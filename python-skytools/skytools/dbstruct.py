...
class TRule(TElem):
    ...
    def get_create_sql(self, curs, new_table_name=None):
        ...
        if self.enabled != "O":
            ...
            # A - rule fires always
            action = {"R": "ENABLE REPLICA", "A": "ENABLE ALWAYS", "D": "DISABLE"}[
                self.enabled
            ]
...
class TParent(TElem):
    ...
    def get_create_sql(self, curs, new_table_name=None):
        return "ALTER TABLE ONLY %s\n  INHERIT %s" % (
            ...
            quote_fqident(self.parent_name),
        )
    ...
    def get_create_sql(self, curs, new_name=None):
        ...
        if self.dist_key_list is not None:
            if self.dist_key_list != []:
                sql += "\ndistributed by(%s)" % ",".join(
                    c.name for c in self.dist_key_list
                )
            else:
