var CodeMirror = (function () {
	function u(aM, aJ) {
		...
		var b9 = (aD.CodeMirror = {
			...
			historySize: function () {
				return { undo: a8.done.length, redo: a8.undone.length };
			},
			...
			setLine: ar(function (cZ, c0) {
				if (br(cZ)) {
					bQ(c0, { line: cZ, ch: 0 }, { line: cZ, ch: cF(cZ).text.length });
				}
			}),
			removeLine: ar(function (cZ) {
				if (br(cZ)) {
					bQ("", { line: cZ, ch: 0 }, aT({ line: cZ + 1, ch: 0 }));
				}
			}),
			replaceRange: ar(bQ),
		...
		function aW(cZ) {
			...
			aO(
				c0,
				{ line: cy.size - 1, ch: cF(cy.size - 1).text.length },
				A(cZ),
				c0,
				c0
			);
		...
		function ck(c8) {
			...
			if (
				...
				function c4(db) {
					...
					c2();
					if (
						Math.abs(c8.clientX - db.clientX) +
							Math.abs(c8.clientY - db.clientY) <
						10
					) {
						T(db);
						a6(cZ.line, cZ.ch, true);
		...
		function ca(c4, c5) {
			...
			for (var c1 = c6.length - 1; c1 >= 0; c1 -= 1) {
				...
				c0.push({ start: c3.start, added: c3.old.length, old: c7 });
				var c2 = aT({
				...
				at(
					{ line: c3.start, ch: 0 },
					{ line: cZ - 1, ch: cF(cZ - 1).text.length },
					c3.old,
					c2,
					c2
				);
		...
		function at(de, c3, dk, cZ, dl) {
			...
			ch = c1;
			bG(100);
			aB.push({ from: de.line, to: c3.line + 1, diff: c7 });
			var c6 = { from: de, to: c3, text: dk };
			if (cN) {
		...
		function bQ(c0, c3, c2) {
			...
			function c1(c6) {
				...
				if (c6.line == c2.line) {
					c5 +=
						c0[c0.length - 1].length -
						(c2.ch - (c2.line == c3.line ? c3.ch : 0));
				}
				return { line: c4, ch: c5 };
			}
			var cZ;
			aC(c0, c3, c2, function (c4) {
				cZ = c4;
				return { from: c1(cW.from), to: c1(cW.to) };
			});
			return cZ;
		}
		function bs(cZ, c0) {
			aC(A(cZ), cW.from, cW.to, function (c1) {
				if (c0 == "end") {
					return { from: c1, to: c1 };
				} else {
					if (c0 == "start") {
						return { from: cW.from, to: cW.from };
					} else {
						return { from: cW.from, to: c1 };
					}
				}
			});
		}
		function aC(c2, c4, c3, cZ) {
			...
			var c0 = cZ({ line: c4.line + c2.length - 1, ch: c1 });
		...
		function bK() {
			...
			if (c1 < ba.length) {
				cW.from = { line: cW.from.line, ch: cW.from.ch - (ba.length - c1) };
			} else {
				if (cn && ad(cW.from, cW.to)) {
					cW.to = {
						line: cW.to.line,
						ch: Math.min(
							cF(cW.to.line).text.length,
							cW.to.ch + (c0.length - c1)
						),
					};
				}
			}
		...
		function bA() {
			...
			return { from: X(cy, c1), to: X(cy, c0) };
		}
		function cd(c7, c3) {
			if (!bk.clientWidth) {
				cQ = bL = bd = 0;
			...
			var db = c7 === true ? [] : b0([{ from: cQ, to: bL, domStart: 0 }], c7);
			var c6 = 0;
				...
				for (var c2 = 0, c0 = c8.length; c2 < c0; ++c2) {
					...
					if (c5.to <= c4.from && c5.diff) {
						...
						if (c5.to <= c4.from || c5.from >= c4.to) {
							...
							if (c5.from > c4.from) {
								cZ.push({ from: c4.from, to: c5.from, domStart: c4.domStart });
							}
							if (c5.to < c4.to) {
								cZ.push({
									from: c5.to + c7,
			...
			cy.iter(c8, c9, function (dc) {
				...
				if (!c5 || c5.from > c2) {
					if (dc.hidden) {
						...
						var dd =
							"<pre" +
							(dc.className ? ' class="' + dc.className + '"' : "") +
							">" +
							dc.getHTML(a9) +
							"</pre>";
						if (dc.bgClassName) {
							dd =
								'<div style="position: relative"><pre class="' +
								dc.bgClassName +
								'" style="position: absolute; left: 0; right: 0; top: 0; bottom: 0; z-index: -2">&#160;</pre>' +
								dd +
								"</div>";
						}
					}
		...
		function aL() {
			...
			cy.iter(cQ, Math.max(bL, cQ + 1), function (da) {
				if (da.hidden) {
					...
					c6.push(
						c9 && c9.style ? '<pre class="' + c9.style + '">' : "<pre>",
						dc
					);
			...
			aH.style.display = "";
			var c5 =
				Math.abs((parseInt(bu.style.marginLeft) || 0) - aH.offsetWidth) > 2;
		...
		function cV() {
			...
			var cZ = ak(aD),
				c1 = ak(aq);
			bX.style.top =
				Math.max(0, Math.min(bk.offsetHeight, c6.y + c1.top - cZ.top)) + "px";
			bX.style.left =
				Math.max(0, Math.min(bk.offsetWidth, c6.x + c1.left - cZ.left)) + "px";
			if (c2) {
				bc.style.top = c6.y + "px";
				bc.style.left =
					(b1.lineWrapping ? Math.min(c6.x, bu.offsetWidth) : c6.x) + "px";
				bc.style.display = "";
				...
				function dc(di, dh, dg, de) {
					var df = E
						? "width: " + (!dg ? c9 : c9 - dg - di) + "px"
						: "right: " + dg + "px";
					c4 +=
						'<div class="CodeMirror-selected" style="position: absolute; left: ' +
						di +
						"px; top: " +
						dh +
						"px; " +
						...
						setTimeout(
							ar(function () {
								if (c0.parent && /^\s+$/.test(c0.text)) {
									var c7 = Y(c0);
									bQ("", { line: c7, ch: 0 }, { line: c7, ch: c0.text.length });
								}
							}, 10)
						);
					}
		...
		function aT(c1) {
			...
			if (c1.line >= cy.size) {
				return { line: cy.size - 1, ch: cF(cy.size - 1).text.length };
			}
			var cZ = c1.ch,
				c0 = cF(c1.line).text.length;
			if (cZ == null || cZ > c0) {
				return { line: c1.line, ch: c0 };
			} else {
				if (cZ < 0) {
					return { line: c1.line, ch: 0 };
				} else {
					return c1;
				}
			}
		...
		function bI(c2) {
			...
			bx({ line: c2.line, ch: c1 }, { line: c2.line, ch: cZ });
		}
		function aK(cZ) {
			bx({ line: cZ, ch: 0 }, aT({ line: cZ + 1, ch: 0 }));
		}
		function cB(c1) {
			if (ad(cW.from, cW.to)) {
		...
		function by(c1, c8) {
			...
			bQ(c6, { line: c1, ch: 0 }, { line: c1, ch: c0.length });
		}
		function bT() {
		...
		function a9(c0) {
			...
			return (ap[cZ] = { html: c3 + "</span>", width: cZ });
		}
		function cD() {
			bk.className =
				bk.className.replace(/\s*cm-s-\S+/g, "") +
				b1.theme.replace(/(^|\s)\s*/g, " cm-s-");
		}
		function cX() {
			this.set = [];
		}
		...
		function bE(c5, c4, c1) {
			...
			aB.push({ from: c5.line, to: c4.line + 1 });
			return cZ;
		}
		function aU(c0) {
			c0 = aT(c0);
		...
		function bo(c4) {
			...
			for (var c0 = 0, c2 = c1.length; c0 < c2; ++c0) {
				var cZ = c1[c0];
				if (
					(cZ.from == null || cZ.from <= c4.ch) &&
					(cZ.to == null || cZ.to >= c4.ch)
				) {
		...
		function bV(cZ, c1, c0) {
			...
			cZ.gutterMarker = { text: c1, style: c0 };
			aS = true;
			return cZ;
		}
		function au(cZ) {
			...
			return aX(c0, function (c1, c4) {
				if (c1.hidden != cZ) {
					...
					var c3 = cW.from.line,
						c2 = cW.to.line;
					if (cZ && (c3 == c4 || c2 == c4)) {
						var c6 = c3 == c4 ? bR({ line: c3, ch: 0 }, c3, 0) : cW.from;
						var c5 = c2 == c4 ? bR({ line: c2, ch: 0 }, c2, 0) : cW.to;
						if (!c5) {
							return;
						}
						bw(c6, c5);
		...
		var cz =
			"CodeMirror-temp-" + Math.floor(Math.random() * 16777215).toString(16);
		function b5(c0, c3) {
			if (c3 == 0) {
				return { top: 0, left: 0 };
			}
			var cZ =
				b1.lineWrapping &&
				c3 < c0.text.length &&
				o.test(c0.text.slice(c3 - 1, c3 + 1));
		...
		function bH(c8, c7) {
			...
			for (;;) {
				if (c1 - dd <= 1) {
					return { line: c9, ch: cZ - c8 > c8 - dc ? dd : c1 };
				}
		...
		function a2(c3, c2) {
			...
			if (
				!c2 &&
				(cZ - c1.left > bk.clientWidth || c4 - c1.top > bk.clientHeight)
			) {
				return null;
			}
			var c0 = ak(bu, true);
		...
		function a1(c0) {
			...
			bm.style.cssText =
				...
				"px; left: " +
				(c0.clientX - 5) +
				"px; z-index: 1000; background: white; border-width: 0; outline: none; overflow: hidden; opacity: .05; filter: alpha(opacity=5);";
			bf = true;
			var c2 = (bm.value = b3());
			bz();
			a(bm);
		...
		function ce(c5) {
			...
			function da(dw, dr, ds) {
				...
				var dv = dw.styles,
					...
					dt;
				for (
					var dm = c6 ? 0 : dv.length - 2, dp = c6 ? dv.length : -2;
					dm != dp;
					dm += 2 * di
				) {
					var du = dv[dm];
					...
					for (
						var dl = c6 ? 0 : du.length - 1, dk = c6 ? du.length : -1;
						dl != dk;
						dl += di, dq += di
					) {
						if (dq >= dr && dq < ds && dc.test((dt = du.charAt(dl)))) {
							...
							if ((dn.charAt(1) == ">") == c6) {
								...
								if (c2.pop() != dn.charAt(0)) {
									return { pos: dq, match: false };
								} else {
									if (!c2.length) {
										return { pos: dq, match: true };
									}
								}
							}
						}
			...
			for (
				var df = cZ.line,
					dh = c6 ? Math.min(df + 100, cy.size) : Math.max(-1, df - 100);
				df != dh;
				df += di
			) {
				var c7 = cF(df),
			...
			if (!c9) {
				c9 = { pos: null, match: false };
			}
			var dg = c9.match
				? "CodeMirror-matchingbracket"
				: "CodeMirror-nonmatchingbracket";
			var de = bE({ line: cZ.line, ch: c0 }, { line: cZ.line, ch: c0 + 1 }, dg),
				c1 =
					c9.pos != null &&
					bE({ line: df, ch: c9.pos }, { line: df, ch: c9.pos + 1 }, dg);
			var db = ar(function () {
				de.clear();
				c1 && c1.clear();
			});
				...
				cy.iter(c6, cy.size, function (da) {
					...
					if (c9 == null) {
						if (dc !== false || !db) {
							...
							if (
								++c4 > 3 &&
								(!cb.indent || cb.indent(db, "") == cb.indent(cZ, ""))
							) {
								c9 = true;
							}
						}
	...
	var L = (u.commands = {
		selectAll: function (am) {
			am.setSelection({ line: 0, ch: 0 }, { line: am.lineCount() - 1 });
		},
		killLine: function (am) {
			...
			if (!an && am.getLine(ap.line).length == ap.ch) {
				am.replaceRange("", ap, { line: ap.line + 1, ch: 0 });
			} else {
				am.replaceRange("", ap, an ? ao : { line: ap.line });
			}
		},
		...
		goDocEnd: function (am) {
			am.setSelection({ line: am.lineCount() - 1 }, null, true);
		},
		...
		goLineEnd: function (am) {
			am.setSelection({ line: am.getCursor().line }, null, true);
		},
		...
		transposeChars: function (am) {
			...
			if (ao.ch > 0 && ao.ch < an.length - 1) {
				am.replaceRange(
					an.charAt(ao.ch) + an.charAt(ao.ch - 1),
					{ line: ao.line, ch: ao.ch - 1 },
					{ line: ao.line, ch: ao.ch + 1 }
				);
			}
		},
	...
	K.prototype = {
		...
		clipTo: function (an, aq, am, ap, ao) {
			...
			if (
				am &&
				(aq < this.to || this.to == null) &&
				(aq > this.from || this.from == null)
			) {
				this.to = null;
			} else {
	...
	G.prototype = {
		...
		find: function () {
			...
			return { line: Y(this.line), ch: this.from };
		},
		clear: function () {
			if (this.line) {
	...
	k.prototype = {
		addChange: function (am, ar, an) {
			...
			if (this.compound && au && !this.closed) {
				au.push({ start: am, added: ar, old: an });
			} else {
				if (
					aq > 400 ||
					!av ||
					this.closed ||
					av.start > am + an.length ||
					av.start + av.added < am
				) {
					this.done.push([{ start: am, added: ar, old: an }]);
					this.closed = false;
				} else {
						...
						if (T == "/") {
							...
							if (T == "#") {
								...
								if (N.test(T)) {
									...
									return U && V.kwAllowed
										? B(U.type, U.style, W)
										: B("variable", "variable", W);
								}
							}
						}
					}
	...
	return {
		startState: function (T) {
			return {
				...
				context: M.localVars && { vars: M.localVars },
				indented: 0,
			};
		},
		token: function (V, U) {
...
CodeMirror.defineMode("xml", function (y, k) {
	...
	function i(A, C) {
		var B =
			x.doNotIndent.hasOwnProperty(A) || (l.context && l.context.noIndent);
		l.context = {
			prev: l.context,
			tagName: A,
			indent: l.indented,
			...
			if (A == "closeTag") {
				...
				if (l.context) {
					if (l.context.tagName != f) {
						if (
							x.implicitlyClosed.hasOwnProperty(l.context.tagName.toLowerCase())
						) {
							u();
						}
	...
	function c(A) {
		return function (B) {
			if (
				B == "selfcloseTag" ||
				(B == "endTag" &&
					x.autoSelfClosers.hasOwnProperty(l.tagName.toLowerCase()))
			) {
	...
	function q(B) {
		...
		while (true) {
			...
			if (
				!x.contextGrabbers.hasOwnProperty(A) ||
				!x.contextGrabbers[A].hasOwnProperty(B)
			) {
				return;
			}
			u();
...
CodeMirror.defineMode(
	"markdown",
	function (B, m) {
		var i = CodeMirror.getMode(B, { name: "xml", htmlMode: true });
		var y = "header",
			d = "comment",
			A = "quote",
			z = "string",
...
Ext.define("Docs.History", {
	...
	parseToken: function (d) {
		var c =
			d && d.match(/!?(\/(api|guide|example|video|comment|tests)(\/(.*))?)/);
		return c ? { type: c[2], url: "#!" + c[1] } : {};
	},
	push: function (e, f) {
		e = this.cleanUrl(e);
		if (!/^#!?/.test(e)) {
		...
		if (this.localStorage) {
			...
			if (window.addEventListener) {
				window.addEventListener(
					"storage",
					Ext.Function.bind(this.onStorageChange, this),
					false
				);
			} else {
				window.attachEvent(
...
Ext.define("Docs.GuideSearch", {
	...
	deferredSearch: function (f, h, j, g) {
		...
		var i = (this.timeout = Ext.Function.defer(
			function () {
				this.search(
					f,
					function (a) {
...
Ext.define("Docs.view.examples.Device", {
	config: { url: "", id: undefined, device: "phone", orientation: "landscape" },
	constructor: function (b) {
		this.initConfig(b);
	...
	getIframeSize: function () {
		...
		if (this.orientation === "landscape") {
			...
			return { width: b.height, height: b.width };
		}
	},
});
...
Ext.define("Docs.view.DocTree", {
	...
	applyNodeTpl: function (b) {
		if (b.get("leaf")) {
			b.set(
				"text",
				this.nodeTpl.apply({ text: b.get("text"), url: b.raw.url })
			);
			b.commit();
		}
	},
	...
	findNodeByUrl: function (b) {
		return this.getRootNode().findChildBy(
			function (a) {
				return b === a.raw.url;
			},
			this,
...
Ext.define("Docs.view.comments.Pager", {
	...
	afterRender: function () {
		...
		this.getEl().on(
			...
			this,
			{ preventDefault: true, delegate: "a.fetchMoreComments" }
		);
	},
...
Ext.define("Docs.view.SimpleSelectBehavior", {
	...
	constructor: function (c, d) {
		this.mixins.observable.constructor.call(this, { listeners: d });
		c.on({ select: this.onSelect, deselect: this.onDeselect, scope: this });
	},
...
Ext.define("Docs.view.comments.TopList", {
	...
	initComponent: function () {
		this.items = [
						...
						items: [
							{
								...
								width: 320,
								height: 20,
								listeners: { filter: this.onFilter, scope: this },
							},
						],
					},
				],
			...
			(this.list = Ext.widget("dataview", {
				...
				autoScroll: true,
				store: new Ext.data.Store({ model: this.model }),
				allowDeselect: true,
				tpl: [
					"<ul>",
					'<tpl for=".">',
	...
	onFilter: function (b) {
		...
		this.list
			.getStore()
			.filter({ property: this.displayField, value: b, anyMatch: true });
	},
...
Ext.define("Docs.view.examples.InlineEditor", {
	...
	selectAll: function () {
		...
		this.codemirror.setSelection({ line: 0, ch: 0 }, { line: d, ch: c });
	},
});
Ext.define("Docs.view.examples.InlinePreview", {
	...
	getHtml: function () {
		if (Docs.data.touchExamplesUi) {
			...
			return b.apply({ id: this.iframeId });
		}
	},
	update: function (h) {
		var f = this.options;
...
Ext.define("Docs.view.comments.Form", {
	...
	bindEvents: function () {
		this.getEl().on(
			...
			this,
			{ preventDefault: true, delegate: "a.toggleCommentGuide" }
		);
		this.getEl().on(
			"click",
			function () {
			...
			this,
			{ preventDefault: true, delegate: "a.cancelUpdateComment" }
		);
		this.getEl().on(
			"click",
			function () {
			...
			this,
			{ preventDefault: true, delegate: "input.submitComment" }
		);
		this.getEl().on(
			"click",
			function (c, d) {
...
Ext.define("Docs.Comments", {
	...
	init: function (c, d) {
		if (
			!(
				Docs.data.commentsUrl &&
				Docs.data.commentsDomain &&
	...
	load: function (d, f, e) {
		this.request("jsonp", {
			url: "/comments",
			method: "GET",
			params: { startkey: Ext.JSON.encode(d) },
			success: f,
			scope: e,
		});
	},
	...
	avatar: function (c, d) {
		return (
			'<img class="avatar ' +
			(d || "") +
			'" width="25" height="25" src="http://www.gravatar.com/avatar/' +
			c +
...
Ext.define("Docs.controller.Auth", {
	...
	refs: [{ ref: "authHeaderForm", selector: "authHeaderForm" }],
	init: function () {
		this.control({
			"authHeaderForm, authForm": { login: this.login, logout: this.logout },
		});
		var b = this.getController("Tabs");
		b.onLaunch = Ext.Function.createSequence(
			b.onLaunch,
			this.afterTabsLaunch,
			this
		);
	},
...
Ext.define("Docs.controller.Welcome", {
	...
	refs: [
		...
		{ ref: "index", selector: "#welcomeindex" },
	],
	init: function () {
...
Ext.define("Docs.controller.Failure", {
	...
	show404: function (c) {
		...
		Ext.getCmp("failure").update(d.apply({ msg: c }));
...
Ext.define("Docs.controller.Search", {
	...
	stores: ["Search"],
	refs: [{ ref: "field", selector: "#search-field" }],
	pageIndex: 0,
	pageSize: 10,
	basicSearchDelay: 50,
		...
		this.control({
			...
			"#search-field": {
					...
					if (l.keyCode === Ext.EventObject.UP) {
						...
						if (l.keyCode === Ext.EventObject.DOWN) {
							...
							if (l.keyCode === Ext.EventObject.ENTER) {
								...
								this.searchTimeout = Ext.Function.defer(
									function () {
										this.search(m.value);
									},
									this.basicSearchDelay,
									this
								);
							}
						}
				...
				blur: function () {
					...
					this.hideTimeout = Ext.Function.defer(
						b.hide,
	...
	guideSearch: function (b) {
		Docs.GuideSearch.deferredSearch(
			b,
			function (a) {
				this.basicSearch(b, a);
			},
			this,
			this.guideSearchDelay
		);
	},
...
Ext.define("Docs.controller.Classes", {
	...
	refs: [
		{ ref: "viewport", selector: "#viewport" },
		{ ref: "index", selector: "#classindex" },
		{ ref: "header", selector: "classheader" },
		{ ref: "overview", selector: "classoverview" },
		{ ref: "tabPanel", selector: "classtabpanel" },
		{ ref: "tree", selector: "#classtree" },
		{ ref: "favoritesGrid", selector: "#favorites-grid" },
	],
	cache: {},
	init: function () {
		...
		Ext.getBody().addListener(
			...
			this,
			{ preventDefault: true, delegate: ".docClass" }
		);
		this.control({
			classtree: {
			...
			classoverview: {
				afterrender: function (b) {
					b.el.addListener(
						...
						this,
						{ preventDefault: true, delegate: ".expandable" }
					);
		...
		if (this.cache[g]) {
			...
			Ext.data.JsonP.request({
				...
				failure: function (b, a) {
					...
					this.getController("Failure").show404(
						"Class <b>" + Ext.String.htmlEncode(g) + "</b> was not found."
					);
				},
				scope: this,
			});
	...
	showClass: function (e, f) {
		...
		this.fireEvent("showClass", e.name, { reRendered: d });
	},
});
Ext.define("Docs.controller.Examples", {
	...
	refs: [
		{ ref: "viewport", selector: "#viewport" },
		{ ref: "index", selector: "#exampleindex" },
		{ ref: "tree", selector: "#exampletree" },
		{ ref: "page", selector: "#example" },
	],
	init: function () {
	...
	loadExample: function (d, f) {
		...
		if (!e) {
			this.getController("Failure").show404(
				"Example <b>" + Ext.String.htmlEncode(d) + "</b> was not found."
			);
			return;
		}
...
Ext.define("Docs.controller.Videos", {
	...
	refs: [
		{ ref: "viewport", selector: "#viewport" },
		{ ref: "index", selector: "#videoindex" },
		{ ref: "tree", selector: "#videotree" },
	],
	init: function () {
	...
	loadVideo: function (j, h) {
		...
		if (!i) {
			this.getController("Failure").show404(
				"Video <b>" + Ext.String.htmlEncode(g) + "</b> was not found."
			);
			return;
		}
		...
		this.fireEvent("showVideo", g, { reRendered: f });
...
Ext.define("Docs.controller.Guides", {
	...
	refs: [
		{ ref: "viewport", selector: "#viewport" },
		{ ref: "index", selector: "#guideindex" },
		{ ref: "tree", selector: "#guidetree" },
		{ ref: "guide", selector: "#guide" },
	],
	cache: {},
	init: function () {
			...
			indexcontainer: {
				afterrender: function (b) {
					b.el.addListener(
						...
						this,
						{ preventDefault: true, delegate: ".guide" }
					);
				},
			},
			doctabs: {
		...
		if (this.cache[f]) {
			...
			Ext.data.JsonP.request({
				...
				failure: function (b, a) {
					this.cache[f] = false;
					this.getController("Failure").show404(
						"Guide <b>" + Ext.String.htmlEncode(f) + "</b> was not found."
					);
				},
				scope: this,
			});
	...
	showGuide: function (i, j, f, h) {
		...
		this.getViewport().setPageTitle(i.title);
		if (this.activeUrl !== j) {
			Ext.getCmp("guide").load({ name: f, content: i.guide });
			g = true;
		}
		this.activeUrl = j;
		...
		this.fireEvent("showGuide", f, { reRendered: g });
		this.getTree().selectUrl(j);
	},
});
Ext.define("Docs.controller.CommentCounts", {
	...
	refs: [
		{ ref: "class", selector: "classoverview" },
		{ ref: "classIndex", selector: "#classindex" },
		{ ref: "guide", selector: "#guide" },
		{ ref: "guideIndex", selector: "#guideindex" },
		{ ref: "video", selector: "#video" },
		{ ref: "videoIndex", selector: "#videoindex" },
	],
	init: function () {
...
Ext.define("Docs.controller.Tests", {
	...
	refs: [
		{ ref: "viewport", selector: "#viewport" },
		{ ref: "index", selector: "#testsindex" },
	],
	init: function () {
		this.addEvents("loadIndex");
		this.control({ "#testsgrid": { afterrender: this.loadExamples } });
	},
	loadIndex: function () {
		this.fireEvent("loadIndex");
		Ext.getCmp("treecontainer").hide();
...
Ext.define("Docs.controller.Tabs", {
	...
	refs: [
		{ ref: "welcomeIndex", selector: "#welcomeindex" },
		{ ref: "classIndex", selector: "#classindex" },
		{ ref: "guideIndex", selector: "#guideindex" },
		{ ref: "videoIndex", selector: "#videoindex" },
		{ ref: "exampleIndex", selector: "#exampleindex" },
		{ ref: "testsIndex", selector: "#testsindex" },
		{ ref: "commentIndex", selector: "#commentindex" },
		{ ref: "classTree", selector: "#classtree" },
		{ ref: "guideTree", selector: "#guidetree" },
		{ ref: "exampleTree", selector: "#exampletree" },
		{ ref: "videoTree", selector: "#videotree" },
		{ ref: "doctabs", selector: "#doctabs" },
	],
	init: function () {
	...
	onLaunch: function () {
		...
		if (b) {
			Ext.Array.forEach(
				b,
				function (a) {
					this.addTabFromTree(a, { animate: false });
				},
				this
			);
		}
	...
	addTab: function (d, c) {
		c = c || { animate: true, activate: true };
		this.getDoctabs().addTab(
			{ href: d.url, text: d.text, iconCls: d.iconCls },
			c
		);
	},
	getTree: function (b) {
...
Ext.define("Docs.controller.Comments", {
	...
	refs: [
		{ ref: "viewport", selector: "#viewport" },
		{ ref: "index", selector: "#commentindex" },
		{ ref: "commentsFullList", selector: "commentsFullList" },
	],
...
Ext.define("Docs.view.Tabs", {
	...
	initListeners: function () {
		...
		this.el.on(
			...
			this,
			{ delegate: ".close", preventDefault: true }
		);
		this.el.on(
			"click",
			function (f, d) {
				...
				this.fireEvent("tabActivate", e, { navigate: true });
			},
			this,
			{ delegate: ".doctab" }
		);
		this.el.on(
			...
			this,
			{ delegate: ".doctab", preventDefault: true }
		);
	...
	refresh: function () {
		...
		var f =
			this.maxTabsInBar() < this.tabs.length
				? this.maxTabsInBar()
				: this.tabs.length;
	...
	addTabToBar: function (e, d) {
		...
		if (d.animate && !Ext.isIE) {
			...
			f.animate({ to: { width: this.tabWidth() } });
		} else {
			f.setStyle({ visibility: "visible" });
		}
		this.resizeTabs(d);
	...
	removeTabFromBar: function (d) {
		...
		if (Ext.isIE) {
			...
			c.animate({ to: { top: 30 }, duration: this.animDuration }).animate({
				to: { width: 10 },
			...
			function (a) {
				...
				if (!d.dom.removed && !d.hasCls("overview")) {
					if (b && b.animate && !Ext.isIE) {
						d.animate({ to: { width: this.tabWidth() } });
					} else {
		...
		var b = new Docs.view.TabMenu({
			listeners: {
				...
				tabItemClick: function (a) {
					this.fireEvent("tabActivate", a.href, { navigate: true });
				},
				scope: this,
			},
		});
	...
	addToolTips: function () {
		Ext.Array.each(this.staticTabs, function (c) {
			...
			if (d) {
				Ext.create("Ext.tip.ToolTip", { target: d, html: c.tooltip });
			}
		});
		Ext.Array.each(
			this.tabsInBar,
	...
	addMainTabTooltip: function (c, d) {
		if (d.tooltip) {
			Ext.create("Ext.tip.ToolTip", { target: c, html: d.tooltip });
		}
	},
	saveTabs: function () {
...
Ext.define("Docs.view.examples.TouchContainer", {
	...
	updateButtons: function () {
		...
		Ext.get(
			Ext.query(
				".example-toolbar .orientations button." + this.device.getOrientation()
			)
		).addCls("selected");
		...
		Ext.get(
			Ext.query(".example-toolbar .devices button." + this.device.getDevice())
		).addCls("selected");
	},
	clear: function () {
		this.update("");
		...
		this.on(
			...
			function () {
				this.el.addListener(
					...
					this,
					{ preventDefault: true, delegate: ".prev" }
				);
				this.el.addListener(
					"click",
					function () {
					...
					this,
					{ preventDefault: true, delegate: ".next" }
				);
				this.el.addListener(
					"click",
					function () {
...
Ext.define("Docs.view.GroupTree", {
	...
	initComponent: function () {
		this.root = { text: "Root", children: this.buildTree(this.data) };
		this.callParent();
	},
...
Ext.define("Docs.view.auth.BaseForm", {
	...
	bindFormSubmitEvent: function () {
		this.getEl()
			.down("form")
			.on("submit", this.submitLogin, this, { preventDefault: true });
	},
...
Ext.define("Docs.view.auth.HeaderForm", {
	...
	afterRender: function () {
		...
		this.getEl().addListener(
			...
			this,
			{ preventDefault: true, delegate: ".logout" }
		);
	},
	...
	showLoggedIn: function (d) {
		...
		this.update(
			c +
				"<div><span>" +
				d.userName +
				'</span> | <a href="#" class="logout">Logout</a></div>'
		);
	},
...
Ext.define("Docs.view.comments.Users", {
	...
	initComponent: function () {
		this.items = [
			(this.tabpanel = Ext.widget("tabpanel", {
				...
				region: "north",
				height: 50,
				items: [{ title: "Votes" }, { title: "Comments" }],
				dockedItems: [
					{
						dock: "bottom",
						items: [
				...
				listeners: { tabchange: this.onTabChange, scope: this },
			})),
			(this.list = Ext.widget("dataview", {
				region: "center",
	...
	onFilter: function (b) {
		...
		this.list
			.getStore()
			.filter({ property: "userName", value: b, anyMatch: true });
	},
	deselectAll: function () {
		this.list.getSelectionModel().deselectAll();
	},
	...
	loadUsers: function (c) {
		...
		if (this.selectedUser) {
			var d = this.list
				.getStore()
				.findExact("userName", this.selectedUser.get("userName"));
		...
		this.tpl = Ext.create(
			...
			"</tpl>",
			{
				...
				renderAliases: function (e) {
					var f = { widget: "xtype", plugin: "ptype", feature: "ftype" };
					var d = [];
					e &&
						Ext.Object.each(e, function (a, b) {
							d.push((f[a] || a) + ": " + b.join(", "));
						});
					if (d.length > 0) {
...
Ext.define("Docs.view.guides.Index", {
	...
	getTab: function () {
		...
		return b ? { cls: "guides", href: "#!/guide", tooltip: "Guides" } : false;
	},
	updateCommentCounts: function () {
...
Ext.define("Docs.view.videos.Index", {
	...
	getTab: function () {
		...
		return b ? { cls: "videos", href: "#!/video", tooltip: "Videos" } : false;
	},
	updateCommentCounts: function () {
		this.down("thumblist").updateCommentCounts();
	},
...
Ext.define("Docs.view.examples.Inline", {
	...
	layout: "card",
	border: 0,
	resizable: { transparent: true, handles: "s", constrainTo: false },
	maxCodeHeight: 400,
	options: {},
...
Ext.define("Docs.view.examples.InlineWrap", {
	...
	initToolbar: function () {
		...
		this.tb = Ext.create("Docs.view.examples.InlineToolbar", { renderTo: b });
	},
	replacePre: function (d) {
		...
		Ext.create("Docs.view.examples.Inline", {
			height: 200,
			renderTo: c,
			value: Ext.String.htmlDecode(
				Ext.util.Format.stripTags(this.pre.innerHTML)
			),
			options: d,
			toolbar: this.tb,
		});
...
Ext.define("Docs.controller.InlineExamples", {
	...
	createResizer: function (b) {
		return function () {
			Ext.Array.each(
				Ext.ComponentQuery.query(b + " .inlineexample"),
				function (a) {
...
Ext.define("Docs.view.tests.BatchRunner", {
	...
	run: function (b) {
		...
		this.runNext({ pass: 0, fail: 0, total: b.length, remaining: b });
	},
	runNext: function (h) {
...
Ext.define("Docs.view.tests.Index", {
	...
	layout: { type: "vbox", align: "stretch", shrinkToFit: true },
	padding: 10,
	initComponent: function () {
		...
		this.items = [
			{ html: "<h1>Inline examples test page</h1>", height: 30 },
			{
				...
				items: [
					{
						...
						items: [
							...
							{ itemId: "testStatus", margin: "5 5 5 15" },
						],
					},
					this.grid,
				],
	...
	getTab: function () {
		return Docs.data.tests
			? { cls: "tests", href: "#!/tests", tooltip: "Tests", text: "Tests" }
			: false;
	},
	...
	updateTestStatus: function (d) {
		...
		this.setStatus(
			d.fail === 0,
			c + "/" + d.total + " examples tested, " + d.fail + " failures"
		);
	},
...
Ext.define("Docs.view.cls.Tree", {
	...
	initComponent: function () {
		this.setLogic(
			Docs.Settings.get("classTreeLogic"),
			Docs.Settings.get("showPrivateClasses")
		);
		this.dockedItems = [
			{
				xtype: "container",
	...
	makeButtonHtml: function (d, c) {
		return Ext.String.format(
			...
			d,
			Docs.Settings.get("classTreeLogic") === d ? "selected" : "",
			c
		);
	},
	...
	setLogic: function (i, f) {
		...
		var g = new Docs.view.cls[i]({ classes: this.data, showPrivateClasses: f });
		if (this.root) {
...
Ext.define("Docs.view.TreeContainer", {
	...
	initComponent: function () {
		this.items = [
			{},
			{ xtype: "classtree", id: "classtree", data: Docs.data.classes },
			{
				xtype: "grouptree",
				id: "exampletree",
				data: Docs.data.examples,
...
Ext.define("Docs.view.comments.Expander", {
	...
	afterRender: function () {
		this.callParent(arguments);
		this.getEl()
			.select(".toggleComments")
			.each(function (b) {
				b.on("click", this.toggle, this, { preventDefault: true });
			}, this);
		new Docs.view.comments.TopLevelDropZone(
			this.getEl().down(".side.toggleComments"),
	...
	loadComments: function () {
		this.list = new Docs.view.comments.ListWithForm({
			...
			renderTo: this.getEl(),
			listeners: { reorder: this.reload, scope: this },
		});
		this.reload();
	},
	reload: function () {
...
Ext.define("Docs.view.guides.Container", {
	...
	scrollToEl: function (c) {
		...
		this.scrollToView(d, { highlight: true, offset: -100 });
	},
	load: function (b) {
		this.guide = b;
		this.tpl =
	...
	updateCommentCounts: function () {
		...
		this.expander
			.getExpander()
			.setCount(Docs.Comments.getCount(["guide", this.guide.name, ""]));
	},
});
Ext.define("Docs.view.videos.Container", {
	...
	load: function (b) {
		...
		this.tpl =
			...
			new Ext.XTemplate(
				...
				{
					linkify: function (a) {
						return a.replace(/(\bhttps?:\/\/\S+)/gi, "<a href='$1'>$1</a>");
					},
				}
			);
		this.update(this.tpl.apply(b));
	...
	updateCommentCounts: function () {
		...
		this.expander
			.getExpander()
			.setCount(Docs.Comments.getCount(["video", this.video.name, ""]));
	},
});
Ext.define("Docs.view.comments.MemberWrap", {
	...
	getNewCommentTitle: function () {
		if (this.getDefinedIn() !== this.className) {
			return [
				"<b>Be aware.</b> This member is inherited from <b>" +
					this.getDefinedIn() +
					"</b>; ",
...
Ext.define("Docs.view.comments.RepliesExpander", {
	...
	initComponent: function () {
		this.tpl = new Ext.XTemplate(
			...
			"{[this.renderCount(values.count)]}",
			"</a>",
			{ renderCount: this.renderCount, getCountCls: this.getCountCls }
		);
		this.data = { count: this.count };
		this.callParent(arguments);
	},
	...
	afterRender: function () {
		...
		this.getEl()
			.down(".replies-button")
			.on("click", this.toggle, this, { preventDefault: true });
	},
	toggle: function () {
		this.expanded ? this.collapse() : this.expand();
	},
	...
	loadComments: function () {
		this.list = new Docs.view.comments.ListWithForm({
			...
			renderTo: this.getEl(),
			listeners: { countChange: this.setCount, scope: this },
		});
		Docs.Comments.loadReplies(
			this.parentId,
			function (b) {
				this.list.load(b);
			},
...
Ext.define("Docs.model.Comment", {
	...
	setParent: function (d, f, e) {
		this.request({
			...
			method: "POST",
			params: d ? { parentId: d.get("id") } : undefined,
			success: f,
			scope: e,
		});
	},
...
Ext.define("Docs.view.comments.TagEditor", {
	...
	initComponent: function () {
		this.items = [
			{
				xtype: "combobox",
				listConfig: { cls: "comments-tageditor-boundlist" },
...
Ext.define("Docs.view.comments.List", {
	...
	initComponent: function () {
		this.store = Ext.create("Ext.data.Store", {
			...
			listeners: { update: this.fireChangeEvent, scope: this },
		});
	...
	delegateClick: function (e, f, d) {
		this.getEl().on(
			...
			this,
			{ preventDefault: true, delegate: e }
		);
	},
	vote: function (e, f, d) {
		if (!Docs.Auth.isLoggedIn()) {
...
Ext.define("Docs.view.comments.FullList", {
	...
	items: [
		{
			...
			region: "north",
			height: 25,
			items: [{ title: "Recent" }, { title: "Votes" }],
		},
		{
			region: "center",
			xtype: "container",
			...
			cls: "iScroll",
			items: [
				{ xtype: "commentsList", id: "recentcomments", showTarget: true },
				{ xtype: "commentsPager" },
			],
		},
	],
	...
	setHideReadVisibility: function () {
		...
		Ext.get("hideRead")
			.up("label")
			.setStyle("display", b ? "inline" : "none");
	},
	initTabs: function () {
		this.down("tabpanel[cls=comments-tabpanel]").on(
			"tabchange",
			function (d, c) {
				if (c.title === "Recent") {
	...
	getTab: function () {
		return Docs.Comments.isEnabled()
			? { cls: "comments", href: "#!/comment", tooltip: "Comments" }
			: false;
	},
});
Ext.define("Docs.view.comments.Index", {
	...
	layout: "border",
	items: [
		{ region: "center", xtype: "commentsFullList" },
		{
			region: "east",
			itemId: "cardPanel",
			layout: "border",
			...
			margin: "0 0 0 20",
			layout: "card",
			dockedItems: [{ xtype: "commentsHeaderMenu", dock: "top", height: 35 }],
			items: [
				{ xtype: "commentsUsers" },
	...
	initComponent: function () {
		...
		this.down("commentsHeaderMenu").on(
			"select",
			function (a) {
...
Ext.define("Docs.view.HoverMenuButton", {
	...
	initComponent: function () {
		...
		if (this.showCount) {
			...
			this.store.on(
				...
				function () {
					this.setText(
						this.initialText + " <sup>" + this.store.getCount() + "</sup>"
					);
				},
				this
			);
	...
	deferShowMenu: function () {
		clearTimeout(Docs.view.HoverMenuButton.showTimeout);
		Docs.view.HoverMenuButton.showTimeout = Ext.Function.defer(
			function () {
				if (!this.menu) {
					this.renderMenu();
				}
				Ext.Array.forEach(
					Docs.view.HoverMenuButton.menus,
					function (a) {
						if (a !== this.menu) {
							a.hide();
						}
				...
				this.menu.getEl().setStyle({ left: k + "px", top: j[1] + 25 + "px" });
			},
			200,
			this
		);
...
Ext.define("Docs.view.cls.Toolbar", {
	...
	createLinkRecord: function (d, c) {
		return {
			...
			url: d + "-" + c.id,
			label:
				c.tagname === "method" && c.name === "constructor"
					? "new " + d
					: c.name,
			inherited: c.owner !== d,
				...
				if (c) {
					c.getStore().filterBy(function (h) {
						return !(
							...
							(!d.removed && h.get("meta")["removed"]) ||
							(e && !f.test(h.get("label")))
						);
					});
					var a = c.menu;
		...
		Ext.Object.each(
			...
			function (c, d) {
				d.getStore().each(function (a) {
					a.set(
						"commentCount",
						Docs.Comments.getCount(["class", this.docClass.name, a.get("id")])
					);
				}, this);
			},
			this
...
Ext.define("Docs.view.cls.Overview", {
	...
	initComments: function () {
		...
		this.toolbar.setCommentCount(
			Docs.Comments.getCount(["class", this.docClass.name, ""])
		);
	...
	initBasicMemberWrappers: function () {
		...
		Ext.Array.forEach(
			Ext.query(".member"),
			function (c) {
				var d = new Docs.view.cls.MemberWrap({ el: c });
				this.memberWrappers[d.getMemberId()] = d;
			},
			this
		);
	...
	filterMembers: function (h, e) {
		...
		Ext.Array.forEach(
			Ext.query(".members-section"),
			function (b) {
				var a = this.getVisibleElements(".member", b);
				Ext.get(b).setStyle({ display: a.length > 0 ? "block" : "none" });
				Ext.Array.forEach(
					Ext.query(".subsection", b),
					function (d) {
						var c = this.getVisibleElements(".member", d);
						if (c.length > 0) {
							c[0].addCls("first-child");
							Ext.get(d).setStyle({ display: "block" });
						} else {
							Ext.get(d).setStyle({ display: "none" });
						}
					},
					this
				);
...
Ext.define("Docs.view.cls.Container", {
	...
	initComponent: function () {
		this.items = [
			Ext.create("Docs.view.cls.Header", { region: "north" }),
			Ext.create("Docs.view.cls.Overview", { region: "center" }),
		];
		this.callParent(arguments);
	},
});
				...
				items: [
					...
					},
					{
						...
						items: {
							...
							items: [
								{ autoScroll: true, xtype: "welcomeindex", id: "welcomeindex" },
								{ xtype: "container", id: "failure" },
								{ autoScroll: true, xtype: "classindex", id: "classindex" },
								{ xtype: "classcontainer", id: "classcontainer" },
								{ autoScroll: true, xtype: "guideindex", id: "guideindex" },
								{
									autoScroll: true,
								...
								{ xtype: "videoindex", id: "videoindex" },
								{
									autoScroll: true,
								...
								{ xtype: "exampleindex", id: "exampleindex" },
								{
									xtype: Docs.data.touchExamplesUi
										? "touchexamplecontainer"
										: "examplecontainer",
									id: "example",
								},
								{ xtype: "testsindex", id: "testsindex" },
								{ xtype: "commentindex", id: "commentindex" },
							],
						},
					},
				],
...
Ext.define("Docs.view.comments.ListWithForm", {
	...
	initComponent: function () {
		this.items = [
			(this.list = new Docs.view.comments.List({ enableDragDrop: true })),
		];
...
Ext.ns("Docs");
Ext.Loader.setConfig({ enabled: true, paths: { Docs: "app" } });
...
!(function () {
	...
	(function () {
		function h(J) {
			...
			function I(R) {
				var M = R.substring(1, R.length - 1).match(
				...
				for (var O = O ? 1 : 0, Q = M.length; O < Q; ++O) {
					...
					if (/\\[bdsw]/i.test(P)) {
						...
						N < 65 ||
							...
							(N < 65 ||
								P > 90 ||
								R.push([Math.max(65, P) | 32, Math.min(N, 90) | 32]),
							N < 97 ||
								P > 122 ||
								R.push([Math.max(97, P) & -33, Math.min(N, 122) & -33]));
					}
				}
				R.sort(function (U, T) {
				...
				for (O = 0; O < M.length; ++O) {
					(P = M[O]),
						S.push(F(P[0])),
						P[1] > P[0] && (P[1] + 1 > P[0] && S.push("-"), S.push(F(P[1])));
				}
				S.push("]");
				return S.join("");
			}
					...
					for (P = 0; P < S; ++P) {
						(M = N[P]),
							...
							M.length >= 2 && Q === "["
								? (N[P] = I(M))
								: Q !== "\\" &&
								  (N[P] = M.replace(/[A-Za-z]/g, function (T) {
										T = T.charCodeAt(0);
			...
			for (var K = 0, y = !1, D = !1, C = 0, H = J.length; C < H; ++C) {
				...
				if (E.ignoreCase) {
					...
					if (
						/[a-z]/i.test(
							E.source.replace(/\\u[\da-f]{4}|\\x[\da-f]{2}|\\[^UXux]/gi, "")
						)
					) {
						y = !0;
						D = !1;
			...
			for (
				var p = { b: 8, t: 9, n: 10, v: 11, f: 12, r: 13 },
					v = [],
					C = 0,
					H = J.length;
				C < H;
		...
		function t(v, C, y, p) {
			C && ((v = { a: C, e: v }), y(v), p.push.apply(p, v.g));
		}
		function f(v) {
			for (var C = void 0, y = v.firstChild; y; y = y.nextSibling) {
				var p = y.nodeType,
					C =
						p === 1 ? (C ? v : y) : p === 3 ? (e.test(y.nodeValue) ? v : C) : C;
			}
			return C === v ? void 0 : C;
		}
		function A(y, E) {
			function D(R) {
				for (
					...
					if (typeof S === "string") {
						...
						if (
							(N = S.length >= 5 && "lang-" === S.substring(0, 5)) &&
							!(T && typeof T[1] === "string")
						) {
							(N = !1), (S = "src");
						}
			...
			(function () {
				for (
					var J = y.concat(E), G = [], F = {}, M = 0, H = J.length;
					M < H;
					++M
				) {
					var I = J[M],
		...
		function u(v) {
			...
			p.length &&
				C.push(["kwd", RegExp("^(?:" + p.replace(/[\s,]+/g, "|") + ")\\b"), a]);
			D.push(["pln", /^\s+/, a, " \r\n\t\u00a0"]);
			p = "^.[^\\s\\w.$@'\"`/\\\\]*";
			v.regexLiterals && (p += "(?!s*/)");
			C.push(
				["lit", /^@[$_a-z][\w$@]*/i, a],
				["typ", /^(?:[@_]?[A-Z]+[a-z][\w$@]*|\w+_t\b)/, a],
				["pln", /^[$_a-z][\w$@]*/i, a],
				[
					"lit",
		...
		function q(J, G, F) {
			...
			for (
				var G = Math.max(0, (G - 1) | 0) || 0, E = 0, v = H.length;
				E < v;
				++E
			) {
				(C = H[E]),
		...
		function B(v, C) {
			for (var y = C.length; --y >= 0; ) {
				var p = C[y];
				w.hasOwnProperty(p)
					? z.console && console.warn("cannot override language handler %s", p)
					: (w[p] = v);
			}
		}
		function s(p, v) {
				...
				try {
					for (; ac < T; ) {
						...
						if (N.nodeType !== 1 && (F = C.substring(X, I))) {
							...
							P.appendChild(N);
							X < S &&
								((U[ac + 1] = N = y.createTextNode(C.substring(I, S))),
		...
		B(A([], [["atv", /^[\S\s]+/]]), ["uq.val"]);
		B(u({ keywords: n, hashComments: !0, cStyleComments: !0, types: i }), [
			"c",
			"cc",
			"cpp",
			"cxx",
		...
		B(u({ keywords: "null,true,false" }), ["json"]);
		B(
			u({
				keywords: l,
				hashComments: !0,
		...
		B(u({ keywords: m, cStyleComments: !0 }), ["java"]);
		B(u({ keywords: r, hashComments: !0, multiLineStrings: !0 }), [
			"bash",
			"bsh",
			"csh",
			"sh",
		...
		B(u({ keywords: x, cStyleComments: !0, regexLiterals: !0 }), [
			"javascript",
			"js",
		]);
		B(
		...
		B(u({ keywords: d, cStyleComments: !0, multilineStrings: !0 }), [
			"rc",
			"rs",
			"rust",
		]);
		...
		var b = (z.PR = {
			...
			prettyPrint: (z.prettyPrint = function (T, Q) {
				function N() {
					for (
						var v = z.PR_SHOULD_USE_CONTINUATION ? R.now() + 250 : Infinity;
						L < G.length && R.now() < v;
						L++
					) {
								...
								if (y.test(Y.tagName)) {
									...
									var X = Y.currentStyle,
										...
										X =
											(X = X
												? X.whiteSpace
												: V && V.getComputedStyle
												? V.getComputedStyle(Y, a).getPropertyValue(
														"white-space"
												  )
				...
				for (
					var S = Q || document.body,
						E = S.ownerDocument || document,
						S = [
