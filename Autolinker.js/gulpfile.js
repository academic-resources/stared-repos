...
function docTask() {
	...
	return merge(
		...
		// by GitHub pages
		gulp.src(`${distFolder}/**/*`).pipe(gulp.dest("./docs/dist")),

		gulp.src(srcFilesGlob).pipe(jsduck.doc())
	);
}

function lintTask() {
	return gulp
		.src([srcFilesGlob, testFilesGlob])
		.pipe(jshint())
...
function domainsToRegex(contents) {
	contents = contents.split("\n").filter(notCommentLine).map(dePunycodeDomain);
