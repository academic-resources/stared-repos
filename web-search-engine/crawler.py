...
def pipeline(response, spider):
    ...
    if (
        response.status in spider.handle_httpstatus_list
...
def create_thumbnail(url_id, lang, link):
    ...
    if r.status_code == 200:
        ...
        img_str = b"data:image/%s;base64,%s" % (
            format_.lower().encode("utf8"),
            base64.b64encode(buffer_.getvalue()),
        )
        ...
        res = client.index(
            index="web-%s" % lang, doc_type="page", id=url_id, body=url["_source"]
        )
        return 1
