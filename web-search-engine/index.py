...
def index_job(link):
    ...
    process.crawl(
        crawler.SingleSpider, start_urls=[link], es_client=client, redis_conn=redis_conn
    )
...
def explore_job(link):
    ...
    res = client.index(
        ...
        doc_type="domain",
        id=domain,
        body={"homepage": link, "domain": domain, "last_crawl": datetime.now()},
    )
...
def reference_job(link, email):
    ...
    res = client.index(
        ...
        doc_type="domain",
        id=domain,
        body={"homepage": link, "domain": domain, "email": email},
    )
...
def search():
    ...
    # analyze user query
    groups = re.search(
        "(site:(?P<domain>[^ ]+))?( ?(?P<query>.*))?", data["query"]
    ).groupdict()
    if groups.get("query", False) and groups.get("domain", False):
        ...
        response = client.search(
            index="web-*",
            doc_type="page",
            body=query.domain_expression_query(groups["domain"], groups["query"]),
            from_=start,
            size=hits,
        )
        results = [
            format_result(hit["_source"], hit.get("highlight", None))
        ...
        response = client.search(
            index="web-*",
            doc_type="page",
            body=query.domain_query(groups["domain"]),
            from_=start,
            size=hits,
        )
        results = [
            format_result(hit["_source"], None) for hit in response["hits"]["hits"]
        ]
        total = response["hits"]["total"]
        ...
        # expression query
        response = client.search(
            index="web-*", doc_type="page", body=query.expression_query(groups["query"])
        )
        results = []
        ...
        results = [
            result[0]
            for result in sorted(results, key=lambda result: result[1], reverse=True)
        ]
        total = len(results)
