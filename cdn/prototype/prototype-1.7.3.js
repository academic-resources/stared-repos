    ...
    (function () {
        ...
        function inspect(useDoubleQuotes) {
            var escapedString = this.replace(
                ...
                function (character) {
                    ...
                    return (
                        "\\u00" + character.charCodeAt().toPaddedString(2, 16)
                    );
                }
            );
            if (useDoubleQuotes)
        ...
        function evalJSON(sanitize) {
            ...
            throw new SyntaxError(
                "Badly formed JSON string: " + this.inspect()
            );
        }
...
var Enumerable = (function () {
    ...
    function partition(iterator, context) {
        ...
        var trues = [],
            falses = [];
        this.each(function (value, index) {
            (iterator.call(context, value, index, this) ? trues : falses).push(
                value
            );
        }, this);
        return [trues, falses];
    ...
    function concat(_) {
        ...
        for (var i = 0, length = items.length; i < length; i++) {
            ...
            if (Object.isArray(item) && !("callee" in item)) {
                for (
                    var j = 0, arrayLength = item.length;
                    j < arrayLength;
                    j++
                ) {
            ...
            return this.inject([], function (results, pair) {
                ...
                if (values && typeof values == "object") {
                    if (Object.isArray(values)) {
                        var queryValues = [];
                        for (
                            var i = 0, len = values.length, value;
                            i < len;
                            i++
                        ) {
                            value = values[i];
...
Ajax.Updater = Class.create(Ajax.Request, {
    initialize: function ($super, container, url, options) {
        this.container = {
            ...
            failure:
                container.failure || (container.success ? null : container),
        };
...
(function (GLOBAL) {
    ...
    function Element(tagName, attributes) {
        ...
        if (!ELEMENT_CACHE[tagName])
            ELEMENT_CACHE[tagName] = Element.extend(
                document.createElement(tagName)
            );
...
(function () {
    ...
    function clonePosition(element, source, options) {
        ...
        if (options.setLeft)
            styles.left =
                p[0] + pageXY.x - delta[0] + options.offsetLeft + "px";
        if (options.setTop)
    ...
    function findElement(event, expression) {
        ...
        while (element) {
            if (
                ...
                selector.match(element, expression)
            )
                return Element.extend(element);
            element = element.parentNode;
        }
