        ...
        $.widget("mobile.h2linker", {
            ...
            _create: function () {
                var self = this,
                    bodyid = "ui-page-top",
                    panel =
                        "<div data-role='panel' class='jqm-nav-panel jqm-quicklink-panel' data-position='right' data-display='overlay' data-theme='a'><ul data-role='listview' data-inset='false' data-theme='a' data-divider-theme='a' data-icon='false' class='jqm-list'><li data-role='list-divider'>Quick Links</li></ul></div>",
                    first = true,
                ...
                this.element.find("div.jqm-content>h2").each(function () {
                    ...
                    if (!first) {
                        $(this).before(
                            "<a href='#" +
                                bodyid +
                                "' class='jqm-deeplink ui-icon-carat-u ui-alt-icon'>Top</a>"
                        );
                    } else {
                        $(this).before(
                            "<a href='#' data-ajax='false' class='jqm-deeplink jqm-open-quicklink-panel ui-icon-carat-l ui-alt-icon'>Quick Links</a>"
                        );
                    }
                    first = false;
                });
                ...
                this._on(document, {
                    pagebeforechange: function () {
                        this.element
                            .find(".jqm-quicklink-panel")
                            .panel("close");
                        this.element
                            .find(".jqm-quicklink-panel .ui-btn-active")
...
if (
    location.protocol.substr(0, 4) === "file" ||
        ...
        $("a[href$='/'], a[href='.'], a[href='..']")
            ...
            .each(function () {
                if (!$(this).attr("href").match("http")) {
                    this.href =
                        $(this).attr("href").replace(/\/$/, "") + "/index.html";
                }
            });
    };
    ...
    $.ajax({
        ...
        $(document).on("mobileinit", function () {
            ...
            message
                .append(
                    "<h3>Note: Navigation may not work if viewed locally</h3>"
                )
                .append(
                    "<p>The Ajax-based navigation used throughout the jQuery Mobile docs may need to be viewed on a web server to work in certain browsers. If you see an error message when you click a link, please try a different browser.</p>"
                );
...
$(document).on("pagecreate", ".jqm-demos", function (event) {
    var search,
        ...
        searchContents = $(".jqm-search ul.jqm-list").find(
            "li:not(.ui-collapsible)"
        ),
    ...
    $(this)
        .find(".jqm-search-results-wrap ul.jqm-list")
        .html(searchContents)
        .listview({
            inset: true,
            theme: null,
    ...
    if ($(event.target).hasClass("jqm-demos-search-results")) {
        ...
        setTimeout(function () {
            ...
            e.which = 65;
            $(that)
                .find(".jqm-content .jqm-search-results-wrap input")
                .val(search)
                .trigger(e)
                .trigger("change");
        }, 0);
...
$(document).one("pagecreate", ".jqm-demos", function (event) {
    $(this)
        .find(".jqm-search-results-list li, .jqm-search li")
        .each(function () {
            ...
            $(this)
                .find("a")
                .append(
                    "<span class='jqm-search-results-keywords ui-li-desc'>" +
                        text +
                        "</span>"
                );
        });
...
jQuery.fn.removeHighlight = function () {
    return this.find("span.jqm-search-results-highlight")
        .each(function () {
    ...
    (function ($, undefined) {
        $.widget("mobile.listview", $.mobile.listview, {
            ...
            submitTo: function () {
                ...
                form.attr("method", "get").attr(
                    "action",
                    this.options.submitTo
                );

                url =
                    this.options.submitTo +
                    "?search=" +
                    this.element.parent().find("input").val();
            ...
            enterToNav: function () {
                ...
                form.append(
                    "<button type='submit' data-icon='carat-r' data-inline='true' class='ui-hidden-accessible' data-iconpos='notext'>Submit</button>"
                )
                    .parent()
                    .trigger("create");
            ...
            handleKeyUp: function (e) {
                ...
                if (e.which === $.ui.keyCode.DOWN) {
                    if (this.element.find("li.ui-btn-active").length === 0) {
                        ...
                        this.element
                            .find("li.ui-btn-active a")
                            .toggleClass("ui-btn-active");
                        this.element
                            .find("li.ui-btn-active")
                            .toggleClass("ui-btn-active")
                            .next()
                            .toggleClass("ui-btn-active")
                            .find("a")
...
function attachPopupHandler(popup, sources) {
    popup.one("popupbeforeposition", function () {
        var collapsibleSet = popup.find("[data-role='collapsibleset']"),
            collapsible,
            pre;

        $.each(sources, function (idx, options) {
            collapsible = $(
                "<div data-role='collapsible' data-collapsed='true' data-theme='" +
                    options.theme +
                    "' data-iconpos='right' data-collapsed-icon='carat-l' data-expanded-icon='carat-d' data-content-theme='b'>" +
                    "<h1>" +
                    options.title +
                    "</h1>" +
                    "<pre class='brush: " +
            ...
            pre.append(options.data.replace(/</gim, "&lt;"));
            collapsible
...
function getSnippet(type, selector, source) {
    ...
    if (!text && selector) {
        ...
        $.ajax(absUrl, { async: false, dataType: "text" }).success(function (
            data,
            textStatus,
            jqXHR
        ) {
            ...
            if (hash) {
                text = $("<div></div>")
                    .append($(data).find(hash).contents().clone())
                    .html();
            }
        });
    }
...
$(document).bind("pagebeforechange", function (e, data) {
    ...
    if (data.options && data.options.role === "popup" && data.options.link) {
        ...
        if (sources) {
            popup = $(
                "<div id='jqm-view-source' class='jqm-view-source' data-role='popup' data-theme='none' data-position-to='window'>" +
...
function makeButton() {
    ...
    a.className =
        "jqm-view-source-link ui-btn ui-corner-all ui-btn-inline ui-mini";
...
$.fn.viewSourceCode = function () {
    return $(this).each(function () {
        var button = makeButton(),
            self = $(this),
            snippetSource = self
                .parents(".ui-page,:jqmData(role='page')")
                .add($("head")),
        ...
        if (self.is("[data-demo-html]")) {
            if (self.attr("data-demo-html") === "true") {
                ...
                data = $("<div></div>")
                    .append($(self.attr("data-demo-html")).clone())
                    .html();
            }
            sources.push({
                title: "HTML",
...
$(document).on("pagebeforecreate", "[data-role='page']", function () {
    $(this)
        .find(
            "[data-demo-html], [data-demo-js], [data-demo-css], [data-demo-php]"
        )
        .viewSourceCode();
...
$(document).on("pagecreate", function (e) {
    ...
    $(e.target).delegate(
        ".jqm-view-source .ui-collapsible",
        "expand",
        function () {
            $(this)
                .parents(":mobile-popup")
    ...
    $(e.target).delegate(
        ...
        function () {
            ...
            $(".ui-collapsible:not(.ui-collapsible-collapsed) .gutter", this)
                .find(".line")
                .css("height", "");

            $(".ui-collapsible:not(.ui-collapsible-collapsed) .code", this)
                .find(".line")
                .each(function () {
                    if ($(this).height() !== 16) {
                        var height = $(this).height(),
                            linenumber =
                                ".number" +
                                /number(\w+)/.exec(this.className)[1],
                            gutter = $(this)
                                .parents("tr")
                                .find("td.gutter")
                                .first(),
