(function ($, undefined) {
    ...
    $.widget("mobile.popup", $.mobile.popup, {
        ...
        _alignmentToCoeffs: function (alignment) {
            return {
                originCoeff: alignment < 0 ? 0 : alignment <= 1 ? alignment : 1,
                popupCoeff:
                    alignment < 0
                        ? alignment
                        : alignment <= 1
        ...
        _alignedCoord: function (start, coeffs, originSize, popupSize) {
            return (
                ...
                start +
                // Apply lignment
                coeffs.originCoeff * originSize +
                coeffs.popupCoeff * popupSize +
        ...
        _desiredCoords: function (options) {
            ...
            if (alignment && options.positionTo === "origin" && this._ui.link) {
                ...
                clampInfo.menuSize.cx = Math.min(
                    clampInfo.menuSize.cx,
                    clampInfo.rc.cx
                );
                ...
                options.x = this._alignedCoord(
                    linkBox.x,
                    alignment.x,
                    linkBox.cx,
                    clampInfo.menuSize.cx
                );
                options.y = this._alignedCoord(
                    linkBox.y,
                    alignment.y,
                    linkBox.cy,
                    clampInfo.menuSize.cy
                );
            }
