...
(function (global, factory) {
    ...
    function setPath(obj, path, val) {
        ...
        for (var i = 0, l = path.length; i < l; i++) {
            ...
            key = path[i];
            if (key.charAt(0) === "*") {
                key = parseExpression(key.slice(1)).get.call(
                    original,
                    original
                );
            }
    ...
    var vFor = {
                ...
                if (frag.reused && !frag.staggerCb) {
                    ...
                    if (
                        ...
                        (!currentPrev ||
                            ...
                            findPrevFrag(currentPrev, start, this.id) !==
                                targetPrev)
                    ) {
        ...
        insert: function insert(frag, index, prevEl, inDocument) {
            ...
            if (inDocument && staggerAmount) {
                ...
                if (!anchor) {
                    anchor = frag.staggerAnchor =
                        createAnchor("stagger-anchor");
        ...
        getStagger: function getStagger(frag, index, total, type) {
            ...
            return hook
                ...
                : index *
                      parseInt(this.params[type] || this.params.stagger, 10);
        },
            ...
            while (i--) {
                ...
                if (raw === null) {
                    ...
                    if (prop.mode === propBindingModes.ONE_TIME) {
                        // one time binding
                        value = (scope || vm._context || vm).$get(
                            prop.parentPath
                        );
                    ...
                    value =
                        stripped === raw ? toBoolean(toNumber(raw)) : stripped;
                    initProp(vm, prop, value);
                } else {
        ...
        for (var i = 0, j = attrs.length; i < j; i++) {
            ...
            if ((matched = name.match(dirAttrRE))) {
                ...
                if (def && def.terminal) {
                    if (
                        !termDef ||
                        (def.priority || DEFAULT_TERMINAL_PRIORITY) >
                            termDef.priority
                    ) {
                        termDef = def;
                        rawName = attr.name;
    ...
    function lifecycleMixin(Vue) {
        ...
        Vue.prototype._bindDir = function (
            descriptor,
            node,
            host,
            scope,
    ...
    function miscMixin(Vue) {
        ...
        Vue.prototype._applyFilters = function (
            value,
            oldValue,
            filters,
            write
