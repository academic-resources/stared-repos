(function (global, factory) {
    ...
    function determinant(rows, rank, rowStart, rowMask, colMask, detCache) {
        ...
        if (rank === 1) {
            ...
            var colStart = Math.round(
                Math.log(((1 << fullRank) - 1) & ~colMask) / LN2
            );
    ...
    function calculateZrXY(el, e, out) {
        ...
        if (el.getBoundingClientRect && env$1.domSupported) {
            ...
            if (el.nodeName.toUpperCase() === "CANVAS") {
                ...
                var transformer = preparePointerTransformer(
                    prepareCoordMarkers(el, saved),
                    saved
                );
                if (transformer) {
        ...
        _recognize: function (event) {
            for (var eventName in recognizers) {
                if (recognizers.hasOwnProperty(eventName)) {
                    var gestureInfo = recognizers[eventName](
                        this._track,
                        event
                    );
                    if (gestureInfo) {
    ...
    Handler.prototype = {
        ...
        mousemove: function (event) {
            ...
            proxy.setCursor &&
                proxy.setCursor(
                    hoveredTarget ? hoveredTarget.cursor : "default"
                );
        ...
        dispatchToElement: function (targetInfo, eventName, event) {
            ...
            while (el) {
                el[eventHandler] &&
                    (eventPacket.cancelBubble = el[eventHandler].call(
                        el,
                        eventPacket
                    ));
    ...
    transformableProto.updateTransform = function () {
        ...
        if (globalScaleRatio != null && globalScaleRatio !== 1) {
            ...
            var sx =
                ((scaleTmp[0] - relX) * globalScaleRatio + relX) /
                    scaleTmp[0] || 0;
            var sy =
                ((scaleTmp[1] - relY) * globalScaleRatio + relY) /
                    scaleTmp[1] || 0;
    ...
    Clip.prototype = {
        ...
        step: function (globalTime, deltaTime) {
            ...
            var easingFunc =
                typeof easing$$1 === "string" ? easing[easing$$1] : easing$$1;
            var schedule =
                typeof easingFunc === "function"
                    ? easingFunc(percent)
                    : percent;
        ...
        if (colorArr) {
            for (var i = 0; i < 3; i++) {
                if (level < 0) {
                    ...
                    colorArr[i] =
                        ((255 - colorArr[i]) * level + colorArr[i]) | 0;
                }
                if (colorArr[i] > 255) {
                    colorArr[i] = 255;
                } else if (color[i] < 0) {
    ...
    Style.prototype = {
        ...
        bind: function (ctx, el, prevEl) {
            ...
            var notCheckCache =
                !prevStyle || ctx.__attrCachedBy !== ContextCachedBy.STYLE_BIND;
            ...
            for (var i = 0; i < STYLE_COMMON_PROPS.length; i++) {
                ...
                if (
                    notCheckCache ||
                    style[styleName] !== prevStyle[styleName]
                ) {
                    // FIXME Invalid property value will cause style leak from previous element.
                    ctx[styleName] = fixShadow(
                        ctx,
                        styleName,
                        style[styleName] || prop[1]
                    );
                }
            }
        ...
        clear: function (clearAll, clearColor) {
            ...
            if (clearColor && clearColor !== "transparent") {
                ...
                // Pattern
                    ...
                    clearColorGradientOrPattern =
                        Pattern.prototype.getCanvasPattern.call(
                            clearColor,
                            ctx
                        );
                }
    ...
    function parseRichText(text, style) {
        ...
        while ((result = STYLE_REG.exec(text)) != null) {
            ...
            if (matchedIndex > lastIndex) {
                pushTokens(
                    contentBlock,
                    text.substring(lastIndex, matchedIndex)
                );
            }
        ...
        for (var i = 0; i < lines.length; i++) {
            ...
            for (var j = 0; j < line.tokens.length; j++) {
                ...
                var tokenStyle =
                    (token.styleName && style.rich[token.styleName]) || {};
                ...
                if (
                    ...
                    if (tokenWidthNotSpecified) {
                        ...
                        var textBackgroundColor =
                            tokenStyle.textBackgroundColor;
                        var bgImg =
                            textBackgroundColor && textBackgroundColor.image;
                    ...
                    var remianTruncWidth =
                        truncateWidth != null
                            ? truncateWidth - lineWidth
                            : null;

                    if (
                        remianTruncWidth != null &&
                        remianTruncWidth < tokenWidth
                    ) {
                        if (
                            !tokenWidthNotSpecified ||
    ...
    function pushTokens(block, str, styleName) {
        ...
        for (var i = 0; i < strs.length; i++) {
            ...
            if (!i) {
                var tokens = (
                    lines[lines.length - 1] || (lines[0] = { tokens: [] })
                ).tokens;
    ...
    function renderPlainText(hostEl, ctx, text, style, rect, prevEl) {
        ...
        if (prevEl !== WILL_BE_RESTORED) {
            ...
            ctx.__attrCachedBy = needDrawBg
                ? ContextCachedBy.NONE
                : ContextCachedBy.PLAIN_TEXT;
        }
        ...
        var strokeWidthChanged =
            !checkCache || textStrokeWidth !== textStrokeWidthPrev;
        var strokeChanged =
            !checkCache ||
            strokeWidthChanged ||
            style.textStroke !== prevStyle.textStroke;
    ...
    function renderRichText(hostEl, ctx, text, style, rect, prevEl) {
        ...
        if (!contentBlock || hostEl.__dirtyText) {
            contentBlock = hostEl.__textCotentBlock = parseRichText(
                text,
                style
            );
        }
    ...
    function getBoxPosition(out, hostEl, style, rect) {
        ...
        if (rect) {
            ...
            if (textPosition instanceof Array) {
                ...
                var res =
                    hostEl && hostEl.calculateTextPosition
                        ? hostEl.calculateTextPosition(
    ...
    Painter.prototype = {
        ...
        _doPaintList: function (list, paintAll) {
            ...
            for (var k = 0; k < layerList.length; k++) {
                ...
                var clearColor =
                    layer.zlevel === this._zlevelList[0]
        ...
        _doPaintEl: function (el, currentLayer, forcePaint, scope) {
            ...
            if (
                ...
                !(
                    el.culling &&
                    isDisplayableCulled(el, this._width, this._height)
                )
            ) {
    ...
    function eventNameFix(name) {
        return name === "mousewheel" && env$1.browser.firefox
            ? "DOMMouseScroll"
            : name;
    }
    ...
    function initDomHandler(instance) {
        ...
        each(mouseHandlerNames, function (name) {
            instance._handlers[name] = makeMouseHandler(
                domHandlers[name],
                instance
            );
        });
    ...
    var ZRender = function (id, dom, opts) {
        ...
        var handerProxy =
            !env$1.node && !env$1.worker
                ? new HandlerDomProxy(painter.getViewportRoot())
                : null;
    ...
    function cubicRootAt(p0, p1, p2, p3, val, roots) {
        ...
        if (isAroundZero(A) && isAroundZero(B)) {
            ...
            if (isAroundZero(disc)) {
                ...
                var t2 =
                    (-b + ASqrt * (tmp + THREE_SQRT * Math.sin(theta))) /
                    (3 * a);
                var t3 =
                    (-b + ASqrt * (tmp - THREE_SQRT * Math.sin(theta))) /
                    (3 * a);
    ...
    PathProxy.prototype = {
        ...
        setScale: function (sx, sy, segmentIgnoreThreshold) {
            ...
            this._ux =
                mathAbs(segmentIgnoreThreshold / devicePixelRatio / sx) || 0;
            this._uy =
                mathAbs(segmentIgnoreThreshold / devicePixelRatio / sy) || 0;
        },
        ...
        _dashedBezierTo: function (x1, y1, x2, y2, x3, y3) {
            ...
            for (t = 0; t < 1; t += 0.1) {
                dx =
                    cubicAt$$1(x0, x1, x2, x3, t + 0.1) -
                    cubicAt$$1(x0, x1, x2, x3, t);
                dy =
                    cubicAt$$1(y0, y1, y2, y3, t + 0.1) -
            ...
            switch (cmd) {
                ...
                case CMD$1.L:
                    if (isStroke) {
                        ...
                        w +=
                            windingLine(xi, yi, data[i], data[i + 1], x, y) ||
                            0;
                    }
                    xi = data[i++];
                    yi = data[i++];
                ...
                case CMD$1.R:
                    ...
                    if (isStroke) {
                        if (
                            containStroke$1(x0, y0, x1, y0, lineWidth, x, y) ||
        ...
        getBoundingRect: function () {
            ...
            if (style.hasStroke()) {
                ...
                if (this.__dirty || needsUpdateRect) {
                    ...
                    var lineScale = style.strokeNoScale
                        ? this.getLineScale()
                        : 1;
            ...
            if (rect.contain(x, y)) {
                ...
                if (style.hasStroke()) {
                    ...
                    if (lineScale > 1e-10) {
                        ...
                        if (
                            containStroke(pathData, lineWidth / lineScale, x, y)
                        ) {
                            return true;
                        }
                    }
    ...
    function subPixelOptimizeLine(outputShape, inputShape, style) {
        ...
        if (round(x1 * 2) === round(x2 * 2)) {
            outputShape.x1 = outputShape.x2 = subPixelOptimize(
                x1,
                lineWidth,
                true
            );
        ...
        if (round(y1 * 2) === round(y2 * 2)) {
            outputShape.y1 = outputShape.y2 = subPixelOptimize(
                y1,
                lineWidth,
                true
            );
    ...
    function subPixelOptimizeRect(outputShape, inputShape, style) {
        ...
        outputShape.width = Math.max(
            subPixelOptimize(originX + originWidth, lineWidth, false) -
                outputShape.x,
            originWidth === 0 ? 0 : 1
        );
    ...
    function parseAttributes(xmlNode, el, defs, onlyInlineStyle) {
        ...
        zrStyle.fill != null &&
            elStyle.set(elFillProp, getPaint(zrStyle.fill, defs));
        zrStyle.stroke != null &&
            elStyle.set(elStrokeProp, getPaint(zrStyle.stroke, defs));
        ...
        each(
            ...
            function (propName) {
                var elPropName =
                    propName === "lineWidth" && isTextEl
                        ? "textStrokeWidth"
                        : propName;
                zrStyle[propName] != null &&
                    elStyle.set(elPropName, parseFloat(zrStyle[propName]));
            }
        );
        ...
        each(
            ...
            function (propName) {
                zrStyle[propName] != null &&
                    elStyle.set(propName, zrStyle[propName]);
            }
        );
    ...
    var transformRegex =
        /(translate|scale|rotate|skewX|skewY|matrix)\(([\-\s0-9\.e,]*)\)/g;
    ...
    function parseStyleAttribute(xmlNode) {
        ...
        for (var svgAttrName in attributesMap) {
            if (
                attributesMap.hasOwnProperty(svgAttrName) &&
                styleList[svgAttrName] != null
            ) {
    ...
    var CompoundPath = Path.extend({
        ...
        beforeBrush: function () {
            ...
            for (var i = 0; i < paths.length; i++) {
                ...
                paths[i].path.setScale(
                    scale[0],
                    scale[1],
                    paths[i].segmentIgnoreThreshold
                );
            }
        },
    ...
    IncrementalDisplayble.prototype.addDisplayable = function (
        displayable,
        notPersistent
    ) {
        if (notPersistent) {
    ...
    IncrementalDisplayble.prototype.addDisplayables = function (
        displayables,
        notPersistent
    ) {
    ...
    IncrementalDisplayble.prototype.brush = function (ctx, prevEl) {
        ...
        for (var i = this._cursor; i < this._displayables.length; i++) {
            ...
            displayable.brush(
                ctx,
                i === this._cursor ? null : this._displayables[i - 1]
            );
        ...
        for (var i = 0; i < this._temporaryDisplayables.length; i++) {
            ...
            displayable.brush(
                ctx,
                i === 0 ? null : this._temporaryDisplayables[i - 1]
            );
            displayable.afterBrush && displayable.afterBrush(ctx);
        }
    ...
    var Trochoid = Path.extend({
        ...
        buildPath: function (ctx, shape) {
            ...
            do {
                theta = (Math.PI / 180) * i;
                x2 =
                    (R + delta * r) * cos$3(theta) -
                    delta * d * cos$3((R / r + delta) * theta) +
                    offsetX;
                y2 =
                    (R + delta * r) * sin$3(theta) -
    ...
    function bindStyle(svgEl, style, isText, el) {
        ...
        if (pathHasStroke(style, isText)) {
            ...
            var strokeScale =
                !isText && style.strokeNoScale ? el.getLineScale() : 1;
            ...
            if (lineDash) {
                ...
                attr(
                    svgEl,
                    "stroke-dashoffset",
                    mathRound(style.lineDashOffset || 0)
                );
            } else {
            ...
            style.miterLimit &&
                attr(svgEl, "stroke-miterlimit", style.miterLimit);
        } else {
        ...
        for (var i = 0; i < dataLength; ) {
            ...
            switch (cmd) {
                ...
                case CMD$3.A:
                    ...
                    var isCircle =
                        isAroundZero$1(dThetaPositive - PI2$4) ||
                        (clockwise ? dTheta >= PI2$4 : -dTheta >= PI2$4);

                    // Mapping to 0~2PI
                    var unifiedTheta =
                        dTheta > 0 ? dTheta % PI2$4 : (dTheta % PI2$4) + PI2$4;
    ...
    var svgTextDrawRectText = function (el, hostRect) {
        ...
        var needTransformTextByHostEl =
            el instanceof Text || style.transformText;
    ...
    Diff.prototype = {
        ...
        extractCommon: function (basePath, newArr, oldArr, diagonalPath) {
            ...
            while (
                newPos + 1 < newLen &&
                oldPos + 1 < oldLen &&
                this.equals(newArr[newPos + 1], oldArr[oldPos + 1])
            ) {
                newPos++;
                oldPos++;
                commonCount++;
    ...
    GradientManager.prototype.update = function (gradient) {
        ...
        Definable.prototype.update.call(this, gradient, function () {
            ...
            if (
                (type === "linear" && tagName === "linearGradient") ||
    ...
    ShadowManager.prototype.add = function (displayable) {
        ...
        dom.setAttribute(
            "id",
            "zr" + this._zrId + "-shadow-" + displayable._shadowDomId
        );
    ...
    var SVGPainter = function (root, storage, opts, zrId) {
        ...
        svgRoot.style.cssText =
            "user-select:none;position:absolute;left:0;top:0;";
                ...
                if (!displayable.invisible) {
                    if (displayable.__dirty) {
                        ...
                        if (displayable.style) {
                            ...
                            this.gradientManager.update(
                                displayable.style.stroke
                            );
            ...
            for (i = 0; i < diff.length; i++) {
                ...
                if (item.added) {
                    for (var k = 0; k < item.count; k++) {
                        ...
                        insertAfter(svgRoot, textSvgElement, svgElement);
                        prevSvgElement =
                            textSvgElement || svgElement || prevSvgElement;

                        // zrender.Text only create textSvgElement.
                        this.gradientManager.addWithoutUpdate(
                            svgElement || textSvgElement,
                            displayable
                        );
                        this.shadowManager.addWithoutUpdate(
                            svgElement || textSvgElement,
                            displayable
                        );
    ...
    function createMethodNotSupport(method) {
        return function () {
            logError$1(
                'In SVG mode painter not support method "' + method + '"'
            );
        };
    }
        ...
        try {
            ...
            doCreateNode = function (tagName) {
                return doc.createElement(
                    "<zrvml:" + tagName + ' class="zrvml">'
                );
            };
        } catch (e) {
            doCreateNode = function (tagName) {
                return doc.createElement(
                    "<" + tagName + ' xmlns="' + urn + '" class="zrvml">'
                );
            };
        }
    }
    ...
    if (!env$1.canvasSupported) {
        ...
        var getZIndex = function (zlevel, z, z2) {
            // z 的取值范围为 [0, 1000]
            return (
                (parseFloat(zlevel) || 0) * ZLEVEL_BASE +
                (parseFloat(z) || 0) * Z_BASE +
                z2
            );
        };
        ...
        var updateFillAndStroke = function (vmlEl, type, style, zrEl) {
            ...
            if (
                ...
                isFill
                    ? updateFillNode(el, style, zrEl)
                    : updateStrokeNode(el, style);
                append(vmlEl, el);
            } else {
            ...
            for (i = 0; i < dataLength; ) {
                ...
                switch (cmd) {
                    ...
                    case A:
                        ...
                        str.push(
                            type,
                            round$1(((cx - rx) * sx + x) * Z - Z2),
                            comma,
                            round$1(((cy - ry) * sy + y) * Z - Z2),
                            comma,
                            round$1(((cx + rx) * sx + x) * Z - Z2),
                            comma,
                            round$1(((cy + ry) * sy + y) * Z - Z2),
                            comma,
                            round$1((x0 * sx + x) * Z - Z2),
                            comma,
                            round$1((y0 * sy + y) * Z - Z2),
                            comma,
                            round$1((x1 * sx + x) * Z - Z2),
                            comma,
        ...
        ZImage.prototype.brushVML = function (vmlRoot) {
            ...
            if (hasRotation) {
                ...
                transformFilter.push(
                    ...
                    comma,
                    "Dx=",
                    round$1(x * scaleX + m[4]),
                    comma,
                    "Dy=",
                    round$1(y * scaleY + m[5])
                );

                vmlElStyle.padding =
                    "0 " + round$1(maxX) + "px " + round$1(maxY) + "px 0";
                // FIXME DXImageTransform 在 IE11 的兼容模式下不起作用
                vmlElStyle.filter =
                    imageTransformPrefix +
                    ".Matrix(" +
                    transformFilter.join("") +
                    ", SizingMethod=clip)";
            } else {
                if (m) {
                    x = x * scaleX + m[4];
            ...
            if (hasCrop) {
                ...
                if (!(ow && oh)) {
                    ...
                    tmpImage.onload = function () {
                        ...
                        imageELStyle.width =
                            round$1((scaleX * ow * dw) / sw) + "px";
                        imageELStyle.height =
                            round$1((scaleY * oh * dh) / sh) + "px";
        ...
        var drawRectText = function (vmlRoot, rect, textRect, fromTextEl) {
            ...
            if (m && fromTextEl) {
                ...
                // Text position
                skewEl.offset =
                    (round$1(coords[0]) || 0) + "," + (round$1(coords[1]) || 0);
    ...
    VMLPainter.prototype = {
        ...
        _getWidth: function () {
            ...
            return (
                ((root.clientWidth || parseInt10$2(stl.width)) -
                    parseInt10$2(stl.paddingLeft) -
                    parseInt10$2(stl.paddingRight)) |
                0
            );
        },
        ...
        _getHeight: function () {
            ...
            return (
                ((root.clientHeight || parseInt10$2(stl.height)) -
                    parseInt10$2(stl.paddingTop) -
                    parseInt10$2(stl.paddingBottom)) |
                0
            );
        },
    };
    ...
    function createMethodNotSupport$1(method) {
        return function () {
            logError$1(
                'In IE8.0 VML mode painter not support method "' + method + '"'
            );
        };
    }
