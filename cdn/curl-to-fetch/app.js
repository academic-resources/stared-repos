            ...
            case !!arg:
                switch (state) {
                    ...
                    case "data-binary":
                        ...
                        out.body =
                            state === "data" ? addArg(out.body, arg) : out.body;
                        state = "";
                        break;
                    case "form-string":
                        formString = arg;
...
const formatOpts = (indent, opts) =>
    ...
    Object.keys(opts)
        ...
        .map((key) => {
            ...
            if (!rawVal) return;
            if (rawVal[VariableSymbol])
                return _class.var(spacer + rawVal[VariableSymbol]);
            const legalKey = matchLegalChars.test(key)
                ? _class.key(key)
                : _class.key.text(key);

            return `${indent}${legalKey}: ${
                isObj(rawVal)
...
const printResult = (url, opts, extra) =>
    (_result.innerHTML = extra
...
const update = () => {
    try {
        ...
        // update render
        const { method, headers, url, body, formData } = parseCurl(
            _textArea.value,
            _clean.checked
        );
        const opts = {};
        ...
        if (isNotEmpty(formData)) {
            ...
            printResult(
                ...
                formData.reduce(
                    (acc, [key, value]) =>
                        `${acc}${_class.var("body")}.${_class.keyword(
                            "append"
                        )}(${_class.value.text(key)}, ${_class.value.text(
                            value
                        )})\n`,
                    `${_class.url("const")} ${_class.var("body")} ${_class.op(
                        "= new"
                    )} ${_class.url("FormData")}\n`
                )
            );
