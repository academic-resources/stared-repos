...
class UAChecker {
    ...
    getOS() {
        // 获取系统
        if (
            UAChecker.regexTester(/windows|win32|win64|wow32|wow64/g, this.ua)
        ) {
            this.os = "Windows";
    ...
    getOSVersion() {
        ...
        switch (this.os) {
            case "Windows":
                if (
                    UAChecker.versionTester(
                        /windows nt 5.0|windows 2000/g,
                        this.ua
                    )
                ) {
                    this.osVersion = "2000";
                    ...
                    UAChecker.versionTester(
                        /windows nt 6.0|windows vista/g,
                        this.ua
                    )
                ) {
                ...
                break;
            case "macOS":
                this.osVersion = UAChecker.versionTester(
                    /os x [\d._]+/g,
                    this.ua
                );
                break;
            ...
            case "Linux":
                ...
                if (this.ua.match(/x11; [\w.]+;/g)) {
                    const version = this.ua
                        .match(/x11; [\w.]+;/g)[0]
                        .replace(/x11; /, "")
                        .replace(/;/, "");
                    this.osVersion = `Linux ${version[0].toUpperCase()}${version.slice(
                        1
                    )}`;
                }
                break;
    ...
    getPlatform() {
        ...
        if (OS === "Windows" || OS === "macOS" || OS === "Linux") {
            ...
            OS === "Android" ||
            OS === "iOS" ||
            UAChecker.regexTester(/mobile/g, this.ua)
        ) {
            // 移动端
    ...
    getBrowser() {
        ...
        if (UAChecker.regexTester(/applewebkit/g, this.ua)) {
            ...
            if (
                UAChecker.regexTester(/chrome/g, this.ua) ||
                UAChecker.regexTester(/crios/g, this.ua)
            ) {
                this.browser = "Chrome";
            }
            if (
                UAChecker.regexTester(/safari/g, this.ua) &&
                !UAChecker.regexTester(/chrome/g, this.ua) &&
                !UAChecker.regexTester(/crios/g, this.ua)
            ) {
                this.browser = "Safari";
            ...
            UAChecker.regexTester(/gecko/g, this.ua) &&
            UAChecker.regexTester(/firefox/g, this.ua)
        ) {
            // gecko 内核
            this.engine = "Gecko";
            ...
            UAChecker.regexTester(/opr/g, this.ua) ||
            UAChecker.regexTester(/presto/g, this.ua)
        ) {
            // presto 内核
            this.engine = "presto";
            this.browser = "Opera";
    ...
    getBrowserVersion() {
        // 获取浏览器版本
        const browser =
            this.browser !== "unknown" ? this.browser : this.getBrowser();
        switch (browser) {
            case "Chrome":
                this.browserVersion = UAChecker.versionTester(
                    /chrome\/[\d._]+/g,
                    this.ua
                );
