...
function AppML(container, appsrc, appmlname) {
        ...
        for (i = appmlObjects.length - 1; i > -1; i -= 1) {
            if (appmlObjects[i].controller) {
                if (typeof window[appmlObjects[i].controller] == "undefined") {
                    ...
                    if (
                        window[appmlObjects[i].controller](appmlObjects[i]) ==
                        -1
                    ) {
                        continue;
                    } else {
    ...
    this.getData = function (nav, id) {
        ...
        if (this.dataSource) {
            if (
                ...
                errmsg =
                    "The datasource " + this.dataSource + " does not exist";
            }
                    ...
                    for (ii = 0; ii < l; ii += 1) {
                        ...
                        for (iii = 0; iii < ll; iii += 1) {
                            if (
                                ...
                                a[ii].value =
                                    this.data.records[i - 1][
                                        this.data.itemNames[iii]
                                    ];
                            }
                        }
                    }
    ...
    this.needleInHaystack = function (elmnt, typ, repeatX, x) {
        ...
        while (pos1 > -1) {
            ...
            if (this.display.value != undefined) {
                r = "{{" + needleToReplace + "}}";
                if (typ == "attribute") {
                    rowClone.value = rowClone.value.replace(
                        r,
                        this.display.value
                    );
                } else {
                ...
                if (b[iii].outerHTML.indexOf(r) > -1) {
                    if (b[iii].tagName !== "TBODY" && b[iii].tagName !== "TR") {
                        if (b[iii].innerHTML.indexOf(r) > -1) {
                            b[iii].innerHTML = b[iii].innerHTML.replace(
                                r,
                                result
                            );
                        }
                        ...
                        if (b[iii].hasAttributes()) {
                            ...
                            for (iiii = 0; iiii < lll; iiii += 1) {
                                if (bb[iiii].value.indexOf(r) > -1) {
                                    bb[iiii].value = bb[iiii].value.replace(
                                        r,
                                        result
                                    );
                                }
    ...
    this.setCommandsClick = function () {
        ...
        for (i = 1; i <= 4; i += 1) {
            ...
            if (i === 4) {
                ...
                if (
                    (to === count &&
                        Number(from) - 1 === Number(to) - Number(fc)) ||
    ...
    this.displayMessage = function (txt) {
        var messageDIV =
            this.messageDIV || this.getElement("appmlmessage") || "";
        if (messageDIV === "") {
            if (txt === "") {
                return;
            }
    ...
    function removeClass(x, removeclass) {
        ...
        if (clsnm === removeclass) {
            ...
            if (clsnm.indexOf(removeclass) === 0) {
                ...
                if (
                    clsnm.indexOf(removeclass) + removeclass.length ===
                    clsnm.length
                ) {
                    clsnm = clsnm.replace(" " + removeclass, "");
                } else {
    ...
    this.appmlQuery = function () {
        ...
        for (i = 0; i < l; i += 1) {
            ...
            if (this.getElement("appml_query_" + qfname, queryElmnt)) {
                this.getElement("appml_query_" + qfname, queryElmnt).value =
                    qvalue;
            }
            if (qoper !== "") {
                if (this.getElement("appml_operator_" + qfname, queryElmnt)) {
                    this.getElement(
                        "appml_operator_" + qfname,
                        queryElmnt
                    ).value = qoper;
                }
            }
        ...
        for (i = 0; i < l; i += 1) {
            if (x[i].id) {
                if (x[i].id.substr(0, 12) === "appml_query_") {
                    ...
                    fvalue = x[i].value;
                    foper = Number(
                        this.getElement("appml_operator_" + fname).value
                    );
