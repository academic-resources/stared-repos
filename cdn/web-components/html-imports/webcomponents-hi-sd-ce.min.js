/**
@license @nocompile
Copyright (c) 2018 The Polymer Project Authors. All rights reserved.
This code may only be used under the BSD style license found at http://polymer.github.io/LICENSE.txt
The complete set of authors may be found at http://polymer.github.io/AUTHORS.txt
The complete set of contributors may be found at http://polymer.github.io/CONTRIBUTORS.txt
Code distributed by Google as part of the polymer project is also
subject to an additional IP rights grant found at http://polymer.github.io/PATENTS.txt
*/
(function () {
    /*

 Copyright (c) 2016 The Polymer Project Authors. All rights reserved.
 This code may only be used under the BSD style license found at http://polymer.github.io/LICENSE.txt
 The complete set of authors may be found at http://polymer.github.io/AUTHORS.txt
 The complete set of contributors may be found at http://polymer.github.io/CONTRIBUTORS.txt
 Code distributed by Google as part of the polymer project is also
 subject to an additional IP rights grant found at http://polymer.github.io/PATENTS.txt
*/
    "use strict";
    var p,
        aa =
            "function" == typeof Object.defineProperties
                ? Object.defineProperty
                : function (a, b, c) {
                      a != Array.prototype &&
                          a != Object.prototype &&
                          (a[b] = c.value);
                  },
        q =
            "undefined" != typeof window && window === this
                ? this
                : "undefined" != typeof global && null != global
                ? global
                : this;
    function ba() {
        ba = function () {};
        q.Symbol || (q.Symbol = ca);
    }
    var ca = (function () {
        var a = 0;
        return function (b) {
            return "jscomp_symbol_" + (b || "") + a++;
        };
    })();
    function ea() {
        ba();
        var a = q.Symbol.iterator;
        a || (a = q.Symbol.iterator = q.Symbol("iterator"));
        "function" != typeof Array.prototype[a] &&
            aa(Array.prototype, a, {
                configurable: !0,
                writable: !0,
                value: function () {
                    return fa(this);
                },
            });
        ea = function () {};
    }
    function fa(a) {
        var b = 0;
        return ha(function () {
            return b < a.length ? { done: !1, value: a[b++] } : { done: !0 };
        });
    }
    function ha(a) {
        ea();
        a = { next: a };
        a[q.Symbol.iterator] = function () {
            return this;
        };
        return a;
    }
    function ia(a) {
        ea();
        ba();
        ea();
        var b = a[Symbol.iterator];
        return b ? b.call(a) : fa(a);
    }
    function ja(a) {
        for (var b, c = []; !(b = a.next()).done; ) c.push(b.value);
        return c;
    }
    (function (a) {
        function b(a, b) {
            if ("function" === typeof window.CustomEvent)
                return new CustomEvent(a, b);
            var c = document.createEvent("CustomEvent");
            c.initCustomEvent(a, !!b.bubbles, !!b.cancelable, b.detail);
            return c;
        }
        function c(a) {
            if (E) return a.ownerDocument !== document ? a.ownerDocument : null;
            var b = a.__importDoc;
            if (!b && a.parentNode) {
                b = a.parentNode;
                if ("function" === typeof b.closest)
                    b = b.closest("link[rel=import]");
                else for (; !h(b) && (b = b.parentNode); );
                a.__importDoc = b;
            }
            return b;
        }
        function d(a) {
            var b = m(document, "link[rel=import]:not([import-dependency])"),
                c = b.length;
            c
                ? n(b, function (b) {
                      return g(b, function () {
                          0 === --c && a();
                      });
                  })
                : a();
        }
        function e(a) {
            function b() {
                "loading" !== document.readyState &&
                    document.body &&
                    (document.removeEventListener("readystatechange", b), a());
            }
            document.addEventListener("readystatechange", b);
            b();
        }
        function f(a) {
            e(function () {
                return d(function () {
                    return a && a();
                });
            });
        }
        function g(a, b) {
            if (a.__loaded) b && b();
            else if (
                ("script" === a.localName && !a.src) ||
                ("style" === a.localName && !a.firstChild)
            )
                (a.__loaded = !0), b && b();
            else {
                var c = function (d) {
                    a.removeEventListener(d.type, c);
                    a.__loaded = !0;
                    b && b();
                };
                a.addEventListener("load", c);
                (Za && "style" === a.localName) ||
                    a.addEventListener("error", c);
            }
        }
        function h(a) {
            return (
                a.nodeType === Node.ELEMENT_NODE &&
                "link" === a.localName &&
                "import" === a.rel
            );
        }
        function k() {
            var a = this;
            this.a = {};
            this.b = 0;
            this.c = new MutationObserver(function (b) {
                return a.Oa(b);
            });
            this.c.observe(document.head, { childList: !0, subtree: !0 });
            this.loadImports(document);
        }
        function l(a) {
            n(m(a, "template"), function (a) {
                n(
                    m(
                        a.content,
                        'script:not([type]),script[type="application/javascript"],script[type="text/javascript"],script[type="module"]'
                    ),
                    function (a) {
                        var b = document.createElement("script");
                        n(a.attributes, function (a) {
                            return b.setAttribute(a.name, a.value);
                        });
                        b.textContent = a.textContent;
                        a.parentNode.replaceChild(b, a);
                    }
                );
                l(a.content);
            });
        }
        function m(a, b) {
            return a.childNodes.length ? a.querySelectorAll(b) : da;
        }
        function n(a, b, c) {
            var d = a ? a.length : 0,
                e = c ? -1 : 1;
            for (c = c ? d - 1 : 0; c < d && 0 <= c; c += e) b(a[c], c);
        }
        var A = document.createElement("link"),
            E = "import" in A,
            da = A.querySelectorAll("*"),
            $a = null;
        !1 === "currentScript" in document &&
            Object.defineProperty(document, "currentScript", {
                get: function () {
                    return (
                        $a ||
                        ("complete" !== document.readyState
                            ? document.scripts[document.scripts.length - 1]
                            : null)
                    );
                },
                configurable: !0,
            });
        var ve = /(url\()([^)]*)(\))/g,
            we = /(@import[\s]+(?!url\())([^;]*)(;)/g,
            xe = /(<link[^>]*)(rel=['|"]?stylesheet['|"]?[^>]*>)/g,
            I = {
                Ia: function (a, b) {
                    a.href &&
                        a.setAttribute("href", I.V(a.getAttribute("href"), b));
                    a.src &&
                        a.setAttribute("src", I.V(a.getAttribute("src"), b));
                    if ("style" === a.localName) {
                        var c = I.sa(a.textContent, b, ve);
                        a.textContent = I.sa(c, b, we);
                    }
                },
                sa: function (a, b, c) {
                    return a.replace(c, function (a, c, d, e) {
                        a = d.replace(/["']/g, "");
                        b && (a = I.V(a, b));
                        return c + "'" + a + "'" + e;
                    });
                },
                V: function (a, b) {
                    if (void 0 === I.aa) {
                        I.aa = !1;
                        try {
                            var c = new URL("b", "http://a");
                            c.pathname = "c%20d";
                            I.aa = "http://a/c%20d" === c.href;
                        } catch (wg) {}
                    }
                    if (I.aa) return new URL(a, b).href;
                    c = I.Aa;
                    c ||
                        ((c =
                            document.implementation.createHTMLDocument("temp")),
                        (I.Aa = c),
                        (c.la = c.createElement("base")),
                        c.head.appendChild(c.la),
                        (c.ka = c.createElement("a")));
                    c.la.href = b;
                    c.ka.href = a;
                    return c.ka.href || a;
                },
            },
            cc = {
                async: !0,
                load: function (a, b, c) {
                    if (a)
                        if (a.match(/^data:/)) {
                            a = a.split(",");
                            var d = a[1];
                            d =
                                -1 < a[0].indexOf(";base64")
                                    ? atob(d)
                                    : decodeURIComponent(d);
                            b(d);
                        } else {
                            var e = new XMLHttpRequest();
                            e.open("GET", a, cc.async);
                            e.onload = function () {
                                var a =
                                    e.responseURL ||
                                    e.getResponseHeader("Location");
                                a &&
                                    0 === a.indexOf("/") &&
                                    (a =
                                        (location.origin ||
                                            location.protocol +
                                                "//" +
                                                location.host) + a);
                                var d = e.response || e.responseText;
                                304 === e.status ||
                                0 === e.status ||
                                (200 <= e.status && 300 > e.status)
                                    ? b(d, a)
                                    : c(d);
                            };
                            e.send();
                        }
                    else c("error: href must be specified");
                },
            },
            Za =
                /Trident/.test(navigator.userAgent) ||
                /Edge\/\d./i.test(navigator.userAgent);
        k.prototype.loadImports = function (a) {
            var b = this;
            a = m(a, "link[rel=import]");
            n(a, function (a) {
                return b.l(a);
            });
        };
        k.prototype.l = function (a) {
            var b = this,
                c = a.href;
            if (void 0 !== this.a[c]) {
                var d = this.a[c];
                d && d.__loaded && ((a.__import = d), this.g(a));
            } else
                this.b++,
                    (this.a[c] = "pending"),
                    cc.load(
                        c,
                        function (a, d) {
                            a = b.Pa(a, d || c);
                            b.a[c] = a;
                            b.b--;
                            b.loadImports(a);
                            b.D();
                        },
                        function () {
                            b.a[c] = null;
                            b.b--;
                            b.D();
                        }
                    );
        };
        k.prototype.Pa = function (a, b) {
            if (!a) return document.createDocumentFragment();
            Za &&
                (a = a.replace(xe, function (a, b, c) {
                    return -1 === a.indexOf("type=")
                        ? b + " type=import-disable " + c
                        : a;
                }));
            var c = document.createElement("template");
            c.innerHTML = a;
            if (c.content) (a = c.content), l(a);
            else
                for (a = document.createDocumentFragment(); c.firstChild; )
                    a.appendChild(c.firstChild);
            if ((c = a.querySelector("base")))
                (b = I.V(c.getAttribute("href"), b)), c.removeAttribute("href");
            c = m(
                a,
                'link[rel=import],link[rel=stylesheet][href][type=import-disable],style:not([type]),link[rel=stylesheet][href]:not([type]),script:not([type]),script[type="application/javascript"],script[type="text/javascript"],script[type="module"]'
            );
            var d = 0;
            n(c, function (a) {
                g(a);
                I.Ia(a, b);
                a.setAttribute("import-dependency", "");
                if ("script" === a.localName && !a.src && a.textContent) {
                    if ("module" === a.type)
                        throw Error(
                            "Inline module scripts are not supported in HTML Imports."
                        );
                    a.setAttribute(
                        "src",
                        "data:text/javascript;charset=utf-8," +
                            encodeURIComponent(
                                a.textContent +
                                    ("\n//# sourceURL=" +
                                        b +
                                        (d ? "-" + d : "") +
                                        ".js\n")
                            )
                    );
                    a.textContent = "";
                    d++;
                }
            });
            return a;
        };
        k.prototype.D = function () {
            var a = this;
            if (!this.b) {
                this.c.disconnect();
                this.flatten(document);
                var b = !1,
                    c = !1,
                    d = function () {
                        c &&
                            b &&
                            (a.loadImports(document),
                            a.b ||
                                (a.c.observe(document.head, {
                                    childList: !0,
                                    subtree: !0,
                                }),
                                a.Ma()));
                    };
                this.Ra(function () {
                    c = !0;
                    d();
                });
                this.Qa(function () {
                    b = !0;
                    d();
                });
            }
        };
        k.prototype.flatten = function (a) {
            var b = this;
            a = m(a, "link[rel=import]");
            n(a, function (a) {
                var c = b.a[a.href];
                (a.__import = c) &&
                    c.nodeType === Node.DOCUMENT_FRAGMENT_NODE &&
                    ((b.a[a.href] = a),
                    (a.readyState = "loading"),
                    (a.__import = a),
                    b.flatten(c),
                    a.appendChild(c));
            });
        };
        k.prototype.Qa = function (a) {
            function b(e) {
                if (e < d) {
                    var f = c[e],
                        h = document.createElement("script");
                    f.removeAttribute("import-dependency");
                    n(f.attributes, function (a) {
                        return h.setAttribute(a.name, a.value);
                    });
                    $a = h;
                    f.parentNode.replaceChild(h, f);
                    g(h, function () {
                        $a = null;
                        b(e + 1);
                    });
                } else a();
            }
            var c = m(document, "script[import-dependency]"),
                d = c.length;
            b(0);
        };
        k.prototype.Ra = function (a) {
            var b = m(
                    document,
                    "style[import-dependency],link[rel=stylesheet][import-dependency]"
                ),
                d = b.length;
            if (d) {
                var e =
                    Za &&
                    !!document.querySelector(
                        "link[rel=stylesheet][href][type=import-disable]"
                    );
                n(b, function (b) {
                    g(b, function () {
                        b.removeAttribute("import-dependency");
                        0 === --d && a();
                    });
                    if (e && b.parentNode !== document.head) {
                        var f = document.createElement(b.localName);
                        f.__appliedElement = b;
                        f.setAttribute("type", "import-placeholder");
                        b.parentNode.insertBefore(f, b.nextSibling);
                        for (f = c(b); f && c(f); ) f = c(f);
                        f.parentNode !== document.head && (f = null);
                        document.head.insertBefore(b, f);
                        b.removeAttribute("type");
                    }
                });
            } else a();
        };
        k.prototype.Ma = function () {
            var a = this,
                b = m(document, "link[rel=import]");
            n(
                b,
                function (b) {
                    return a.g(b);
                },
                !0
            );
        };
        k.prototype.g = function (a) {
            a.__loaded ||
                ((a.__loaded = !0),
                a.import && (a.import.readyState = "complete"),
                a.dispatchEvent(
                    b(a.import ? "load" : "error", {
                        bubbles: !1,
                        cancelable: !1,
                        detail: void 0,
                    })
                ));
        };
        k.prototype.Oa = function (a) {
            var b = this;
            n(a, function (a) {
                return n(a.addedNodes, function (a) {
                    a &&
                        a.nodeType === Node.ELEMENT_NODE &&
                        (h(a) ? b.l(a) : b.loadImports(a));
                });
            });
        };
        var ab = null;
        if (E)
            (A = m(document, "link[rel=import]")),
                n(A, function (a) {
                    (a.import && "loading" === a.import.readyState) ||
                        (a.__loaded = !0);
                }),
                (A = function (a) {
                    a = a.target;
                    h(a) && (a.__loaded = !0);
                }),
                document.addEventListener("load", A, !0),
                document.addEventListener("error", A, !0);
        else {
            var ma = Object.getOwnPropertyDescriptor(Node.prototype, "baseURI");
            Object.defineProperty(
                (!ma || ma.configurable ? Node : Element).prototype,
                "baseURI",
                {
                    get: function () {
                        var a = h(this) ? this : c(this);
                        return a
                            ? a.href
                            : ma && ma.get
                            ? ma.get.call(this)
                            : (
                                  document.querySelector("base") ||
                                  window.location
                              ).href;
                    },
                    configurable: !0,
                    enumerable: !0,
                }
            );
            Object.defineProperty(HTMLLinkElement.prototype, "import", {
                get: function () {
                    return this.__import || null;
                },
                configurable: !0,
                enumerable: !0,
            });
            e(function () {
                ab = new k();
            });
        }
        f(function () {
            return document.dispatchEvent(
                b("HTMLImportsLoaded", {
                    cancelable: !0,
                    bubbles: !0,
                    detail: void 0,
                })
            );
        });
        a.useNative = E;
        a.whenReady = f;
        a.importForElement = c;
        a.loadImports = function (a) {
            ab && ab.loadImports(a);
        };
    })((window.HTMLImports = window.HTMLImports || {})); /*

Copyright (c) 2016 The Polymer Project Authors. All rights reserved.
This code may only be used under the BSD style license found at http://polymer.github.io/LICENSE.txt
The complete set of authors may be found at http://polymer.github.io/AUTHORS.txt
The complete set of contributors may be found at http://polymer.github.io/CONTRIBUTORS.txt
Code distributed by Google as part of the polymer project is also
subject to an additional IP rights grant found at http://polymer.github.io/PATENTS.txt
*/
    function ka() {
        this.ra = this.root = null;
        this.S = !1;
        this.A =
            this.N =
            this.ea =
            this.assignedSlot =
            this.assignedNodes =
            this.F =
                null;
        this.childNodes =
            this.nextSibling =
            this.previousSibling =
            this.lastChild =
            this.firstChild =
            this.parentNode =
            this.H =
                void 0;
        this.wa = this.na = !1;
        this.L = {};
    }
    ka.prototype.toJSON = function () {
        return {};
    };
    function r(a) {
        a.Z || (a.Z = new ka());
        return a.Z;
    }
    function t(a) {
        return a && a.Z;
    }
    var u = window.ShadyDOM || {};
    u.Ka = !(!Element.prototype.attachShadow || !Node.prototype.getRootNode);
    var la = Object.getOwnPropertyDescriptor(Node.prototype, "firstChild");
    u.u = !!(la && la.configurable && la.get);
    u.ga = u.force || !u.Ka;
    var na = navigator.userAgent.match("Trident"),
        oa = navigator.userAgent.match("Edge");
    void 0 === u.ua && (u.ua = u.u && (na || oa));
    function v(a) {
        return (a = t(a)) && void 0 !== a.firstChild;
    }
    function w(a) {
        return "ShadyRoot" === a.Da;
    }
    function pa(a) {
        a = a.getRootNode();
        if (w(a)) return a;
    }
    var qa = Element.prototype,
        ra =
            qa.matches ||
            qa.matchesSelector ||
            qa.mozMatchesSelector ||
            qa.msMatchesSelector ||
            qa.oMatchesSelector ||
            qa.webkitMatchesSelector;
    function sa(a, b) {
        if (a && b)
            for (
                var c = Object.getOwnPropertyNames(b), d = 0, e;
                d < c.length && (e = c[d]);
                d++
            ) {
                var f = Object.getOwnPropertyDescriptor(b, e);
                f && Object.defineProperty(a, e, f);
            }
    }
    function ta(a, b) {
        for (var c = [], d = 1; d < arguments.length; ++d)
            c[d - 1] = arguments[d];
        for (d = 0; d < c.length; d++) sa(a, c[d]);
        return a;
    }
    function ua(a, b) {
        for (var c in b) a[c] = b[c];
    }
    var va = document.createTextNode(""),
        wa = 0,
        xa = [];
    new MutationObserver(function () {
        for (; xa.length; )
            try {
                xa.shift()();
            } catch (a) {
                throw ((va.textContent = wa++), a);
            }
    }).observe(va, { characterData: !0 });
    function ya(a) {
        xa.push(a);
        va.textContent = wa++;
    }
    var za = !!document.contains;
    function Aa(a, b) {
        for (; b; ) {
            if (b == a) return !0;
            b = b.parentNode;
        }
        return !1;
    }
    function Ba(a) {
        for (var b = a.length - 1; 0 <= b; b--) {
            var c = a[b],
                d = c.getAttribute("id") || c.getAttribute("name");
            d && "length" !== d && isNaN(d) && (a[d] = c);
        }
        a.item = function (b) {
            return a[b];
        };
        a.namedItem = function (b) {
            if ("length" !== b && isNaN(b) && a[b]) return a[b];
            for (var c = ia(a), d = c.next(); !d.done; d = c.next())
                if (
                    ((d = d.value),
                    (d.getAttribute("id") || d.getAttribute("name")) == b)
                )
                    return d;
            return null;
        };
        return a;
    }
    var Ca = [],
        Da;
    function Ea(a) {
        Da || ((Da = !0), ya(Fa));
        Ca.push(a);
    }
    function Fa() {
        Da = !1;
        for (var a = !!Ca.length; Ca.length; ) Ca.shift()();
        return a;
    }
    Fa.list = Ca;
    function Ga() {
        this.a = !1;
        this.addedNodes = [];
        this.removedNodes = [];
        this.R = new Set();
    }
    function Ha(a) {
        a.a ||
            ((a.a = !0),
            ya(function () {
                Ia(a);
            }));
    }
    function Ia(a) {
        if (a.a) {
            a.a = !1;
            var b = a.takeRecords();
            b.length &&
                a.R.forEach(function (a) {
                    a(b);
                });
        }
    }
    Ga.prototype.takeRecords = function () {
        if (this.addedNodes.length || this.removedNodes.length) {
            var a = [
                {
                    addedNodes: this.addedNodes,
                    removedNodes: this.removedNodes,
                },
            ];
            this.addedNodes = [];
            this.removedNodes = [];
            return a;
        }
        return [];
    };
    function Ja(a, b) {
        var c = r(a);
        c.F || (c.F = new Ga());
        c.F.R.add(b);
        var d = c.F;
        return {
            Ba: b,
            C: d,
            Ea: a,
            takeRecords: function () {
                return d.takeRecords();
            },
        };
    }
    function Ka(a) {
        var b = a && a.C;
        b && (b.R.delete(a.Ba), b.R.size || (r(a.Ea).F = null));
    }
    function La(a, b) {
        var c = b.getRootNode();
        return a
            .map(function (a) {
                var b = c === a.target.getRootNode();
                if (b && a.addedNodes) {
                    if (
                        ((b = Array.from(a.addedNodes).filter(function (a) {
                            return c === a.getRootNode();
                        })),
                        b.length)
                    )
                        return (
                            (a = Object.create(a)),
                            Object.defineProperty(a, "addedNodes", {
                                value: b,
                                configurable: !0,
                            }),
                            a
                        );
                } else if (b) return a;
            })
            .filter(function (a) {
                return a;
            });
    }
    var x = {},
        Ma = Element.prototype.insertBefore,
        Na = Element.prototype.replaceChild,
        Oa = Element.prototype.removeChild,
        Pa = Element.prototype.setAttribute,
        Qa = Element.prototype.removeAttribute,
        Ra = Element.prototype.cloneNode,
        Sa = Document.prototype.importNode,
        Ta = Element.prototype.addEventListener,
        Ua = Element.prototype.removeEventListener,
        Va = Window.prototype.addEventListener,
        Wa = Window.prototype.removeEventListener,
        Xa = Element.prototype.dispatchEvent,
        Ya = Node.prototype.contains || HTMLElement.prototype.contains,
        bb = Document.prototype.getElementById,
        cb = Element.prototype.querySelector,
        db = DocumentFragment.prototype.querySelector,
        eb = Document.prototype.querySelector,
        fb = Element.prototype.querySelectorAll,
        gb = DocumentFragment.prototype.querySelectorAll,
        hb = Document.prototype.querySelectorAll;
    x.appendChild = Element.prototype.appendChild;
    x.insertBefore = Ma;
    x.replaceChild = Na;
    x.removeChild = Oa;
    x.setAttribute = Pa;
    x.removeAttribute = Qa;
    x.cloneNode = Ra;
    x.importNode = Sa;
    x.addEventListener = Ta;
    x.removeEventListener = Ua;
    x.Xa = Va;
    x.Ya = Wa;
    x.dispatchEvent = Xa;
    x.contains = Ya;
    x.getElementById = bb;
    x.hb = cb;
    x.kb = db;
    x.fb = eb;
    x.querySelector = function (a) {
        switch (this.nodeType) {
            case Node.ELEMENT_NODE:
                return cb.call(this, a);
            case Node.DOCUMENT_NODE:
                return eb.call(this, a);
            default:
                return db.call(this, a);
        }
    };
    x.ib = fb;
    x.lb = gb;
    x.gb = hb;
    x.querySelectorAll = function (a) {
        switch (this.nodeType) {
            case Node.ELEMENT_NODE:
                return fb.call(this, a);
            case Node.DOCUMENT_NODE:
                return hb.call(this, a);
            default:
                return gb.call(this, a);
        }
    };
    var ib = /[&\u00A0"]/g,
        jb = /[&\u00A0<>]/g;
    function kb(a) {
        switch (a) {
            case "&":
                return "&amp;";
            case "<":
                return "&lt;";
            case ">":
                return "&gt;";
            case '"':
                return "&quot;";
            case "\u00a0":
                return "&nbsp;";
        }
    }
    function lb(a) {
        for (var b = {}, c = 0; c < a.length; c++) b[a[c]] = !0;
        return b;
    }
    var mb = lb(
            "area base br col command embed hr img input keygen link meta param source track wbr".split(
                " "
            )
        ),
        nb = lb(
            "style script xmp iframe noembed noframes plaintext noscript".split(
                " "
            )
        );
    function ob(a, b) {
        "template" === a.localName && (a = a.content);
        for (
            var c = "", d = b ? b(a) : a.childNodes, e = 0, f = d.length, g;
            e < f && (g = d[e]);
            e++
        ) {
            a: {
                var h = g;
                var k = a;
                var l = b;
                switch (h.nodeType) {
                    case Node.ELEMENT_NODE:
                        for (
                            var m = h.localName,
                                n = "<" + m,
                                A = h.attributes,
                                E = 0;
                            (k = A[E]);
                            E++
                        )
                            n +=
                                " " +
                                k.name +
                                '="' +
                                k.value.replace(ib, kb) +
                                '"';
                        n += ">";
                        h = mb[m] ? n : n + ob(h, l) + "</" + m + ">";
                        break a;
                    case Node.TEXT_NODE:
                        h = h.data;
                        h = k && nb[k.localName] ? h : h.replace(jb, kb);
                        break a;
                    case Node.COMMENT_NODE:
                        h = "\x3c!--" + h.data + "--\x3e";
                        break a;
                    default:
                        throw (
                            (window.console.error(h), Error("not implemented"))
                        );
                }
            }
            c += h;
        }
        return c;
    }
    var y = {},
        z = document.createTreeWalker(document, NodeFilter.SHOW_ALL, null, !1),
        B = document.createTreeWalker(
            document,
            NodeFilter.SHOW_ELEMENT,
            null,
            !1
        );
    function pb(a) {
        var b = [];
        z.currentNode = a;
        for (a = z.firstChild(); a; ) b.push(a), (a = z.nextSibling());
        return b;
    }
    y.parentNode = function (a) {
        z.currentNode = a;
        return z.parentNode();
    };
    y.firstChild = function (a) {
        z.currentNode = a;
        return z.firstChild();
    };
    y.lastChild = function (a) {
        z.currentNode = a;
        return z.lastChild();
    };
    y.previousSibling = function (a) {
        z.currentNode = a;
        return z.previousSibling();
    };
    y.nextSibling = function (a) {
        z.currentNode = a;
        return z.nextSibling();
    };
    y.childNodes = pb;
    y.parentElement = function (a) {
        B.currentNode = a;
        return B.parentNode();
    };
    y.firstElementChild = function (a) {
        B.currentNode = a;
        return B.firstChild();
    };
    y.lastElementChild = function (a) {
        B.currentNode = a;
        return B.lastChild();
    };
    y.previousElementSibling = function (a) {
        B.currentNode = a;
        return B.previousSibling();
    };
    y.nextElementSibling = function (a) {
        B.currentNode = a;
        return B.nextSibling();
    };
    y.children = function (a) {
        var b = [];
        B.currentNode = a;
        for (a = B.firstChild(); a; ) b.push(a), (a = B.nextSibling());
        return Ba(b);
    };
    y.innerHTML = function (a) {
        return ob(a, function (a) {
            return pb(a);
        });
    };
    y.textContent = function (a) {
        switch (a.nodeType) {
            case Node.ELEMENT_NODE:
            case Node.DOCUMENT_FRAGMENT_NODE:
                a = document.createTreeWalker(
                    a,
                    NodeFilter.SHOW_TEXT,
                    null,
                    !1
                );
                for (var b = "", c; (c = a.nextNode()); ) b += c.nodeValue;
                return b;
            default:
                return a.nodeValue;
        }
    };
    var C = {},
        qb = u.u,
        rb = [Node.prototype, Element.prototype, HTMLElement.prototype];
    function D(a) {
        var b;
        a: {
            for (b = 0; b < rb.length; b++) {
                var c = rb[b];
                if (c.hasOwnProperty(a)) {
                    b = c;
                    break a;
                }
            }
            b = void 0;
        }
        if (!b) throw Error("Could not find descriptor for " + a);
        return Object.getOwnPropertyDescriptor(b, a);
    }
    var F = qb
            ? {
                  parentNode: D("parentNode"),
                  firstChild: D("firstChild"),
                  lastChild: D("lastChild"),
                  previousSibling: D("previousSibling"),
                  nextSibling: D("nextSibling"),
                  childNodes: D("childNodes"),
                  parentElement: D("parentElement"),
                  previousElementSibling: D("previousElementSibling"),
                  nextElementSibling: D("nextElementSibling"),
                  innerHTML: D("innerHTML"),
                  textContent: D("textContent"),
                  firstElementChild: D("firstElementChild"),
                  lastElementChild: D("lastElementChild"),
                  children: D("children"),
              }
            : {},
        sb = qb
            ? {
                  firstElementChild: Object.getOwnPropertyDescriptor(
                      DocumentFragment.prototype,
                      "firstElementChild"
                  ),
                  lastElementChild: Object.getOwnPropertyDescriptor(
                      DocumentFragment.prototype,
                      "lastElementChild"
                  ),
                  children: Object.getOwnPropertyDescriptor(
                      DocumentFragment.prototype,
                      "children"
                  ),
              }
            : {},
        tb = qb
            ? {
                  firstElementChild: Object.getOwnPropertyDescriptor(
                      Document.prototype,
                      "firstElementChild"
                  ),
                  lastElementChild: Object.getOwnPropertyDescriptor(
                      Document.prototype,
                      "lastElementChild"
                  ),
                  children: Object.getOwnPropertyDescriptor(
                      Document.prototype,
                      "children"
                  ),
              }
            : {};
    C.qa = F;
    C.jb = sb;
    C.eb = tb;
    C.parentNode = function (a) {
        return F.parentNode.get.call(a);
    };
    C.firstChild = function (a) {
        return F.firstChild.get.call(a);
    };
    C.lastChild = function (a) {
        return F.lastChild.get.call(a);
    };
    C.previousSibling = function (a) {
        return F.previousSibling.get.call(a);
    };
    C.nextSibling = function (a) {
        return F.nextSibling.get.call(a);
    };
    C.childNodes = function (a) {
        return Array.prototype.slice.call(F.childNodes.get.call(a));
    };
    C.parentElement = function (a) {
        return F.parentElement.get.call(a);
    };
    C.previousElementSibling = function (a) {
        return F.previousElementSibling.get.call(a);
    };
    C.nextElementSibling = function (a) {
        return F.nextElementSibling.get.call(a);
    };
    C.innerHTML = function (a) {
        return F.innerHTML.get.call(a);
    };
    C.textContent = function (a) {
        return F.textContent.get.call(a);
    };
    C.children = function (a) {
        switch (a.nodeType) {
            case Node.DOCUMENT_FRAGMENT_NODE:
                return sb.children.get.call(a);
            case Node.DOCUMENT_NODE:
                return tb.children.get.call(a);
            default:
                return F.children.get.call(a);
        }
    };
    C.firstElementChild = function (a) {
        switch (a.nodeType) {
            case Node.DOCUMENT_FRAGMENT_NODE:
                return sb.firstElementChild.get.call(a);
            case Node.DOCUMENT_NODE:
                return tb.firstElementChild.get.call(a);
            default:
                return F.firstElementChild.get.call(a);
        }
    };
    C.lastElementChild = function (a) {
        switch (a.nodeType) {
            case Node.DOCUMENT_FRAGMENT_NODE:
                return sb.lastElementChild.get.call(a);
            case Node.DOCUMENT_NODE:
                return tb.lastElementChild.get.call(a);
            default:
                return F.lastElementChild.get.call(a);
        }
    };
    var G = u.ua ? C : y;
    function ub(a) {
        for (; a.firstChild; ) a.removeChild(a.firstChild);
    }
    var vb = u.u,
        wb = document.implementation.createHTMLDocument("inert"),
        xb = Object.getOwnPropertyDescriptor(Node.prototype, "isConnected"),
        yb = xb && xb.get,
        zb = Object.getOwnPropertyDescriptor(
            Document.prototype,
            "activeElement"
        ),
        Ab = {
            parentElement: {
                get: function () {
                    var a = t(this);
                    (a = a && a.parentNode) &&
                        a.nodeType !== Node.ELEMENT_NODE &&
                        (a = null);
                    return void 0 !== a ? a : G.parentElement(this);
                },
                configurable: !0,
            },
            parentNode: {
                get: function () {
                    var a = t(this);
                    a = a && a.parentNode;
                    return void 0 !== a ? a : G.parentNode(this);
                },
                configurable: !0,
            },
            nextSibling: {
                get: function () {
                    var a = t(this);
                    a = a && a.nextSibling;
                    return void 0 !== a ? a : G.nextSibling(this);
                },
                configurable: !0,
            },
            previousSibling: {
                get: function () {
                    var a = t(this);
                    a = a && a.previousSibling;
                    return void 0 !== a ? a : G.previousSibling(this);
                },
                configurable: !0,
            },
            nextElementSibling: {
                get: function () {
                    var a = t(this);
                    if (a && void 0 !== a.nextSibling) {
                        for (
                            a = this.nextSibling;
                            a && a.nodeType !== Node.ELEMENT_NODE;

                        )
                            a = a.nextSibling;
                        return a;
                    }
                    return G.nextElementSibling(this);
                },
                configurable: !0,
            },
            previousElementSibling: {
                get: function () {
                    var a = t(this);
                    if (a && void 0 !== a.previousSibling) {
                        for (
                            a = this.previousSibling;
                            a && a.nodeType !== Node.ELEMENT_NODE;

                        )
                            a = a.previousSibling;
                        return a;
                    }
                    return G.previousElementSibling(this);
                },
                configurable: !0,
            },
        },
        Bb = {
            className: {
                get: function () {
                    return this.getAttribute("class") || "";
                },
                set: function (a) {
                    this.setAttribute("class", a);
                },
                configurable: !0,
            },
        },
        Cb = {
            childNodes: {
                get: function () {
                    if (v(this)) {
                        var a = t(this);
                        if (!a.childNodes) {
                            a.childNodes = [];
                            for (var b = this.firstChild; b; b = b.nextSibling)
                                a.childNodes.push(b);
                        }
                        var c = a.childNodes;
                    } else c = G.childNodes(this);
                    c.item = function (a) {
                        return c[a];
                    };
                    return c;
                },
                configurable: !0,
            },
            childElementCount: {
                get: function () {
                    return this.children.length;
                },
                configurable: !0,
            },
            firstChild: {
                get: function () {
                    var a = t(this);
                    a = a && a.firstChild;
                    return void 0 !== a ? a : G.firstChild(this);
                },
                configurable: !0,
            },
            lastChild: {
                get: function () {
                    var a = t(this);
                    a = a && a.lastChild;
                    return void 0 !== a ? a : G.lastChild(this);
                },
                configurable: !0,
            },
            textContent: {
                get: function () {
                    if (v(this)) {
                        for (
                            var a = [], b = 0, c = this.childNodes, d;
                            (d = c[b]);
                            b++
                        )
                            d.nodeType !== Node.COMMENT_NODE &&
                                a.push(d.textContent);
                        return a.join("");
                    }
                    return G.textContent(this);
                },
                set: function (a) {
                    if ("undefined" === typeof a || null === a) a = "";
                    switch (this.nodeType) {
                        case Node.ELEMENT_NODE:
                        case Node.DOCUMENT_FRAGMENT_NODE:
                            if (!v(this) && vb) {
                                var b = this.firstChild;
                                (b != this.lastChild ||
                                    (b && b.nodeType != Node.TEXT_NODE)) &&
                                    ub(this);
                                C.qa.textContent.set.call(this, a);
                            } else
                                ub(this),
                                    (0 < a.length ||
                                        this.nodeType === Node.ELEMENT_NODE) &&
                                        this.appendChild(
                                            document.createTextNode(a)
                                        );
                            break;
                        default:
                            this.nodeValue = a;
                    }
                },
                configurable: !0,
            },
            firstElementChild: {
                get: function () {
                    var a = t(this);
                    if (a && void 0 !== a.firstChild) {
                        for (
                            a = this.firstChild;
                            a && a.nodeType !== Node.ELEMENT_NODE;

                        )
                            a = a.nextSibling;
                        return a;
                    }
                    return G.firstElementChild(this);
                },
                configurable: !0,
            },
            lastElementChild: {
                get: function () {
                    var a = t(this);
                    if (a && void 0 !== a.lastChild) {
                        for (
                            a = this.lastChild;
                            a && a.nodeType !== Node.ELEMENT_NODE;

                        )
                            a = a.previousSibling;
                        return a;
                    }
                    return G.lastElementChild(this);
                },
                configurable: !0,
            },
            children: {
                get: function () {
                    return v(this)
                        ? Ba(
                              Array.prototype.filter.call(
                                  this.childNodes,
                                  function (a) {
                                      return a.nodeType === Node.ELEMENT_NODE;
                                  }
                              )
                          )
                        : G.children(this);
                },
                configurable: !0,
            },
            innerHTML: {
                get: function () {
                    return v(this)
                        ? ob(
                              "template" === this.localName
                                  ? this.content
                                  : this
                          )
                        : G.innerHTML(this);
                },
                set: function (a) {
                    var b = "template" === this.localName ? this.content : this;
                    ub(b);
                    var c = this.localName || "div";
                    c =
                        this.namespaceURI &&
                        this.namespaceURI !== wb.namespaceURI
                            ? wb.createElementNS(this.namespaceURI, c)
                            : wb.createElement(c);
                    vb ? C.qa.innerHTML.set.call(c, a) : (c.innerHTML = a);
                    for (
                        a = "template" === this.localName ? c.content : c;
                        a.firstChild;

                    )
                        b.appendChild(a.firstChild);
                },
                configurable: !0,
            },
        },
        Db = {
            shadowRoot: {
                get: function () {
                    var a = t(this);
                    return (a && a.ra) || null;
                },
                configurable: !0,
            },
        },
        Eb = {
            activeElement: {
                get: function () {
                    var a =
                        zb && zb.get
                            ? zb.get.call(document)
                            : u.u
                            ? void 0
                            : document.activeElement;
                    if (a && a.nodeType) {
                        var b = !!w(this);
                        if (
                            this === document ||
                            (b &&
                                this.host !== a &&
                                x.contains.call(this.host, a))
                        ) {
                            for (b = pa(a); b && b !== this; )
                                (a = b.host), (b = pa(a));
                            a =
                                this === document
                                    ? b
                                        ? null
                                        : a
                                    : b === this
                                    ? a
                                    : null;
                        } else a = null;
                    } else a = null;
                    return a;
                },
                set: function () {},
                configurable: !0,
            },
        };
    function H(a, b, c) {
        for (var d in b) {
            var e = Object.getOwnPropertyDescriptor(a, d);
            (e && e.configurable) || (!e && c)
                ? Object.defineProperty(a, d, b[d])
                : c && console.warn("Could not define", d, "on", a);
        }
    }
    function J(a) {
        H(a, Ab);
        H(a, Bb);
        H(a, Cb);
        H(a, Eb);
    }
    function Fb() {
        var a = Gb.prototype;
        a.__proto__ = DocumentFragment.prototype;
        H(a, Ab, !0);
        H(a, Cb, !0);
        H(a, Eb, !0);
        Object.defineProperties(a, {
            nodeType: { value: Node.DOCUMENT_FRAGMENT_NODE, configurable: !0 },
            nodeName: { value: "#document-fragment", configurable: !0 },
            nodeValue: { value: null, configurable: !0 },
        });
        ["localName", "namespaceURI", "prefix"].forEach(function (b) {
            Object.defineProperty(a, b, { value: void 0, configurable: !0 });
        });
        ["ownerDocument", "baseURI", "isConnected"].forEach(function (b) {
            Object.defineProperty(a, b, {
                get: function () {
                    return this.host[b];
                },
                configurable: !0,
            });
        });
    }
    var Hb = u.u
            ? function () {}
            : function (a) {
                  var b = r(a);
                  b.na || ((b.na = !0), H(a, Ab, !0), H(a, Bb, !0));
              },
        Ib = u.u
            ? function () {}
            : function (a) {
                  r(a).wa || (H(a, Cb, !0), H(a, Db, !0));
              };
    var Jb = G.childNodes;
    function Kb(a, b, c) {
        Hb(a);
        c = c || null;
        var d = r(a),
            e = r(b),
            f = c ? r(c) : null;
        d.previousSibling = c ? f.previousSibling : b.lastChild;
        if ((f = t(d.previousSibling))) f.nextSibling = a;
        if ((f = t((d.nextSibling = c)))) f.previousSibling = a;
        d.parentNode = b;
        c
            ? c === e.firstChild && (e.firstChild = a)
            : ((e.lastChild = a), e.firstChild || (e.firstChild = a));
        e.childNodes = null;
    }
    function Lb(a) {
        var b = r(a);
        if (void 0 === b.firstChild) {
            b.childNodes = null;
            var c = Jb(a);
            b.firstChild = c[0] || null;
            b.lastChild = c[c.length - 1] || null;
            Ib(a);
            for (b = 0; b < c.length; b++) {
                var d = c[b],
                    e = r(d);
                e.parentNode = a;
                e.nextSibling = c[b + 1] || null;
                e.previousSibling = c[b - 1] || null;
                Hb(d);
            }
        }
    }
    var Mb = G.parentNode;
    function Nb(a, b, c) {
        if (b === a)
            throw Error(
                "Failed to execute 'appendChild' on 'Node': The new child element contains the parent."
            );
        if (c) {
            var d = t(c);
            d = d && d.parentNode;
            if ((void 0 !== d && d !== a) || (void 0 === d && Mb(c) !== a))
                throw Error(
                    "Failed to execute 'insertBefore' on 'Node': The node before which the new node is to be inserted is not a child of this node."
                );
        }
        if (c === b) return b;
        b.parentNode && Ob(b.parentNode, b);
        d = !0;
        var e, f;
        if (!b.__noInsertionPoint) {
            if ((f = e = pa(a))) {
                var g;
                "slot" === b.localName
                    ? (g = [b])
                    : b.querySelectorAll && (g = b.querySelectorAll("slot"));
                f = g && g.length ? g : void 0;
            }
            if (f) {
                g = e;
                var h = f;
                g.a = g.a || [];
                g.f = g.f || [];
                g.h = g.h || {};
                g.a.push.apply(
                    g.a,
                    [].concat(h instanceof Array ? h : ja(ia(h)))
                );
            }
        }
        ("slot" === a.localName || f) && (e = e || pa(a)) && K(e);
        if (v(a)) {
            e = c;
            Ib(a);
            f = r(a);
            void 0 !== f.firstChild && (f.childNodes = null);
            if (b.nodeType === Node.DOCUMENT_FRAGMENT_NODE) {
                f = b.childNodes;
                for (g = 0; g < f.length; g++) Kb(f[g], a, e);
                e = r(b);
                f = void 0 !== e.firstChild ? null : void 0;
                e.firstChild = e.lastChild = f;
                e.childNodes = f;
            } else Kb(b, a, e);
            e = t(a);
            Pb(a) ? (K(e.root), (d = !1)) : e.root && (d = !1);
        }
        d
            ? ((d = w(a) ? a.host : a),
              c
                  ? ((c = Qb(c)), x.insertBefore.call(d, b, c))
                  : x.appendChild.call(d, b))
            : b.ownerDocument !== a.ownerDocument &&
              a.ownerDocument.adoptNode(b);
        Rb(a, b);
        return b;
    }
    function Ob(a, b) {
        if (b.parentNode !== a)
            throw Error(
                "The node to be removed is not a child of this node: " + b
            );
        var c = pa(b),
            d = t(a);
        if (v(a)) {
            var e = r(b),
                f = r(a);
            b === f.firstChild && (f.firstChild = e.nextSibling);
            b === f.lastChild && (f.lastChild = e.previousSibling);
            var g = e.previousSibling,
                h = e.nextSibling;
            g && (r(g).nextSibling = h);
            h && (r(h).previousSibling = g);
            e.parentNode = e.previousSibling = e.nextSibling = void 0;
            void 0 !== f.childNodes && (f.childNodes = null);
            if (Pb(a)) {
                K(d.root);
                var k = !0;
            }
        }
        Sb(b);
        if (c) {
            (e = a && "slot" === a.localName) && (k = !0);
            if (c.f) {
                Tb(c);
                f = c.h;
                for (da in f)
                    for (g = f[da], h = 0; h < g.length; h++) {
                        var l = g[h];
                        if (Aa(b, l)) {
                            g.splice(h, 1);
                            var m = c.f.indexOf(l);
                            0 <= m && c.f.splice(m, 1);
                            h--;
                            m = t(l);
                            if ((l = m.A))
                                for (var n = 0; n < l.length; n++) {
                                    var A = l[n],
                                        E = Ub(A);
                                    E && x.removeChild.call(E, A);
                                }
                            m.A = [];
                            m.assignedNodes = [];
                            m = !0;
                        }
                    }
                var da = m;
            } else da = void 0;
            (da || e) && K(c);
        }
        k ||
            ((k = w(a) ? a.host : a),
            ((!d.root && "slot" !== b.localName) || k === Mb(b)) &&
                x.removeChild.call(k, b));
        Rb(a, null, b);
        return b;
    }
    function Sb(a) {
        var b = t(a);
        if (b && void 0 !== b.H) {
            b = a.childNodes;
            for (var c = 0, d = b.length, e; c < d && (e = b[c]); c++) Sb(e);
        }
        if ((a = t(a))) a.H = void 0;
    }
    function Qb(a) {
        var b = a;
        a &&
            "slot" === a.localName &&
            (b =
                (b = (b = t(a)) && b.A) && b.length ? b[0] : Qb(a.nextSibling));
        return b;
    }
    function Pb(a) {
        return (a = (a = t(a)) && a.root) && Vb(a);
    }
    function Wb(a, b) {
        if ("slot" === b) (a = a.parentNode), Pb(a) && K(t(a).root);
        else if ("slot" === a.localName && "name" === b && (b = pa(a))) {
            if (b.f) {
                Tb(b);
                var c = a.za,
                    d = Xb(a);
                if (d !== c) {
                    c = b.h[c];
                    var e = c.indexOf(a);
                    0 <= e && c.splice(e, 1);
                    c = b.h[d] || (b.h[d] = []);
                    c.push(a);
                    1 < c.length && (b.h[d] = Yb(c));
                }
            }
            K(b);
        }
    }
    function Rb(a, b, c) {
        if ((a = (a = t(a)) && a.F))
            b && a.addedNodes.push(b), c && a.removedNodes.push(c), Ha(a);
    }
    function Zb(a) {
        if (a && a.nodeType) {
            var b = r(a),
                c = b.H;
            void 0 === c &&
                (w(a)
                    ? ((c = a), (b.H = c))
                    : ((c = (c = a.parentNode) ? Zb(c) : a),
                      x.contains.call(document.documentElement, a) &&
                          (b.H = c)));
            return c;
        }
    }
    function $b(a, b, c) {
        var d = [];
        ac(a.childNodes, b, c, d);
        return d;
    }
    function ac(a, b, c, d) {
        for (var e = 0, f = a.length, g; e < f && (g = a[e]); e++) {
            var h;
            if ((h = g.nodeType === Node.ELEMENT_NODE)) {
                h = g;
                var k = b,
                    l = c,
                    m = d,
                    n = k(h);
                n && m.push(h);
                l && l(n) ? (h = n) : (ac(h.childNodes, k, l, m), (h = void 0));
            }
            if (h) break;
        }
    }
    var bc = null;
    function dc(a, b, c) {
        bc || (bc = window.ShadyCSS && window.ShadyCSS.ScopingShim);
        bc && "class" === b
            ? bc.setElementClass(a, c)
            : (x.setAttribute.call(a, b, c), Wb(a, b));
    }
    function ec(a, b) {
        if (a.ownerDocument !== document || "template" === a.localName)
            return x.importNode.call(document, a, b);
        var c = x.importNode.call(document, a, !1);
        if (b) {
            a = a.childNodes;
            b = 0;
            for (var d; b < a.length; b++) (d = ec(a[b], !0)), c.appendChild(d);
        }
        return c;
    }
    var fc = "__eventWrappers" + Date.now(),
        gc = (function () {
            var a = Object.getOwnPropertyDescriptor(
                Event.prototype,
                "composed"
            );
            return a
                ? function (b) {
                      return a.get.call(b);
                  }
                : null;
        })(),
        hc = {
            blur: !0,
            focus: !0,
            focusin: !0,
            focusout: !0,
            click: !0,
            dblclick: !0,
            mousedown: !0,
            mouseenter: !0,
            mouseleave: !0,
            mousemove: !0,
            mouseout: !0,
            mouseover: !0,
            mouseup: !0,
            wheel: !0,
            beforeinput: !0,
            input: !0,
            keydown: !0,
            keyup: !0,
            compositionstart: !0,
            compositionupdate: !0,
            compositionend: !0,
            touchstart: !0,
            touchend: !0,
            touchmove: !0,
            touchcancel: !0,
            pointerover: !0,
            pointerenter: !0,
            pointerdown: !0,
            pointermove: !0,
            pointerup: !0,
            pointercancel: !0,
            pointerout: !0,
            pointerleave: !0,
            gotpointercapture: !0,
            lostpointercapture: !0,
            dragstart: !0,
            drag: !0,
            dragenter: !0,
            dragleave: !0,
            dragover: !0,
            drop: !0,
            dragend: !0,
            DOMActivate: !0,
            DOMFocusIn: !0,
            DOMFocusOut: !0,
            keypress: !0,
        },
        ic = {
            DOMAttrModified: !0,
            DOMAttributeNameChanged: !0,
            DOMCharacterDataModified: !0,
            DOMElementNameChanged: !0,
            DOMNodeInserted: !0,
            DOMNodeInsertedIntoDocument: !0,
            DOMNodeRemoved: !0,
            DOMNodeRemovedFromDocument: !0,
            DOMSubtreeModified: !0,
        };
    function jc(a, b) {
        var c = [],
            d = a;
        for (a = a === window ? window : a.getRootNode(); d; )
            c.push(d),
                (d = d.assignedSlot
                    ? d.assignedSlot
                    : d.nodeType === Node.DOCUMENT_FRAGMENT_NODE &&
                      d.host &&
                      (b || d !== a)
                    ? d.host
                    : d.parentNode);
        c[c.length - 1] === document && c.push(window);
        return c;
    }
    function kc(a, b) {
        if (!w) return a;
        a = jc(a, !0);
        for (var c = 0, d, e, f, g; c < b.length; c++)
            if (
                ((d = b[c]),
                (f = d === window ? window : d.getRootNode()),
                f !== e && ((g = a.indexOf(f)), (e = f)),
                !w(f) || -1 < g)
            )
                return d;
    }
    var lc = {
        get composed() {
            void 0 === this.K &&
                (gc
                    ? (this.K = gc(this))
                    : !1 !== this.isTrusted && (this.K = hc[this.type]));
            return this.K || !1;
        },
        composedPath: function () {
            this.ma || (this.ma = jc(this.__target, this.composed));
            return this.ma;
        },
        get target() {
            return kc(
                this.currentTarget || this.__previousCurrentTarget,
                this.composedPath()
            );
        },
        get relatedTarget() {
            if (!this.Y) return null;
            this.oa || (this.oa = jc(this.Y, !0));
            return kc(
                this.currentTarget || this.__previousCurrentTarget,
                this.oa
            );
        },
        stopPropagation: function () {
            Event.prototype.stopPropagation.call(this);
            this.X = !0;
        },
        stopImmediatePropagation: function () {
            Event.prototype.stopImmediatePropagation.call(this);
            this.X = this.va = !0;
        },
    };
    function mc(a) {
        function b(b, d) {
            b = new a(b, d);
            b.K = d && !!d.composed;
            return b;
        }
        ua(b, a);
        b.prototype = a.prototype;
        return b;
    }
    var nc = { focus: !0, blur: !0 };
    function oc(a) {
        return a.__target !== a.target || a.Y !== a.relatedTarget;
    }
    function pc(a, b, c) {
        if (
            (c =
                b.__handlers && b.__handlers[a.type] && b.__handlers[a.type][c])
        )
            for (
                var d = 0, e;
                (e = c[d]) &&
                (!oc(a) || a.target !== a.relatedTarget) &&
                (e.call(b, a), !a.va);
                d++
            );
    }
    function qc(a) {
        var b = a.composedPath();
        Object.defineProperty(a, "currentTarget", {
            get: function () {
                return d;
            },
            configurable: !0,
        });
        for (var c = b.length - 1; 0 <= c; c--) {
            var d = b[c];
            pc(a, d, "capture");
            if (a.X) return;
        }
        Object.defineProperty(a, "eventPhase", {
            get: function () {
                return Event.AT_TARGET;
            },
        });
        var e;
        for (c = 0; c < b.length; c++) {
            d = b[c];
            var f = t(d);
            f = f && f.root;
            if (0 === c || (f && f === e))
                if (
                    (pc(a, d, "bubble"),
                    d !== window && (e = d.getRootNode()),
                    a.X)
                )
                    break;
        }
    }
    function rc(a, b, c, d, e, f) {
        for (var g = 0; g < a.length; g++) {
            var h = a[g],
                k = h.type,
                l = h.capture,
                m = h.once,
                n = h.passive;
            if (b === h.node && c === k && d === l && e === m && f === n)
                return g;
        }
        return -1;
    }
    function sc(a, b, c) {
        if (b) {
            var d = typeof b;
            if ("function" === d || "object" === d)
                if (
                    "object" !== d ||
                    (b.handleEvent && "function" === typeof b.handleEvent)
                ) {
                    var e = this instanceof Window ? x.Xa : x.addEventListener;
                    if (ic[a]) return e.call(this, a, b, c);
                    if (c && "object" === typeof c) {
                        var f = !!c.capture;
                        var g = !!c.once;
                        var h = !!c.passive;
                    } else (f = !!c), (h = g = !1);
                    var k = (c && c.$) || this,
                        l = b[fc];
                    if (l) {
                        if (-1 < rc(l, k, a, f, g, h)) return;
                    } else b[fc] = [];
                    l = function (e) {
                        g && this.removeEventListener(a, b, c);
                        e.__target || tc(e);
                        if (k !== this) {
                            var f = Object.getOwnPropertyDescriptor(
                                e,
                                "currentTarget"
                            );
                            Object.defineProperty(e, "currentTarget", {
                                get: function () {
                                    return k;
                                },
                                configurable: !0,
                            });
                        }
                        e.__previousCurrentTarget = e.currentTarget;
                        if (!w(k) || -1 != e.composedPath().indexOf(k))
                            if (e.composed || -1 < e.composedPath().indexOf(k))
                                if (oc(e) && e.target === e.relatedTarget)
                                    e.eventPhase === Event.BUBBLING_PHASE &&
                                        e.stopImmediatePropagation();
                                else if (
                                    e.eventPhase === Event.CAPTURING_PHASE ||
                                    e.bubbles ||
                                    e.target === k ||
                                    k instanceof Window
                                ) {
                                    var h =
                                        "function" === d
                                            ? b.call(k, e)
                                            : b.handleEvent && b.handleEvent(e);
                                    k !== this &&
                                        (f
                                            ? (Object.defineProperty(
                                                  e,
                                                  "currentTarget",
                                                  f
                                              ),
                                              (f = null))
                                            : delete e.currentTarget);
                                    return h;
                                }
                    };
                    b[fc].push({
                        node: k,
                        type: a,
                        capture: f,
                        once: g,
                        passive: h,
                        Za: l,
                    });
                    nc[a]
                        ? ((this.__handlers = this.__handlers || {}),
                          (this.__handlers[a] = this.__handlers[a] || {
                              capture: [],
                              bubble: [],
                          }),
                          this.__handlers[a][f ? "capture" : "bubble"].push(l))
                        : e.call(this, a, l, c);
                }
        }
    }
    function uc(a, b, c) {
        if (b) {
            var d = this instanceof Window ? x.Ya : x.removeEventListener;
            if (ic[a]) return d.call(this, a, b, c);
            if (c && "object" === typeof c) {
                var e = !!c.capture;
                var f = !!c.once;
                var g = !!c.passive;
            } else (e = !!c), (g = f = !1);
            var h = (c && c.$) || this,
                k = void 0;
            var l = null;
            try {
                l = b[fc];
            } catch (m) {}
            l &&
                ((f = rc(l, h, a, e, f, g)),
                -1 < f &&
                    ((k = l.splice(f, 1)[0].Za), l.length || (b[fc] = void 0)));
            d.call(this, a, k || b, c);
            k &&
                nc[a] &&
                this.__handlers &&
                this.__handlers[a] &&
                ((a = this.__handlers[a][e ? "capture" : "bubble"]),
                (k = a.indexOf(k)),
                -1 < k && a.splice(k, 1));
        }
    }
    function vc() {
        for (var a in nc)
            window.addEventListener(
                a,
                function (a) {
                    a.__target || (tc(a), qc(a));
                },
                !0
            );
    }
    function tc(a) {
        a.__target = a.target;
        a.Y = a.relatedTarget;
        if (u.u) {
            var b = Object.getPrototypeOf(a);
            if (!b.hasOwnProperty("__patchProto")) {
                var c = Object.create(b);
                c.ab = b;
                sa(c, lc);
                b.__patchProto = c;
            }
            a.__proto__ = b.__patchProto;
        } else sa(a, lc);
    }
    var wc = mc(window.Event),
        xc = mc(window.CustomEvent),
        yc = mc(window.MouseEvent);
    function zc() {
        window.Event = wc;
        window.CustomEvent = xc;
        window.MouseEvent = yc;
        vc();
        if (
            !gc &&
            Object.getOwnPropertyDescriptor(Event.prototype, "isTrusted")
        ) {
            var a = function () {
                var a = new MouseEvent("click", {
                    bubbles: !0,
                    cancelable: !0,
                    composed: !0,
                });
                this.dispatchEvent(a);
            };
            Element.prototype.click
                ? (Element.prototype.click = a)
                : HTMLElement.prototype.click &&
                  (HTMLElement.prototype.click = a);
        }
    }
    function Ac(a, b) {
        return { index: a, I: [], P: b };
    }
    function Bc(a, b, c, d) {
        var e = 0,
            f = 0,
            g = 0,
            h = 0,
            k = Math.min(b - e, d - f);
        if (0 == e && 0 == f)
            a: {
                for (g = 0; g < k; g++) if (a[g] !== c[g]) break a;
                g = k;
            }
        if (b == a.length && d == c.length) {
            h = a.length;
            for (var l = c.length, m = 0; m < k - g && Cc(a[--h], c[--l]); )
                m++;
            h = m;
        }
        e += g;
        f += g;
        b -= h;
        d -= h;
        if (0 == b - e && 0 == d - f) return [];
        if (e == b) {
            for (b = Ac(e, 0); f < d; ) b.I.push(c[f++]);
            return [b];
        }
        if (f == d) return [Ac(e, b - e)];
        k = e;
        g = f;
        d = d - g + 1;
        h = b - k + 1;
        b = Array(d);
        for (l = 0; l < d; l++) (b[l] = Array(h)), (b[l][0] = l);
        for (l = 0; l < h; l++) b[0][l] = l;
        for (l = 1; l < d; l++)
            for (m = 1; m < h; m++)
                if (a[k + m - 1] === c[g + l - 1]) b[l][m] = b[l - 1][m - 1];
                else {
                    var n = b[l - 1][m] + 1,
                        A = b[l][m - 1] + 1;
                    b[l][m] = n < A ? n : A;
                }
        k = b.length - 1;
        g = b[0].length - 1;
        d = b[k][g];
        for (a = []; 0 < k || 0 < g; )
            0 == k
                ? (a.push(2), g--)
                : 0 == g
                ? (a.push(3), k--)
                : ((h = b[k - 1][g - 1]),
                  (l = b[k - 1][g]),
                  (m = b[k][g - 1]),
                  (n = l < m ? (l < h ? l : h) : m < h ? m : h),
                  n == h
                      ? (h == d ? a.push(0) : (a.push(1), (d = h)), k--, g--)
                      : n == l
                      ? (a.push(3), k--, (d = l))
                      : (a.push(2), g--, (d = m)));
        a.reverse();
        b = void 0;
        k = [];
        for (g = 0; g < a.length; g++)
            switch (a[g]) {
                case 0:
                    b && (k.push(b), (b = void 0));
                    e++;
                    f++;
                    break;
                case 1:
                    b || (b = Ac(e, 0));
                    b.P++;
                    e++;
                    b.I.push(c[f]);
                    f++;
                    break;
                case 2:
                    b || (b = Ac(e, 0));
                    b.P++;
                    e++;
                    break;
                case 3:
                    b || (b = Ac(e, 0)), b.I.push(c[f]), f++;
            }
        b && k.push(b);
        return k;
    }
    function Cc(a, b) {
        return a === b;
    }
    var Ub = G.parentNode,
        Dc = G.childNodes,
        Ec = {},
        L = u.deferConnectionCallbacks && "loading" === document.readyState,
        Fc;
    function Gc(a) {
        var b = [];
        do b.unshift(a);
        while ((a = a.parentNode));
        return b;
    }
    function Gb(a, b, c) {
        if (a !== Ec) throw new TypeError("Illegal constructor");
        this.Da = "ShadyRoot";
        this.host = b;
        this.c = c && c.mode;
        Lb(b);
        a = r(b);
        a.root = this;
        a.ra = "closed" !== this.c ? this : null;
        a = r(this);
        a.firstChild =
            a.lastChild =
            a.parentNode =
            a.nextSibling =
            a.previousSibling =
                null;
        a.childNodes = [];
        this.b = this.O = !1;
        this.a = this.h = this.f = null;
        K(this);
    }
    function K(a) {
        a.O ||
            ((a.O = !0),
            Ea(function () {
                return Hc(a);
            }));
    }
    function Hc(a) {
        for (var b; a; ) {
            a.O && (b = a);
            a: {
                var c = a;
                a = c.host.getRootNode();
                if (w(a))
                    for (var d = c.host.childNodes, e = 0; e < d.length; e++)
                        if (((c = d[e]), "slot" == c.localName)) break a;
                a = void 0;
            }
        }
        b && b._renderRoot();
    }
    Gb.prototype._renderRoot = function () {
        var a = L;
        L = !0;
        this.O = !1;
        if (this.f) {
            Tb(this);
            for (var b = 0, c; b < this.f.length; b++) {
                c = this.f[b];
                var d = t(c),
                    e = d.assignedNodes;
                d.assignedNodes = [];
                d.A = [];
                if ((d.ea = e))
                    for (d = 0; d < e.length; d++) {
                        var f = t(e[d]);
                        f.N = f.assignedSlot;
                        f.assignedSlot === c && (f.assignedSlot = null);
                    }
            }
            for (c = this.host.firstChild; c; c = c.nextSibling) Ic(this, c);
            for (b = 0; b < this.f.length; b++) {
                c = this.f[b];
                e = t(c);
                if (!e.assignedNodes.length)
                    for (d = c.firstChild; d; d = d.nextSibling) Ic(this, d, c);
                (d = (d = t(c.parentNode)) && d.root) &&
                    Vb(d) &&
                    d._renderRoot();
                Jc(this, e.A, e.assignedNodes);
                if ((d = e.ea)) {
                    for (f = 0; f < d.length; f++) t(d[f]).N = null;
                    e.ea = null;
                    d.length > e.assignedNodes.length && (e.S = !0);
                }
                e.S && ((e.S = !1), Kc(this, c));
            }
            b = this.f;
            c = [];
            for (e = 0; e < b.length; e++)
                (d = b[e].parentNode),
                    ((f = t(d)) && f.root) || !(0 > c.indexOf(d)) || c.push(d);
            for (b = 0; b < c.length; b++) {
                e = c[b];
                d = e === this ? this.host : e;
                f = [];
                e = e.childNodes;
                for (var g = 0; g < e.length; g++) {
                    var h = e[g];
                    if ("slot" == h.localName) {
                        h = t(h).A;
                        for (var k = 0; k < h.length; k++) f.push(h[k]);
                    } else f.push(h);
                }
                e = void 0;
                g = Dc(d);
                h = Bc(f, f.length, g, g.length);
                for (var l = (k = 0); k < h.length && (e = h[k]); k++) {
                    for (var m = 0, n; m < e.I.length && (n = e.I[m]); m++)
                        Ub(n) === d && x.removeChild.call(d, n),
                            g.splice(e.index + l, 1);
                    l -= e.P;
                }
                for (l = 0; l < h.length && (e = h[l]); l++)
                    for (k = g[e.index], m = e.index; m < e.index + e.P; m++)
                        (n = f[m]),
                            x.insertBefore.call(d, n, k),
                            g.splice(m, 0, n);
            }
        }
        if (!this.b)
            for (n = this.host.childNodes, c = 0, b = n.length; c < b; c++)
                (e = n[c]),
                    (d = t(e)),
                    Ub(e) !== this.host ||
                        ("slot" !== e.localName && d.assignedSlot) ||
                        x.removeChild.call(this.host, e);
        this.b = !0;
        L = a;
        Fc && Fc();
    };
    function Ic(a, b, c) {
        var d = r(b),
            e = d.N;
        d.N = null;
        c || (c = (a = a.h[b.slot || "__catchall"]) && a[0]);
        c
            ? (r(c).assignedNodes.push(b), (d.assignedSlot = c))
            : (d.assignedSlot = void 0);
        e !== d.assignedSlot && d.assignedSlot && (r(d.assignedSlot).S = !0);
    }
    function Jc(a, b, c) {
        for (var d = 0, e; d < c.length && (e = c[d]); d++)
            if ("slot" == e.localName) {
                var f = t(e).assignedNodes;
                f && f.length && Jc(a, b, f);
            } else b.push(c[d]);
    }
    function Kc(a, b) {
        x.dispatchEvent.call(b, new Event("slotchange"));
        b = t(b);
        b.assignedSlot && Kc(a, b.assignedSlot);
    }
    function Tb(a) {
        if (a.a && a.a.length) {
            for (var b = a.a, c, d = 0; d < b.length; d++) {
                var e = b[d];
                Lb(e);
                Lb(e.parentNode);
                var f = Xb(e);
                a.h[f]
                    ? ((c = c || {}), (c[f] = !0), a.h[f].push(e))
                    : (a.h[f] = [e]);
                a.f.push(e);
            }
            if (c) for (var g in c) a.h[g] = Yb(a.h[g]);
            a.a = [];
        }
    }
    function Xb(a) {
        var b = a.name || a.getAttribute("name") || "__catchall";
        return (a.za = b);
    }
    function Yb(a) {
        return a.sort(function (a, c) {
            a = Gc(a);
            for (var b = Gc(c), e = 0; e < a.length; e++) {
                c = a[e];
                var f = b[e];
                if (c !== f)
                    return (
                        (a = Array.from(c.parentNode.childNodes)),
                        a.indexOf(c) - a.indexOf(f)
                    );
            }
        });
    }
    function Vb(a) {
        Tb(a);
        return !(!a.f || !a.f.length);
    }
    if (window.customElements && u.ga) {
        var Lc = new Map();
        Fc = function () {
            var a = Array.from(Lc);
            Lc.clear();
            a = ia(a);
            for (var b = a.next(); !b.done; b = a.next()) {
                b = ia(b.value);
                var c = b.next().value;
                b.next().value ? c.xa() : c.ya();
            }
        };
        L &&
            document.addEventListener(
                "readystatechange",
                function () {
                    L = !1;
                    Fc();
                },
                { once: !0 }
            );
        var Mc = function (a, b, c) {
                var d = 0,
                    e = "__isConnected" + d++;
                if (b || c)
                    (a.prototype.connectedCallback = a.prototype.xa =
                        function () {
                            L
                                ? Lc.set(this, !0)
                                : this[e] ||
                                  ((this[e] = !0), b && b.call(this));
                        }),
                        (a.prototype.disconnectedCallback = a.prototype.ya =
                            function () {
                                L
                                    ? this.isConnected || Lc.set(this, !1)
                                    : this[e] &&
                                      ((this[e] = !1), c && c.call(this));
                            });
                return a;
            },
            define = window.customElements.define;
        Object.defineProperty(
            window.CustomElementRegistry.prototype,
            "define",
            {
                value: function (a, b) {
                    var c = b.prototype.connectedCallback,
                        d = b.prototype.disconnectedCallback;
                    define.call(window.customElements, a, Mc(b, c, d));
                    b.prototype.connectedCallback = c;
                    b.prototype.disconnectedCallback = d;
                },
            }
        );
    }
    function Nc(a) {
        var b = a.getRootNode();
        w(b) && Hc(b);
        return ((a = t(a)) && a.assignedSlot) || null;
    }
    var Oc = {
            addEventListener: sc.bind(window),
            removeEventListener: uc.bind(window),
        },
        Pc = {
            addEventListener: sc,
            removeEventListener: uc,
            appendChild: function (a) {
                return Nb(this, a);
            },
            insertBefore: function (a, b) {
                return Nb(this, a, b);
            },
            removeChild: function (a) {
                return Ob(this, a);
            },
            replaceChild: function (a, b) {
                Nb(this, a, b);
                Ob(this, b);
                return a;
            },
            cloneNode: function (a) {
                if ("template" == this.localName)
                    var b = x.cloneNode.call(this, a);
                else if (
                    ((b = x.cloneNode.call(this, !1)),
                    a && b.nodeType !== Node.ATTRIBUTE_NODE)
                ) {
                    a = this.childNodes;
                    for (var c = 0, d; c < a.length; c++)
                        (d = a[c].cloneNode(!0)), b.appendChild(d);
                }
                return b;
            },
            getRootNode: function () {
                return Zb(this);
            },
            contains: function (a) {
                return Aa(this, a);
            },
            dispatchEvent: function (a) {
                Fa();
                return x.dispatchEvent.call(this, a);
            },
        };
    Object.defineProperties(Pc, {
        isConnected: {
            get: function () {
                if (yb && yb.call(this)) return !0;
                if (this.nodeType == Node.DOCUMENT_FRAGMENT_NODE) return !1;
                var a = this.ownerDocument;
                if (za) {
                    if (x.contains.call(a, this)) return !0;
                } else if (
                    a.documentElement &&
                    x.contains.call(a.documentElement, this)
                )
                    return !0;
                for (a = this; a && !(a instanceof Document); )
                    a = a.parentNode || (w(a) ? a.host : void 0);
                return !!(a && a instanceof Document);
            },
            configurable: !0,
        },
    });
    var Qc = {
            get assignedSlot() {
                return Nc(this);
            },
        },
        Rc = {
            querySelector: function (a) {
                return (
                    $b(
                        this,
                        function (b) {
                            return ra.call(b, a);
                        },
                        function (a) {
                            return !!a;
                        }
                    )[0] || null
                );
            },
            querySelectorAll: function (a, b) {
                if (b) {
                    b = Array.prototype.slice.call(
                        x.querySelectorAll.call(this, a)
                    );
                    var c = this.getRootNode();
                    return b.filter(function (a) {
                        return a.getRootNode() == c;
                    });
                }
                return $b(this, function (b) {
                    return ra.call(b, a);
                });
            },
        },
        Sc = {
            assignedNodes: function (a) {
                if ("slot" === this.localName) {
                    var b = this.getRootNode();
                    w(b) && Hc(b);
                    return (b = t(this))
                        ? (a && a.flatten ? b.A : b.assignedNodes) || []
                        : [];
                }
            },
        },
        Tc = ta(
            {
                setAttribute: function (a, b) {
                    dc(this, a, b);
                },
                removeAttribute: function (a) {
                    x.removeAttribute.call(this, a);
                    Wb(this, a);
                },
                attachShadow: function (a) {
                    if (!this) throw "Must provide a host.";
                    if (!a) throw "Not enough arguments.";
                    return new Gb(Ec, this, a);
                },
                get slot() {
                    return this.getAttribute("slot");
                },
                set slot(a) {
                    dc(this, "slot", a);
                },
                get assignedSlot() {
                    return Nc(this);
                },
            },
            Rc,
            Sc
        );
    Object.defineProperties(Tc, Db);
    var Uc = ta(
        {
            importNode: function (a, b) {
                return ec(a, b);
            },
            getElementById: function (a) {
                return (
                    $b(
                        this,
                        function (b) {
                            return b.id == a;
                        },
                        function (a) {
                            return !!a;
                        }
                    )[0] || null
                );
            },
        },
        Rc
    );
    Object.defineProperties(Uc, { _activeElement: Eb.activeElement });
    for (
        var Vc = HTMLElement.prototype.blur,
            Wc = {
                blur: function () {
                    var a = t(this);
                    (a = (a = a && a.root) && a.activeElement)
                        ? a.blur()
                        : Vc.call(this);
                },
            },
            Xc = {},
            Yc = ia(Object.getOwnPropertyNames(Document.prototype)),
            Zc = Yc.next();
        !Zc.done;
        Xc = { o: Xc.o }, Zc = Yc.next()
    )
        (Xc.o = Zc.value),
            "on" === Xc.o.substring(0, 2) &&
                Object.defineProperty(Wc, Xc.o, {
                    set: (function (a) {
                        return function (b) {
                            var c = r(this),
                                d = a.o.substring(2);
                            c.L[a.o] && this.removeEventListener(d, c.L[a.o]);
                            this.addEventListener(d, b, {});
                            c.L[a.o] = b;
                        };
                    })(Xc),
                    get: (function (a) {
                        return function () {
                            var b = t(this);
                            return b && b.L[a.o];
                        };
                    })(Xc),
                    configurable: !0,
                });
    var $c = {
        addEventListener: function (a, b, c) {
            "object" !== typeof c && (c = { capture: !!c });
            c.$ = this;
            this.host.addEventListener(a, b, c);
        },
        removeEventListener: function (a, b, c) {
            "object" !== typeof c && (c = { capture: !!c });
            c.$ = this;
            this.host.removeEventListener(a, b, c);
        },
        getElementById: function (a) {
            return (
                $b(
                    this,
                    function (b) {
                        return b.id == a;
                    },
                    function (a) {
                        return !!a;
                    }
                )[0] || null
            );
        },
    };
    function M(a, b) {
        for (var c = Object.getOwnPropertyNames(b), d = 0; d < c.length; d++) {
            var e = c[d],
                f = Object.getOwnPropertyDescriptor(b, e);
            f.value ? (a[e] = f.value) : Object.defineProperty(a, e, f);
        }
    }
    if (u.ga) {
        var ShadyDOM = {
            inUse: u.ga,
            patch: function (a) {
                Ib(a);
                Hb(a);
                return a;
            },
            isShadyRoot: w,
            enqueue: Ea,
            flush: Fa,
            settings: u,
            filterMutations: La,
            observeChildren: Ja,
            unobserveChildren: Ka,
            nativeMethods: x,
            nativeTree: G,
            deferConnectionCallbacks: u.deferConnectionCallbacks,
        };
        window.ShadyDOM = ShadyDOM;
        zc();
        var ad =
            (window.customElements &&
                window.customElements.nativeHTMLElement) ||
            HTMLElement;
        M(Gb.prototype, $c);
        M(window.Node.prototype, Pc);
        M(window.Window.prototype, Oc);
        M(window.Text.prototype, Qc);
        M(window.DocumentFragment.prototype, Rc);
        M(window.Element.prototype, Tc);
        M(window.Document.prototype, Uc);
        window.HTMLSlotElement && M(window.HTMLSlotElement.prototype, Sc);
        M(ad.prototype, Wc);
        u.u &&
            (J(window.Node.prototype),
            J(window.Text.prototype),
            J(window.DocumentFragment.prototype),
            J(window.Element.prototype),
            J(ad.prototype),
            J(window.Document.prototype),
            window.HTMLSlotElement && J(window.HTMLSlotElement.prototype));
        Fb();
        window.ShadowRoot = Gb;
    }
    var bd = new Set(
        "annotation-xml color-profile font-face font-face-src font-face-uri font-face-format font-face-name missing-glyph".split(
            " "
        )
    );
    function cd(a) {
        var b = bd.has(a);
        a = /^[a-z][.0-9_a-z]*-[\-.0-9_a-z]*$/.test(a);
        return !b && a;
    }
    function N(a) {
        var b = a.isConnected;
        if (void 0 !== b) return b;
        for (; a && !(a.__CE_isImportDocument || a instanceof Document); )
            a =
                a.parentNode ||
                (window.ShadowRoot && a instanceof ShadowRoot
                    ? a.host
                    : void 0);
        return !(!a || !(a.__CE_isImportDocument || a instanceof Document));
    }
    function dd(a, b) {
        for (; b && b !== a && !b.nextSibling; ) b = b.parentNode;
        return b && b !== a ? b.nextSibling : null;
    }
    function ed(a, b, c) {
        c = void 0 === c ? new Set() : c;
        for (var d = a; d; ) {
            if (d.nodeType === Node.ELEMENT_NODE) {
                var e = d;
                b(e);
                var f = e.localName;
                if ("link" === f && "import" === e.getAttribute("rel")) {
                    d = e.import;
                    if (d instanceof Node && !c.has(d))
                        for (c.add(d), d = d.firstChild; d; d = d.nextSibling)
                            ed(d, b, c);
                    d = dd(a, e);
                    continue;
                } else if ("template" === f) {
                    d = dd(a, e);
                    continue;
                }
                if ((e = e.__CE_shadowRoot))
                    for (e = e.firstChild; e; e = e.nextSibling) ed(e, b, c);
            }
            d = d.firstChild ? d.firstChild : dd(a, d);
        }
    }
    function O(a, b, c) {
        a[b] = c;
    }
    function fd() {
        this.a = new Map();
        this.l = new Map();
        this.g = [];
        this.c = !1;
    }
    function gd(a, b, c) {
        a.a.set(b, c);
        a.l.set(c.constructor, c);
    }
    function hd(a, b) {
        a.c = !0;
        a.g.push(b);
    }
    function id(a, b) {
        a.c &&
            ed(b, function (b) {
                return a.b(b);
            });
    }
    fd.prototype.b = function (a) {
        if (this.c && !a.__CE_patched) {
            a.__CE_patched = !0;
            for (var b = 0; b < this.g.length; b++) this.g[b](a);
        }
    };
    function P(a, b) {
        var c = [];
        ed(b, function (a) {
            return c.push(a);
        });
        for (b = 0; b < c.length; b++) {
            var d = c[b];
            1 === d.__CE_state ? a.connectedCallback(d) : jd(a, d);
        }
    }
    function Q(a, b) {
        var c = [];
        ed(b, function (a) {
            return c.push(a);
        });
        for (b = 0; b < c.length; b++) {
            var d = c[b];
            1 === d.__CE_state && a.disconnectedCallback(d);
        }
    }
    function R(a, b, c) {
        c = void 0 === c ? {} : c;
        var d = c.Wa || new Set(),
            e =
                c.W ||
                function (b) {
                    return jd(a, b);
                },
            f = [];
        ed(
            b,
            function (b) {
                if (
                    "link" === b.localName &&
                    "import" === b.getAttribute("rel")
                ) {
                    var c = b.import;
                    c instanceof Node &&
                        ((c.__CE_isImportDocument = !0),
                        (c.__CE_hasRegistry = !0));
                    c && "complete" === c.readyState
                        ? (c.__CE_documentLoadHandled = !0)
                        : b.addEventListener("load", function () {
                              var c = b.import;
                              if (!c.__CE_documentLoadHandled) {
                                  c.__CE_documentLoadHandled = !0;
                                  var f = new Set(d);
                                  f.delete(c);
                                  R(a, c, { Wa: f, W: e });
                              }
                          });
                } else f.push(b);
            },
            d
        );
        if (a.c) for (b = 0; b < f.length; b++) a.b(f[b]);
        for (b = 0; b < f.length; b++) e(f[b]);
    }
    function jd(a, b) {
        if (void 0 === b.__CE_state) {
            var c = b.ownerDocument;
            if (
                c.defaultView ||
                (c.__CE_isImportDocument && c.__CE_hasRegistry)
            )
                if ((c = a.a.get(b.localName))) {
                    c.constructionStack.push(b);
                    var d = c.constructor;
                    try {
                        try {
                            if (new d() !== b)
                                throw Error(
                                    "The custom element constructor did not produce the element being upgraded."
                                );
                        } finally {
                            c.constructionStack.pop();
                        }
                    } catch (g) {
                        throw ((b.__CE_state = 2), g);
                    }
                    b.__CE_state = 1;
                    b.__CE_definition = c;
                    if (c.attributeChangedCallback)
                        for (
                            c = c.observedAttributes, d = 0;
                            d < c.length;
                            d++
                        ) {
                            var e = c[d],
                                f = b.getAttribute(e);
                            null !== f &&
                                a.attributeChangedCallback(b, e, null, f, null);
                        }
                    N(b) && a.connectedCallback(b);
                }
        }
    }
    fd.prototype.connectedCallback = function (a) {
        var b = a.__CE_definition;
        b.connectedCallback && b.connectedCallback.call(a);
    };
    fd.prototype.disconnectedCallback = function (a) {
        var b = a.__CE_definition;
        b.disconnectedCallback && b.disconnectedCallback.call(a);
    };
    fd.prototype.attributeChangedCallback = function (a, b, c, d, e) {
        var f = a.__CE_definition;
        f.attributeChangedCallback &&
            -1 < f.observedAttributes.indexOf(b) &&
            f.attributeChangedCallback.call(a, b, c, d, e);
    };
    function kd(a) {
        var b = document;
        this.i = a;
        this.a = b;
        this.C = void 0;
        R(this.i, this.a);
        "loading" === this.a.readyState &&
            ((this.C = new MutationObserver(this.b.bind(this))),
            this.C.observe(this.a, { childList: !0, subtree: !0 }));
    }
    function ld(a) {
        a.C && a.C.disconnect();
    }
    kd.prototype.b = function (a) {
        var b = this.a.readyState;
        ("interactive" !== b && "complete" !== b) || ld(this);
        for (b = 0; b < a.length; b++)
            for (var c = a[b].addedNodes, d = 0; d < c.length; d++)
                R(this.i, c[d]);
    };
    function md() {
        var a = this;
        this.b = this.a = void 0;
        this.c = new Promise(function (b) {
            a.b = b;
            a.a && b(a.a);
        });
    }
    function nd(a) {
        if (a.a) throw Error("Already resolved.");
        a.a = void 0;
        a.b && a.b(void 0);
    }
    function S(a) {
        this.ba = !1;
        this.i = a;
        this.fa = new Map();
        this.ca = function (a) {
            return a();
        };
        this.M = !1;
        this.da = [];
        this.Ca = new kd(a);
    }
    p = S.prototype;
    p.define = function (a, b) {
        var c = this;
        if (!(b instanceof Function))
            throw new TypeError(
                "Custom element constructors must be functions."
            );
        if (!cd(a))
            throw new SyntaxError("The element name '" + a + "' is not valid.");
        if (this.i.a.get(a))
            throw Error(
                "A custom element with name '" +
                    a +
                    "' has already been defined."
            );
        if (this.ba) throw Error("A custom element is already being defined.");
        this.ba = !0;
        try {
            var d = function (a) {
                    var b = e[a];
                    if (void 0 !== b && !(b instanceof Function))
                        throw Error(
                            "The '" + a + "' callback must be a function."
                        );
                    return b;
                },
                e = b.prototype;
            if (!(e instanceof Object))
                throw new TypeError(
                    "The custom element constructor's prototype is not an object."
                );
            var f = d("connectedCallback");
            var g = d("disconnectedCallback");
            var h = d("adoptedCallback");
            var k = d("attributeChangedCallback");
            var l = b.observedAttributes || [];
        } catch (m) {
            return;
        } finally {
            this.ba = !1;
        }
        b = {
            localName: a,
            constructor: b,
            connectedCallback: f,
            disconnectedCallback: g,
            adoptedCallback: h,
            attributeChangedCallback: k,
            observedAttributes: l,
            constructionStack: [],
        };
        gd(this.i, a, b);
        this.da.push(b);
        this.M ||
            ((this.M = !0),
            this.ca(function () {
                return od(c);
            }));
    };
    p.W = function (a) {
        R(this.i, a);
    };
    function od(a) {
        if (!1 !== a.M) {
            a.M = !1;
            for (var b = a.da, c = [], d = new Map(), e = 0; e < b.length; e++)
                d.set(b[e].localName, []);
            R(a.i, document, {
                W: function (b) {
                    if (void 0 === b.__CE_state) {
                        var e = b.localName,
                            f = d.get(e);
                        f ? f.push(b) : a.i.a.get(e) && c.push(b);
                    }
                },
            });
            for (e = 0; e < c.length; e++) jd(a.i, c[e]);
            for (; 0 < b.length; ) {
                var f = b.shift();
                e = f.localName;
                f = d.get(f.localName);
                for (var g = 0; g < f.length; g++) jd(a.i, f[g]);
                (e = a.fa.get(e)) && nd(e);
            }
        }
    }
    p.get = function (a) {
        if ((a = this.i.a.get(a))) return a.constructor;
    };
    p.whenDefined = function (a) {
        if (!cd(a))
            return Promise.reject(
                new SyntaxError(
                    "'" + a + "' is not a valid custom element name."
                )
            );
        var b = this.fa.get(a);
        if (b) return b.c;
        b = new md();
        this.fa.set(a, b);
        this.i.a.get(a) &&
            !this.da.some(function (b) {
                return b.localName === a;
            }) &&
            nd(b);
        return b.c;
    };
    p.Sa = function (a) {
        ld(this.Ca);
        var b = this.ca;
        this.ca = function (c) {
            return a(function () {
                return b(c);
            });
        };
    };
    window.CustomElementRegistry = S;
    S.prototype.define = S.prototype.define;
    S.prototype.upgrade = S.prototype.W;
    S.prototype.get = S.prototype.get;
    S.prototype.whenDefined = S.prototype.whenDefined;
    S.prototype.polyfillWrapFlushCallback = S.prototype.Sa;
    var pd = window.Document.prototype.createElement,
        qd = window.Document.prototype.createElementNS,
        rd = window.Document.prototype.importNode,
        sd = window.Document.prototype.prepend,
        td = window.Document.prototype.append,
        ud = window.DocumentFragment.prototype.prepend,
        vd = window.DocumentFragment.prototype.append,
        wd = window.Node.prototype.cloneNode,
        xd = window.Node.prototype.appendChild,
        yd = window.Node.prototype.insertBefore,
        zd = window.Node.prototype.removeChild,
        Ad = window.Node.prototype.replaceChild,
        Bd = Object.getOwnPropertyDescriptor(
            window.Node.prototype,
            "textContent"
        ),
        Cd = window.Element.prototype.attachShadow,
        Dd = Object.getOwnPropertyDescriptor(
            window.Element.prototype,
            "innerHTML"
        ),
        Ed = window.Element.prototype.getAttribute,
        Fd = window.Element.prototype.setAttribute,
        Gd = window.Element.prototype.removeAttribute,
        Hd = window.Element.prototype.getAttributeNS,
        Id = window.Element.prototype.setAttributeNS,
        Jd = window.Element.prototype.removeAttributeNS,
        Kd = window.Element.prototype.insertAdjacentElement,
        Ld = window.Element.prototype.insertAdjacentHTML,
        Md = window.Element.prototype.prepend,
        Nd = window.Element.prototype.append,
        Od = window.Element.prototype.before,
        Pd = window.Element.prototype.after,
        Qd = window.Element.prototype.replaceWith,
        Rd = window.Element.prototype.remove,
        Sd = window.HTMLElement,
        Td = Object.getOwnPropertyDescriptor(
            window.HTMLElement.prototype,
            "innerHTML"
        ),
        Ud = window.HTMLElement.prototype.insertAdjacentElement,
        Vd = window.HTMLElement.prototype.insertAdjacentHTML;
    var Wd = new (function () {})();
    function Xd() {
        var a = Yd;
        window.HTMLElement = (function () {
            function b() {
                var b = this.constructor,
                    d = a.l.get(b);
                if (!d)
                    throw Error(
                        "The custom element being constructed was not registered with `customElements`."
                    );
                var e = d.constructionStack;
                if (0 === e.length)
                    return (
                        (e = pd.call(document, d.localName)),
                        Object.setPrototypeOf(e, b.prototype),
                        (e.__CE_state = 1),
                        (e.__CE_definition = d),
                        a.b(e),
                        e
                    );
                d = e.length - 1;
                var f = e[d];
                if (f === Wd)
                    throw Error(
                        "The HTMLElement constructor was either called reentrantly for this constructor or called multiple times."
                    );
                e[d] = Wd;
                Object.setPrototypeOf(f, b.prototype);
                a.b(f);
                return f;
            }
            b.prototype = Sd.prototype;
            Object.defineProperty(b.prototype, "constructor", {
                writable: !0,
                configurable: !0,
                enumerable: !1,
                value: b,
            });
            return b;
        })();
    }
    function Zd(a, b, c) {
        function d(b) {
            return function (c) {
                for (var d = [], e = 0; e < arguments.length; ++e)
                    d[e - 0] = arguments[e];
                e = [];
                for (var f = [], l = 0; l < d.length; l++) {
                    var m = d[l];
                    m instanceof Element && N(m) && f.push(m);
                    if (m instanceof DocumentFragment)
                        for (m = m.firstChild; m; m = m.nextSibling) e.push(m);
                    else e.push(m);
                }
                b.apply(this, d);
                for (d = 0; d < f.length; d++) Q(a, f[d]);
                if (N(this))
                    for (d = 0; d < e.length; d++)
                        (f = e[d]), f instanceof Element && P(a, f);
            };
        }
        void 0 !== c.U && (b.prepend = d(c.U));
        void 0 !== c.append && (b.append = d(c.append));
    }
    function $d() {
        var a = Yd;
        O(Document.prototype, "createElement", function (b) {
            if (this.__CE_hasRegistry) {
                var c = a.a.get(b);
                if (c) return new c.constructor();
            }
            b = pd.call(this, b);
            a.b(b);
            return b;
        });
        O(Document.prototype, "importNode", function (b, c) {
            b = rd.call(this, b, c);
            this.__CE_hasRegistry ? R(a, b) : id(a, b);
            return b;
        });
        O(Document.prototype, "createElementNS", function (b, c) {
            if (
                this.__CE_hasRegistry &&
                (null === b || "http://www.w3.org/1999/xhtml" === b)
            ) {
                var d = a.a.get(c);
                if (d) return new d.constructor();
            }
            b = qd.call(this, b, c);
            a.b(b);
            return b;
        });
        Zd(a, Document.prototype, { U: sd, append: td });
    }
    function ae() {
        var a = Yd;
        function b(b, d) {
            Object.defineProperty(b, "textContent", {
                enumerable: d.enumerable,
                configurable: !0,
                get: d.get,
                set: function (b) {
                    if (this.nodeType === Node.TEXT_NODE) d.set.call(this, b);
                    else {
                        var c = void 0;
                        if (this.firstChild) {
                            var e = this.childNodes,
                                h = e.length;
                            if (0 < h && N(this)) {
                                c = Array(h);
                                for (var k = 0; k < h; k++) c[k] = e[k];
                            }
                        }
                        d.set.call(this, b);
                        if (c) for (b = 0; b < c.length; b++) Q(a, c[b]);
                    }
                },
            });
        }
        O(Node.prototype, "insertBefore", function (b, d) {
            if (b instanceof DocumentFragment) {
                var c = Array.prototype.slice.apply(b.childNodes);
                b = yd.call(this, b, d);
                if (N(this)) for (d = 0; d < c.length; d++) P(a, c[d]);
                return b;
            }
            c = N(b);
            d = yd.call(this, b, d);
            c && Q(a, b);
            N(this) && P(a, b);
            return d;
        });
        O(Node.prototype, "appendChild", function (b) {
            if (b instanceof DocumentFragment) {
                var c = Array.prototype.slice.apply(b.childNodes);
                b = xd.call(this, b);
                if (N(this)) for (var e = 0; e < c.length; e++) P(a, c[e]);
                return b;
            }
            c = N(b);
            e = xd.call(this, b);
            c && Q(a, b);
            N(this) && P(a, b);
            return e;
        });
        O(Node.prototype, "cloneNode", function (b) {
            b = wd.call(this, b);
            this.ownerDocument.__CE_hasRegistry ? R(a, b) : id(a, b);
            return b;
        });
        O(Node.prototype, "removeChild", function (b) {
            var c = N(b),
                e = zd.call(this, b);
            c && Q(a, b);
            return e;
        });
        O(Node.prototype, "replaceChild", function (b, d) {
            if (b instanceof DocumentFragment) {
                var c = Array.prototype.slice.apply(b.childNodes);
                b = Ad.call(this, b, d);
                if (N(this)) for (Q(a, d), d = 0; d < c.length; d++) P(a, c[d]);
                return b;
            }
            c = N(b);
            var f = Ad.call(this, b, d),
                g = N(this);
            g && Q(a, d);
            c && Q(a, b);
            g && P(a, b);
            return f;
        });
        Bd && Bd.get
            ? b(Node.prototype, Bd)
            : hd(a, function (a) {
                  b(a, {
                      enumerable: !0,
                      configurable: !0,
                      get: function () {
                          for (
                              var a = [], b = 0;
                              b < this.childNodes.length;
                              b++
                          )
                              a.push(this.childNodes[b].textContent);
                          return a.join("");
                      },
                      set: function (a) {
                          for (; this.firstChild; )
                              zd.call(this, this.firstChild);
                          xd.call(this, document.createTextNode(a));
                      },
                  });
              });
    }
    function be(a) {
        var b = Element.prototype;
        function c(b) {
            return function (c) {
                for (var d = [], e = 0; e < arguments.length; ++e)
                    d[e - 0] = arguments[e];
                e = [];
                for (var h = [], k = 0; k < d.length; k++) {
                    var l = d[k];
                    l instanceof Element && N(l) && h.push(l);
                    if (l instanceof DocumentFragment)
                        for (l = l.firstChild; l; l = l.nextSibling) e.push(l);
                    else e.push(l);
                }
                b.apply(this, d);
                for (d = 0; d < h.length; d++) Q(a, h[d]);
                if (N(this))
                    for (d = 0; d < e.length; d++)
                        (h = e[d]), h instanceof Element && P(a, h);
            };
        }
        void 0 !== Od && (b.before = c(Od));
        void 0 !== Od && (b.after = c(Pd));
        void 0 !== Qd &&
            O(b, "replaceWith", function (b) {
                for (var c = [], d = 0; d < arguments.length; ++d)
                    c[d - 0] = arguments[d];
                d = [];
                for (var g = [], h = 0; h < c.length; h++) {
                    var k = c[h];
                    k instanceof Element && N(k) && g.push(k);
                    if (k instanceof DocumentFragment)
                        for (k = k.firstChild; k; k = k.nextSibling) d.push(k);
                    else d.push(k);
                }
                h = N(this);
                Qd.apply(this, c);
                for (c = 0; c < g.length; c++) Q(a, g[c]);
                if (h)
                    for (Q(a, this), c = 0; c < d.length; c++)
                        (g = d[c]), g instanceof Element && P(a, g);
            });
        void 0 !== Rd &&
            O(b, "remove", function () {
                var b = N(this);
                Rd.call(this);
                b && Q(a, this);
            });
    }
    function ce() {
        var a = Yd;
        function b(b, c) {
            Object.defineProperty(b, "innerHTML", {
                enumerable: c.enumerable,
                configurable: !0,
                get: c.get,
                set: function (b) {
                    var d = this,
                        e = void 0;
                    N(this) &&
                        ((e = []),
                        ed(this, function (a) {
                            a !== d && e.push(a);
                        }));
                    c.set.call(this, b);
                    if (e)
                        for (var f = 0; f < e.length; f++) {
                            var g = e[f];
                            1 === g.__CE_state && a.disconnectedCallback(g);
                        }
                    this.ownerDocument.__CE_hasRegistry
                        ? R(a, this)
                        : id(a, this);
                    return b;
                },
            });
        }
        function c(b, c) {
            O(b, "insertAdjacentElement", function (b, d) {
                var e = N(d);
                b = c.call(this, b, d);
                e && Q(a, d);
                N(b) && P(a, d);
                return b;
            });
        }
        function d(b, c) {
            function d(b, c) {
                for (var d = []; b !== c; b = b.nextSibling) d.push(b);
                for (c = 0; c < d.length; c++) R(a, d[c]);
            }
            O(b, "insertAdjacentHTML", function (a, b) {
                a = a.toLowerCase();
                if ("beforebegin" === a) {
                    var e = this.previousSibling;
                    c.call(this, a, b);
                    d(e || this.parentNode.firstChild, this);
                } else if ("afterbegin" === a)
                    (e = this.firstChild),
                        c.call(this, a, b),
                        d(this.firstChild, e);
                else if ("beforeend" === a)
                    (e = this.lastChild),
                        c.call(this, a, b),
                        d(e || this.firstChild, null);
                else if ("afterend" === a)
                    (e = this.nextSibling),
                        c.call(this, a, b),
                        d(this.nextSibling, e);
                else
                    throw new SyntaxError(
                        "The value provided (" +
                            String(a) +
                            ") is not one of 'beforebegin', 'afterbegin', 'beforeend', or 'afterend'."
                    );
            });
        }
        Cd &&
            O(Element.prototype, "attachShadow", function (a) {
                return (this.__CE_shadowRoot = a = Cd.call(this, a));
            });
        Dd && Dd.get
            ? b(Element.prototype, Dd)
            : Td && Td.get
            ? b(HTMLElement.prototype, Td)
            : hd(a, function (a) {
                  b(a, {
                      enumerable: !0,
                      configurable: !0,
                      get: function () {
                          return wd.call(this, !0).innerHTML;
                      },
                      set: function (a) {
                          var b = "template" === this.localName,
                              c = b ? this.content : this,
                              d = qd.call(
                                  document,
                                  this.namespaceURI,
                                  this.localName
                              );
                          for (d.innerHTML = a; 0 < c.childNodes.length; )
                              zd.call(c, c.childNodes[0]);
                          for (a = b ? d.content : d; 0 < a.childNodes.length; )
                              xd.call(c, a.childNodes[0]);
                      },
                  });
              });
        O(Element.prototype, "setAttribute", function (b, c) {
            if (1 !== this.__CE_state) return Fd.call(this, b, c);
            var d = Ed.call(this, b);
            Fd.call(this, b, c);
            c = Ed.call(this, b);
            a.attributeChangedCallback(this, b, d, c, null);
        });
        O(Element.prototype, "setAttributeNS", function (b, c, d) {
            if (1 !== this.__CE_state) return Id.call(this, b, c, d);
            var e = Hd.call(this, b, c);
            Id.call(this, b, c, d);
            d = Hd.call(this, b, c);
            a.attributeChangedCallback(this, c, e, d, b);
        });
        O(Element.prototype, "removeAttribute", function (b) {
            if (1 !== this.__CE_state) return Gd.call(this, b);
            var c = Ed.call(this, b);
            Gd.call(this, b);
            null !== c && a.attributeChangedCallback(this, b, c, null, null);
        });
        O(Element.prototype, "removeAttributeNS", function (b, c) {
            if (1 !== this.__CE_state) return Jd.call(this, b, c);
            var d = Hd.call(this, b, c);
            Jd.call(this, b, c);
            var e = Hd.call(this, b, c);
            d !== e && a.attributeChangedCallback(this, c, d, e, b);
        });
        Ud
            ? c(HTMLElement.prototype, Ud)
            : Kd
            ? c(Element.prototype, Kd)
            : console.warn(
                  "Custom Elements: `Element#insertAdjacentElement` was not patched."
              );
        Vd
            ? d(HTMLElement.prototype, Vd)
            : Ld
            ? d(Element.prototype, Ld)
            : console.warn(
                  "Custom Elements: `Element#insertAdjacentHTML` was not patched."
              );
        Zd(a, Element.prototype, { U: Md, append: Nd });
        be(a);
    }
    var de = window.customElements;
    if (
        !de ||
        de.forcePolyfill ||
        "function" != typeof de.define ||
        "function" != typeof de.get
    ) {
        var Yd = new fd();
        Xd();
        $d();
        Zd(Yd, DocumentFragment.prototype, { U: ud, append: vd });
        ae();
        ce();
        document.__CE_hasRegistry = !0;
        var customElements = new S(Yd);
        Object.defineProperty(window, "customElements", {
            configurable: !0,
            enumerable: !0,
            value: customElements,
        });
    } /*

Copyright (c) 2017 The Polymer Project Authors. All rights reserved.
This code may only be used under the BSD style license found at http://polymer.github.io/LICENSE.txt
The complete set of authors may be found at http://polymer.github.io/AUTHORS.txt
The complete set of contributors may be found at http://polymer.github.io/CONTRIBUTORS.txt
Code distributed by Google as part of the polymer project is also
subject to an additional IP rights grant found at http://polymer.github.io/PATENTS.txt
*/
    function ee() {
        this.end = this.start = 0;
        this.rules = this.parent = this.previous = null;
        this.cssText = this.parsedCssText = "";
        this.atRule = !1;
        this.type = 0;
        this.parsedSelector = this.selector = this.keyframesName = "";
    }
    function fe(a) {
        a = a.replace(ge, "").replace(he, "");
        var b = ie,
            c = a,
            d = new ee();
        d.start = 0;
        d.end = c.length;
        for (var e = d, f = 0, g = c.length; f < g; f++)
            if ("{" === c[f]) {
                e.rules || (e.rules = []);
                var h = e,
                    k = h.rules[h.rules.length - 1] || null;
                e = new ee();
                e.start = f + 1;
                e.parent = h;
                e.previous = k;
                h.rules.push(e);
            } else "}" === c[f] && ((e.end = f + 1), (e = e.parent || d));
        return b(d, a);
    }
    function ie(a, b) {
        var c = b.substring(a.start, a.end - 1);
        a.parsedCssText = a.cssText = c.trim();
        a.parent &&
            ((c = b.substring(
                a.previous ? a.previous.end : a.parent.start,
                a.start - 1
            )),
            (c = je(c)),
            (c = c.replace(ke, " ")),
            (c = c.substring(c.lastIndexOf(";") + 1)),
            (c = a.parsedSelector = a.selector = c.trim()),
            (a.atRule = 0 === c.indexOf("@")),
            a.atRule
                ? 0 === c.indexOf("@media")
                    ? (a.type = le)
                    : c.match(me) &&
                      ((a.type = ne),
                      (a.keyframesName = a.selector.split(ke).pop()))
                : (a.type = 0 === c.indexOf("--") ? oe : pe));
        if ((c = a.rules))
            for (var d = 0, e = c.length, f; d < e && (f = c[d]); d++) ie(f, b);
        return a;
    }
    function je(a) {
        return a.replace(/\\([0-9a-f]{1,6})\s/gi, function (a, c) {
            a = c;
            for (c = 6 - a.length; c--; ) a = "0" + a;
            return "\\" + a;
        });
    }
    function qe(a, b, c) {
        c = void 0 === c ? "" : c;
        var d = "";
        if (a.cssText || a.rules) {
            var e = a.rules,
                f;
            if ((f = e))
                (f = e[0]),
                    (f = !(f && f.selector && 0 === f.selector.indexOf("--")));
            if (f) {
                f = 0;
                for (var g = e.length, h; f < g && (h = e[f]); f++)
                    d = qe(h, b, d);
            } else
                b
                    ? (b = a.cssText)
                    : ((b = a.cssText),
                      (b = b.replace(re, "").replace(se, "")),
                      (b = b.replace(te, "").replace(ue, ""))),
                    (d = b.trim()) && (d = "  " + d + "\n");
        }
        d &&
            (a.selector && (c += a.selector + " {\n"),
            (c += d),
            a.selector && (c += "}\n\n"));
        return c;
    }
    var pe = 1,
        ne = 7,
        le = 4,
        oe = 1e3,
        ge = /\/\*[^*]*\*+([^/*][^*]*\*+)*\//gim,
        he = /@import[^;]*;/gim,
        re = /(?:^[^;\-\s}]+)?--[^;{}]*?:[^{};]*?(?:[;\n]|$)/gim,
        se = /(?:^[^;\-\s}]+)?--[^;{}]*?:[^{};]*?{[^}]*?}(?:[;\n]|$)?/gim,
        te = /@apply\s*\(?[^);]*\)?\s*(?:[;\n]|$)?/gim,
        ue = /[^;:]*?:[^;]*?var\([^;]*\)(?:[;\n]|$)?/gim,
        me = /^@[^\s]*keyframes/,
        ke = /\s+/g;
    var T = !(window.ShadyDOM && window.ShadyDOM.inUse),
        ye;
    function ze(a) {
        ye =
            a && a.shimcssproperties
                ? !1
                : T ||
                  !(
                      navigator.userAgent.match(/AppleWebKit\/601|Edge\/15/) ||
                      !window.CSS ||
                      !CSS.supports ||
                      !CSS.supports("box-shadow", "0 0 0 var(--foo)")
                  );
    }
    window.ShadyCSS && void 0 !== window.ShadyCSS.nativeCss
        ? (ye = window.ShadyCSS.nativeCss)
        : window.ShadyCSS
        ? (ze(window.ShadyCSS), (window.ShadyCSS = void 0))
        : ze(window.WebComponents && window.WebComponents.flags);
    var U = ye;
    var Ae =
            /(?:^|[;\s{]\s*)(--[\w-]*?)\s*:\s*(?:((?:'(?:\\'|.)*?'|"(?:\\"|.)*?"|\([^)]*?\)|[^};{])+)|\{([^}]*)\}(?:(?=[;\s}])|$))/gi,
        Be = /(?:^|\W+)@apply\s*\(?([^);\n]*)\)?/gi,
        Ce = /(--[\w-]+)\s*([:,;)]|$)/gi,
        De = /(animation\s*:)|(animation-name\s*:)/,
        Ee = /@media\s(.*)/,
        Fe = /\{[^}]*\}/g;
    var Ge = new Set();
    function He(a, b) {
        if (!a) return "";
        "string" === typeof a && (a = fe(a));
        b && Ie(a, b);
        return qe(a, U);
    }
    function Je(a) {
        !a.__cssRules && a.textContent && (a.__cssRules = fe(a.textContent));
        return a.__cssRules || null;
    }
    function Ke(a) {
        return !!a.parent && a.parent.type === ne;
    }
    function Ie(a, b, c, d) {
        if (a) {
            var e = !1,
                f = a.type;
            if (d && f === le) {
                var g = a.selector.match(Ee);
                g && (window.matchMedia(g[1]).matches || (e = !0));
            }
            f === pe ? b(a) : c && f === ne ? c(a) : f === oe && (e = !0);
            if ((a = a.rules) && !e) {
                e = 0;
                f = a.length;
                for (var h; e < f && (h = a[e]); e++) Ie(h, b, c, d);
            }
        }
    }
    function Le(a, b, c, d) {
        var e = document.createElement("style");
        b && e.setAttribute("scope", b);
        e.textContent = a;
        Me(e, c, d);
        return e;
    }
    var Ne = null;
    function Oe(a) {
        a = document.createComment(" Shady DOM styles for " + a + " ");
        var b = document.head;
        b.insertBefore(a, (Ne ? Ne.nextSibling : null) || b.firstChild);
        return (Ne = a);
    }
    function Me(a, b, c) {
        b = b || document.head;
        b.insertBefore(a, (c && c.nextSibling) || b.firstChild);
        Ne
            ? a.compareDocumentPosition(Ne) ===
                  Node.DOCUMENT_POSITION_PRECEDING && (Ne = a)
            : (Ne = a);
    }
    function Pe(a, b) {
        for (var c = 0, d = a.length; b < d; b++)
            if ("(" === a[b]) c++;
            else if (")" === a[b] && 0 === --c) return b;
        return -1;
    }
    function Qe(a, b) {
        var c = a.indexOf("var(");
        if (-1 === c) return b(a, "", "", "");
        var d = Pe(a, c + 3),
            e = a.substring(c + 4, d);
        c = a.substring(0, c);
        a = Qe(a.substring(d + 1), b);
        d = e.indexOf(",");
        return -1 === d
            ? b(c, e.trim(), "", a)
            : b(c, e.substring(0, d).trim(), e.substring(d + 1).trim(), a);
    }
    function Re(a, b) {
        T
            ? a.setAttribute("class", b)
            : window.ShadyDOM.nativeMethods.setAttribute.call(a, "class", b);
    }
    function V(a) {
        var b = a.localName,
            c = "";
        b
            ? -1 < b.indexOf("-") ||
              ((c = b), (b = (a.getAttribute && a.getAttribute("is")) || ""))
            : ((b = a.is), (c = a.extends));
        return { is: b, J: c };
    }
    function Se(a) {
        for (var b = [], c = "", d = 0; 0 <= d && d < a.length; d++)
            if ("(" === a[d]) {
                var e = Pe(a, d);
                c += a.slice(d, e + 1);
                d = e;
            } else "," === a[d] ? (b.push(c), (c = "")) : (c += a[d]);
        c && b.push(c);
        return b;
    }
    function Te() {}
    function Ue(a, b) {
        var c = W;
        a.__styleScoped
            ? (a.__styleScoped = null)
            : Ve(c, a, function (a) {
                  We(a, b || "");
              });
    }
    function Ve(a, b, c) {
        b.nodeType === Node.ELEMENT_NODE && c(b);
        if (
            (b =
                "template" === b.localName
                    ? (b.content || b.bb || b).childNodes
                    : b.children || b.childNodes)
        )
            for (var d = 0; d < b.length; d++) Ve(a, b[d], c);
    }
    function We(a, b, c) {
        if (b)
            if (a.classList)
                c
                    ? (a.classList.remove("style-scope"), a.classList.remove(b))
                    : (a.classList.add("style-scope"), a.classList.add(b));
            else if (a.getAttribute) {
                var d = a.getAttribute(Xe);
                c
                    ? d &&
                      ((b = d.replace("style-scope", "").replace(b, "")),
                      Re(a, b))
                    : Re(a, (d ? d + " " : "") + "style-scope " + b);
            }
    }
    function Ye(a, b, c) {
        var d = W;
        a.__styleScoped
            ? (a.__styleScoped = null)
            : Ve(d, a, function (a) {
                  We(a, b, !0);
                  We(a, c);
              });
    }
    function Ze(a, b) {
        var c = W;
        a.__styleScoped
            ? (a.__styleScoped = null)
            : Ve(c, a, function (a) {
                  We(a, b || "", !0);
              });
    }
    function $e(a, b, c) {
        var d = W,
            e = a.__cssBuild;
        T || "shady" === e
            ? (b = He(b, c))
            : ((a = V(a)), (b = af(d, b, a.is, a.J, c) + "\n\n"));
        return b.trim();
    }
    function af(a, b, c, d, e) {
        var f = bf(c, d);
        c = c ? cf + c : "";
        return He(b, function (b) {
            b.c || ((b.selector = b.m = df(a, b, a.b, c, f)), (b.c = !0));
            e && e(b, c, f);
        });
    }
    function bf(a, b) {
        return b ? "[is=" + a + "]" : a;
    }
    function df(a, b, c, d, e) {
        var f = Se(b.selector);
        if (!Ke(b)) {
            b = 0;
            for (var g = f.length, h; b < g && (h = f[b]); b++)
                f[b] = c.call(a, h, d, e);
        }
        return f
            .filter(function (a) {
                return !!a;
            })
            .join(ef);
    }
    function ff(a) {
        return a.replace(gf, function (a, c, d) {
            -1 < d.indexOf("+")
                ? (d = d.replace(/\+/g, "___"))
                : -1 < d.indexOf("___") && (d = d.replace(/___/g, "+"));
            return ":" + c + "(" + d + ")";
        });
    }
    function hf(a) {
        for (var b = [], c; (c = a.match(jf)); ) {
            var d = c.index,
                e = Pe(a, d);
            if (-1 === e) throw Error(c.input + " selector missing ')'");
            c = a.slice(d, e + 1);
            a = a.replace(c, "\ue000");
            b.push(c);
        }
        return { ia: a, matches: b };
    }
    function kf(a, b) {
        var c = a.split("\ue000");
        return b.reduce(function (a, b, f) {
            return a + b + c[f + 1];
        }, c[0]);
    }
    Te.prototype.b = function (a, b, c) {
        var d = !1;
        a = a.trim();
        var e = gf.test(a);
        e &&
            ((a = a.replace(gf, function (a, b, c) {
                return ":" + b + "(" + c.replace(/\s/g, "") + ")";
            })),
            (a = ff(a)));
        var f = jf.test(a);
        if (f) {
            var g = hf(a);
            a = g.ia;
            g = g.matches;
        }
        a = a.replace(lf, mf + " $1");
        a = a.replace(nf, function (a, e, f) {
            d ||
                ((a = of(f, e, b, c)),
                (d = d || a.stop),
                (e = a.Ha),
                (f = a.value));
            return e + f;
        });
        f && (a = kf(a, g));
        e && (a = ff(a));
        return a;
    };
    function of(a, b, c, d) {
        var e = a.indexOf(pf);
        0 <= a.indexOf(mf) ? (a = qf(a, d)) : 0 !== e && (a = c ? rf(a, c) : a);
        c = !1;
        0 <= e && ((b = ""), (c = !0));
        if (c) {
            var f = !0;
            c &&
                (a = a.replace(sf, function (a, b) {
                    return " > " + b;
                }));
        }
        a = a.replace(tf, function (a, b, c) {
            return '[dir="' + c + '"] ' + b + ", " + b + '[dir="' + c + '"]';
        });
        return { value: a, Ha: b, stop: f };
    }
    function rf(a, b) {
        a = a.split(uf);
        a[0] += b;
        return a.join(uf);
    }
    function qf(a, b) {
        var c = a.match(vf);
        return (c = (c && c[2].trim()) || "")
            ? c[0].match(wf)
                ? a.replace(vf, function (a, c, f) {
                      return b + f;
                  })
                : c.split(wf)[0] === b
                ? c
                : xf
            : a.replace(mf, b);
    }
    function yf(a) {
        a.selector === zf && (a.selector = "html");
    }
    Te.prototype.c = function (a) {
        return a.match(mf)
            ? ""
            : a.match(pf)
            ? this.b(a, Af)
            : rf(a.trim(), Af);
    };
    q.Object.defineProperties(Te.prototype, {
        a: {
            configurable: !0,
            enumerable: !0,
            get: function () {
                return "style-scope";
            },
        },
    });
    var gf = /:(nth[-\w]+)\(([^)]+)\)/,
        Af = ":not(.style-scope)",
        ef = ",",
        nf = /(^|[\s>+~]+)((?:\[.+?\]|[^\s>+~=[])+)/g,
        wf = /[[.:#*]/,
        mf = ":host",
        zf = ":root",
        pf = "::slotted",
        lf = new RegExp("^(" + pf + ")"),
        vf = /(:host)(?:\(((?:\([^)(]*\)|[^)(]*)+?)\))/,
        sf = /(?:::slotted)(?:\(((?:\([^)(]*\)|[^)(]*)+?)\))/,
        tf = /(.*):dir\((?:(ltr|rtl))\)/,
        cf = ".",
        uf = ":",
        Xe = "class",
        xf = "should_not_match",
        jf = /:(?:matches|any|-(?:webkit|moz)-any)/,
        W = new Te();
    function Bf(a, b, c, d) {
        this.w = a || null;
        this.b = b || null;
        this.ha = c || [];
        this.G = null;
        this.J = d || "";
        this.a = this.s = this.B = null;
    }
    function X(a) {
        return a ? a.__styleInfo : null;
    }
    function Cf(a, b) {
        return (a.__styleInfo = b);
    }
    Bf.prototype.c = function () {
        return this.w;
    };
    Bf.prototype._getStyleRules = Bf.prototype.c;
    function Df(a) {
        var b =
            this.matches ||
            this.matchesSelector ||
            this.mozMatchesSelector ||
            this.msMatchesSelector ||
            this.oMatchesSelector ||
            this.webkitMatchesSelector;
        return b && b.call(this, a);
    }
    var Ef = navigator.userAgent.match("Trident");
    function Ff() {}
    function Gf(a) {
        var b = {},
            c = [],
            d = 0;
        Ie(
            a,
            function (a) {
                Hf(a);
                a.index = d++;
                a = a.j.cssText;
                for (var c; (c = Ce.exec(a)); ) {
                    var e = c[1];
                    ":" !== c[2] && (b[e] = !0);
                }
            },
            function (a) {
                c.push(a);
            }
        );
        a.b = c;
        a = [];
        for (var e in b) a.push(e);
        return a;
    }
    function Hf(a) {
        if (!a.j) {
            var b = {},
                c = {};
            If(a, c) && ((b.v = c), (a.rules = null));
            b.cssText = a.parsedCssText.replace(Fe, "").replace(Ae, "");
            a.j = b;
        }
    }
    function If(a, b) {
        var c = a.j;
        if (c) {
            if (c.v) return Object.assign(b, c.v), !0;
        } else {
            c = a.parsedCssText;
            for (var d; (a = Ae.exec(c)); ) {
                d = (a[2] || a[3]).trim();
                if ("inherit" !== d || "unset" !== d) b[a[1].trim()] = d;
                d = !0;
            }
            return d;
        }
    }
    function Jf(a, b, c) {
        b &&
            (b =
                0 <= b.indexOf(";")
                    ? Kf(a, b, c)
                    : Qe(b, function (b, e, f, g) {
                          if (!e) return b + g;
                          (e = Jf(a, c[e], c)) && "initial" !== e
                              ? "apply-shim-inherit" === e && (e = "inherit")
                              : (e = Jf(a, c[f] || f, c) || f);
                          return b + (e || "") + g;
                      }));
        return (b && b.trim()) || "";
    }
    function Kf(a, b, c) {
        b = b.split(";");
        for (var d = 0, e, f; d < b.length; d++)
            if ((e = b[d])) {
                Be.lastIndex = 0;
                if ((f = Be.exec(e))) e = Jf(a, c[f[1]], c);
                else if (((f = e.indexOf(":")), -1 !== f)) {
                    var g = e.substring(f);
                    g = g.trim();
                    g = Jf(a, g, c) || g;
                    e = e.substring(0, f) + g;
                }
                b[d] =
                    e && e.lastIndexOf(";") === e.length - 1
                        ? e.slice(0, -1)
                        : e || "";
            }
        return b.join(";");
    }
    function Lf(a, b) {
        var c = {},
            d = [];
        Ie(
            a,
            function (a) {
                a.j || Hf(a);
                var e = a.m || a.parsedSelector;
                b &&
                    a.j.v &&
                    e &&
                    Df.call(b, e) &&
                    (If(a, c),
                    (a = a.index),
                    (e = parseInt(a / 32, 10)),
                    (d[e] = (d[e] || 0) | (1 << a % 32)));
            },
            null,
            !0
        );
        return { v: c, key: d };
    }
    function Mf(a, b, c, d) {
        b.j || Hf(b);
        if (b.j.v) {
            var e = V(a);
            a = e.is;
            e = e.J;
            e = a ? bf(a, e) : "html";
            var f = b.parsedSelector,
                g = ":host > *" === f || "html" === f,
                h = 0 === f.indexOf(":host") && !g;
            "shady" === c &&
                ((g = f === e + " > *." + e || -1 !== f.indexOf("html")),
                (h = !g && 0 === f.indexOf(e)));
            "shadow" === c &&
                ((g = ":host > *" === f || "html" === f), (h = h && !g));
            if (g || h)
                (c = e),
                    h &&
                        (b.m || (b.m = df(W, b, W.b, a ? cf + a : "", e)),
                        (c = b.m || e)),
                    d({ ia: c, Na: h, mb: g });
        }
    }
    function Nf(a, b) {
        var c = {},
            d = {},
            e = b && b.__cssBuild;
        Ie(
            b,
            function (b) {
                Mf(a, b, e, function (e) {
                    Df.call(a.cb || a, e.ia) && (e.Na ? If(b, c) : If(b, d));
                });
            },
            null,
            !0
        );
        return { Ta: d, La: c };
    }
    function Of(a, b, c, d) {
        var e = V(b),
            f = bf(e.is, e.J),
            g = new RegExp(
                "(?:^|[^.#[:])" +
                    (b.extends ? "\\" + f.slice(0, -1) + "\\]" : f) +
                    "($|[.:[\\s>+~])"
            );
        e = X(b).w;
        var h = Pf(e, d);
        return $e(b, e, function (b) {
            var e = "";
            b.j || Hf(b);
            b.j.cssText && (e = Kf(a, b.j.cssText, c));
            b.cssText = e;
            if (!T && !Ke(b) && b.cssText) {
                var k = (e = b.cssText);
                null == b.pa && (b.pa = De.test(e));
                if (b.pa)
                    if (null == b.T) {
                        b.T = [];
                        for (var n in h)
                            (k = h[n]),
                                (k = k(e)),
                                e !== k && ((e = k), b.T.push(n));
                    } else {
                        for (n = 0; n < b.T.length; ++n)
                            (k = h[b.T[n]]), (e = k(e));
                        k = e;
                    }
                b.cssText = k;
                b.m = b.m || b.selector;
                e = "." + d;
                n = Se(b.m);
                k = 0;
                for (var A = n.length, E; k < A && (E = n[k]); k++)
                    n[k] = E.match(g) ? E.replace(f, e) : e + " " + E;
                b.selector = n.join(",");
            }
        });
    }
    function Pf(a, b) {
        a = a.b;
        var c = {};
        if (!T && a)
            for (var d = 0, e = a[d]; d < a.length; e = a[++d]) {
                var f = e,
                    g = b;
                f.g = new RegExp("\\b" + f.keyframesName + "(?!\\B|-)", "g");
                f.a = f.keyframesName + "-" + g;
                f.m = f.m || f.selector;
                f.selector = f.m.replace(f.keyframesName, f.a);
                c[e.keyframesName] = Qf(e);
            }
        return c;
    }
    function Qf(a) {
        return function (b) {
            return b.replace(a.g, a.a);
        };
    }
    function Rf(a, b) {
        var c = Sf,
            d = Je(a);
        a.textContent = He(d, function (a) {
            var d = (a.cssText = a.parsedCssText);
            a.j &&
                a.j.cssText &&
                ((d = d.replace(re, "").replace(se, "")),
                (a.cssText = Kf(c, d, b)));
        });
    }
    q.Object.defineProperties(Ff.prototype, {
        a: {
            configurable: !0,
            enumerable: !0,
            get: function () {
                return "x-scope";
            },
        },
    });
    var Sf = new Ff();
    var Tf = {},
        Uf = window.customElements;
    if (Uf && !T) {
        var Vf = Uf.define;
        Uf.define = function (a, b, c) {
            Tf[a] || (Tf[a] = Oe(a));
            Vf.call(Uf, a, b, c);
        };
    }
    function Wf() {
        this.cache = {};
    }
    Wf.prototype.store = function (a, b, c, d) {
        var e = this.cache[a] || [];
        e.push({ v: b, styleElement: c, s: d });
        100 < e.length && e.shift();
        this.cache[a] = e;
    };
    Wf.prototype.fetch = function (a, b, c) {
        if ((a = this.cache[a]))
            for (var d = a.length - 1; 0 <= d; d--) {
                var e = a[d],
                    f;
                a: {
                    for (f = 0; f < c.length; f++) {
                        var g = c[f];
                        if (e.v[g] !== b[g]) {
                            f = !1;
                            break a;
                        }
                    }
                    f = !0;
                }
                if (f) return e;
            }
    };
    function Xf() {}
    function Yf(a) {
        for (var b = 0; b < a.length; b++) {
            var c = a[b];
            if (
                c.target !== document.documentElement &&
                c.target !== document.head
            )
                for (var d = 0; d < c.addedNodes.length; d++) {
                    var e = c.addedNodes[d];
                    if (e.nodeType === Node.ELEMENT_NODE) {
                        var f = e.getRootNode();
                        var g = e;
                        var h = [];
                        g.classList
                            ? (h = Array.from(g.classList))
                            : g instanceof window.SVGElement &&
                              g.hasAttribute("class") &&
                              (h = g.getAttribute("class").split(/\s+/));
                        g = h;
                        h = g.indexOf(W.a);
                        if (
                            (g = -1 < h ? g[h + 1] : "") &&
                            f === e.ownerDocument
                        )
                            Ze(e, g);
                        else if (
                            f.nodeType === Node.DOCUMENT_FRAGMENT_NODE &&
                            (f = f.host)
                        )
                            for (
                                f = V(f).is,
                                    f !== g && Ye(e, g, f),
                                    e =
                                        window.ShadyDOM.nativeMethods.querySelectorAll.call(
                                            e,
                                            ":not(." + W.a + ")"
                                        ),
                                    f = 0;
                                f < e.length;
                                f++
                            )
                                if (((g = e[f]), (h = g.getRootNode().host)))
                                    (h = V(h).is), We(g, h);
                    }
                }
        }
    }
    if (!T) {
        var Zf = new MutationObserver(Yf),
            $f = function (a) {
                Zf.observe(a, { childList: !0, subtree: !0 });
            };
        if (
            window.customElements &&
            !window.customElements.polyfillWrapFlushCallback
        )
            $f(document);
        else {
            var ag = function () {
                $f(document.body);
            };
            window.HTMLImports
                ? window.HTMLImports.whenReady(ag)
                : requestAnimationFrame(function () {
                      if ("loading" === document.readyState) {
                          var a = function () {
                              ag();
                              document.removeEventListener(
                                  "readystatechange",
                                  a
                              );
                          };
                          document.addEventListener("readystatechange", a);
                      } else ag();
                  });
        }
        Xf = function () {
            Yf(Zf.takeRecords());
        };
    }
    var bg = Xf;
    var cg = {};
    var dg = Promise.resolve();
    function eg(a) {
        if ((a = cg[a]))
            (a._applyShimCurrentVersion = a._applyShimCurrentVersion || 0),
                (a._applyShimValidatingVersion =
                    a._applyShimValidatingVersion || 0),
                (a._applyShimNextVersion = (a._applyShimNextVersion || 0) + 1);
    }
    function fg(a) {
        return a._applyShimCurrentVersion === a._applyShimNextVersion;
    }
    function gg(a) {
        a._applyShimValidatingVersion = a._applyShimNextVersion;
        a.b ||
            ((a.b = !0),
            dg.then(function () {
                a._applyShimCurrentVersion = a._applyShimNextVersion;
                a.b = !1;
            }));
    }
    var hg = new Wf();
    function Y() {
        this.D = {};
        this.c = document.documentElement;
        var a = new ee();
        a.rules = [];
        this.g = Cf(this.c, new Bf(a));
        this.l = !1;
        this.b = this.a = null;
    }
    p = Y.prototype;
    p.ja = function () {
        bg();
    };
    p.Ja = function (a) {
        return Je(a);
    };
    p.Va = function (a) {
        return He(a);
    };
    p.prepareTemplate = function (a, b, c) {
        this.prepareTemplateDom(a, b);
        this.prepareTemplateStyles(a, b, c);
    };
    p.prepareTemplateStyles = function (a, b, c) {
        if (!a.l) {
            T || Tf[b] || (Tf[b] = Oe(b));
            a.l = !0;
            a.name = b;
            a.extends = c;
            cg[b] = a;
            var d = (d = a.content.querySelector("style"))
                ? d.getAttribute("css-build") || ""
                : "";
            var e = [];
            for (
                var f = a.content.querySelectorAll("style"), g = 0;
                g < f.length;
                g++
            ) {
                var h = f[g];
                if (h.hasAttribute("shady-unscoped")) {
                    if (!T) {
                        var k = h.textContent;
                        Ge.has(k) ||
                            (Ge.add(k),
                            (k = h.cloneNode(!0)),
                            document.head.appendChild(k));
                        h.parentNode.removeChild(h);
                    }
                } else e.push(h.textContent), h.parentNode.removeChild(h);
            }
            e = e.join("").trim();
            c = { is: b, extends: c, $a: d };
            ig(this);
            f = Be.test(e) || Ae.test(e);
            Be.lastIndex = 0;
            Ae.lastIndex = 0;
            e = fe(e);
            f && U && this.a && this.a.transformRules(e, b);
            a._styleAst = e;
            a.D = d;
            d = [];
            U || (d = Gf(a._styleAst));
            if (!d.length || U)
                (e = T ? a.content : null),
                    (b = Tf[b] || null),
                    (f = $e(c, a._styleAst)),
                    (b = f.length ? Le(f, c.is, e, b) : void 0),
                    (a.a = b);
            a.g = d;
        }
    };
    p.prepareTemplateDom = function (a, b) {
        T || a.c || ((a.c = !0), Ue(a.content, b));
    };
    function jg(a) {
        !a.b &&
            window.ShadyCSS &&
            window.ShadyCSS.CustomStyleInterface &&
            ((a.b = window.ShadyCSS.CustomStyleInterface),
            (a.b.transformCallback = function (b) {
                a.ta(b);
            }),
            (a.b.validateCallback = function () {
                requestAnimationFrame(function () {
                    (a.b.enqueued || a.l) && a.flushCustomStyles();
                });
            }));
    }
    function ig(a) {
        !a.a &&
            window.ShadyCSS &&
            window.ShadyCSS.ApplyShim &&
            ((a.a = window.ShadyCSS.ApplyShim), (a.a.invalidCallback = eg));
        jg(a);
    }
    p.flushCustomStyles = function () {
        ig(this);
        if (this.b) {
            var a = this.b.processStyles();
            if (this.b.enqueued) {
                if (U)
                    for (var b = 0; b < a.length; b++) {
                        var c = this.b.getStyleForCustomStyle(a[b]);
                        if (c && U && this.a) {
                            var d = Je(c);
                            ig(this);
                            this.a.transformRules(d);
                            c.textContent = He(d);
                        }
                    }
                else
                    for (kg(this, this.c, this.g), b = 0; b < a.length; b++)
                        (c = this.b.getStyleForCustomStyle(a[b])) &&
                            Rf(c, this.g.B);
                this.b.enqueued = !1;
                this.l && !U && this.styleDocument();
            }
        }
    };
    p.styleElement = function (a, b) {
        var c = V(a).is,
            d = X(a);
        if (!d) {
            var e = V(a);
            d = e.is;
            e = e.J;
            var f = Tf[d] || null;
            d = cg[d];
            if (d) {
                var g = d._styleAst;
                var h = d.g;
            }
            g = new Bf(g, f, h, e);
            d && Cf(a, g);
            d = g;
        }
        a !== this.c && (this.l = !0);
        b && ((d.G = d.G || {}), Object.assign(d.G, b));
        if (U) {
            if (d.G) {
                b = d.G;
                for (var k in b)
                    null === k
                        ? a.style.removeProperty(k)
                        : a.style.setProperty(k, b[k]);
            }
            if (((k = cg[c]) || a === this.c) && k && k.a && !fg(k)) {
                if (
                    fg(k) ||
                    k._applyShimValidatingVersion !== k._applyShimNextVersion
                )
                    ig(this),
                        this.a && this.a.transformRules(k._styleAst, c),
                        (k.a.textContent = $e(a, d.w)),
                        gg(k);
                T &&
                    (c = a.shadowRoot) &&
                    (c.querySelector("style").textContent = $e(a, d.w));
                d.w = k._styleAst;
            }
        } else if ((this.ja(), kg(this, a, d), d.ha && d.ha.length)) {
            c = d;
            k = V(a).is;
            d = (b = hg.fetch(k, c.B, c.ha)) ? b.styleElement : null;
            g = c.s;
            (h = b && b.s) ||
                ((h = this.D[k] = (this.D[k] || 0) + 1), (h = k + "-" + h));
            c.s = h;
            h = c.s;
            e = Sf;
            e = d ? d.textContent || "" : Of(e, a, c.B, h);
            f = X(a);
            var l = f.a;
            l &&
                !T &&
                l !== d &&
                (l._useCount--,
                0 >= l._useCount &&
                    l.parentNode &&
                    l.parentNode.removeChild(l));
            T
                ? f.a
                    ? ((f.a.textContent = e), (d = f.a))
                    : e && (d = Le(e, h, a.shadowRoot, f.b))
                : d
                ? d.parentNode ||
                  (Ef && -1 < e.indexOf("@media") && (d.textContent = e),
                  Me(d, null, f.b))
                : e && (d = Le(e, h, null, f.b));
            d &&
                ((d._useCount = d._useCount || 0),
                f.a != d && d._useCount++,
                (f.a = d));
            h = d;
            T ||
                ((d = c.s),
                (f = e = a.getAttribute("class") || ""),
                g &&
                    (f = e.replace(
                        new RegExp("\\s*x-scope\\s*" + g + "\\s*", "g"),
                        " "
                    )),
                (f += (f ? " " : "") + "x-scope " + d),
                e !== f && Re(a, f));
            b || hg.store(k, c.B, h, c.s);
        }
    };
    function lg(a, b) {
        return (b = b.getRootNode().host) ? (X(b) ? b : lg(a, b)) : a.c;
    }
    function kg(a, b, c) {
        a = lg(a, b);
        var d = X(a);
        a = Object.create(d.B || null);
        var e = Nf(b, c.w);
        b = Lf(d.w, b).v;
        Object.assign(a, e.La, b, e.Ta);
        b = c.G;
        for (var f in b) if ((e = b[f]) || 0 === e) a[f] = e;
        f = Sf;
        b = Object.getOwnPropertyNames(a);
        for (e = 0; e < b.length; e++) (d = b[e]), (a[d] = Jf(f, a[d], a));
        c.B = a;
    }
    p.styleDocument = function (a) {
        this.styleSubtree(this.c, a);
    };
    p.styleSubtree = function (a, b) {
        var c = a.shadowRoot;
        (c || a === this.c) && this.styleElement(a, b);
        if ((b = c && (c.children || c.childNodes)))
            for (a = 0; a < b.length; a++) this.styleSubtree(b[a]);
        else if ((a = a.children || a.childNodes))
            for (b = 0; b < a.length; b++) this.styleSubtree(a[b]);
    };
    p.ta = function (a) {
        var b = this,
            c = Je(a);
        Ie(c, function (a) {
            if (T) yf(a);
            else {
                var c = W;
                a.selector = a.parsedSelector;
                yf(a);
                a.selector = a.m = df(c, a, c.c, void 0, void 0);
            }
            U && (ig(b), b.a && b.a.transformRule(a));
        });
        U ? (a.textContent = He(c)) : this.g.w.rules.push(c);
    };
    p.getComputedStyleValue = function (a, b) {
        var c;
        U || (c = (X(a) || X(lg(this, a))).B[b]);
        return (c = c || window.getComputedStyle(a).getPropertyValue(b))
            ? c.trim()
            : "";
    };
    p.Ua = function (a, b) {
        var c = a.getRootNode();
        b = b ? b.split(/\s/) : [];
        c = c.host && c.host.localName;
        if (!c) {
            var d = a.getAttribute("class");
            if (d) {
                d = d.split(/\s/);
                for (var e = 0; e < d.length; e++)
                    if (d[e] === W.a) {
                        c = d[e + 1];
                        break;
                    }
            }
        }
        c && b.push(W.a, c);
        U || ((c = X(a)) && c.s && b.push(Sf.a, c.s));
        Re(a, b.join(" "));
    };
    p.Fa = function (a) {
        return X(a);
    };
    Y.prototype.flush = Y.prototype.ja;
    Y.prototype.prepareTemplate = Y.prototype.prepareTemplate;
    Y.prototype.styleElement = Y.prototype.styleElement;
    Y.prototype.styleDocument = Y.prototype.styleDocument;
    Y.prototype.styleSubtree = Y.prototype.styleSubtree;
    Y.prototype.getComputedStyleValue = Y.prototype.getComputedStyleValue;
    Y.prototype.setElementClass = Y.prototype.Ua;
    Y.prototype._styleInfoForNode = Y.prototype.Fa;
    Y.prototype.transformCustomStyleForDocument = Y.prototype.ta;
    Y.prototype.getStyleAst = Y.prototype.Ja;
    Y.prototype.styleAstToString = Y.prototype.Va;
    Y.prototype.flushCustomStyles = Y.prototype.flushCustomStyles;
    Object.defineProperties(Y.prototype, {
        nativeShadow: {
            get: function () {
                return T;
            },
        },
        nativeCss: {
            get: function () {
                return U;
            },
        },
    });
    var Z = new Y(),
        mg,
        ng;
    window.ShadyCSS &&
        ((mg = window.ShadyCSS.ApplyShim),
        (ng = window.ShadyCSS.CustomStyleInterface));
    window.ShadyCSS = {
        ScopingShim: Z,
        prepareTemplate: function (a, b, c) {
            Z.flushCustomStyles();
            Z.prepareTemplate(a, b, c);
        },
        prepareTemplateDom: function (a, b) {
            Z.prepareTemplateDom(a, b);
        },
        prepareTemplateStyles: function (a, b, c) {
            Z.flushCustomStyles();
            Z.prepareTemplateStyles(a, b, c);
        },
        styleSubtree: function (a, b) {
            Z.flushCustomStyles();
            Z.styleSubtree(a, b);
        },
        styleElement: function (a) {
            Z.flushCustomStyles();
            Z.styleElement(a);
        },
        styleDocument: function (a) {
            Z.flushCustomStyles();
            Z.styleDocument(a);
        },
        flushCustomStyles: function () {
            Z.flushCustomStyles();
        },
        getComputedStyleValue: function (a, b) {
            return Z.getComputedStyleValue(a, b);
        },
        nativeCss: U,
        nativeShadow: T,
    };
    mg && (window.ShadyCSS.ApplyShim = mg);
    ng && (window.ShadyCSS.CustomStyleInterface = ng); /*

 Copyright (c) 2014 The Polymer Project Authors. All rights reserved.
 This code may only be used under the BSD style license found at http://polymer.github.io/LICENSE.txt
 The complete set of authors may be found at http://polymer.github.io/AUTHORS.txt
 The complete set of contributors may be found at http://polymer.github.io/CONTRIBUTORS.txt
 Code distributed by Google as part of the polymer project is also
 subject to an additional IP rights grant found at http://polymer.github.io/PATENTS.txt
*/
    var og = window.customElements,
        pg = window.HTMLImports,
        qg = window.HTMLTemplateElement;
    window.WebComponents = window.WebComponents || {};
    if (og && og.polyfillWrapFlushCallback) {
        var rg,
            sg = function () {
                if (rg) {
                    qg.Ga && qg.Ga(window.document);
                    var a = rg;
                    rg = null;
                    a();
                    return !0;
                }
            },
            tg = pg.whenReady;
        og.polyfillWrapFlushCallback(function (a) {
            rg = a;
            tg(sg);
        });
        pg.whenReady = function (a) {
            tg(function () {
                sg() ? pg.whenReady(a) : a();
            });
        };
    }
    pg.whenReady(function () {
        requestAnimationFrame(function () {
            window.WebComponents.ready = !0;
            document.dispatchEvent(
                new CustomEvent("WebComponentsReady", { bubbles: !0 })
            );
        });
    });
    var ug = document.createElement("style");
    ug.textContent =
        "body {transition: opacity ease-in 0.2s; } \nbody[unresolved] {opacity: 0; display: block; overflow: hidden; position: relative; } \n";
    var vg = document.querySelector("head");
    vg.insertBefore(ug, vg.firstChild);
}.call(this));

//# sourceMappingURL=webcomponents-hi-sd-ce.js.map
