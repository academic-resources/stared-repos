...
function build(release) {
    ...
    if (releases[release].postProcessing) {
        releases[release].postProcessing.forEach(function (file) {
            ...
            for (var i in postProcessing.insert) {
                value =
                    postProcessing.insert[i].substr(
                        postProcessing.insert[i].length - 3
                    ) == ".js"
            ...
            for (i in postProcessing.replace) {
                value =
                    postProcessing.replace[i].substr(
                        postProcessing.replace[i].length - 3
                    ) == ".js"
    ...
    // Write build file
    var buildFile =
        "./build/iscroll" + (release != "iscroll" ? "-" + release : "") + ".js";
