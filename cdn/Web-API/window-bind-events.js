...
document.addEventListener("readystatechange", (event) => {
    ...
    pre.insertAdjacentHTML(
        "beforeend",
        `<span>readyState: ${(document.readyState, order)}</span><br/>`
    );
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    pre.insertAdjacentHTML(
        "beforeend",
        `<span>DOMContentLoaded, 2</span><br/>`
    );
});
...
document.addEventListener(
    ...
    () => {
        ...
        pre.insertAdjacentHTML(
            "beforeend",
            `<span>document.hidden ${document.hidden}</span><br/>`
        );
        pre.insertAdjacentHTML(
            "beforeend",
            `<span>document.visibilityState ${document.visibilityState}</span><br/>`
        );
    },
    false
);
