...
(function (root, window, document, factory, undefined) {
    ...
    function init(callback) {
        ...
        //trying to use fullpage without a selector?
            ...
            showError(
                "error",
                "Error! Fullpage.js needs to be initialized with a selector. For example: fullpage('#fullpage');"
            );
        }
    ...
    function afterDomReady() {
        ...
        if (
            ...
            (activeSectionIndex !== 0 ||
                (activeSectionIndex === 0 && getNodeIndex(activeSlide) !== 0))
        ) {
    ...
    function scrollHandler() {
        ...
        if (!options.autoScrolling || options.scrollBar) {
            ...
            var initial = Math.abs(
                currentScroll - $$(SECTION_SEL)[0].offsetTop
            );
        ...
        if (!options.autoScrolling || options.scrollBar) {
            ...
            if (!hasClass(currentSection, ACTIVE)) {
                ...
                if (activeSlide) {
                    var slideAnchorLink =
                        activeSlide.getAttribute("data-anchor");
            ...
            if (options.fitToSection) {
                ...
                scrollId2 = setTimeout(function () {
                    if (
                        (canScroll && !options.autoScrolling) ||
                        options.scrollBar
                    ) {
                        //allows to scroll to an active section and
                        //if the section is already active, we prevent firing callbacks
                        if (
                            getNodeIndex($(SECTION_ACTIVE_SEL)) ==
                            getNodeIndex(currentSection)
                        ) {
                            isResizing = true;
                        }
    ...
    */
        ...
        if (isReallyTouch(e)) {
            ...
            if (canScroll && !slideMoving) {
                ...
                //vertical scrolling (only when autoScrolling is enabled)
                    ...
                    if (
                        Math.abs(touchStartY - touchEndY) >
                        (getWindowHeight() / 100) * options.touchSensitivity
                    ) {
    ...
    function scrollPage(element, callback, isMovementUp) {
        ...
        if (
            (getNodeIndex(v.activeSection) == v.sectionIndex && !isResizing) ||
            (options.scrollBar && getScrollTop() === v.dtop)
        ) {
            return;
        }
    ...
    function landscapeScroll(slides, destiny) {
        ...
        if (!options.loopHorizontal && options.controlArrows) {
            ...
            toggle(
                $(SLIDES_ARROW_NEXT_SEL, section),
                !destiny.is(":last-child")
            );
        }
