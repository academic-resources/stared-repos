...
QUnit.test("Testing setScrollingSpeed 0", function (assert) {
    ...
    setTimeout(function () {
        $.fn.fullpage.moveSectionDown();
        assert.ok(
            $(".section").eq(1).hasClass("active"),
            "We expect section 2 to be active"
        );
    });
...
QUnit.test("Testing setLockAnchors true", function (assert) {
    ...
    assert.equal(
        "",
        window.location.hash,
        "Anchor link should be an empty string"
    );
    assert.ok(
        $(".section").eq(1).hasClass("active"),
        "We expect section 2 to be active"
    ...
    setTimeout(function () {
        assert.ok(
            !$(".section").eq(2).hasClass("active"),
            "We don't expect section 3 to be active"
        );
        done();
    }, 20);
});
...
QUnit.test("Testing setLockAnchors false", function (assert) {
    ...
    setTimeout(function () {
        ...
        assert.equal(
            "#page2",
            window.location.hash,
            "Anchor link should not be an empty"
        );
        assert.ok(
            $(".section").eq(1).hasClass("active"),
            "We expect section 2 to be active"
...
QUnit.test("Testing setKeyboardScrolling false", function (assert) {
    ...
    setTimeout(function () {
        assert.equal(
            $(SECTION_ACTIVE_SEL).index(),
            0,
            "We expect expect section 1 to be active"
        );
        done();
    }, 200);
});
...
QUnit.test("Testing moveSectionDown", function (assert) {
    ...
    $.fn.fullpage.moveSectionDown();
    assert.ok(
        !$(SECTION_SEL).first().hasClass(ACTIVE),
        "We don't expect section 1 to be active anymore"
    );
    ...
    assert.equal(
        $(SECTION_ACTIVE_SEL).length,
        1,
        "We expect a single active section"
    );
...
QUnit.test("Testing moveSectionUp", function (assert) {
    ...
    $.fn.fullpage.moveSectionUp();
    assert.ok(
        !$(SECTION_SEL).last().hasClass("active"),
        "We don't expect the last section to be active anymore"
    );
...
QUnit.test("Testing silentMoveTo", function (assert) {
    ...
    assert.equal(
        $(SECTION_ACTIVE_SEL).find(SLIDE_ACTIVE_SEL).index(),
        3,
        "We expect section 2 slide 4 to be active"
    );
...
QUnit.test(
    ".destroy('all') removes all inline CSS and fullpage classes",
    function (assert) {
        ...
        assert.equal(
            $SECTION_SEL.get(0).style.height,
            "",
            SECTION_SEL + " should have an empty string height"
        );
        assert.equal(
            $SECTION_SEL.get(0).style.backgroundColor,
            "",
            SECTION_SEL + " should have an empty string background-color"
        );
        assert.equal(
            $SECTION_SEL.get(0).style.padding,
            "",
            SECTION_SEL + " should have an empty string padding"
        );
        assert.equal(
            $SLIDE_SEL.get(0).style.width,
            "",
            SLIDE_SEL + " should have an empty string width"
        );
        assert.equal(
            $html.get(0).style.overflow,
            "",
            "html should have an empty string overflow"
        );
        assert.equal(
            $html.get(0).style.height,
            "",
            "html should have an empty string height"
        );
        assert.equal(
            $body.get(0).style.overflow,
            "",
            "body should have an empty string overflow"
        );
        assert.equal(
            $body.get(0).style.height,
            "",
            "body should have an empty string height"
        );

        // check for <html/> classes
        assert.equal(
            $html.hasClass(ENABLED),
            false,
            "html should not have the " + ENABLED + " class"
        );
        ...
        $.each(
            ...
            function (index, className) {
                assert.equal(
                    className.indexOf(VIEWING_PREFIX),
                    -1,
                    "body should not have a " + VIEWING_PREFIX + "-* class"
                );
            }
        );
    }
