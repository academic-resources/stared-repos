        ...
        idb_connect.onupgradeneeded = function (e) {
            ...
            if (!length) {
                storeNames.forEach((name, i) => {
                    let store = idb.createObjectStore(name, {
                        keyPath: "uid",
                    });
                    let msgIdIndex = store.createIndex("by_msgId", "msgId", {
                        unique: true,
                    });
                    let timestampIndex = store.createIndex(
                        "by_timestamp",
                        "timestamp"
                    );
                    if (name === `othersDB`) {
...
const idbTest = () => {
    ...
    readBtn.addEventListener(`click`, () => {
        log(`async window.idb `, window.idb);
        if (window.idb) {
            let { name, version, objectStoreNames, error } = idb;
...
const readStoreDatas = (
    options = {},
    callback = (obj) => log(`read idb callback`, obj)
) => {
    ...
    if (idb) {
        ...
        // DOMStringList {0: "errorDB", 1: "othersDB", 2: "selfDB", length: 3} 3
        let storeNames = [...objectStoreNames];
        let { datas, storeName, indexName } = options;
        if (storeNames.includes(storeName)) {
            // transaction
            ...
            request.onsuccess = function (e) {
                ...
                if (matching !== undefined) {
                    let { uid, msgId, timestamp } = matching;
                    log(
                        `A match was found.`,
...
const idb_operations = (idb, datas = []) => {
    ...
    let storeRead = (datas = []) => {
        ...
        request.onsuccess = function () {
            let matching = request.result;
            if (matching !== undefined) {
                let { isbn, title, author } = matching;
