  ...
  """
    ...
    with tf.control_dependencies(
        [
            ...
            tf.compat.v1.assert_less_equal(encoded_r, math.pow(2, 16) - 1.0),
        ]
    ):
...
def encode_lidar_features(lidar_point_feature):
  ...
  """
    ...
    encoded_intensity = tf.cast(
        tf.bitwise.bitwise_or(encoded_r_shifted, encoded_intensity), dtype=tf.int64
    )
    encoded_elongation = tf.cast(
        tf.bitwise.bitwise_or(encoded_r_shifted, encoded_elongation), dtype=tf.int64
    )
...
def scatter_nd_with_pool(index, value, shape, pool_method=tf.math.unsorted_segment_max):
  ...
  """
    ...
    index_unique = tf.stack(
        [index_encoded // width, tf.math.mod(index_encoded, width)], axis=-1
    )
    shape = [height, width]
...
def compute_range_image_polar(
    range_image, extrinsic, inclination, dtype=tf.float32, scope=None
):
  ...
  """
    ...
    with tf.compat.v1.name_scope(
        scope, "ComputeRangeImagePolar", [range_image, extrinsic, inclination]
    ):
        with tf.compat.v1.name_scope("Azimuth"):
            # [B].
            az_correction = tf.atan2(extrinsic[..., 1, 0], extrinsic[..., 0, 0])
            # [W].
            ratios = (tf.cast(tf.range(width, 0, -1), dtype=dtype) - 0.5) / tf.cast(
                width, dtype=dtype
            )
            # [B, W].
        ...
        range_image_polar = tf.stack(
            [azimuth_tile, inclination_tile, range_image], axis=-1
        )
...
def build_camera_depth_image(
  ...
  """
    with tf.compat.v1.name_scope(
        ...
        range_image_camera = (
            tf.einsum(
                "bij,bhwj->bhwi", vehicle_to_camera_rotation, range_image_cartesian
            )
            + vehicle_to_camera_translation[:, tf.newaxis, tf.newaxis, :]
        )
        # [B, H, W]
        ...
        camera_projection_selected = (
            tf.ones_like(camera_projection[..., 1:3], dtype=camera_projection.dtype)
            * -1
        )
        camera_projection_selected = tf.compat.v1.where(
            camera_projection_mask_2,
            camera_projection[..., 4:6],
            camera_projection_selected,
        )
        # [B, H, W, 2]
        camera_projection_selected = tf.compat.v1.where(
            camera_projection_mask_1,
            camera_projection[..., 1:3],
            camera_projection_selected,
        )
        # [B, H, W]
        camera_projection_mask = tf.logical_or(
            camera_projection_mask_1, camera_projection_mask_2
        )[..., 0]
        ...
        def fn(args):
            ...
            index = tf.gather_nd(tf.stack([cp[..., 1], cp[..., 0]], axis=-1), mask_ids)
        ...
        images = tf.map_fn(
            fn,
            elems=[
                camera_projection_mask,
                range_image_camera_norm,
                camera_projection_selected,
            ],
...
def build_range_image_from_point_cloud(
    points_vehicle_frame,
    num_points,
    extrinsic,
    inclination,
    range_image_size,
  ...
  """
    ...
    with tf.compat.v1.name_scope(
        ...
        # [B, N, 3]
        points = tf.einsum("bij,bkj->bik", points_vehicle_frame, rotation) + translation
        # [B, N]
        ...
        point_azimuth = (
            point_azimuth - tf.cast(point_azimuth_gt_pi_mask, dtype=dtype) * 2 * np.pi
        )
        point_azimuth = (
            point_azimuth
            + tf.cast(point_azimuth_lt_minus_pi_mask, dtype=dtype) * 2 * np.pi
        )
        ...
        point_ri_col_indices = tf.cast(tf.round(point_ri_col_indices), dtype=tf.int32)

        with tf.control_dependencies(
            [
                tf.compat.v1.assert_non_negative(point_ri_col_indices),
                tf.compat.v1.assert_less(
                    point_ri_col_indices, tf.cast(width, tf.int32)
                ),
            ]
        ):
            # [B, N, 2]
        ...
        """
                if len(args) == 3:
                    ...
                    ri_value = tf.concat(
                        [ri_value[..., tf.newaxis], point_feature], axis=-1
                    )
                ...
                range_image = scatter_nd_with_pool(
                    ri_index, ri_value, [height, width], tf.math.unsorted_segment_min
                )
                if len(args) != 3:
...
def crop_range_image(range_images, new_width, shift=None, scope=None):
  ...
  """
    ...
    if width is not None and new_width >= width:
        raise ValueError(
            "new_width {} should be < the old width {}.".format(new_width, width)
        )
...
def compute_inclination(inclination_range, height, scope=None):
  ...
  """
    with tf.compat.v1.name_scope(scope, "ComputeInclination", [inclination_range]):
        ...
        inclination = (
            (0.5 + tf.cast(tf.range(0, height), dtype=inclination_range.dtype))
            / tf.cast(height, dtype=inclination_range.dtype)
            * tf.expand_dims(diff, axis=-1)
            + inclination_range[..., 0:1]
        )
        return inclination
