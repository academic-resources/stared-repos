...
class TransformUtilsTest(tf.test.TestCase):
    def test_get_transform(self):
        # [3, 3]
        point = tf.constant(
            [[5.0, 0.0, 1.5], [20.0, 0.0, 1.6], [20.0, 0.0, 2.6]], dtype=tf.float32
        )
        ...
        point_transformed = (
            tf.einsum("ij,kj->ki", transform[0:3, 0:3], point) + transform[0:3, 3]
        )

        transform = tf.linalg.inv(transform)
        point_transformed_back = (
            tf.einsum("ij,kj->ki", transform[0:3, 0:3], point_transformed)
            + transform[0:3, 3]
        )
