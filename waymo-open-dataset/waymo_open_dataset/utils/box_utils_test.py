...
class BoxUtilsTest(tf.test.TestCase):
    def test_is_within_box_3d(self):
        ...
        point = tf.constant(
            [[5.0, 0.0, 1.5], [20.0, 0.0, 1.6], [20.0, 0.0, 2.6]],  # in  # in  # not in
            dtype=tf.float32,
        )
        ...
        with self.test_session() as sess:
            ...
            self.assertAllEqual(
                point_in_box, [[True, False], [False, True], [False, False]]
            )
    ...
    def test_transform_point_among_frames(self):
        ...
        with self.test_session():
            self.assertAllClose(
                pp[0, 0, 0, ...].eval(),
                [[math.cos(math.pi * 0.4), math.sin(math.pi * 0.4), 2.0]],
            )

    def test_transform_box_among_frames(self):
        b = tf.constant([[1.0, 0, 0, 2.0, 2.0, 2.0, math.pi * 0.1]], dtype=tf.float32)
        from_frame_pose = transform_utils.get_transform(
            transform_utils.get_yaw_rotation(math.pi * 0.5),
