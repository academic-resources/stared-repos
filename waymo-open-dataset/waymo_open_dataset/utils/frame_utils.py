...
def convert_frame_to_dict(frame):
  ...
  """
    range_images, camera_projection_protos, range_image_top_pose = parse_range_image_and_camera_projection(
        frame
    )
    first_return_cartesian_range_images = convert_range_image_to_cartesian(
        frame, range_images, range_image_top_pose, ri_index=0, keep_polar_features=True
    )
    second_return_cartesian_range_images = convert_range_image_to_cartesian(
        frame, range_images, range_image_top_pose, ri_index=1, keep_polar_features=True
    )
    ...
    for c in frame.context.laser_calibrations:
        ...
        if len(c.beam_inclinations) == 0:  # pylint: disable=g-explicit-length-test
            ...
            data_dict[beam_inclination_key] = np.array(c.beam_inclinations, np.float32)
        ...
        data_dict[
            f"{laser_name_str}_RANGE_IMAGE_FIRST_RETURN"
        ] = first_return_cartesian_range_images[c.name].numpy()
        data_dict[
            f"{laser_name_str}_RANGE_IMAGE_SECOND_RETURN"
    ...
    data_dict["POSE"] = np.reshape(np.array(frame.pose.transform, np.float32), (4, 4))
