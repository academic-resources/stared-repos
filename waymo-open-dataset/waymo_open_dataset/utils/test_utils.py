...
def generate_boxes(center_x, batch=1):
  ...
  """
    ...
    # [len(center_x), 7]
    boxes = np.stack([center_x, center_y, center_z, dim, dim, dim, heading], axis=-1)
    ...
    return (
        tf.convert_to_tensor(value=boxes, dtype=tf.float32),
...
def generate_range_image(indices, values, shape, batch=1):
  ...
  """
    multiples = tf.concat(
        [tf.constant([batch]), tf.ones([len(shape)], dtype=tf.int32)], axis=-1
    )
