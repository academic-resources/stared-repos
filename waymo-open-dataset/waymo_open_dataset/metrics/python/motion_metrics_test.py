...
class MotionMetricsEstimatorTest(tf.test.TestCase):
    ...
    def _BuildGraph(
        ...
        with graph.as_default():
            ...
            self._scenario_id = tf.compat.v1.placeholder(dtype=tf.string, shape=[None])
    ...
    def testComputeMinADEBatch2(self):
        ...
        metric_dict = self._RunEval(
            [pred_score, pred_score], [pred_trajectory, pred_trajectory_2], gt
        )
        # ADE of Vehicle.
        self.assertAlmostEqual(
    ...
    def testComputeMinADEDefaultIds(self):
        ...
        gt = self._CreateTestScenario(1)
        metric_dict = self._RunEval(
            [pred_score], [pred_trajectory], gt, set_scenario_and_object_id=False
        )
        # ADE of Vehicle.
        self.assertAlmostEqual(
            metric_dict["TYPE_VEHICLE_3/minADE"], 5.97487, delta=1e-4
