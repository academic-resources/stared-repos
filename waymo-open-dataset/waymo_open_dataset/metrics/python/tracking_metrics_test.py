...
class TrackinMetricsEstimatorTest(tf.test.TestCase):
    def _GenerateRandomBBoxes(self, num_sequences, num_frames, num_bboxes):
        ...
        types = [1, 2, 3, 4] + np.random.randint(1, 4, size=[num_bboxes - 4]).tolist()
    ...
    def testMetricsBasic(self):
        ...
        with self.test_session(graph=graph) as sess:
            ...
            self._EvalUpdateOps(
                ...
                pd_score[:split],
                pd_frame_id[:split],
                pd_sequence_id[:split],
                pd_object_id[:split],
                gt_bbox[:split],
                gt_type[:split],
                gt_frame_id[:split],
                gt_sequence_id[:split],
                gt_object_id[:split],
                gt_speed[:split],
            )
            self._EvalUpdateOps(
            ...
            for i in range(0, mot_metrics_len):
                self.assertTrue(
                    -ERROR <= list(mot_metrics.values())[i][0]
                    and list(mot_metrics.values())[i][0] <= 1.0 + ERROR
                )
