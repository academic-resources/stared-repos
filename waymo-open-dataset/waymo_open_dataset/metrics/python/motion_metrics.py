...
def _update(name, update, init_shape, dtype):
  ...
  """
    with tf.compat.v1.variable_scope("motion_metrics", reuse=tf.compat.v1.AUTO_REUSE):
...
def get_motion_metric_ops(
  ...
  """
    (
        ...
        num_pred_steps,
        _,
    ) = prediction_trajectory.shape.as_list()
    ...
    eval_dict = {
        "ground_truth_trajectory": (
            ground_truth_trajectory,
            [0, num_agents, num_gt_steps, 7],
            tf.float32,
        ),
        "ground_truth_is_valid": (
            ground_truth_is_valid,
            [0, num_agents, num_gt_steps],
            tf.bool,
        ),
        "prediction_trajectory": (
            prediction_trajectory,
            [0, num_groups, top_k, num_agents, num_pred_steps, 2],
            tf.float32,
        ),
        "prediction_score": (prediction_score, [0, num_groups, top_k], tf.float32),
        "prediction_ground_truth_indices": (
            prediction_ground_truth_indices,
            [0, num_groups, num_agents],
            tf.int64,
        ),
        "prediction_ground_truth_indices_mask": (
            prediction_ground_truth_indices_mask,
    ...
    ) = py_metrics_ops.motion_metrics(config=config_str, **variable_map)
    ...
    for i, name in enumerate(breakdown_names):
        ...
        metric_ops["{}/OverlapRate".format(name)] = (overlap_rate[i], tf.constant([]))
        metric_ops["{}/mAP".format(name)] = (mean_average_precision[i], tf.constant([]))
    return metric_ops
