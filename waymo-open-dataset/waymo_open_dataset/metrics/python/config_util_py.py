...
__all__ = ["get_breakdown_names_from_config", "get_breakdown_names_from_motion_config"]
...
def _get_num_breakdown_shards(breakdown_generator_id):
  ...
  """
    if breakdown_generator_id == breakdown_pb2.Breakdown.GeneratorId.Value("ONE_SHARD"):
        return 1
    elif breakdown_generator_id == (
        breakdown_pb2.Breakdown.GeneratorId.Value("ALL_BUT_SIGN")
    ):
        return 2
    elif breakdown_generator_id == (breakdown_pb2.Breakdown.GeneratorId.Value("SIZE")):
        return 2
    elif breakdown_generator_id == (
        breakdown_pb2.Breakdown.GeneratorId.Value("OBJECT_TYPE")
    ):
        return 4
    elif breakdown_generator_id == breakdown_pb2.Breakdown.GeneratorId.Value("RANGE"):
        return 3 * 4
...
def _get_breakdown_shard_name(breakdown_generator_id, shard):
  ...
  """
    ...
    if breakdown_generator_id == breakdown_pb2.Breakdown.GeneratorId.Value("ONE_SHARD"):
        ...
        return "{}_{}_{}".format(
            breakdown_pb2.Breakdown.GeneratorId.Name(breakdown_generator_id),
            label_pb2.Label.Type.Name(object_type),
            velocity_shard_name,
        )
    elif breakdown_generator_id == breakdown_pb2.Breakdown.GeneratorId.Value("RANGE"):
...
def get_breakdown_names_from_config(config):
  ...
  """
    ...
    names = []
    for index, breakdown_generator_id in enumerate(config.breakdown_generator_ids):
        for shard_id in range(_get_num_breakdown_shards(breakdown_generator_id)):
            ...
            if not difficulty_levels:
                ...
                for dl in difficulty_levels:
                    names.append(
                        "{}_{}".format(
                            shard_name, label_pb2.Label.DifficultyLevel.Name(dl)
                        )
                    )
    return names
