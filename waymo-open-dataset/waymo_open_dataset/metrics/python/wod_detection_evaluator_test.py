...
class WODDetectionEvaluatorTest(tf.test.TestCase):
    def _GenerateRandomBBoxes(self, num_frames, num_bboxes):
        ...
        center_xyz = np.random.uniform(low=-1.0, high=1.0, size=(num_bboxes, 3)).astype(
            np.float32
        )
        dimension = np.random.uniform(low=0.1, high=1.0, size=(num_bboxes, 3)).astype(
            np.float32
        )
        rotation = np.random.uniform(
            low=-np.pi, high=np.pi, size=(num_bboxes, 1)
        ).astype(np.float32)
    ...
    def testBasic(self):
        ...
        self.assertAllClose(
            metric_dict.average_precision_ha_weighted, [0.5, 0.5, 0.5, 0.5], atol=0.01
        )
