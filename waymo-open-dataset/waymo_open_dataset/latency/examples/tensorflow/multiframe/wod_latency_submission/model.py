  ...
  """
    ...
    for i, layer_size in enumerate((128, 256, 512)):
        ...
        feat = tf.keras.layers.Dense(
            layer_size, activation="relu", name=f"linear_{i + 1}"
        )(feat)

    # Run the global max pool and an FC layer on the global features.
    # (1, 512)
    global_feat = tf.keras.layers.GlobalMaxPool1D()(feat)
    global_feat = tf.keras.layers.Dense(512, activation="relu", name="global_mlp")(
        global_feat
    )
    ...
    length = tf.keras.layers.Dense(num_boxes, activation="softplus", name="length")(
        global_feat
    )
    width = tf.keras.layers.Dense(num_boxes, activation="softplus", name="width")(
        global_feat
    )
    height = tf.keras.layers.Dense(num_boxes, activation="softplus", name="height")(
        global_feat
    )
    heading = np.pi * tf.keras.layers.Dense(
        num_boxes, activation="tanh", name="heading"
    )(global_feat)
    scores = tf.keras.layers.Dense(num_boxes, activation="sigmoid", name="scores")(
        global_feat
    )
