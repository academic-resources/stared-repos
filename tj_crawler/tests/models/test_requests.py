...
@pytest.mark.parametrize(
    "value", ("1234", "1234567-000-1234.00.00", "1234567-12.1234.1.12.12345")
)
...
def test_legal_process_number_field_description():
    assert (
        LegalProcess.__fields__["number"].field_info.description
        == "Valid format: XXXXXXXX-XX-XXXX.XX.XXXX"
    )


def test_legal_process_number_field_example():
    assert (
        LegalProcess.__fields__["number"].field_info.extra["example"]
        == "1234567-12.1234.1.12.1234"
    )


def test_check_digit_legal_process_number():
    assert (
        LegalProcess.check_digit_validation("1234567-69.1234.1.12.1234")
        == "1234567-69.1234.1.12.1234"
    )
