...
def test_tjms_parse_second_degree_legal_process_detail(
    tjms_crawler, tjms_second_degree_html
):
        ...
        "parties_involved": [
            {
                ...
                "representatives": [
                    {"type": "Advogada", "name": "Adriana Catelan Skowronski"},
                    {"type": "Advogada", "name": "Ana Silvia Pessoa Salgado Moura"},
                ],
            },
            {
                "type": "Apelado",
...
def test_tjms_crawler_parse_not_found_data(tjms_crawler):
    result = tjms_crawler.parse(
        Selector(text="<table><tbody><tr></tr></tbody></table>")
    )
