...
def fake_crawler():
    ...
    session.get.return_value.__aenter__.return_value.text = CoroutineMock(
        return_value="<html><h1>Test</h1></html>"
    )
...
def test_type_error_when_create_a_instance_without_implement_abstract_method():
    ...
    assert (
        str(error.value)
        == "Can't instantiate abstract class FakeCrawler with abstract methods parse"
    )
