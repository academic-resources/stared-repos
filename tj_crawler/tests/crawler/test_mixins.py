...
def test_softplan_crawler_parse_parties_involved(
    softplan_crawler, softplan_parties_involved_html
):
    ...
    expected_result = [
        {
            ...
            "representatives": [
                {"type": "Advogado", "name": "Vinicius Faria de Cerqueira"}
            ],
        },
        {
            "type": "Ré",
            ...
            "representatives": [
                {"type": "Advogado", "name": "Marcus Vinicius Cavalcante Lins Filho"},
                {"type": "Advogado", "name": "Thiago Maia Nobre Rocha"},
            ],
        },
    ]
...
def test_softplan_crawler_parse_parties_involved_return_empty_value(softplan_crawler):
    selector = Selector(
        text='<table id="tablePartesPrincipais"><tr><td></td><td></td></tr></table>'
    )
    result = softplan_crawler.parse_parties_involved(selector)
    expected_result = [{"type": None, "name": None, "representatives": []}]
    assert expected_result == result
...
def test_softplan_crawler_parse_updates(softplan_crawler, softplan_updates):
    ...
    expected_result = [
        ...
        {
            "date": "16/08/2020",
            "description": "Visto em Autoinspeção   Despacho Visto em Autoinspeção",
        },
        {
            "date": "11/05/2020",
            "description": "Documento  Nº Protocolo: WMAC.20.70092549-0 Data: 11/05/2020 13:28",
        },
...
def test_softplan_crawler_parse_updates_return_empty_value(softplan_crawler):
    selector = Selector(
        text="<table><tbody><tr><td></td><td></td></tr></tbody></table>"
    )
...
def test_softplan_crawler_parse_updates_xpath_not_found(softplan_crawler):
    selector = Selector(
        text='<tbody id="tabelaTodasMovimentacoes"><tr><td></td><td></td></tr></tbody>'
    )
    result = softplan_crawler.parse_updates(selector)
