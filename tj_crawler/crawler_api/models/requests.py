...
class LegalProcess(BaseModel):
    number: str = Field(
        description="Valid format: XXXXXXXX-XX-XXXX.XX.XXXX",
        example="1234567-12.1234.1.12.1234",
    )
    ...
    def check_number(cls, value):
        ...
        if not is_full_match:
            raise ValueError(
                "Invalid number format. Example: 1234567-12.1234.1.12.1234"
            )
        return value
    ...
    def check_digit_validation(cls, value):
        ...
        assert (
            int(check_digit) == check_digit_calculated
        ), "Invalid Number. The check digit (DV) is not correct"
        return value
