...
class TJMSCrawler(SoftplanTJCrawlerMixin, BaseCrawler):
    ...
    def parse_legal_process_detail(self, data):
        class_ = data.xpath(
            ".//div[span[contains(text(), 'Classe')]]/div/span/text()"
        ).get()
        area = data.xpath(
            ".//div[span[contains(text(), 'Área')]]/div/span/text()"
        ).get()
        subject = data.xpath(
            ".//div[span[contains(text(), 'Assunto')]]/div/span/text()"
        ).get()
        distribution = data.xpath(
            ".//div[span[contains(text(), 'Distribuição')]]/div/text()"
        ).get()
        judge = data.xpath(
            ".//div[span[contains(text(), 'Juiz')]]/div/span/text()"
        ).get()
        value_div = data.xpath(".//div[span[contains(text(), 'Valor da ação')]]")
        value = (
            value_div.xpath("./div/text()").get()
            or value_div.xpath("./div/span/text()").get()
        )
        return {
            "class": class_,
            "area": area,
