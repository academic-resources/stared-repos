...
class SoftplanTJCrawlerMixin:
    def parse_parties_involved(self, data):
        trs = data.xpath(
            '//table[@id="tableTodasPartes"]|//table[@id="tablePartesPrincipais"]'
        )
        trs = trs and trs[-1]
        parties_involved = []
        for tr in trs.css("tr"):
            ...
            representatives = [
                {"type": item and re.sub(":|&nbsp", "", item), "name": all_texts[index]}
                for index, item in enumerate(
                    filter(lambda x: x and True, all_types), start=1
                )
            ]
            item = {
    ...
    def parse_updates(self, table):
        data = table.xpath(
            '//tbody[@id="tabelaTodasMovimentacoes"]|//tbody[@id="tabelaUltimasMovimentacoes"]'
        )
