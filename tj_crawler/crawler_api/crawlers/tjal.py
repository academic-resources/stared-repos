...
class TJALCrawler(SoftplanTJCrawlerMixin, BaseCrawler):
    ...
    def parse_legal_process_detail(self, data):
        class_ = data.xpath(
            ".//tr[td[label[contains(text(), 'Classe')]]]/td[2]//span//span/text()"
        ).get()
        area = data.xpath(".//tr/td[span[contains(text(), 'Área:')]]/text()[2]").get()
        subject = data.xpath(
            ".//tr[td[label[contains(text(), 'Assunto')]]]/td[2]//span/text()"
        ).get()
        distribution = data.xpath(
            ".//tr[td[label[contains(text(), 'Distribuição')]]]/td[2]//span/text()"
        ).get()
        judge = data.xpath(
            ".//tr[td[label[contains(text(), 'Juiz')]]]/td[2]//span/text()"
        ).get()
        value = data.xpath(
            ".//tr[td[label[contains(text(), 'Valor da ação')]]]/td[2]//span/text()"
        ).get()
        return {
            "class": class_,
