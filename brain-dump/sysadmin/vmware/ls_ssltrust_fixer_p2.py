    ...
    with open(spec, "r") as spec_fh:
        for line in spec_fh.read().splitlines():
            if "endpoint0.url" in line:
                url = (
                    line.split("=", 1)[1].split("//", 2)[1].split("/")[0].split(":")[0]
                )
                if url == "localhost":
    ...
    with open(spec, "r") as spec_fh:
        for line in spec_fh.read().splitlines():
            if endpointurl in line:
                ...
                if url == "localhost":
                    print(
                        "First and second end points found to be using localhost as fqdn - manually  new cert and update .NewCert file before fix"
                    )
                    break
                port = 443
                ...
                if endpoint in certcache:
                    ...
                    try:
                        ...
                        logger.error(
                            "**Failed to get in use certificate from node %s:%s**",
                            url,
                            port,
                        )
                        ConnectFailure_ct = ConnectFailure_ct + 1
                        if url not in ConnectFailure_nodes:
                            ConnectFailure_nodes.append(url)
                    finally:
                        if conn is not None:
...
def _doScan():
    ...
    rc, ids = lstoolcommunicate(
        ["list", "--no-check-cert", "--url", lsUrl, "--id-only"]
    )
    if rc != 0:
    ...
    for id in ids:
        ...
        rc, oldSpec = lstoolcommunicate(
            ["get", "--no-check-cert", "--url", lsUrl, "--id", id, "--as-spec"]
        )
        if rc != 0:
        ...
        if cert:
            ...
            logger.debug(
                "ID: %s Old thumbprint: %s new thumbprint %s", id, othumb, nthumb
            )
            if othumb == nthumb:
                matchedIDs.append(id)
                logger.debug(
                    "Trust matches the current certificate. Added %s to matchedIDs", id
                )
            else:
                mismatchedIDs.append(id)
                logger.debug(
                    "***Trust DOES NOT match the current certificate***. Added %s to mismatchedIDs",
                    id,
                )
    logger.info("")
    if len(matchedIDs) != 0:
        for id in matchedIDs:
            (specfile, certfile) = get_filename_from_id(id)
            logger.debug(
                "Matched: id: %s spec: %s cert in use: %s", id, specfile, certfile
            )
    ...
    if ConnectFailure_ct != 0:
        logger.info("")
        logger.info(
            "***WARNING*** %s ID(s) skipped comparison due to connect failure, ignore if node is dead, use KB:2121701 for manual update procedure. Note: Port 443 is hardcoded",
            str(ConnectFailure_ct),
        )
...
def _doFix():
    ...
    try:
        ...
        logger.error(
            "Mismatch ID list file does not exist, Please run tool with 'scan' function"
        )
        return
    if not mismatchlist:
        ...
        return
    user = (
        raw_input("SSO administrator user (Default:Administrator@vsphere.local):")
    ...
    for id in mismatchlist:
        ...
        if updated != 0:
            ...
            if rc != 0:
                logger.error(
                    "'lstool reregister' failed for ID: %s with error %d", id, rc
                )
            else:
                updated_endpoint = updated_endpoint + updated
                logger.info("Fixing ID: %s completed\n", id)
    logger.info(
        "*** %d endpoints for %d service IDs updated with current cetificates and trust ***",
        updated_endpoint,
        update_idct,
    )
