...
(function ($) {
  ...
  function Sequence(element, options, defaults, get) {
    ...
    self.settings.preloader = renderUiElements(
      self,
      self.settings.preloader,
      ".sequence-preloader"
    ); //set up the preloader and save it
    ...
    self.settings.unpauseDelay =
      self.settings.unpauseDelay === null
        ...
        : self.settings.unpauseDelay; //if the unpauseDelay is not specified, make it the same as the autoPlayDelay speed
    self.getHashTagFrom = self.settings.hashDataAttribute
      ? "data-sequence-hashtag"
      : "id"; //get the hashtag from the ID or data attribute?
      ...
      if (!srcOnly) {
        ...
        for (var j = length; j > 0; j--) {
          ...
          imagesToPreload.push(
            $("body").find(
              'img[src="' + self.settings.preloadTheseImages[j - 1] + '"]'
            )
    ...
    if (
      ...
      var individualImagesToPreload = saveImagesToArray(
      ...
      ); //combine frame images and individual images
    ...
    function init() {
      ...
      self.nextButton = renderUiElements(
        self,
        self.settings.nextButton,
        ".sequence-next"
      ); //set up the next button
      self.prevButton = renderUiElements(
        self,
        self.settings.prevButton,
        ".sequence-prev"
      ); //set up the previous button
      self.pauseButton = renderUiElements(
        self,
        self.settings.pauseButton,
        ".sequence-pause"
      ); //set up the pause button
      self.pagination = renderUiElements(
        self,
      ...
      if (
        ...
        self.settings.showNextButtonOnInit === true
      ) {
      ...
      if (
        ...
        self.settings.showPrevButtonOnInit === true
      ) {
      ...
      if (self.settings.pauseIcon !== false) {
        self.pauseIcon = renderUiElements(
          self,
      ...
      if (self.settings.hashTags === true) {
        ...
        if (self.currentHashTag === undefined || self.currentHashTag === "") {
          ...
          self.frameHashIndex = $.inArray(
            self.currentHashTag,
            self.frameHashID
          ); //get the index of the frame that matches the hashtag
      ...
      if (self.pagination !== undefined) {
        ...
        $(self.paginationLinks[self.settings.startingFrameID - 1]).addClass(
          "current"
        ); //add the 'current' class to the current frame
      }
      ...
      if (self.transitionsSupported) {
        ...
        self.frames
          .not(":eq(" + (self.nextFrameID - 1) + ")")
          .css({ display: "none", opacity: 0 }); //set all frames (except the next one) to display: none, opacity: 0
        resetAutoPlay(self, true, self.settings.autoPlayDelay);
      }
      //END INIT
      //EVENTS
  ...
  Sequence.prototype = {
    ...
    prev: function () {
      ...
      self.nextFrameID =
        self.currentFrameID === 1
          ? self.numberOfFrames
          : self.currentFrameID - 1; //work out the prev frame
      if (self.active === false || self.active === undefined) {
        //if Sequence isn't currently animating...
    ...
    goTo: function (id, direction, ignoreTransitionThreshold) {
      ...
      var transitionThreshold =
        ignoreTransitionThreshold === true
          ? 0
          : self.settings.transitionThreshold; //if transitionThreshold is to be ignored, set it to zero
      ...
      if (
        ...
        (!self.settings.cycle &&
          direction === 1 &&
          self.currentFrameID === self.numberOfFrames) || //or cycling is disabled, the user is navigating forward and this is the last frame...
        (!self.settings.cycle &&
          direction === -1 &&
          self.currentFrameID === 1) || //or cycling is disabled, the user is navigating backwards and this is the first frame...
        ...
        if (self.settings.fadeFrameWhenSkipped) {
          ...
          self.nextFrame
            .stop()
            .animate({ opacity: 0 }, self.settings.fadeFrameTime); //fade
        }
          ...
          if (
            ...
            setTimeout(function () {
              ...
              waitForAnimationsToComplete(
                ...
                "in"
              ); //wait for the next frame to animate in
              if (
                self.afterCurrentFrameAnimatesOut !== "function () {}" ||
                (self.settings.transitionThreshold === true &&
                  ignoreTransitionThreshold !== true)
              ) {
                ...
                waitForAnimationsToComplete(
                  self,
                  self.currentFrame,
                  self.currentFrameChildren,
                  "out",
                  true,
                  1
                ); //wait for the current frame to animate out as well
              }
            }, 50);
  ...
  var reverseTransitionProperties = function (sequence) {
    ...
    var maximumCurrentFrameDuration = Math.max.apply(
      Math,
      currentFrameChildrenDurations
    ); //find which transition duration is the longest
    var maximumNextFrameDuration = Math.max.apply(
      Math,
      nextFrameChildrenDurations
    ); //find which transition duration is the longest
    var transitionDifference =
      maximumCurrentFrameDuration - maximumNextFrameDuration; //get the overal transition difference between the current and next frame
    var currentDelay = 0;
    var nextDelay = 0;

    if (
      transitionDifference < 0 &&
      !sequence.settings.preventDelayWhenReversingAnimations
    ) {
    ...
    var reverseEachProperty = function (
      ...
      currentFrameChildren.each(function () {
        ...
        var transitionFunction = $(this).css(
          sequence.prefix + "transition-timing-function"
        ...
        if (transitionFunction.indexOf("cubic-bezier") >= 0) {
          ...
          var cubicBezier = transitionFunction
            ...
            .replace(")", "")
            .split(","); //remove the CSS function and just get the array
        ...
        transitionProperties["transition-delay"] =
          maximumFrameDuration - frameDuration + frameDelay + "s"; //add a delay if required
    ...
    reverseEachProperty(
      sequence.transitionProperties,
      sequence.nextFrameChildren,
      maximumNextFrameDuration,
      nextDelay
    ); //reverse properties for each of the next frame's elements
  };
  ...
  var renderUiElements = function (sequence, devOption, defaultOption) {
    switch (devOption) {
      ...
      default:
        //if using a developer defined selector...
  ...
  var setHashTag = function (sequence) {
    if (sequence.settings.hashTags) {
      ...
      sequence.currentHashTag = sequence.nextFrame.prop(
        sequence.getHashTagFrom
      ); //get the hashtag name
      sequence.frameHashIndex = $.inArray(
        sequence.currentHashTag,
        sequence.frameHashID
      ); //get the index of the frame that matches the hashtag
      if (
        sequence.frameHashIndex !== -1 &&
        (sequence.settings.hashChangesOnFirstFrame ||
          !sequence.isStartingFrame ||
          !sequence.transitionsSupported)
      ) {
        //if the hashtag matches a Sequence frame ID...
