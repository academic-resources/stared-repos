...
def petition_submission_view(request, pk):
    ...
    try:
        ...
        return JsonResponse(
            {"error": "Invalid petition id"}, status=status.HTTP_400_BAD_REQUEST
        )
...
def signup_submission(request, signup):
    ...
    if source is None:
        return JsonResponse(
            {"error": "Unknown source"}, status=status.HTTP_400_BAD_REQUEST
        )
    ...
    if settings.MOFO_NEWSLETTER_SUBSCRIBE_METHOD == "BASKET":
        response = basket.subscribe(
            data["email"], data["newsletters"], lang=data["lang"]
        )
        ...
        message = json.dumps(
            {
                "app": settings.HEROKU_APP_NAME,
                "timestamp": datetime.now().isoformat(),
                "data": {"json": True, "form": data, "event_type": "crm_petition_data"},
            }
        )

    if (
        settings.MOFO_NEWSLETTER_SUBSCRIBE_METHOD == "BASKET"
...
def send_to_sqs(sqs, queue_url, message, type="petition"):
    ...
    if queue_url is None:
        ...
        return JsonResponse({"message": "success", "details": "nq"}, status=201)
    ...
    if "MessageId" in response and response["MessageId"]:
        return JsonResponse(
            {"message": "success", "details": response["MessageId"]}, status=201
        )
    else:
        return JsonResponse(
