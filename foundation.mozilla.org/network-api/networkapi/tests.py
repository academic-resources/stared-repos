...
class RedirectDefaultSiteDecoratorTests(TestCase):
    def setUp(self):
        ...
        self.original_default_site = Site.objects.get(
        ...
        self.default_site = SiteFactory(
            hostname="default-site.com", is_default_site=True
        )
    ...
    def test_PNI_homepage_redirect_to_foundation_site(self):
        ...
        response = self.client.get(
            "/en/privacynotincluded/", HTTP_HOST="secondary-site.com"
        )
...
class WagtailPagesTestCase(TestCase):
    ...
    def test_parse_accept_lang_header_returns_iso_3166_language(self):
        self.assertEqual(
            parse_accept_lang_header("en-GB,en;q=0.5"), (("en-GB", 1.0), ("en", 0.5))
        )
...
class TestPNIAirtableConnections(TestCase):
    ...
    def test_product_page_import_mappings(self):
        ...
        self.assertEqual(
            mappings["Signup requires phone number"], "signup_requires_phone"
        )
        self.assertEqual(
        ...
        self.assertEqual(
            mappings["Signup explanation"], "signup_requirement_explanation"
        )
        self.assertEqual(
            mappings["How it collects data"], "how_does_it_use_data_collected"
        )
        self.assertEqual(
            mappings["Data collection privacy ding"], "data_collection_policy_is_bad"
        )
        self.assertEqual(
            mappings["User friendly privacy policy"], "user_friendly_privacy_policy"
        )
        self.assertEqual(
        ...
        self.assertEqual(
            mappings["Security updates help text"], "security_updates_helptext"
        )
        self.assertEqual(mappings["Strong password"], "strong_password")
        self.assertEqual(
            mappings["Strong password help text"], "strong_password_helptext"
        )
        self.assertEqual(
            mappings["Manages security vulnerabilities"], "manage_vulnerabilities"
        )
        self.assertEqual(
            mappings["Manages security help text"], "manage_vulnerabilities_helptext"
        )
        self.assertEqual(mappings["Has privacy policy"], "privacy_policy")
        self.assertEqual(
            mappings["Privacy policy help text"], "privacy_policy_helptext"
        )
    ...
    def test_general_product_page_import_mappings(self):
        ...
        self.assertEqual(
            mappings["Biometric data collected"], "biometric_data_collected"
        )
        self.assertEqual(mappings["Social data collected"], "social_data_collected")
        self.assertEqual(
            mappings["How you can control your data"], "how_can_you_control_your_data"
        )
        self.assertEqual(mappings["Company track record"], "company_track_record")
        self.assertEqual(
            mappings["Show company track record privacy ding"], "track_record_is_bad"
        )
    ...
    def test_software_product_page_import_mappings(self):
        ...
        self.assertEqual(
            mappings["Recording alert help text"], "recording_alert_helptext"
        )
        self.assertEqual(
            mappings["Medical privacy compliant"], "medical_privacy_compliant"
        )
        self.assertEqual(
            mappings["Medical privacy compliant help text"],
        ...
        self.assertEqual(
            mappings["Easy to learn and use help text"],
...
class TestCreateWagtailImageUtility(TestCase):
    def setUp(self):
        self.image_path = abspath(
            join(dirname(__file__), "../media/images/placeholders/products/teddy.jpg")
        )

    def create_new_image(self):
        """A generic test to ensure the image is created properly."""
        new_image = create_wagtail_image(
            self.image_path, image_name="fake teddy.jpg", collection_name="pni products"
        )
        # Image was created
