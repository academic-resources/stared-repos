...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if root.id != 1:
            print(
                "Incorrect root Collection ID. Stopping this script to prevent any mistakes"
            )
            return

        duplicate_root_collections = Collection.objects.filter(depth=1).exclude(
            name="Root"
        )

        print(
            f"There are {duplicate_root_collections.count()} duplicate root Collections"
        )
