...
class DonationModal(TranslatableMixin, models.Model):
    ...
    donate_text = models.CharField(
        max_length=150, help_text="Donate button label", default="Yes, I'll chip in"
    )

    dismiss_text = models.CharField(
        max_length=150, help_text="Dismiss button label", default="No thanks"
    )
...
class DonationModals(TranslatableMixin, models.Model):
    page = ParentalKey("wagtailpages.CampaignPage", related_name="donation_modals")
    ...
    panels = [SnippetChooserPanel("donation_modal")]
