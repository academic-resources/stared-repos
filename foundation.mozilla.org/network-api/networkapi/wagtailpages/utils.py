...
def insert_panels_after(panels, after_label, additional_panels):
    ...
    """
    position = next(
        (i for i, item in enumerate(panels) if item.heading == after_label), None
    )
...
def create_wagtail_image(
    img_src: str, image_name: str = None, collection_name: str = None
) -> Union[None, Image]:
    """
    ...
    if collection_name:
        ...
        collection = (
            ...
            else collection.add_child(name=collection_name)
        )
    ...
    wagtail_image = Image.objects.create(
        title=image_name, file=ImageFile(f, name=image_name), collection=collection
    )
