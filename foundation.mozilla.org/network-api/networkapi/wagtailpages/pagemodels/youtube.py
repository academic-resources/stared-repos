...
class YoutubeRegretsPage(FoundationMetadataPageMixin, Page):
    headline = models.CharField(max_length=500, help_text="Page headline", blank=True)

    intro_text = StreamField([("text", blocks.CharBlock())])

    intro_images = StreamField([("image", customblocks.ImageBlock())])
    ...
    regret_stories = StreamField([("regret_story", customblocks.YoutubeRegretBlock())])
