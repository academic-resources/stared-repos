...
class InitiativeSection(TranslatableMixin, models.Model):
    page = ParentalKey(
        "wagtailpages.InitiativesPage", related_name="initiative_sections"
    )
    ...
    sectionHeader = models.CharField(verbose_name="Header", max_length=250)

    sectionCopy = models.TextField(verbose_name="Subheader")

    sectionButtonTitle = models.CharField(verbose_name="Button Text", max_length=250)

    sectionButtonURL = models.TextField(verbose_name="Button URL")

    sectionButtonTitle2 = models.CharField(
        verbose_name="Button 2 Text", max_length=250, blank="True"
    )
...
class ParticipatePage2(PrimaryPage):
    ...
    ctaHeroSubhead = RichTextField(features=["bold", "italic", "link"], blank=True)

    ctaButtonTitle = models.CharField(
        verbose_name="Button Text", max_length=250, blank=True
    )

    ctaButtonURL = models.TextField(verbose_name="Button URL", blank=True)
    ...
    ctaHeroSubhead2 = RichTextField(features=["bold", "italic", "link"], blank=True)

    ctaButtonTitle2 = models.CharField(
        verbose_name="Button Text", max_length=250, blank=True
    )

    ctaButtonURL2 = models.TextField(verbose_name="Button URL", blank=True)
    ...
    ctaHeroSubhead3 = RichTextField(features=["bold", "italic", "link"], blank=True)
...
class HomepageSpotlightPosts(TranslatableMixin, WagtailOrderable):
    page = ParentalKey("wagtailpages.Homepage", related_name="spotlight_posts")
...
class HomepageNewsYouCanUse(TranslatableMixin, WagtailOrderable):
    page = ParentalKey("wagtailpages.Homepage", related_name="news_you_can_use")
    blog = models.ForeignKey("BlogPage", on_delete=models.CASCADE, related_name="+")
    panels = [PageChooserPanel("blog")]
...
class InitiativesHighlights(TranslatableMixin, WagtailOrderable, models.Model):
    page = ParentalKey(
        "wagtailpages.InitiativesPage", related_name="featured_highlights"
    )
    highlight = models.ForeignKey(
        "highlights.Highlight", on_delete=models.CASCADE, related_name="+"
    )
    panels = [SnippetChooserPanel("highlight")]
...
class CTABase(WagtailOrderable, models.Model):
    ...
    subhead = RichTextField(features=["bold", "italic", "link"], blank=True)

    buttonTitle = models.CharField(
        verbose_name="Button Text", max_length=250, blank=True
    )

    buttonURL = models.TextField(verbose_name="Button URL", blank=True)
...
class CTA4(TranslatableMixin, CTABase):
    page = ParentalKey("wagtailpages.ParticipatePage2", related_name="cta4")
...
class ParticipateHighlightsBase(TranslatableMixin, WagtailOrderable, models.Model):
    page = ParentalKey(
        "wagtailpages.ParticipatePage2", related_name="featured_highlights"
    )
    highlight = models.ForeignKey(
        "highlights.Highlight", on_delete=models.CASCADE, related_name="+"
    )
...
class ParticipateHighlights2(ParticipateHighlightsBase):
    page = ParentalKey(
        "wagtailpages.ParticipatePage2", related_name="featured_highlights2"
    )
...
class FocusArea(TranslatableMixin, models.Model):
    ...
    name = models.CharField(
        max_length=100, help_text="The name of this area of focus. Max. 100 characters."
    )
...
class HomepageFocusAreas(TranslatableMixin, WagtailOrderable):
    page = ParentalKey("wagtailpages.Homepage", related_name="focus_areas")
...
class HomepageTakeActionCards(TranslatableMixin, WagtailOrderable):
    page = ParentalKey("wagtailpages.Homepage", related_name="take_action_cards")
...
class PartnerLogos(TranslatableMixin, WagtailOrderable):
    page = ParentalKey("wagtailpages.Homepage", related_name="partner_logos")
...
class Homepage(FoundationMetadataPageMixin, Page):
    hero_headline = models.CharField(
        max_length=80, help_text="Hero story headline", blank=True
    )
    ...
    spotlight_headline = models.CharField(
        max_length=140, help_text="Spotlight headline", blank=True
    )

    cause_statement = models.CharField(max_length=250, default="")

    cause_statement_link_text = models.CharField(max_length=80, blank=True)
    ...
    quote_text = models.CharField(max_length=450, default="")

    quote_source_name = models.CharField(max_length=100, default="")

    quote_source_job_title = models.CharField(max_length=100, default="")
    ...
    content_panels = Page.content_panels + [
        ...
        MultiFieldPanel(
            [
                ...
                InlinePanel(
                    "partner_logos", label="Partner Logo", max_num=7, min_num=1
                ),
            ],
            heading="Partner",
            classname="collapsible collapsed",
