...
class DearInternetPage(FoundationMetadataPageMixin, Page):
    intro_texts = StreamField(
        [("intro_text", blocks.RichTextBlock(features=["bold", "italic", "link"]))]
    )

    letters_section_heading = models.CharField(
        max_length=300, default="Stories from around the world"
    )

    letters = StreamField([("letter", customblocks.DearInternetLetterBlock())])
