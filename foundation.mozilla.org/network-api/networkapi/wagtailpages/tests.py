...
class BuyersGuideViewTest(TestCase):
    ...
    def test_homepage(self):
        ...
        self.assertEqual(
            response.status_code, 302, "Homepage should be forwarded to /en/ by default"
        )

    @skip(
        "TODO: REENABLE: THIS HAS BEEN TESTED MANUALLY BUT FAILS IN THIS CODE FORM ATM"
    )
    def test_localised_homepage(self):
        ...
        response = self.client.get(
            "/privacynotincluded", follow=True, HTTP_ACCEPT_LANGUAGE="fr"
        )
        self.assertEqual(
        ...
        response = self.client.get(
            "/privacynotincluded", follow=True, HTTP_ACCEPT_LANGUAGE="foo"
        )
        self.assertEqual(
            response.redirect_chain[0][0],
        ...
        response = self.client.get(
            "/de/privacynotincluded", follow=True, HTTP_ACCEPT_LANGUAGE="it"
        )
        self.assertEqual(
            response.redirect_chain[0][0],
    ...
    def test_product_view_404(self):
        ...
        """
        response = self.client.get(
            "/en/privacynotincluded/products/this is not a product", follow=True
        )
        self.assertEqual(response.status_code, 404, "this is not a product")

        response = self.client.get(
            "/en/privacynotincluded/products/this is not a product/"
        )
        self.assertEqual(response.status_code, 404, "this is not a product")
    ...
    def test_category_view_404(self):
        ...
        """
        response = self.client.get(
            "/en/privacynotincluded/categories/this is not a category", follow=True
        )
        self.assertEqual(response.status_code, 404, "this is not a category")

        response = self.client.get(
            "/en/privacynotincluded/categories/this is not a category/"
        )
        self.assertEqual(response.status_code, 404, "this is not a category")
    ...
    def test_category_view(self):
        ...
        self.assertEqual(
            response.status_code, 302, 'The category "Smart Home" should work by name'
        )

        response = self.client.get("/privacynotincluded/categories/smart-home/")
        self.assertEqual(
            response.status_code, 302, 'The category "Smart Home" should work by slug'
        )
    ...
    def get_or_create_product_page(self):
        ...
        if not product_page:
            ...
            wagtail_image = create_wagtail_image(
                image_path, collection_name="pni products"
            )
...
class TestBuyersGuidePage(BuyersGuideTestMixin):
    ...
    def test_buyersguide_about_mss_view(self):
        self.about_url_test(
            "min-security-view",
            "about/meets-minimum-security-standards/",
            "minimum_security",
        )
    ...
    def test_buyersguide_category_route(self):
        ...
        category_url = self.bg.reverse_subpage(
            "category-view", args=(missing_category,)
        )
    ...
    def test_buyersguide_product_redirect_route(self):
        ...
        response = self.client.get(
            f"/en/{self.bg.slug}/products/{product.slug}/", follow=True
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_category_filter_view(self):
        category = BuyersGuideProductCategory.objects.first()
        url = self.bg.url + self.bg.reverse_subpage(
            "category-view", args=(category.slug,)
        )
        ...
        with self.settings(
            ...
            category_orderable = ProductPageCategory(
                product=self.product_page, category=category
            )
...
class TestProductPage(BuyersGuideTestMixin):
    ...
    def test_get_voting_json(self):
        ...
        comparable_data = {
            "creepiness": {
                "vote_breakdown": {"0": 1, "1": 2, "2": 3, "3": 4, "4": 5},
                "average": 4.0,
            },
            "total": 15,
        }
...
class WagtailBuyersGuideVoteTest(APITestCase, BuyersGuideTestMixin):
    def test_successful_vote(self):
        ...
        response = self.client.post(self.product_page.url, {"value": 25}, format="json")
        self.assertEqual(response.status_code, 200)
        ...
        response = self.client.post(
            self.product_page.url, {"value": 100}, format="json"
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_bad_vote_value(self):
        # vote = 500
        response = self.client.post(self.product_page.url, {"value": -1}, format="json")
        self.assertEqual(response.status_code, 405)

        response = self.client.post(
            self.product_page.url, {"value": 101}, format="json"
        )
        self.assertEqual(response.status_code, 405)
