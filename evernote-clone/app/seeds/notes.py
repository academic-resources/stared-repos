...
def seed_notes():
    ...
    demo_note = Note(
        title="Destructuring",
        content="Destructuring is lifee",
        user_id=1,
        notebook_id=1,
    )
    db.session.add(demo_note)
    demo_note = Note(
        title="Programming Notes", content="<code/>", user_id=1, notebook_id=1
    )
    db.session.add(demo_note)
    demo_note = Note(
    ...
    demo_note = Note(
        title="Wonderful :D",
        content="Isn't this note just wonderful? <3 ^_^",
        user_id=1,
        notebook_id=1,
    )
    db.session.add(demo_note)
    demo_note = Note(
        title="More notes", content="a b c d e f g :)", user_id=1, notebook_id=1
    )
    db.session.add(demo_note)
    demo_note = Note(
        title="Puppies",
        content="Mason, Misa, Buddy, Beth, and Macho",
        user_id=1,
        notebook_id=1,
    )
    db.session.add(demo_note)
    demo_note = Note(
        title="Fun stuff",
        content="<strong>super fun super fun, let's do stuff</strong>",
        user_id=1,
        notebook_id=2,
    )
    db.session.add(demo_note)
    demo_note = Note(
        title="Grocery list",
        content="milk, eggs, soda, candy, bread, veggies, soap, sponges",
        user_id=1,
        notebook_id=2,
    )
    db.session.add(demo_note)
    demo_note = Note(
        ...
        content="Stay Home and drink coffeez",
        user_id=1,
        notebook_id=3,
    )
    db.session.add(demo_note)
    ...
    demo_note = Note(
        title="Basic Workout",
        content="<strong>Pushups</strong><br><strong>Bench Press</strong>",
        user_id=1,
        notebook_id=5,
    )
    db.session.add(demo_note)
