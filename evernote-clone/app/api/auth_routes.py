...
def get_user_data(user):
    tags = (
        Tag.query.filter(Tag.user_id == user["id"]).options(joinedload(Tag.notes)).all()
    )
    tags_data = {
    ...
    notebooks = (
        Notebook.query.filter(Notebook.user_id == user["id"])
        .options(joinedload(Notebook.notes).joinedload(Note.tags))
        .all()
    )
    ...
    notes = (
        Note.query.filter(Note.user_id == user["id"])
