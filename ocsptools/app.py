...
def lint(filename, chain_file):
    try:
        cert = hf.return_cert_from_file(
            os.path.join(app.instance_path, "cert", filename)
        )
    except ValueError:
        return "{} is not a valid x509 certificate".format(cert_file.filename)

    try:
        issuer = hf.return_cert_from_file(
            os.path.join(app.instance_path, "chain", chain_file)
        )
    except ValueError:
...
def response_check():
    ...
    if request.method == "POST":
        ...
        if check == "Check OCSP Response":
            ...
            cert_file.save(
                os.path.join(
                    app.instance_path, "cert", secure_filename(cert_file.filename)
                )
            )
            ...
            issuer_file.save(
                os.path.join(
                    app.instance_path, "chain", secure_filename(issuer_file.filename)
                )
            )

            return lint(
                secure_filename(cert_file.filename),
                secure_filename(issuer_file.filename),
            )
            ...
            if cert_data[0] is not None:
                ...
                with open(os.path.join(app.instance_path, "cert", filename), "w+") as f:
                    ...
                    f.close()
                with open(
                    os.path.join(app.instance_path, "chain", filename_chain), "w+"
                ) as f:
