...
def return_ocsp_request_object(cert, issuer, algo, nonce=True):
    ...
    request_obj = ocsp.Request({"req_cert": cert_details})

    tbs_request_obj = ocsp.TBSRequest({"request_list": ocsp.Requests([request_obj])})
    ...
    ocsp_request_obj = ocsp.OCSPRequest({"tbs_request": tbs_request_obj})
