...
if __name__ == "__main__":
    ...
    for row in c.execute(
        ...
        uri_proc = sp.run(
            """openssl x509 -noout -text -in {0} | grep -A 4 'X509v3 CRL Distribution Points' | grep 'URI:' | 
                sed -n 's/.*\(http.*\).*/\\1/p'""".format(
                filepath
            ),
            shell=True,
            stdout=sp.PIPE,
        ...
        wget_proc = sp.run(
            """wget -O crl.der {0}""".format(uri),
            shell=True,
            stdout=sp.PIPE,
            stderr=sp.PIPE,
        )
        transform_proc = sp.run(
            """openssl crl -inform DER -in crl.der -outform PEM -out crl.pem""",
            shell=True,
        )
        chain_proc = sp.run(
            """OLDIFS=$IFS; IFS=':' certificates=$(openssl s_client -connect {0}:443 -showcerts -servername {0} 2>&1 </dev/null | sed -n '/-----BEGIN/,/-----END/ {{/-----BEGIN/ s/^/:/; p}}'); for certificate in ${{certificates#:}}; do echo $certificate | tee -a chain.pem ; done; IFS=$OLDIFS""".format(
                row[0]
            ),
            shell=True,
            stdout=sp.PIPE,
        ...
        check_proc = sp.run(
            """openssl verify -crl_check -CAfile crl_chain.pem {0}""".format(filepath),
            shell=True,
        )
