...
def get_ocsp_response(cert, issuer, algo="sha1", nonce=True, timeout=20):
    ...
    for i in range(len(cert.ocsp_urls)):
        try:
            ocsp_response_obj = hf.make_ocsp_request(
                cert.ocsp_urls[i], ocsp_request_obj, timeout
            )
...
def validate_ocsp_response(
    cert, issuer, ocsp_request_obj, ocsp_response_objs, current_time
):
    ...
    for (ocsp_url, ocsp_response_obj) in ocsp_response_objs:
        ...
        if (
            request_nonce
            and response_nonce
            and request_nonce.native != response_nonce.native
        ):
            errors[
                "NonceVerificationFailure"
        ...
        if certificate_serial_number != certificate_serial_number_from_file:
            errors[
                "CertificateSerialMismatchFailure"
        ...
        if certificate_issuer_key_hash != certificate_issuer_key_hash_from_file:
            errors[
                "IssuerKeyMismatchFailure"
            ] = "OCSP response issuer key hash does not match request certificate issuer key hash"

        if certificate_issuer_name_hash != certificate_issuer_name_hash_from_file:
            errors[
                "IssuerNameHashMismatchFailure"
        ...
        if current_time < this_update_time:
            errors[
                "ThisUpdateTimeError"
            ] = "OCSP reponse update time is from the future"

        if (
            "next_update" not in certificate_response
            or certificate_response["next_update"].native is None
        ):
            warnings[
                "NextUpdateTimeMissing"
            ] = "OCSP response does not contain next update time"
        else:
            next_update_time = certificate_response["next_update"].native
            if current_time > next_update_time:
                errors[
                    "NextUpdateTimeFailure"
                ] = "OCSP reponse next update time is in the past"
        ...
        if signing_cert is None:
            errors[
                "SigningCetificateNotFoundFailure"
            ] = "OCSP response signing certificate not found"
            lints["errors"] = errors
            lints["warnings"] = warnings
            lints_list.append(lints)
            continue
        ...
        if issuer.issuer_serial != signing_cert.issuer_serial:
            if signing_cert_issuer.issuer_serial != issuer.issuer_serial:
                errors[
                    "UnauthorizedSigningCertificateFailure"
                ] = "OCSP response signed by unauthorized certificate"

            extended_key_usage = signing_cert.extended_key_usage_value
            if "ocsp_signing" not in extended_key_usage.native:
                errors[
                    "ExtendedKeyUsageExtensionValueFailure"
        ...
        try:
            ...
            if sig_algo == "rsassa_pkcs1v15":
                ...
                errors[
                    "UnsupportedAlgorithmFailure"
                ] = "OCSP response signature uses unsupported algorithm"
        ...
        if certificate_response["cert_status"].name == "revoked":
            ...
            if revocation_data["revocation_reason"].native is None:
                errors[
                    "CertificateValidityFailure"
                ] = "Certificate revoked due to unknown reason"
            else:
                errors["CertificateValidityFailure"] = (
                    "Certicate revoked due to "
                    + revocation_data["revocation_reason"].human_friendly
                )
...
if __name__ == "__main__":
    ...
    lints_list = validate_ocsp_response(
        cert, issuer, ocsp_request, ocsp_responses, current_time
    )
    ...
    for lint in lints_list:
        ...
        cursor.execute(
            insert_query,
            (domain, ocsp_url, count, json.dumps(errors), json.dumps(warnings)),
        )
