    ...
    try:
        if reading_list is not None:
            ...
            if not os.path.exists(acl_anthology_file):
                data = (
                    urllib.request.urlopen(args.acl_anthology_file)
                    .read()
                    .decode("utf-8")
                )
            ...
            acl_anthology_by_id, acl_anthology_by_title = process_acl_anthology_file(
                filename=acl_anthology_file, show_progress=False
            )
            ...
            if result is None:
                ...
                easygui.msgbox(
                    "Sorry, I could not get the bib info for this paper.", title="Error"
                )
            else:
                bib_entry, title = result
                add_to_reading_list(reading_list, bib_entry, title)
                easygui.msgbox(
                    'Successfully added the following paper to the reading list: "{}"'.format(
                        title
                    ),
                    title="Success",
                )
        ...
        easygui.msgbox(
            "Failed adding paper to the reading list. {}".format(sys.exc_info()[0]),
            title="Error",
        )
        logger.error(
            "Failed adding paper to the reading list. {}".format(sys.exc_info()[0])
