...
def main():
    args = docopt(
        """Draws a 2d graph of the given list of words and word embeddings, using
    ...
    for word, x, y in zip(vocabulary, Y[:, 0], Y[:, 1]):
        ax.annotate(
            word, xy=(x, y), xytext=(0, 0), textcoords="offset points", fontsize=8
        )
...
def load_embeddings(embedding_file, dim, vocab=None):
    ...
    words, vectors = zip(
        *[
            ...
            if len(vector.split()) == dim and (vocab is None or word in vocab)
        ]
    )
