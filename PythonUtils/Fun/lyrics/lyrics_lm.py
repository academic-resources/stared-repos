...
def main():
    ...
    for iter in range(MAX_ITERS):
        ...
        for i, sent in enumerate(train):
            ...
            if i % DISPLAY_FREQ == 0:
                ...
                sample = lm.sample(
                    first=vocab.w2i[START], stop=vocab.w2i[END], nwords=10
                )
    ...
    with codecs.open(model_name + ".sample", "w", "utf-8") as f_out:
        ...
        for i in range(25):
            ...
            sentence = " ".join(
                [vocab.i2w[w] for w in sample if w != start_index and w != end_index]
            ).strip()
...
def load_text_embeddings(embeddings_file, dim, vocabulary):
    ...
    vectors = {
        word: np.fromstring(vector, sep=" ")
        for (word, vector) in lines
        if len(vector.split()) == dim
    }
