...
def main():
    ...
    auth = tweepy.OAuthHandler(
        access_keys["consumer_key"], access_keys["consumer_secret"]
    )
    auth.set_access_token(
        access_keys["access_token"], access_keys["access_token_secret"]
    )
...
def generate_random_venn2(intersections, sets):
    ...
    all_members = [A.difference(B), B.difference(A), A.intersection(B)]

    all_members = [
        random.sample(list(members), 3) if len(members) > 3 else members
...
def generate_random_venn(wordnet, filename=None):
    ...
    while not found:
        ...
        sets = {
            wordnet.get_name(subclass): wordnet.get_members(subclass)
        ...
        sets = {
            name: [wordnet.get_name(member) for member in subclass]
