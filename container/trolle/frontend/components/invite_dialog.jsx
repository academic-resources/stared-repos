...
const mdtp = (dispatch) => ({
  ...
  inviteUsers: (user_ids, board_id) =>
    dispatch(inviteUsers(user_ids, board_id)),
});
...
class InviteDialog extends React.Component {
  ...
  render() {
    ...
    const selectedIcons = Object.values(this.state.selected_users).map(
      (user) => (
...
export default connect(mstp, mdtp)(InviteDialog);
