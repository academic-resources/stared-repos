...
(function (global) {
  ...
  var Assertions = (function () {
    ...
    function assert(test, message) {
      this._assertExpression(
        ...
        "Expected %o to evaluate to true.",
        test
      );
    }
    ...
    function refute(test, message) {
      this._assertExpression(
        ...
        "Expected %o to evaluate to false.",
        test
      );
    }
    ...
    function refuteUndefined(test, message) {
      this._assertExpression(
        typeof test !== "undefined",
        message || "Failed refutation.",
        "Expected %o to not be undefined.",
        test
      );
    }
    ...
    function assertIn(property, object, message) {
      this._assertExpression(
        property in object,
        message || "Failed assertion.",
        'Expected "%s" to be a property of %o.',
        property,
        object
      );
    }
    ...
    function refuteIn(property, object, message) {
      this._assertExpression(
        !(property in object),
        message || "Failed refutation.",
        'Expected "%s" to not be a property of %o.',
        property,
        object
      );
    }
    ...
    function pause(assertions) {
      ...
      self._timeoutId = global.setTimeout(function () {
        self.resume(function () {
          self.fail(
            "Test timed out. Testing was not resumed after being paused."
          );
        });
  ...
  function AutoRunner() {
    if (global.console && global.console.log) {
      ...
      Object.prototype.toString.call(global.environment) ===
        "[object Environment]" &&
      global.print
    ) {
  ...
  (function (p) {
    var BASIC_STYLES =
      "color: #333; background-color: #fff; font-family: monospace; border-bottom: 1px solid #ccc;";
    var STYLES = {
  ...
  (function (p) {
    ...
    function addFailure(testcase, msg) {
      ...
      this.logger.error(
        testcase + ": " + msg.message + " " + msg.template,
        msg.args
      );
    }
    ...
    function printf(template, args, inspector) {
      ...
      while ((m = regexp.exec(template))) {
        ...
        if (match.indexOf("%%") === 0) {
          ...
          parts.push(
            template.substr(0, match.indexOf("%" === 0) ? index + 1 : index)
          );
          type = m[2];
          arg = args.shift();
