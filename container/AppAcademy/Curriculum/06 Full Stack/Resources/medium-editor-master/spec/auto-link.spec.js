...
describe("Autolink", function () {
    ...
    describe("extension", function () {
        ...
        it("should reset browser auto-link (if supported) during destroy", function () {
            ...
            spyOn(document, "queryCommandSupported").and.callFake(function (
                command
            ) {
                if (command === "AutoUrlDetect") {
                    return true;
        ...
        describe("auto-linking typed-in text", function () {
            ...
            function generateLinkTest(link, href) {
                return function () {
                    ...
                    newRange.setStart(
                        this.el.firstChild.childNodes[0],
                        link.length + 1
                    );
                    newRange.setEnd(
                        this.el.firstChild.childNodes[0],
                    ...
                    expect("href: " + anchors[0].getAttribute("href")).toBe(
                        "href: " + href
                    );
                    expect("Text content: " + anchors[0].textContent).toBe(
                        "Text content: " + link
                    );
                    expect(
                        anchors[0].firstChild.getAttribute("data-auto-link")
                    ).toBe("true");
                };
            }
            ...
            links.forEach(function (link) {
                it(
                    'should auto-link "' + link + '" when typed in',
                ...
                it(
                    'should auto-link "' + noProtocolLink + '" when typed in',
            ...
            notLinks.forEach(function (link) {
                it(
                    'should not auto-link "' + link + '" when typed in',
                    generateNotLinkTest(link)
                );
            });
            ...
            it("should auto-link text on its own", function () {
                ...
                expect(links[0].firstChild.getAttribute("data-auto-link")).toBe(
                    "true"
                );
            ...
            it("should auto-link link within basic text", function () {
                ...
                expect(
                    this.el.childNodes[this.el.childNodes.length - 1].nodeValue
                ).toBe(" inside!");
            });

            it("should auto-link basic text within a parent element", function () {
                this.el.innerHTML =
                    "<span>Text with http://www.example.com inside!</span>";
                ...
                expect(this.el.firstChild.textContent).toBe(
                    "Text with http://www.example.com inside!"
                );
                expect(
                    this.el.firstChild.getElementsByTagName("a").length
                ).toBe(1);
                expect(links[0].getAttribute("href")).toBe(
                    "http://www.example.com"
                );
            ...
            it("should auto-link text that is partially styled and preserve the SPAN and B tags", function () {
                var selection = window.getSelection(),
                    newRange = document.createRange();
                this.el.innerHTML =
                    '<p><span class="a"><b>Here is the link: http://www.</b>exa</span>mple.com </p>';
                selection.removeAllRanges();
                newRange.setStart(
                    this.el.firstChild.lastChild,
                    this.el.firstChild.lastChild.nodeValue.length
                );
                newRange.setEnd(
                    this.el.firstChild.lastChild,
                    this.el.firstChild.lastChild.nodeValue.length
                ...
                expect(
                    this.el.innerHTML.indexOf(
                        '<p><span class="a"><b>Here is the link: </b></span>'
                    )
                ).toBe(0);
                ...
                expect(links[0].firstChild.innerHTML).toBe(
                    '<span class="a"><b>http://www.</b>exa</span>mple.com'
                );
            ...
            it("should auto-link text that is partially styled with a profusion of mixed bold sections", function () {
                var selection = window.getSelection(),
                    newRange = document.createRange();
                this.el.innerHTML =
                    "<p><b>Here is the link: http://www.</b>exampl<b>e</b>.com </p>";
                selection.removeAllRanges();
                newRange.setStart(
                    this.el.firstChild.lastChild,
                    this.el.firstChild.lastChild.nodeValue.length
                ...
                expect(
                    this.el.innerHTML.indexOf("<p><b>Here is the link: </b>")
                ).toBe(0);
                var links = this.el.getElementsByTagName("a");
                expect(links.length).toBe(1);
                expect(links[0].firstChild.getAttribute("data-auto-link")).toBe(
            ...
            it("should auto-link text in a really hideous example", function () {
                ...
                expect(links[0].firstChild.getAttribute("data-href")).toBe(
            ...
            it("should not auto-link text inside a span with data-auto-link=true", function () {
                this.el.innerHTML =
                    'Click this <span data-href="http://www.example.com" data-auto-link="true">' +
                    "http://www.example.com</span> link";

                selectElementContentsAndFire(this.el.firstChild);

                triggerAutolinking(this.el);
                expect(this.el.getElementsByTagName("a").length).toBe(
                    0,
                    "should not create a link"
                );
                expect(this.el.getElementsByTagName("span").length).toBe(
                    1,
                    "span should remain in place"
                );
            });
            ...
            it("should remove a span with data-auto-link=true when the text no longer matches the original link", function () {
                this.el.innerHTML =
                    'Click this <span data-auto-link="true" data-href="http://www.example.com">' +
                    "foo</span> link";

                triggerAutolinking(this.el);
                expect(this.el.getElementsByTagName("span").length).toBe(
                    0,
                    "span should have been removed"
                );
            });

            it(
                "should create a link with data-auto-link=true when the text no longer matches the original link" +
                    " and it has been unlinked",
                function () {
                    this.el.innerHTML =
                        'Click this <span data-auto-link="true" data-href="http://www.example.com">' +
            ...
            it("should create a link for a url within a list item", function () {
                this.el.innerHTML =
                    "<p>This is my list of links:</p><ol><li>http://www.example.com</li></ol><p>It is very impressive</p>";
                ...
                expect(links.length).toBe(
                    1,
                    "A single link was not automatically created"
                );
            ...
            it("should not create a link when text in consecutive list items could be a valid url when combined", function () {
                this.el.innerHTML =
                    "<ul><li>text ending in a period.</li><li>name - text starting with a TLD</li></ul>";
                ...
                expect(links.length).toBe(
                    0,
                    "A link was created without a valid url being in the text"
                );
            ...
            it("should not create a link which spans multiple list items", function () {
                this.el.innerHTML =
                    "<blockquote><ol><li>abc</li><li>www.example.com</li></ol></blockquote>";
                ...
                expect(links.length).toBe(
                    1,
                    "There should have been exactly 1 link created"
                );
                expect(links[0].getAttribute("href")).toBe(
                    "http://www.example.com"
                );
            ...
            it("should not create a link which spans multiple list items", function () {
                this.el.innerHTML =
                    "<blockquote><ol><li>www</li><li>.example.com</li></ol></blockquote>";
                ...
                expect(links.length).toBe(
                    0,
                    "There should not have been any links created"
                );
            });
        });
    });
