...
describe("Extensions TestCase", function () {
    ...
    describe("Editor", function () {
        ...
        it("should not override the base or name properties of an extension if overriden", function () {
            ...
            expect(editor.getExtensionByName("tempExtension").base).toBe(
                "something"
            );
        });
            ...
            var TempExtension = MediumEditor.Extension.extend({}),
                ...
                fakeDocument = {
                    ...
                    querySelectorAll: function () {
                        return document.querySelectorAll.apply(
                            document,
                            arguments
                        );
                    },
                    createElement: function () {
                        return document.createElement.apply(
                            document,
                            arguments
                        );
                    },
                ...
                fakeWindow = {
                    ...
                    removeEventListener: function () {
                        return window.removeEventListener.apply(
                            window,
                            arguments
                        );
                    },
    ...
    describe("Core Extension", function () {
        ...
        it("should not add default extensions when overriden", function () {
            ...
            expect(editor.getExtensionByName("anchor-preview")).toBe(
                extPreview
            );
            expect(editor.getExtensionByName("placeholder")).toBe(
                extPlaceholder
            );
            expect(editor.getExtensionByName("auto-link")).toBe(extAutoLink);
            expect(editor.getExtensionByName("fileDragging")).toBe(
                extFileDragging
            );
        });
    ...
    describe("All extensions", function () {
        ...
        it("should be able to access editor options via getEditorOption()", function () {
            ...
            expect(tempExtension.getEditorOption("spellcheck")).toBe(
                editor.options.spellcheck
            );
        });
        ...
        it("should be able to prevent blur on the editor when user iteracts with extension elements", function () {
            ...
            expect(editor.getExtensionByName("toolbar").isDisplayed()).toBe(
                true
            );
    ...
    describe("Button integration", function () {
        ...
        it("should include extensions button into toolbar", function () {
            ...
            expect(
                toolbar
                    .getToolbarElement()
                    .querySelectorAll(".extension-button").length
            ).toBe(1);
        });
        ...
        it("should not include buttons into the toolbar when an overriding extension is present", function () {
            ...
            expect(
                toolbar
                    .getToolbarElement()
                    .querySelectorAll('button[data-action="italic"]').length
            ).toBe(1);
            expect(
                toolbar
                    .getToolbarElement()
                    .querySelectorAll('button[data-action="bold"]').length
            ).toBe(0);
