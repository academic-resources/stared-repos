  ...
  db.get()
    ...
    .catch((err) => {
      res
        .status(500)
        .json({
          message: "Could not fetch those actions. They've disappeared!",
        });
    });
});
  ...
  db.get(id)
    .then((action) => {
      if (Object.keys(action).length === 0) {
        res
          .status(404)
          .json({ message: "That's not a valid action ID, didn't you know?" });
      } else {
        ...
        contextDb
          ...
          .catch((err) =>
            res
              .status(500)
              .json({ message: "Ooops could not fetch that info." })
          );
      }
    })
    .catch((err) => {
      res
        .status(500)
        .json({
          message: "That action is MIA and could not be fetched. Oops.",
        });
    });
});
...
router.post("/", (req, res) => {
  ...
  if (action.action_description && action.action_notes && action.project_id) {
          ...
          db.add(action)
            ...
            .catch((err) => {
              res
                ...
                .json({
                  message:
                    "There was an error adding this action to the project",
                });
            });
        }
      })
    ...
    res
      ...
      .json({
        message:
          "We need to know which project this goes to. What's the project ID?",
      });
    ...
    res
      ...
      .json({
        message:
          "Add a note. What's this action about? How will you get it done?",
      });
  } else if (action.action_notes && action.project_id) {
    res
      .status(400)
      .json({ message: "New actions require a description! Get to it." });
  } else {
    res
      .status(400)
      .json({
  ...
  db.get(id)
        ...
        db.remove(id)
          ...
          .catch((err) => {
            res
              .status(500)
              .json({
                message: "This action is impenetrable and cannot be deleted.",
              });
          });
      }
    })
    .catch((err) => {
      res
        ...
        .json({
          message:
            "This action could not be located. It sensed your sinister intentions and ran.",
        });
    });
});
