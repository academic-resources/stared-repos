...
var SECURITY_READ_WRITE_ERROR = new Error(
...
var updateFileProperties = function (file, path) {
  ...
  file._usablePath = file._isAbsolute
    ? file._path
    : windowsRoot + PATH.join(file._relative, path);
};
        ...
        FS.readdir(me._usablePath, function (error, files) {
          if (error) {
            ...
            files.forEach(function (file) {
              new File(PATH.join(me._path, file)).copy(
                PATH.join(path, file),
...
File.createTempFile = function (settings, cb) {
  ...
  PATH.exists(f._usablePath, function (exists) {
    if (exists) {
...
File.prototype.exists = function (cb) {
  ...
  PATH.exists(this._usablePath, function (exists) {
    cb(exists);
  });
};
...
File.prototype.list = function (filter, cb) {
  ...
  FS.stat(this._usablePath, function (error, stats) {
    if (error) {
      ...
      search(me._usablePath, me._path, {}, filter, function (error, files) {
        cb(error, files);
      });
    }
  });
  ...
  FS.stat(this._usablePath, function (error, stats) {
    ...
    if (stats.isFile()) {
      ...
      FS.readdir(me._usablePath, function (error, files) {
        ...
        if (finish()) return;
        for (var i in files) {
          new File(PATH.join(me._path, files[i])).remove(function (
            error,
            removed
          ) {
            if (error) {
...
var removeSynchronous = function (file) {
  ...
  if (!(checkSecurity(file._usablePath) & SecurityManager.WRITE.id)) {
    return SECURITY_WRITE_ERROR;
  }
  if (!PATH.existsSync(file._usablePath)) return false;
...
var checkSecurity = function (path) {
    ...
    for (var i in array) {
      ...
      if (path.match(re)) {
        if (!last.dir || !last.dir.match(re)) {
          last.dir = dir;
          last.perm = mode
            ? negatePermissions(p.permissions.id)
            : p.permissions.id;
        }
      }
    }
