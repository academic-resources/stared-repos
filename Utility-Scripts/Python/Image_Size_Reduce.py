...
)  # taking path of the image from the user
img = Image.open(str(img_path))  # making an Image object in python
widthper = width_pixels / float(img.size[0])
height_pixels = int(
    (float(img.size[1]) * float(widthper))
)  # height of final image in pixels
img = img.resize(
    (width_pixels, height_pixels), PIL.Image.ANTIALIAS
...
final_img = (
    q + "compressed_image." + img_extension
)  # creating the address of final compressed image
