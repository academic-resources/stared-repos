...
class TestFetcher(unittest.TestCase):
    ...
    def test_a160_cookie(self):
        ...
        self.assertEqual(response.status_code, 200, result)
        self.assertEqual(
            response.cookies, {"a": "b", "k1": "v1", "k2": "v2", "c": "d"}, result
        )
...
@unittest.skipIf(
    os.environ.get("IGNORE_SPLASH") or os.environ.get("IGNORE_ALL"),
    "no splash server for test.",
)
class TestSplashFetcher(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        self.httpbin_thread = utils.run_in_subprocess(
            httpbin.app.run, host="0.0.0.0", port=14887, passthrough_errors=False
        )
    ...
    def test_95_splash_js_script_2(self):
        ...
        request["fetch"][
            "js_script"
        ] = 'function() { document.querySelector("a").click(); return "abc" }'
        result = self.fetcher.sync_fetch(request)
        self.assertEqual(result["status_code"], 200)
        self.assertNotIn("loading", result["content"])
        self.assertIn("done", result["content"])
