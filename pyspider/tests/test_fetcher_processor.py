...
class TestFetcherProcessor(Handler, unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        self.httpbin_thread = utils.run_in_subprocess(
            httpbin.app.run, port=14887, passthrough_errors=False
        )
    ...
    def assertStatusOk(self, status):
        self.assertTrue(
            self.status_ok(status, "fetch"), status.get("track", {}).get("fetch")
        )
        self.assertTrue(
            self.status_ok(status, "process"), status.get("track", {}).get("process")
        )
    ...
    def test_30_catch_status_code_error(self):
        status, newtasks, result = self.crawl(
            self.httpbin + "/status/418", callback=self.json
        )
        ...
        status, newtasks, result = self.crawl(
            self.httpbin + "/status/400", callback=self.catch_http_error
        )
        ...
        status, newtasks, result = self.crawl(
            self.httpbin + "/status/500", callback=self.catch_http_error
        )
        self.assertFalse(self.status_ok(status, "fetch"))
        self.assertTrue(self.status_ok(status, "process"))
        self.assertEqual(len(newtasks), 1, newtasks)
    ...
    def test_40_method(self):
        status, newtasks, result = self.crawl(
            self.httpbin + "/delete", method="DELETE", callback=self.json
        )
        ...
        status, newtasks, result = self.crawl(
            self.httpbin + "/get", method="DELETE", callback=self.catch_http_error
        )
    ...
    def test_70_redirect(self):
        status, newtasks, result = self.crawl(
            self.httpbin + "/redirect-to?url=/get", callback=self.json
        )

        self.assertStatusOk(status)
        self.assertEqual(
            status["track"]["fetch"]["redirect_url"], self.httpbin + "/get"
        )
        self.assertFalse(newtasks)

    def test_80_redirect_too_many(self):
        status, newtasks, result = self.crawl(
            self.httpbin + "/redirect/10", callback=self.json
        )
    ...
    def test_a115_user_agent(self):
        status, newtasks, result = self.crawl(
            self.httpbin + "/get", user_agent="binux", callback=self.json
        )
    ...
    def test_a140_response_cookie(self):
        status, newtasks, result = self.crawl(
            self.httpbin + "/cookies/set?k1=v1&k2=v2", callback=self.cookies
        )
        self.assertStatusOk(status)
        self.assertFalse(newtasks)
    ...
    def test_a145_redirect_cookie(self):
        status, newtasks, result = self.crawl(
            self.httpbin + "/cookies/set?k1=v1&k2=v2", callback=self.json
        )
        self.assertStatusOk(status)
        self.assertFalse(newtasks)
    ...
    def test_a150_timeout(self):
        status, newtasks, result = self.crawl(
            self.httpbin + "/delay/2", timeout=1, callback=self.json
        )
    ...
    def test_a160_etag(self):
        status, newtasks, result = self.crawl(
            self.httpbin + "/cache", etag="abc", callback=self.json
        )
    ...
    def test_a170_last_modified(self):
        status, newtasks, result = self.crawl(
            self.httpbin + "/cache", last_modified="0", callback=self.json
        )
    ...
    def test_a180_save(self):
        status, newtasks, result = self.crawl(
            callback=self.get_save, save={"roy": "binux", u"中文": "value"}
        )
    ...
    def test_a190_taskid(self):
        status, newtasks, result = self.crawl(
            callback=self.get_save, taskid="binux-taskid"
        )
    ...
    def test_a220_proxy_ok(self):
        ...
        status, newtasks, result = self.crawl(
            self.httpbin + "/get",
            params={"test": "a220", "username": "binux", "password": "123456"},
            callback=self.catch_http_error,
        )
    ...
    def test_a240_proxy_parameter_ok(self):
        status, newtasks, result = self.crawl(
            self.httpbin + "/post",
            method="POST",
            data={"test": "a240", "username": "binux", "password": "123456"},
            proxy=self.proxy,
            callback=self.catch_http_error,
        )
    ...
    def test_a260_process_save(self):
        ...
        status, newtasks, result = self.crawl(
            callback=self.get_process_save, track=status["track"]
        )
    ...
    def test_zzz_links(self):
        status, newtasks, result = self.crawl(
            self.httpbin + "/links/10/0", callback=self.links
        )
    ...
    def test_zzz_html(self):
        status, newtasks, result = self.crawl(
            self.httpbin + "/html", callback=self.html
        )
    ...
    def test_zzz_etag_enabled(self):
        status, newtasks, result = self.crawl(
            self.httpbin + "/cache", callback=self.json
        )
        self.assertStatusOk(status)
        self.assertTrue(result)

        status, newtasks, result = self.crawl(
            self.httpbin + "/cache", track=status["track"], callback=self.json
        )
        self.assertStatusOk(status)
        self.assertFalse(newtasks)
        self.assertFalse(result)
    ...
    def test_zzz_robots_txt(self):
        status, newtasks, result = self.crawl(
            self.httpbin + "/deny", robots_txt=True, callback=self.catch_http_error
        )
    ...
    def test_zzz_connect_timeout(self):
        ...
        status, newtasks, result = self.crawl(
            "http://240.0.0.1/", connect_timeout=5, callback=self.catch_http_error
        )
        end_time = time.time()
