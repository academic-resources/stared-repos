...
class TestRun(unittest.TestCase):
    ...
    @unittest.skipIf(
        os.environ.get("IGNORE_COUCHDB") or os.environ.get("IGNORE_ALL"),
        "no couchdb server for test.",
    )
    ...
    @unittest.skipIf(
        os.environ.get("IGNORE_MYSQL") or os.environ.get("IGNORE_ALL"),
        "no mysql server for test.",
    )
    ...
    def test_a100_all(self):
        ...
        try:
            ...
            while limit >= 0:
                ...
                try:
                    requests.post(
                        ...
                        data={"project": "data_sample_handler"},
                    )
            ...
            rv = requests.get(
                "http://localhost:5000/results?project=data_sample_handler"
            )
...
class TestSendMessage(unittest.TestCase):
    ...
    def test_10_send_message(self):
        ctx = run.send_message.make_context(
            "send_message", ["test_project", "test_message"], self.ctx
        )
