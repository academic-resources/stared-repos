...
class TestFetcher(unittest.TestCase):
    ...
    def test_format_data(self):
        ...
        self.assertRegex(
            utils.format_date(now - 24 * 60 * 60), r"^yesterday at \d{1,2}:\d{2}$"
        )
        self.assertRegex(
            utils.format_date(now - 2 * 24 * 60 * 60), r"^[A-Z][a-z]+ at \d{1,2}:\d{2}$"
        )
        self.assertRegex(
            utils.format_date(now - 3 * 24 * 60 * 60), r"^[A-Z][a-z]+ at \d{1,2}:\d{2}$"
        )
        self.assertRegex(
            utils.format_date(now - 4 * 24 * 60 * 60), r"^[A-Z][a-z]+ at \d{1,2}:\d{2}$"
        )
        self.assertRegex(
            utils.format_date(now - 5 * 24 * 60 * 60),
            r"^\d{1,2}-\d{1,2} at \d{1,2}:\d{2}$",
        )
        self.assertRegex(
            utils.format_date(now - 333 * 24 * 60 * 60),
            r"^\d{1,2}-\d{1,2} at \d{1,2}:\d{2}$",
        )
        self.assertRegex(
            utils.format_date(now - 334 * 24 * 60 * 60),
