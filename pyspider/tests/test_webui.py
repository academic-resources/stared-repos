...
class TestWebUI(unittest.TestCase):
    ...
    def test_30_run(self):
        rv = self.app.post(
            "/debug/test_project/run",
            data={"script": self.script_content, "task": self.task_content},
        )
        self.assertEqual(rv.status_code, 200)
    ...
    def test_40_save(self):
        rv = self.app.post(
            "/debug/test_project/save", data={"script": self.script_content}
        )
        self.assertEqual(rv.status_code, 200)
    ...
    def test_45_run_with_saved_script(self):
        rv = self.app.post(
            "/debug/test_project/run",
            data={"webdav_mode": "true", "script": "", "task": self.task_content},
        )
        self.assertEqual(rv.status_code, 200)
        data = json.loads(utils.text(rv.data))
        self.assertIn(b"follows", rv.data)
    ...
    def test_52_change_status(self):
        rv = self.app.post(
            "/update", data={"name": "status", "value": "RUNNING", "pk": "test_project"}
        )
        self.assertEqual(rv.status_code, 200)
        self.assertIn(b"ok", rv.data)
    ...
    def test_60_change_rate(self):
        rv = self.app.post(
            "/update", data={"name": "rate", "value": "1/4", "pk": "test_project"}
        )
        self.assertEqual(rv.status_code, 200)
        self.assertIn(b"ok", rv.data)
    ...
    def test_80_change_group(self):
        rv = self.app.post(
            "/update",
            data={"name": "group", "value": "test_binux", "pk": "test_project"},
        )
        self.assertEqual(rv.status_code, 200)
        self.assertIn(b"ok", rv.data)
    ...
    def test_a10_counter(self):
        for i in range(30):
            time.sleep(1)
            if (
                self.rpc.counter("5m", "sum").get("test_project", {}).get("success", 0)
                > 5
            ):
                break
    ...
    def test_a60_fetch_via_cannot_connect_fetcher(self):
        ctx = run.webui.make_context(
            "webui", ["--fetcher-rpc", "http://localhost:20000/"], self.ctx
        )
        app = run.webui.invoke(ctx)
        app = app.test_client()
        rv = app.post(
    ...
    def test_a70_fetch_via_fetcher(self):
        ctx = run.webui.make_context(
            "webui", ["--fetcher-rpc", "http://localhost:24444/"], self.ctx
        )
        app = run.webui.invoke(ctx)
        app = app.test_client()
        rv = app.post(
    ...
    def test_h005_no_such_project(self):
        rv = self.app.post(
            "/update",
            data={"name": "group", "value": "lock", "pk": "not_exist_project"},
        )
    ...
    def test_h005_unknown_field(self):
        rv = self.app.post(
            "/update",
            data={"name": "unknown_field", "value": "lock", "pk": "test_project"},
        )
        self.assertEqual(rv.status_code, 400)

    def test_h005_rate_wrong_format(self):
        rv = self.app.post(
            "/update", data={"name": "rate", "value": "xxx", "pk": "test_project"}
        )
        self.assertEqual(rv.status_code, 400)

    def test_h010_change_group(self):
        rv = self.app.post(
            "/update", data={"name": "group", "value": "lock", "pk": "test_project"}
        )
        self.assertEqual(rv.status_code, 200)
        self.assertIn(b"ok", rv.data)
    ...
    def test_h020_change_group_lock_failed(self):
        rv = self.app.post(
            "/update", data={"name": "group", "value": "", "pk": "test_project"}
        )
    ...
    def test_h020_change_group_lock_ok(self):
        rv = self.app.post(
            "/update",
            data={"name": "group", "value": "test_binux", "pk": "test_project"},
            headers={"Authorization": "Basic YmludXg6NDMyMQ=="},
        )
        self.assertEqual(rv.status_code, 200)
    ...
    def test_x0_disconnected_scheduler(self):
        ctx = run.webui.make_context(
            "webui", ["--scheduler-rpc", "http://localhost:23458/"], self.ctx
        )
        app = run.webui.invoke(ctx)
        self.__class__.app = app.test_client()
        self.__class__.rpc = app.config["scheduler_rpc"]
