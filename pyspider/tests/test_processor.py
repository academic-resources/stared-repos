...
class TestProjectModule(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.script = open(
            os.path.join(os.path.dirname(__file__), "data_handler.py")
        ).read()
        self.env = {"test": True}
        self.project_info = {"name": self.project, "status": "DEBUG"}
        data = ProjectManager.build_module(
            {"name": self.project, "script": self.script}, {"test": True}
        )
    ...
    def test_20_get_info(self):
        task = {
            ...
            "fetch": {"save": ["min_tick", "retry_delay"]},
    ...
    def test_60_timeout_in_thread(self):
        ...
        thread = utils.run_in_thread(
            lambda self=self: self.instance.run_task(
                self.module, base_task, fetch_result
            )
        )
        thread.join()
...
class TestProcessor(unittest.TestCase):
    ...
    def test_70_update_project(self):
        ...
        self.projectdb.update(
            "test_broken_project", {"script": inspect.getsource(sample_handler)}
        )
        ...
        self.projectdb.update(
            "test_broken_project", {"script": inspect.getsource(sample_handler)[:10]}
        )
