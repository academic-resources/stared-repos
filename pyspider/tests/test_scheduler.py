...
class TestScheduler(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        self.rpc = xmlrpc_client.ServerProxy(
            "http://localhost:%d" % self.scheduler_xmlrpc_port
        )
        ...
        def run_scheduler():
            ...
            self.xmlrpc_thread = run_in_thread(
                scheduler.xmlrpc_run, port=self.scheduler_xmlrpc_port
            )
            scheduler.run()
    ...
    def test_32_get_info(self):
        self.status_queue.put(
            {"taskid": "_on_get_info", "project": "test_project", "track": {"save": {}}}
        )
    ...
    def test_a30_task_verify(self):
        ...
        self.assertFalse(
            self.rpc.newtask(
                {"taskid": "taskid#", "project": "not_exist_project", "url": "url"}
            )
    ...
    def test_x20_delete_project(self):
        ...
        self.projectdb.update(
            "test_inqueue_project", status="STOP", group="lock,delete"
        )
        time.sleep(1)
...
class TestProject(unittest.TestCase):
    ...
    def test_pause_20_no_enough_fail_tasks(self):
        ...
        for i in range(1):
            self.project.active_tasks.appendleft(
                (time.time(), dict(self.status_ok_pack))
            )
        for i in range(self.scheduler.FAIL_PAUSE_NUM - 5):
            self.project.active_tasks.appendleft(
                (time.time(), dict(self.status_fail_pack))
            )
        self.assertFalse(self.project.paused)
