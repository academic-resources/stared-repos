...
class ProjectManager(object):
    ...
    def build_module(project, env=None):
        ...
        if "__handler_cls__" not in module.__dict__:
            ...
            for each in list(six.itervalues(module.__dict__)):
                if (
                    inspect.isclass(each)
                    and each is not BaseHandler
    ...
    def _need_update(self, project_name, updatetime=None, md5sum=None):
        ...
        if project_name not in self.projects:
            ...
            return True
        elif updatetime and updatetime > self.projects[project_name]["info"].get(
            "updatetime", 0
        ):
            return True
        elif (
            time.time() - self.projects[project_name]["load_time"]
            > self.RELOAD_PROJECT_INTERVAL
        ):
            return True
        return False
    ...
    def _check_projects(self):
        ...
        for project in self.projectdb.check_update(
            self.last_check_projects, ["name", "updatetime"]
        ):
            if project["name"] not in self.projects:
                continue
            if project["updatetime"] > self.projects[project["name"]]["info"].get(
                "updatetime", 0
            ):
