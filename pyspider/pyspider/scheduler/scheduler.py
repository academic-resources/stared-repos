...
class Project(object):
    ...
    def __init__(self, scheduler, project_info):
        ...
        self._send_finished_event_wait = (
            0
        )  # wait for scheduler.FAIL_PAUSE_NUM loop steps before sending the event
    ...
    def paused(self):
        ...
        elif self._paused is True and (
            self._paused_time + self.scheduler.PAUSE_TIME < time.time()
        ):
            self._paused = "checking"
            self._unpause_last_seen = (
                self.active_tasks[0][1] if len(self.active_tasks) else None
            )
...
class Scheduler(object):
    ...
    def __init__(
        ...
        self._cnt = {
            ...
            "all": counter.CounterManager(lambda: counter.TotalCounter()),
        }
            ...
            self.on_select_task(
                {
                    ...
                    "fetch": {"save": self.get_info_attributes},
    ...
    def _update_project_cnt(self, project_name):
        ...
        self._cnt["all"].value(
            (project_name, "success"), status_count.get(self.taskdb.SUCCESS, 0)
        )
        self._cnt["all"].value(
            (project_name, "failed"),
            status_count.get(self.taskdb.FAILED, 0)
            + status_count.get(self.taskdb.BAD, 0),
        )
        self._cnt["all"].value(
            (project_name, "pending"), status_count.get(self.taskdb.ACTIVE, 0)
        )
    ...
    def task_verify(self, task):
        ...
        if not project.active:
            logger.error(
                "project %s not started, please set status to RUNNING or DEBUG",
                task["project"],
            )
            return False
        return True
            ...
            self.on_select_task(
                {
                    ...
                    "status": self.taskdb.SUCCESS,
                    "fetch": {"save": {"tick": self._last_tick}},
    ...
    def _check_select(self):
        ...
        min_project_limit = int(
            limit / 10.0
        )  # ensure minimum select limit for each project
        max_project_limit = int(
            limit / 3.0
        )  # ensure maximum select limit for each project
                    ...
                    self._postpone_request.append(
                        {
                            ...
                            "process": {"callback": "on_finished"},
                            "schedule": {"age": 0, "priority": 9, "force_update": True},
                        }
                    )
    ...
    def _load_put_task(self, project, taskid):
        try:
            task = self.taskdb.get_task(
                project, taskid, fields=self.request_task_fields
            )
        except ValueError:
    ...
    def _print_counter_log(self):
        ...
        top_3_actives = sorted(
            [x for x in project_actives if x[1] not in top_2_fails], reverse=True
        )[: 5 - len(top_2_fails)]

        log_str = (
            "in 5m: new:%(pending)d,success:%(success)d,"
    ...
    def xmlrpc_run(self, port=23333, bind="127.0.0.1", logRequests=False):
        ...
        application.register_function(
            get_projects_pause_status, "get_projects_pause_status"
        )
    ...
    def on_request(self, task):
        if (
            self.INQUEUE_LIMIT
            and len(self.projects[task["project"]].task_queue) >= self.INQUEUE_LIMIT
        ):
            logger.debug("overflow task %(project)s:%(taskid)s %(url)s", task)
            return

        oldtask = self.taskdb.get_task(
            task["project"], task["taskid"], fields=self.merge_task_fields
        )
        if oldtask:
    ...
    def on_old_request(self, task, old_task):
        ...
        if _schedule.get("force_update") and self.projects[
            task["project"]
        ].task_queue.is_processing(task["taskid"]):
        ...
        if _schedule.get("itag") and _schedule["itag"] != old_schedule.get("itag"):
            ...
            schedule_age >= 0
            and schedule_age + (old_task.get("lastcrawltime", 0) or 0) < now
        ):
            restart = True
        ...
        if old_task["status"] == self.taskdb.SUCCESS:
            self._cnt["all"].event((project, "success"), -1).event(
                (project, "pending"), +1
            )
        elif old_task["status"] == self.taskdb.FAILED:
            self._cnt["all"].event((project, "failed"), -1).event(
                (project, "pending"), +1
            )
    ...
    def on_task_status(self, task):
        ...
        try:
            ...
            if not self.projects[task["project"]].task_queue.done(task["taskid"]):
                logging.error(
                    "not processing pack: %(project)s:%(taskid)s %(url)s", task
                )
                return None
        ...
        if task["track"]["fetch"].get("time"):
            self._cnt["5m_time"].event(
                (task["project"], "fetch_time"), task["track"]["fetch"]["time"]
            )
        if task["track"]["process"].get("time"):
            self._cnt["5m_time"].event(
                (task["project"], "process_time"), task["track"]["process"].get("time")
            )
    ...
    def on_task_failed(self, task):
        ...
        if "schedule" not in task:
            old_task = self.taskdb.get_task(
                task["project"], task["taskid"], fields=["schedule"]
            )
            if old_task is None:
        ...
        next_exetime = retry_delay.get(
            retried, retry_delay.get("", self.DEFAULT_RETRY_DELAY[""])
        )
        ...
        if task["schedule"].get("auto_recrawl") and "age" in task["schedule"]:
            ...
            if retried >= retries:
                next_exetime = -1
            elif "age" in task["schedule"] and next_exetime > task["schedule"].get(
                "age"
            ):
                next_exetime = task["schedule"].get("age")
...
class OneScheduler(Scheduler):
        ...
        def crawl(url, project=None, **kwargs):
            ...
            if project is None:
                if len(self.projects) == 1:
                    ...
                    raise LookupError(
                        "You need specify the project: %r" % list(self.projects.keys())
                    )
            ...
            if not kwargs:
                dbtask = self.taskdb.get_task(
                    task["project"], task["taskid"], fields=self.request_task_fields
                )
                if not dbtask:
                    dbtask = self.taskdb.get_task(
                        task["project"], task["url"], fields=self.request_task_fields
                    )
                if dbtask:
                    task = dbtask
    ...
    def init_one(
        self, ioloop, fetcher, processor, result_worker=None, interactive=False
    ):
        self.ioloop = ioloop
    ...
    def run(self):
        ...
        tornado.ioloop.PeriodicCallback(self.run_once, 100, io_loop=self.ioloop).start()
        self.ioloop.start()
            ...
            while True:
                for queue in self.thread_queues:
                    ...
                    if block:
                        ...
                        queue = self.thread_queues[
                            random.randint(0, len(self.thread_queues) - 1)
                        ]
                break
        else:
