...
def debug(project):
    ...
    if info:
        ...
        script = (
            ...
            .replace(
                "__START_URL__", request.values.get("start-urls") or "__START_URL__"
            )
        )
...
def run(project):
    ...
    try:
        ...
        return (
            json.dumps(utils.unicode_obj(result)),
            200,
    ...
    try:
        module = ProjectManager.build_module(
            project_info,
            {"debugger": True, "process_time_limit": app.config["process_time_limit"]},
        )
...
def save(project):
    ...
    if (
        project_info
        and "lock" in projectdb.split_group(project_info.get("group"))
...
def get_script(project):
    ...
    info = projectdb.get(project, fields=["name", "script"])
    return (
        json.dumps(utils.unicode_obj(info)),
        200,
        {"Content-Type": "application/json"},
    )
