...
class TaskDB(SplitTableMixin, BaseTaskDB):
    ...
    def _create_project(self, project):
        ...
        # create index
        payload = {"index": {"fields": ["status", "taskid"]}, "name": collection_name}
    ...
    def load_tasks(self, status, project=None, fields=None):
        ...
        for project in projects:
            collection_name = self._get_collection_name(project)
            for task in self.get_docs(
                collection_name, {"selector": {"status": status}, "fields": fields}
            ):
                yield task
    ...
    def get_task(self, project, taskid, fields=None):
        ...
        collection_name = self._get_collection_name(project)
        ret = self.get_docs(
            collection_name, {"selector": {"taskid": taskid}, "fields": fields}
        )
        if len(ret) == 0:
            return None
        return ret[0]
    ...
    def status_count(self, project):
        ...
        def _count_for_status(collection_name, status):
            total = len(
                self.get_docs(collection_name, {"selector": {"status": status}})
            )
