...
class TaskDB(BaseTaskDB):
    ...
    def projects(self):
        ret = self.es.search(
            index=self.index,
            doc_type=self.__type__,
            body={"aggs": {"projects": {"terms": {"field": "project"}}}},
            _source=False,
        )
        return [
            each["key"] for each in ret["aggregations"]["projects"].get("buckets", [])
        ]
