...
class ProjectDB(BaseProjectDB):
    ...
    def __init__(self, hosts, index="pyspider"):
        ...
        if not self.es.indices.get_mapping(index=self.index, doc_type=self.__type__):
            self.es.indices.put_mapping(
                ...
                body={
                    "_all": {"enabled": False},
                    "properties": {"updatetime": {"type": "double"}},
                },
            )
    ...
    def insert(self, name, obj={}):
        ...
        return self.es.index(
            index=self.index, doc_type=self.__type__, body=obj, id=name, refresh=True
        )
    ...
    def get_all(self, fields=None):
        for record in elasticsearch.helpers.scan(
            self.es,
            index=self.index,
            doc_type=self.__type__,
    ...
    def check_update(self, timestamp, fields=None):
        for record in elasticsearch.helpers.scan(
            ...
            index=self.index,
            doc_type=self.__type__,
            query={"query": {"range": {"updatetime": {"gte": timestamp}}}},
            _source_include=fields or [],
        ):
            yield record["_source"]
    ...
    def drop(self, name):
        return self.es.delete(
            index=self.index, doc_type=self.__type__, id=name, refresh=True
        )
