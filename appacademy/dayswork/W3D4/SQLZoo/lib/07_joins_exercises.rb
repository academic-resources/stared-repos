...
def ford_films
  ...
  execute(<<-SQL)
    SELECT title
    FROM movies
    LEFT JOIN castings ON movies.id = castings.movie_id
    LEFT JOIN actors ON actors.id = castings.actor_id
...
def films_by_cast_size
  ...
  execute(<<-SQL)
    SELECT title, COUNT(actors.name)
    FROM movies
    LEFT JOIN castings ON movies.id = castings.movie_id
    LEFT JOIN actors ON actors.id = castings.actor_id
    WHERE yr = 1978 
    GROUP BY title
    ORDER BY COUNT(actors.name) DESC, title 
  SQL
end
