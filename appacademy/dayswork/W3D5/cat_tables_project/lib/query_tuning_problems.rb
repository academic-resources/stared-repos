...
def cats_like_johnson
  ...
  execute(<<-SQL)
    SELECT all_cats.name
    FROM cats all_cats
    INNER JOIN cats johnnylav ON johnnylav.breed = all_cats.breed
    WHERE johnnylav.name = 'Johnson'
    AND johnnylav.color = 'Lavender'
    ORDER BY all_cats.name;
  SQL
end
...
def cats_with_a_lot
  ...
  execute(<<-SQL)
    ...
    LEFT JOIN (
      SELECT cat_id, COUNT(*) as toy_count
      FROM cattoys
      GROUP BY cat_id
    ) as A ON cats.id = A.cat_id
    WHERE A.toy_count > 7
    ORDER BY cats.name;
  SQL
end
