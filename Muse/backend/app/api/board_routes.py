    ...
    for i in range(16):
        if i < len(board_pads):
            ...
            sequenceData["columnOrder"] = [
                f"block-{(current_pad_block_seq[index] if current_pad_block_seq[index] != None else index) + 1}"
                for index in range(len(current_pad["block_seq"]))
            ]
            ...
            for column_index in range(len(sequenceData["columnOrder"])):
                ...
                sequenceData["columns"][current_block_id] = {
                    ...
                    "taskIds": temp_block[current_pad["block_seq"][column_index]]
                    if current_pad["block_seq"][column_index] != None
                    else [],
                }
...
def handle_board_save(userId):
    ...
    board = Board(title=project_title, bpm=bpm, user_id=userId)
    ...
    for seq_id in sequence_data.keys():
        ...
        block_list = [
            int(block_id.split("-")[1]) - 1
            if len(current_pad_sequence["columns"][block_id]["taskIds"]) > 0
            else None
