...
class AppPage {
  ...
  async waitForStep(n: number) {
    ...
    await this.page.waitForFunction(
      `Number(document.querySelector('.step-view .step-number').innerHTML)>=${n}`,
      { timeout: 1000 * 60 * 5 }
    );
  }
  ...
  async defineDS(ds: DS) {
    ...
    await this.page.waitForFunction(
      `!!document.querySelector('#StructureSettings')`
    );
    ...
    await this.page.waitForFunction(
      `!!document.querySelector('.struct-settings-${ds.name}')`
    );

    await this.page.evaluate(function (name) {
      const elem = document.querySelector(
        `.struct-settings-${name} .toggle-edit`
      );
      //@ts-ignore
      elem.click();
      elem.scrollIntoView();
  ...
  describe("BST", () => {
    it("->", async () => {
      ...
      await page.defineDS({
        ...
        pointers: [{ name: "children", type: "multiple" }],
        displayKey: "value",
        numChildren: 2,
      });
      await page.normalFlow(false);
