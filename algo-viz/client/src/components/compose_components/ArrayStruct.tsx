...
const ArrayStruct: React.FC<Props> = observer(
  ({ structure, objectId, ratio, pointed, renderId }) => {
    ...
    const maxWidth =
      store.windowWidth * (store.structsWidth / 18) * store.widths.array;

    const len = structure.get("length").value;
    if (display === "column") {
      ratio *= 0.7;
    }
    const valSize = Math.max(Math.min(maxWidth / (len * 2), 30) * ratio, 0.001);
    ...
    const bkgExtend = active
      ? `${Math.min(len * valSize * 2 + 100, 2000)}px`
      : "20px";
    ...
    if (display === "row") {
      ...
      styles.maxHeight =
        store.windowHeight /
        (store.structsWidth < 10 ? store.numStructs[2] + 1 : 1);
