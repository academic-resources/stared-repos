...
const ArcPointer: React.FC<Props> = observer(
  ({ from, to, children, get, set, prop }) => {
    ...
    if (!willRender) {
      ...
      const lineStyle: React.CSSProperties = {
        stroke: get
          ? configColors["Arc Pointer: GET"]
          : set
          ? configColors["Arc Pointer: SET"]
          : configColors["Arc Pointer"],
      ...
      return (
        <div>
          <svg
            style={{ position: "absolute", top, left, zIndex: 0 }}
            height={height}
            width={width}
            ...
            {fromCoords.radius && toCoords.radius && (
              <>
                {d && (
                  <path
                    d={d}
                    strokeDashoffset={toCoords.radius}
                    stroke={configColors["Arc Pointer"]}
                    strokeWidth={1}
                    fill="transparent"
                  />
                )}
