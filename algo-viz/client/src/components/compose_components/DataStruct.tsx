...
const DataStruct: React.FC<Props> = observer(
  ({
    structure,
    objectId,
    ratio,
    ...
    const width =
      store.windowWidth *
      (store.structsWidth >= 10 ? store.structsWidth / 24 : 0.5) *
      ratio;
    ...
    if (isList) {
      styles.width = Math.max(Math.min(width, 30) * ratio, 0.001);
    ...
    for (const [key, prop] of structure.entries()) {
      ...
      if (
        typeof value === "string" &&
        value in store.structs.objects &&
        value !== objectId &&
        (key in settings.pointers || key in settings.order)
      ) {
        if (key in settings.pointers) {
          ...
          if (!pointer) {
            ...
            for (const key of object.keys()) {
              ...
              if (
                ...
                info.value in store.structs.objects
              ) {
                pointers.push(
                  <ArcPointer
                    prop={key}
          ...
          if (
            parent &&
            (objectId !== parent.id || key !== parent.key) &&
            store.structs.bindings.has(parent.id)
          ) {
            pointers.push(
              <ArcPointer
                prop={key}
            ...
            if (store.settings.viableParents.has(type)) {
              ...
              for (const key of object.keys()) {
                ...
                if (
                  ...
                  children.push({
                    order,
                    key: store.settings.arrayTypes.has(type)
                      ? Number(key)
                      : key,
                    child: info.value,
                    parent: object,
      ...
      >
              ...
              {otherKeys.map(([key, value]) => {
                return (
                  <div key={key} className="has-text-weight-bold">
                    ...
                    {value === objectId ? (
                      <span className="has-text-danger">this</span>
                    ) : (
                      <ValText
                        value={value}
                        type={getType(value)}
        ...
        >
          <div
            ...
            onMouseLeave={() =>
              (store.structs.activePointers[objectId] = false)
            }
            style={{
              display: "flex",
              justifyContent: "center",
              ...
              if (!child) {
                ...
                return (
                  <DataChild
                    ...
                    ratio={
                      ratio /
                      (settings.numChildren === null
                        ? children.length
                        : settings.numChildren)
                    }
                    prop={key}
