...
const ArrayVal: React.FC<Props> = observer(
  ({ array, index, objectId, size, ratio, setChildren }) => {
    ...
    if (typeof value === "string" && value in store.structs.objects) {
      if (!store.structs.bindings.has(value)) {
        ...
        if (
          store.settings.arrayTypes.has(type) &&
          parent &&
          parent.id === objectId &&
          parent.key === index
        ) {
