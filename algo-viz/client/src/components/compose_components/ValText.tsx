...
const ValText: React.FC<Props> = observer(
  ({ value, type, textOnly = false, size = 30 }) => {
    let color: string;
    ...
    if (Array.isArray(value)) {
      ...
      return (
        <span style={{ color: store.settings.valueColors.other }}>
          ({nodes})
        </span>
      );
    }
    ...
    return (
      <span
        style={{
          color,
          fontSize: `${value.length > 20 ? 100 / value.length : 100}%`,
        }}
      >
        {value}
      </span>
