...
const LinePointer: React.FC<Props> = observer(
  ({ from, to, children, get, set, prop }) => {
    ...
    if (!willRender) {
      ...
      const lineStyle: React.CSSProperties = {
        stroke: get
          ? configColors["Line Pointer: GET"]
          : set
          ? configColors["Line Pointer: SET"]
          : configColors["Line Pointer"],
          ...
          >
            {fromCoords.radius && toCoords.radius && (
              <>
                <line
                  {...lineCoords}
                  stroke={configColors["Line Pointer"]}
                  strokeWidth={1}
                ></line>
