...
const Pointer: React.FC<Props> = observer(
  ({ active, id, size, isInsideDisplay = false }) => {
    const [hovered, toggle] = useState(false);
    const config = store.settings.config;
    const isActive = config["Active Pointer on GET"]
      ? hovered || active
      : hovered;
    useEffect(() => {
    ...
    return (
      ...
      >
        <path
          d="M39.54545454545544,299l295.4545454545455,291.8181818181881l196.9999999999991,-478.8181818181881l-489.72727272727184,185.18181818181722Z"
          style={{
