...
const HashStruct: React.FC<Props> = observer(
  ({ structure, objectId, ratio, pointed, renderId }) => {
    ...
    renderId = useMemo(
      () => renderId || genId(objectId.length),
      [objectId, renderId]
    );
    ...
    const willRender: boolean = !(
      pos &&
      pos.renderId &&
      pos.renderId !== renderId
    );
    const len = structure.size;

    const dimensionsMultiplier =
    ...
    return (
      <div
        style={{ ...styles, maxHeight: maxHeight + 100 }}
        className={`hash-struct has-text-centered`}
      >
        <div
        ...
        <div
          style={{
            ...styles,
            minWidth:
              store.settings.hashTypes.has(type) &&
        ...
        <div className="is-size-1 has-text-centered" style={braceStyle}>
          {"}"}
        </div>
      </div>
    );
