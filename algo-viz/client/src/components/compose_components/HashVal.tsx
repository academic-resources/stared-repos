...
const HashVal: React.FC<ValProps> = observer(
  ({ object, prop, objectId, size, ratio }) => {
    const [hovered, toggle] = useState(false);
    const willRender = object.has(prop);
    const info = willRender
      ? object.get(prop)
      : { get: false, set: false, value: null };
    ...
    if (
      ...
      store.settings.arrayTypes.has(store.viz.types[value])
    ) {
        ...
        return (
          ...
          >
            {!store.settings.setTypes.has(type) && (
              <div
                className={`column ${
                  store.settings.mapTypes.has(type) && "has-text-right"
                }`}
              >
                <p
                  className={`is-size-6 ${
                    (displayProps.anim[0] || displayProps.anim[1]) &&
                    "has-text-white"
                  }`}
                >
                  <Tooltip
                    ...
                    trigger={["hover"]}
                    visible={
                      store.settings.config.tooltips
                        ? visible || hovered
                        : hovered
                    }
                    overlay={() => (
    ...
    return (
      ...
      >
        <Tooltip
          ...
          trigger={["hover"]}
          visible={
            store.settings.config.tooltips ? visible || hovered : hovered
          }
          defaultVisible={false}
        >
          <div
            style={{ overflowX: "visible" }}
            className="columns is-paddingless is-multiline"
          >
            {!store.settings.setTypes.has(type) && (
              <div
                style={{ overflowX: "visible" }}
                ...
                <p
                  style={{
                    color:
                      displayProps.anim[0] || displayProps.anim[1]
                        ? "white"
                        : store.settings.structSettings[type].color,
                    fontWeight:
                      displayProps.anim[0] || displayProps.anim[1]
                        ? "bold"
                        : "normal",
