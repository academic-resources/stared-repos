...
class DataStructContainer extends Component<Props, State> {
  ...
  setDepth = (d: number, val: number) => {
    ...
    const multiple =
      (this.depths.length > 10 ? 0.5 : 0.7) **
      Math.floor((this.depths.length - 1) / (5 * this.props.heightMultiple));
  ...
  render() {
    ...
    return (
      <div
        style={{
          overflow: "visible",
          display: numChildren === 1 ? "block" : "inline-flex",
        }}
      >
        <DataStruct
          depthMultiplier={
            numChildren === 1
              ? Math.min(depthMultiplier * 1.4, 1)
              : depthMultiplier
          }
