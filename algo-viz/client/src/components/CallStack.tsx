    ...
    >
      <li
        style={{
          ...
          fontSize: `${Math.min(
            height / 2,
            100 / (name ? (name as string).length : 1)
          )}px`,
  ...
  if (len > 100) {
    return (
      <div className="call-stack">
        {renderItem([0, `${len} stack frames`], 40, {
          backgroundColor: "red",
    ...
    for (let i = len - 1; i >= 0; --i) {
      stack.push([i, store.state.callStack[i].funcName]);
    }
  }
        ...
        <div className="call-stack">
          <ul>
            <Transition
              config={{
                duration:
                  (store.iterator.baseTime /
                    (store.iterator.iterating ? store.iterator.speed : 2)) *
                  3,
              }}
              items={stack}
            ...
            >
              {([i, name]) =>
                (props: React.CSSProperties) => {
                  ...
                  if (i === stack.length - 1) {
                    props.borderTopLeftRadius = props.borderTopRightRadius =
                      "50%";
                  }
                  return renderItem([i as number, name as any], height, props);
                }}
            </Transition>
          </ul>
        </div>
