...
class StructSettings extends Component<Props> {
  ...
  updatePositions = (items: string[]) => {
    ...
    items.forEach(
      (k, i) =>
        (newKeys[k].pos = Math.max(1, Math.min(i + 1, this.state.numKeys)))
    );
    this.setState({
      order: newKeys,
    });
      ...
      <div className={`box struct-settings-${name}`} style={style}>
        <div className="columns">
          ...
          <div className="column has-text-centered">
            <input
              type="color"
              value={structSettings.color}
              onChange={(e) => (structSettings.color = e.target.value)}
            />
          </div>
          {!isNative && (
            <div className="column has-text-centered">
              <input
                type="color"
                value={structSettings.textColor}
                onChange={(e) => (structSettings.textColor = e.target.value)}
              />
            </div>
          )}
          <div
            className={`column has-text-right ${this.state.deleting && "is-3"}`}
          >
            {!isNative &&
              !isBuiltin &&
              !settings.dependentTypes.has(name) &&
              (this.state.deleting ? (
                <div className="columns">
                  <div className="column is-one-third">Sure?</div>
                  <div className="column is-one-third">
                    <button
                      onClick={() => settings.deleteStruct(name)}
                  ...
                  <div className="column is-one-third">
                    <button
                      onClick={() => this.toggleDelete(false)}
                      className="button is-small"
                    >
                      No
                    </button>
        ...
        {this.state.editing && (
          <div>
            <h1
              className="title is-6"
              style={{ color: store.settings.configColors["Text"] }}
            >
              Children
            </h1>
            <DraggableList
            ...
            {!isBuiltin && (
              <div className="columns">
                <div className="column">
                  <input
                    type="text"
                    className="input add-child-input"
                    onChange={(e) =>
                ...
                <div className="column has-text-right">
                  <button
                    className="button is-primary add-child-button"
                    disabled={!this.state.newKeyName}
                    onClick={this.addKey}
                  >
                    Add Child
                  </button>
            ...
            <ul className="list">
              {pointers.map((key) => {
                const configurable = isBuiltin
                  ? !(key in structInfo[name].pointers)
                  : true;
                return (
                  <li key={key} className="list-item">
                    <div className="columns ">
                          ...
                          onChange={
                            configurable
                              ? (e) =>
                                  this.configPointer(
                                    key,
                                    e.target.value as pointerType
                                  )
                              : undefined
                          }
                          value={
                            this.state.pointers[key] ? "multiple" : "single"
                          }
                          className="select"
                        >
                      ...
                      <div className="column has-text-centered">{key}</div>
                      {configurable && (
                        <div className="column has-text-right">
                          <button
                            className="delete"
                            onClick={() => this.removePointer(key)}
                          />
                        </div>
                      )}
                    </div>
            ...
            <div className="columns">
              <div className="column">
                <input
                  type="text"
                  className="input add-pointer-input"
                  onChange={(e) =>
                    this.setState({ newPointerName: e.target.value })
                  }
                  value={this.state.newPointerName}
                />
              </div>
              <div className="column has-text-right">
                <button
                  className="button is-primary add-pointer-button"
                  disabled={!this.state.newPointerName}
            ...
            <div>
              ...
              <input
                className="input display-key-input"
                type="text"
                value={this.state.main}
                onChange={
                  isBuiltin ? undefined : (e) => this.changeMain(e.target.value)
                }
              />
            </div>
            <hr />
            <div>
              ...
              <Slider
                min={0}
                max={12}
                marks={new Array(13).fill(1).reduce((a, _, i) => {
                  if (i === 0) {
                    a[i] = (
                      <span
                        style={{
                ...
                onChange={
                  isBuiltin
                    ...
                    : (v) =>
                        v === 0
                          ? this.toggleNumChildren()
                          : this.setState({ numChildren: v })
                }
                trackStyle={{
            ...
            <div className="has-text-centered">
              <button
                onClick={this.submit}
                className="button is-link finished-editing"
              >
