...
const Structs: React.FC = observer(() => {
    ...
    if (store.settings.arrayTypes.has(store.viz.types[id])) {
      arrays.push(
        <ArrayStruct
          ...
          pointed={false}
          ratio={1}
          structure={store.structs.objects[id]}
          objectId={id}
        />
      );
  ...
  store.setWidths(
    {
      array: Math.max(arrayCol / 6, 0.5),
      object: 1,
      data: dataCol / 6,
    },
    [arrays.length, objects.length, data.length]
  );

  return (
    <div className="structs columns is-multiline">
      {data.length ? (
        <div className={`column is-narrow is-${dataCol}`}>
          {data.map((d) => (
            <DataStructContainer
              key={d.id}
              {...d}
      ...
      {arrays.length ? (
        <div className={`column is-narrow is-${arrayCol}`}>{arrays}</div>
      ) : null}
      {objects.length ? (
        <div className={`column is-narrow is-${objCol}`}>{objects}</div>
      ) : null}
    </div>
  );
});
