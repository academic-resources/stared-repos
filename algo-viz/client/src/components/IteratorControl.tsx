...
const handle = (props: any) => {
  ...
  return (
    <Tooltip
      ...
      overlay={
        value +
        (store.viz.steps[value] ? ": " + store.viz.steps[value].type : "")
      }
      visible={dragging}
      placement="top"
      key={index}
        ...
        {() => (
          <Handle
            ...
            style={{
              ...
              marginTop: -2,
              backgroundColor:
                store.settings.configColors["Primary Background"],
            }}
          />
        )}
      </Observer>
  ...
  return (
    <div className="iterator-control">
      <Slider
        ...
        onBeforeChange={() => {
          iterator.beforeChange();
        }}
        onChange={(e) => iterator.change(e)}
        onAfterChange={() => {
          iterator.afterChange();
        }}
        trackStyle={{
          backgroundColor:
            store.viz.steps[store.viz.steps.length - 1].type === "ERROR"
              ? "#FF0000"
              : settings.configColors["Step Slider Track"],
          height: 10,
        }}
        handle={handle}
        railStyle={{
          backgroundColor:
            store.viz.steps[store.viz.steps.length - 1].type === "ERROR"
              ? "#FF8080"
              : settings.configColors["Step Slider Rail"],
          height: 10,
        }}
      />
      <br />
