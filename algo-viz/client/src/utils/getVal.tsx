...
const getVal = (
  value: any,
  displayProps: Viz.DisplayProps,
  type: Viz.valType,
  includePointer = false,
  isDataDisplay = false
) => {
  const {
    settings: { valueColors: colors },
  } = store;
  if (type === "null") {
    displayProps.color = displayProps.color || "transparent";
  } else if (type === "object") {
    displayProps.color =
      displayProps.color || store.settings.configColors["Primary Background"];
    if (includePointer) {
      displayProps.component = (
        <Pointer
          active={!!displayProps.anim[0]}
          id={value}
          size={displayProps.size * 3}
  ...
  if (type === "boolean") {
    ...
    if (typeof value === "string" && value.length <= 4) {
      displayProps.textDisplay = value;
    } else if (Array.isArray(value)) {
      displayProps.textDisplay = null;
    }
  } else if (type === "number") {
