...
class Structures {
  ...
  constructor(store: RootStore) {
    ...
    for (const id in objs) {
      if (!this.pointers.has(id))
        this.pointers.set(id, new PointerQueue(this, id));
  ...
  @action addPointers(
    ...
    parent: string,
    key: string | number,
    idx = this.root.iterator.index
  ) {
    if (id !== parent) {
      const parentType = this.root.viz.types[parent];
      const isChild =
        key in this.root.settings.structSettings[parentType].order;
  ...
  @action prev(step: Viz.Step.Any) {
    ...
    if (step.type === "GET") {
      ...
      if (
        prop &&
        !this.root.settings.arrayTypes.has(this.root.viz.types[object])
      ) {
