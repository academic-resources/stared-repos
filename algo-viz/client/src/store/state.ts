...
class StateStore {
  ...
  @action next(step: Viz.Step.Any) {
    ...
    if (
      ["ASSIGNMENT", "DECLARATION", "DELETE_VARIABLE"].includes(step.type) &&
      step.scope &&
      step.varName
    ) {
          ...
          if (typeof scope === "number") {
            ...
            for (const id in ids) {
              ...
              if (idens.length === 1 && idens[idens.length - 1] === undefined) {
                ...
                if (step.prevVals) ids[id].push(step.prevVals[scope][id]);
              }
            }
            const { children } = this.scopeChain[scope];
            for (const child of children) {
