...
class PointerQueue {
  ...
  @action private _siftUp(idx: number) {
    ...
    while (
      idx > 0 &&
      this._compare(this.heap[idx], this.heap[parent], [idx, parent])
    ) {
  ...
  @action private _siftDown(idx: number, end: number) {
    ...
    while (child1 <= end) {
      ...
      if (
        child2 !== -1 &&
        this._compare(this.heap[child2], this.heap[child1], [child2, child1])
      ) {
        swapIdx = child2;
      }
  ...
  private getAffinity(parent: string, child: string): number {
    ...
    if (
      this.baseTypes.arrayTypes.has(parentType) &&
      this.baseTypes.arrayTypes.has(childType)
    ) {
      return 2;
    }

    if (
      !this.baseTypes.unconfigurables.has(parentType) &&
      !!this.baseTypes.unconfigurables.has(childType)
    ) {
      return 3;
    }
    ...
    if (
      this.baseTypes.hashTypes.has(parentType) &&
      !this.baseTypes.setTypes.has(parentType)
    ) {
