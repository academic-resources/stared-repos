...
const PORT =
  process.env.PORT ||
  (env === "test" ? 8080 : env === "production" ? 3000 : 3001);
...
async function initialize() {
  let hasError;
  const mongoUri =
    env === "production" ? process.env.MONGO_URI : "mongodb://localhost:27017";
  ...
  app.use(
    session({
      store: !hasError
        ? new MongoStore({
            url: env === "production" ? mongoUri : mongoUri + "/" + dbName,
          })
        : null,
      ...
      .catch((e) => {
        if (e.isRunnerError || e.isTranspilerError) {
          ...
          if (e.isRunnerError)
            return next(
              new Error(
                "The runner made a fatal mistake with your code. It will be investigated ASAP. In the meantime, please do not run the same code again."
              )
            );
          if (e.isTranspilerError)
            return next(
              new Error(
                "The transpiler made a fatal mistake with your code. It will be investigated ASAP. In the meantime, please do not run the same code again."
              )
            );
        } else {
          next(e);
