...
others |= {getattr(threading, name) for name in dir(threading) if name[0] != "_"}

viz_types = {getattr(viz, name) for name in dir(viz) if name[0] != "_"}
collections_types = {
    getattr(collections, name) for name in dir(collections) if name[0] != "_"
}
queue_types = {getattr(myqueue, name) for name in dir(myqueue) if name[0] != "_"}
...
class Runner:
    ...
    def gen_id(self, l=3, num_=2):
        ...
        while not _id or _id in self.types:
            _id = "".join("_" for i in range(num_)) + "".join(
