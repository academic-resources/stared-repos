    ...
    class GenericProxy(ObjectProxy):
        def __getattr__(self, name):
            ...
            if t == "method" or t == "builtin_function_or_method":
                ...
                runner.__(
                    attr,
                    {"type": TYPES.GET, "object": self.__wrapped__, "access": name},
                )
                return attr
        ...
        def __getitem__(self, key):
            ...
            runner.__(
                item, {"type": TYPES.GET, "object": self.__wrapped__, "access": key}
            )
        ...
        def __setattr__(self, name, value):
            if name.startswith("_self_"):
                ...
                runner.__(
                    ...
                    {"type": TYPES.SET, "object": self.__wrapped__, "access": name},
                )
        ...
        def __setitem__(self, key, value):
            ...
            runner.__(
                ...
                {"type": TYPES.SET, "object": self.__wrapped__, "access": key},
            )
            return result
        ...
        def __delattr__(self, name):
            if name.startswith("_self_"):
                ...
                runner.__(
                    True,
                    {"type": TYPES.DELETE, "object": self.__wrapped__, "access": name},
                )
                delattr(self.__wrapped__, name)

        def __delitem__(self, key):
            runner.__(
                True, {"type": TYPES.DELETE, "object": self.__wrapped__, "access": key}
            )
    ...
    class ListProxy(ObjectProxy):
        def __getitem__(self, key):
            ...
            if isinstance(key, slice):
                for i in range(len(self))[key]:
                    runner.__(
                        self.__wrapped__[i],
                        {"type": TYPES.GET, "object": self.__wrapped__, "access": i},
                    )
            else:
                key = key if key >= 0 else len(self) + key
                runner.__(
                    self.__wrapped__[key],
                    {"type": TYPES.GET, "object": self.__wrapped__, "access": int(key)},
                )
        ...
        def __setitem__(self, key, value):
            ...
            if not isinstance(key, slice):
                ...
                runner.__(
                    self.__wrapped__[key],
                    {"type": TYPES.SET, "object": self.__wrapped__, "access": int(key)},
                )
                return
            if ln == len(self):
                for i in range(len(self))[key]:
                    runner.__(
                        self.__wrapped__[i],
                        {"type": TYPES.SET, "object": self.__wrapped__, "access": i},
                    )
            else:
                rng = range(ln)[key]
                start = rng[0]
                ...
                runner.__(
                    len(self),
                    {"type": TYPES.SET, "object": self.__wrapped__, "access": "length"},
                )
        ...
        def __delitem__(self, key):
            ...
            for i in range(ln - length, ln):
                runner.__(
                    True,
                    {"type": TYPES.DELETE, "object": self.__wrapped__, "access": i},
                )
        ...
        def pop(self, *args, **kwargs):
            ...
            runner.__(
                result, {"type": TYPES.GET, "object": self.__wrapped__, "access": idx}
            )
            runner.__(
                True, {"type": TYPES.DELETE, "object": self.__wrapped__, "access": idx}
            )
        ...
        def sort(self, *args, **kwargs):
            ...
            for i, n in enumerate(self.__wrapped__):
                runner.__(
                    n, {"type": TYPES.SET, "object": self.__wrapped__, "access": i}
                )
            return result
        ...
        def insert(self, *args, **kwargs):
            ...
            if idx > ln:
                runner.__(
                    args[i],
                    {"type": TYPES.SET, "object": self.__wrapped__, "access": ln},
                )
        ...
        def clear(self, *args, **kwargs):
            ...
            runner.__(
                0, {"type": TYPES.SET, "object": self.__wrapped__, "access": "length"}
            )
            return result
        ...
        def copy(self, *args, **kwargs):
            ...
            for i, n in enumerate(self.__wrapped__):
                runner.__(
                    n, {"type": TYPES.GET, "object": self.__wrapped__, "access": i}
                )
            return result
    ...
    class DictProxy(runner.GenericProxy):
        def __getitem__(self, key):
            ...
            if not had_key and key in self.__wrapped__:
                ...
                item = super().__getitem__(key)
                t = TYPES.SET
            runner.__(item, {"type": t, "object": self.__wrapped__, "access": key})
        ...
        def get(self, *args, **kwargs):
            ...
            if key in self.__wrapped__:
                runner.__(
                    self.__wrapped__[key],
                    {"type": TYPES.GET, "object": self.__wrapped__, "access": (key)},
                )
            return result
        ...
        def clear(self, *args, **kwargs):
            result = super().__getattr__("clear")(*args, **kwargs)
            runner.__(None, {"type": TYPES.CLEAR, "object": self.__wrapped__})
            return result
        ...
        def pop(self, *args, **kwargs):
            has_key = False if len(args) < 1 else args[0] in self.__wrapped__
            result = super().__getattr__("pop")(*args, **kwargs)
            if has_key:
                key = args[0]
                runner.__(
                ...
                runner.__(
                    True,
                    {"type": TYPES.DELETE, "object": self.__wrapped__, "access": key},
                )
            return result
...
def set_proxy(runner):
    class SetProxy(runner.GenericProxy):
        ...
        def add(self, *args, **kwargs):
            ...
            runner.__(
                args[0],
                {"type": TYPES.SET, "object": self.__wrapped__, "access": args[0]},
            )
            return result

        def discard(self, *args, **kwargs):
            had_key = False if len(args) < 1 else (args[0] in self.__wrapped__)
        ...
        def pop(self, *args, **kwargs):
            ...
            runner.__(
                result,
                {"type": TYPES.GET, "object": self.__wrapped__, "access": result},
            )
            runner.__(
                True,
                {"type": TYPES.DELETE, "object": self.__wrapped__, "access": result},
            )
            return result
        ...
        def update(self, *args, **kwargs):
            ...
            for val in self.__wrapped__:
                if val not in vals:
                    runner.__(
                        val,
                        {"type": TYPES.SET, "object": self.__wrapped__, "access": val},
                    )
            return result
        ...
        def __ior__(self, other):
            ...
            for val in other:
                runner.__(val, {"type": TYPES.SET, "object": self, "access": val})
            return ret
        ...
        def __iand__(self, other):
            ...
            for val in vals:
                if not val in other:
                    runner.__(
                        val, {"type": TYPES.DELETE, "object": self, "access": val}
                    )
            return ret
        ...
        def __contains__(self, val):
            ...
            if has:
                runner.__(
                    val, {"type": TYPES.GET, "object": self.__wrapped__, "access": val}
                )
            return has
