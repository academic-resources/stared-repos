...
describe("RECONSTRUCT", () => {
  ...
  async function testRunner(func) {
    const { steps, objects, types, normalResult, map, transpiledResult, code } =
      await main(func);
    ...
    const {
      objects: { [key]: reconstructed },
    } = reconstructor({ types, steps, objects, code });
...
describe("DECONSTRUCT(reverse)", () => {
  ...
  async function testRunner(func, copies, original) {
    const { steps, objects, types, map, transpiledResult, code } = await main(
      func,
      copies
    );
