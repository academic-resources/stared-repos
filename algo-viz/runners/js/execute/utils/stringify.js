module.exports = function (obj) {
  ...
  if (
    ...
    if (i > 1000) {
      this.throw(
        new Error(
          `${type} is too large. All objects are limited to 1000 elements.`
        )
      );
    }
    if (!type) {
      this.throw(
        new Error("Class name not found. All classes must have a name.")
      );
    }
    if (obj instanceof (this.global ? this.global.String : String)) {
      type = "Object";
    }
    if (this.Viz && this.Viz[type] && this.Viz[type] === obj.constructor)
      type = "Viz." + type;
