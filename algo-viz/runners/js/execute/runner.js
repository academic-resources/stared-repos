...
class Runner {
  constructor(name, code, limit = 30000) {
    ...
    const { reassignMapMethods, reassignSetMethods } =
      reassignMutative.call(this);
  ...
  __(val, info) {
    ...
    try {
      ...
      info.value = info.value
        ...
        : this.stringify(val);
      if (
        [TYPES.FUNC, TYPES.METHOD, TYPES.BLOCK, TYPES.RETURN].includes(
          info.type
        )
      ) {
      ...
      this.numSteps++;
      if (this.numSteps > this.limit)
        this.throw(new Error("Step limit exceeded."));
      if (this.calls > 500)
        this.throw(new Error("Maximum callstack size of 500 exceeded"));
