  ...
  return class Trie {
    ...
    add(word: string) {
      if (typeof word !== "string") {
        throw new Error(
          `Trie.add: word parameter must be a string. Got ${word}`
        );
      }
      let node: Trie = this;
    ...
    remove(word: string) {
      if (typeof word !== "string") {
        throw new Error(
          `Trie.remove: word parameter must be a string. Got ${word}`
        );
      }
      let value: string = "",
        node: Trie = this;
    ...
    find(word: string): Trie {
      if (typeof word !== "string")
        throw new Error(`Trie.find: word must be a string. Got ${word}`);
      let value: string = "",
        node: Trie = this;
    ...
    addMany(words: string[]) {
      if (!Array.isArray(words)) {
        throw new Error(
          `Trie.add: words parameter must be an array of words. Got ${words}`
        );
      }
