  ...
  return class DLL {
    ...
    static create(elems: any[]): DLL {
      if (!Array.isArray(elems))
        throw new Error(
          "Viz.DLL.create: DLL elements must be given in array form"
        );
      if (!elems.length)
        throw new Error(
          "Viz.DLL.create: Elements must have a length of at least 1"
        );
      runner.ignore(true);
    ...
    static forEach(list: DLL, callback: (val: any) => any): void {
      ...
      for (let current: DLL | null = list; !!current; current = current.next) {
        if (seen.has(current))
          throw new Error("Viz.DLL.forEach: Cannot traverse cyclic lists");
    ...
    private static assert(list: DLL) {
      if (!("next" in list) || !("value" in list) || !("prev" in list))
        throw new Error(
          'Viz.DLL: List must have properties "next" and "value"'
        );
    }
