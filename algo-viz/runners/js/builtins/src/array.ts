const check = (length: number, name: string) => {
  if (typeof length !== "number")
    throw new Error(`Viz.array.${name}: length must be a number`);
  if (length > 1000)
    throw new Error(
      `Viz.array.${name}: Requested length is too high. length < 1000`
    );
};
const array = {
  ...
  randomInts: (length: number = 10, allowNegative: boolean = true) => {
    ...
    const arr = [];
    for (let i = 0; i < length; i++) {
      const negator = allowNegative ? (Math.random() > 0.5 ? 1 : -1) : 1;
  ...
  filterDuplicates: (arr: any[], mutate: boolean = true) => {
    if (!Array.isArray(arr))
      throw new Error("Viz.array.filterDuplicates: Input must be an array");
  ...
  matrix: (
    rows: number,
    cols: number,
    callbackOrVal?: (row: number, col: number, matrix?: any[][]) => any
  ) => {
    if (typeof rows !== "number")
      throw new Error("Viz.array.matrix: rows must be a number");
    if (typeof cols !== "number")
      throw new Error("Viz.array.matrix: cols must be a number");
    const cb =
