...
export default function instantiateBTree(
  runner: Runner = { ignore: () => {} }
) {
  return class BTree {
    left: BTree | null;
    right: BTree | null;
    value: any;
    private static traversalTypes = [
      "inOrder",
      "postOrder",
      "preOrder",
      "breadthFirst",
    ...
    static create(elems: any[], method: "inOrder" | "binary" = "inOrder") {
      if (!Array.isArray(elems))
        throw new Error("Viz.BTree.create: elems must be an array");
      if (!elems.length) return null;
      if (method !== "inOrder" && method !== "binary")
        throw new Error(
          `Viz.BTree.create: Method must be "inOrder" or "binary". Received ${method}`
        );
      runner.ignore(true);
      elems = [...elems];
      runner.ignore(false);
    ...
    traverse(
      ...
      if (!BTree.traversalTypes.includes(order)) {
        throw new Error(
          "Viz.BTree.traverse: traversal order must be one of " +
            BTree.traversalTypes.join(", ")
        );
      }
