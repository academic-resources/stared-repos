  ...
  return function () {
    ...
    return {
      visitor: {
        ...
        Function: {
          enter(path, { opts }) {
            if (path.node.async && opts.disallow.async)
              throw new SyntaxError("async functions are disallowed");
            ...
            if (path.node.id) {
              ...
              const parent = path.findParent(
                (p) => t.isClassDeclaration(p) || t.isClassExpression(p)
              );
            ...
            if (t.isBlockStatement(path.node.body)) {
              ...
              if (isClassMethod) {
                const blockNode = proxy(t.nullLiteral(), {
                  type: TYPES.BLOCK,
                  scope: t.arrayExpression([
                    t.numericLiteral(path.scope.parent.parent.uid),
                    t.numericLiteral(path.scope.parent.uid),
                  ]),
                });
                block.body = [
                  blockNode,
        ...
        VariableDeclaration: {
          exit(path) {
            ...
            path
              .get("declarations")
              .forEach(traverseDeclarations(path, isFor, declarations));
        ...
        "ForOfStatement|ForInStatement": {
          exit(path) {
            if (!t.isVariableDeclaration(path.node.left)) {
              ...
              path.node.body.body = [
                ...details.map((d) => proxy(t.identifier(d.varName), d)),
                ...path.node.body.body,
              ];
            }
          },
        ...
        "CallExpression|NewExpression": {
          exit(path) {
            ...
            if (
              ...
              isBarredObject(call.callee.object.name)
            ) {
              return;
            }
        ...
        Expression: {
          exit(path) {
            if (t.isMemberExpression(path) && path.node.object.name === _name)
              return;
