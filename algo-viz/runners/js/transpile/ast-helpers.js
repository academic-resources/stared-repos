...
module.exports = function ({ t = types, input, code, Node }) {
  const createId = (l = 3, num_ = 2) => {
    let id;
    while (!id || id in input.references)
      id = "_".repeat(num_) + randomString(l);
  ...
  const getNameSlice = (node) =>
    t.arrayExpression([
      t.numericLiteral(node.start),
      t.numericLiteral(node.end),
    ]);
      ...
      if (t.isIdentifier(id)) {
        ...
        const details = {
          ...
          name: t.arrayExpression([
            t.numericLiteral(id.node.start),
            t.numericLiteral(id.node.end),
          ]),
        };
        ...
        Identifier: {
          enter(path) {
            ...
            if (isFor) {
              ...
              decs.push(
                t.variableDeclarator(variableName, proxy(path.node, details))
              );
            }
          },
        },
