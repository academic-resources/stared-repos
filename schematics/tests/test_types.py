...
from schematics.exceptions import ValidationError, ConversionError, MockCreationError
        ...
        def validate_contains_m_chars(self, value):
            if value.count("M") != self.number_of_m_chars:
                raise ValidationError(
                    "Value must contain {0} 'm' characters".format(
...
def test_multilingual_string_should_emit_string_with_explicit_locale():
    ...
    assert (
        mls.to_primitive(
            {"en_US": "snake", "fr_FR": "serpent"}, context={"locale": "fr_FR"}
        )
        == "serpent"
    )
...
def test_multilingual_string_should_accept_lists_of_locales():
    strings = {"en_US": "snake", "fr_FR": "serpent", "es_MX": "serpiente"}

    mls = MultilingualStringType(default_locale=["foo", "fr_FR", "es_MX"])

    assert mls.to_primitive(strings) == "serpent"
    assert (
        mls.to_primitive(strings, context={"locale": ["es_MX", "bar"]}) == "serpiente"
    )

    mls = MultilingualStringType()

    assert (
        mls.to_primitive(strings, context={"locale": ["foo", "es_MX", "fr_FR"]})
        == "serpiente"
    )
