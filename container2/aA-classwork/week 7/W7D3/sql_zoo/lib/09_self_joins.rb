...
def start_at_craiglockhart
  ...
  execute(<<-SQL)
    SELECT
      stopb.name, routes.company, routes.num
    FROM
      stops
    JOIN
      routes ON stops.id = routes.stop_id
    JOIN
      stops stopa ON (routes.stop_id = stopa.id)
    JOIN
      stops stopb ON (routes.stop_id = stopb.id)
    WHERE
      stopa.name = 'Craiglockhart' AND stopb.name = '%';
