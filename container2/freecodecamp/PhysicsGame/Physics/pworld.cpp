...
void GroundContacts::init(physics::ParticleWorld::Particles *particles)
{
    GroundContacts::particles = particles;
}

unsigned GroundContacts::addContact(physics::ParticleContact *contact,
                                    unsigned limit) const
{
    unsigned count = 0;
    for (physics::ParticleWorld::Particles::iterator p = particles->begin();
        p != particles->end();
        p++)
    {
        ...
        if (y < 0.0f)
        {
            contact->contactNormal = physics::Vector3::UP;
