...
const CSSTranslate = (position, axis) => {
  const positionCss =
    axis === 'horizontal' ? [position, 0, 0] : [0, position, 0]
  const translatedPosition = '(' + positionCss.join(',') + ')'

  return 'translate3d' + translatedPosition
}
...
class Carousel extends Component {
  ...
  setPosition = (position, forceReflow) => {
    ...
    if (forceReflow) {
      list.offsetLeft // eslint-disable-line
    }
  }
  ...
  render() {
    ...
    if (isHorizontal) {
      ...
      swiperProps.onSwipeUp =
        this.props.verticalSwipe === 'natural' ? this.increment : this.decrement
      swiperProps.onSwipeDown =
        this.props.verticalSwipe === 'natural' ? this.decrement : this.increment
