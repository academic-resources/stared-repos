...
class Process:
    ...
    wait = Event(
        from_states=(created, running, blocked, swapped_out_waiting), to_state=waiting
    )
...
def transition(process, event, event_name):
    try:
        ...
        print(
            f"Error: transition of {process.name} from {process.current_state} to {event_name} failed"
        )
