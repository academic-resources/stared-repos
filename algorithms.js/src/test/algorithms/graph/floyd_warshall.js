...
describe("Floyd-Warshall Algorithm", function () {
  it("should compute all-pairs shortest paths in the graph", function () {
    ...
    assert.deepEqual(result.distance, {
      a: { a: 0, b: -2, c: -3, x: -1, y: -6, z: Infinity },
      b: { a: 3, b: 0, c: -1, x: 1, y: -4, z: Infinity },
      c: { a: 4, b: 2, c: 0, x: 2, y: -3, z: Infinity },
      x: {
        a: Infinity,
        b: Infinity,
        c: Infinity,
      ...
      z: { a: Infinity, b: Infinity, c: Infinity, x: 1, y: -4, z: 0 },
    });
  ...
  it("should determine if the graph contains a negative-weighted cycle", function () {
    var graph = new Graph();
    graph.addEdge("a", "b", -2);
    graph.addEdge("b", "c", -1);
    graph.addEdge("c", "y", -3);
