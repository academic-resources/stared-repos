...
options = {"api_key": api_key, "app_key": application_key}
...
api.ServiceCheck.check(
    check=check, host_name=host, status=status, message="Response: 200 OK"
)
...
api.Comment.create(message="Smalltalk?", related_event_id=23234)
...
# Schedule downtime
api.Downtime.create(
    scope="env:staging", start=start_ts, end=end_ts, recurrence=recurrence
)
...
api.Event.query(
    start=start_time, end=end_time, priority="normal", tags=["application:web"]
)
