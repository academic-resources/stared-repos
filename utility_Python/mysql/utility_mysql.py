...
def get_mysql_conn(host, user, database):
    ...
    # user = 'root',
    # db = 'local_dev')
    conn = pymysql.connect(host=host, port=3306, user=user, db=database)
    return conn
...
def insert_data_csv():
    try:
        ...
        for row in csv_data:
            a.execute(
                "INSERT INTO `taxi` (`id`,`lon`,`lat`,`time`) VALUES (%s, %s, %s,%s)",
...
def insert_data_df_new_table():
    ...
    try:
        ...
        engine = create_engine(
            "mysql+pymysql://<user>:<password>@<host>[:<port>]/<dbname>", echo=True
        )
        df = taxi_df()
