...
def filter_outside_uk(lat, lon):
    if (
        float(lat) < 58.576745
        and float(lat) > 50.474865
        and float(lon) < 1.727596
        and float(lon) > -9.745551
    ):
        # 58.576745, -4.252387
        # 50.474865, -3.681098
        # 53.425787, -9.745551
...
def main(df):
    ...
    """
    df["located_UK"] = df.apply(
        lambda row: pd.Series(filter_outside_uk(row["LATITUDE"], row["LONGITUDE"])),
        axis=1,
    )
