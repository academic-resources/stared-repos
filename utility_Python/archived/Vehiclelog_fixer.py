...
def main():
    ...
    df__vehiclelog_sample_dev[
        "category_shifted"
    ] = df__vehiclelog_sample_dev.category.shift(-1)
    ...
    for index, row in df__vehiclelog_sample_dev.iloc[0:].iterrows():
		...
		"""
        if (str(row["category"]) == "Out of Service") and (
            str(row["category_shifted"]) == "Create booking"
        ):
            dataframe2.loc[index] = row
        ...
        elif (str(row["category"]) == "Create booking") and (
            str(row["category_shifted"]) == "Out of Service"
        ):
            dataframe2.loc[index] = row
