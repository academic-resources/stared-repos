class TicTacToe:
    ...
    def check_left_diagonal(self):
        # left diagonal
        sum_of_left_d_X = sum(
            [self.matrix[i][i] == "x" for i in range(len(self.matrix))]
        )
        sum_of_left_d_y = sum(
            [self.matrix[i][i] == "y" for i in range(len(self.matrix))]
        )
    ...
    def check_right_diagonal(self):
        ...
        sum_of_left_d_X = sum(
            [
                self.matrix[i][len(self.matrix) - i - 1] == "x"
                for i in range(len(self.matrix))
            ]
        )
        sum_of_left_d_y = sum(
            [
                self.matrix[i][len(self.matrix) - i - 1] == "y"
                for i in range(len(self.matrix))
            ]
        )
