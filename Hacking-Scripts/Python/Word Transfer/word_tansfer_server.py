...
def create_socket():
    try:
        ...
        )  # Error message in case socket can't be created
...
def socket_accept():
    ...
    received_word = clt_soc.recv(
        1024
    )  # receive the word sent by client in form of bytes
    received_word = received_word.decode(
        "utf-8"
    )  # to decode the word from bytes to text
...
def check_word(received_word, clt_soc):
    ...
    if flag == 0:
        clt_soc.send(
            bytes("Word not found in the list.", "utf-8")
        )  # to send back to client in case word is found
    else:
        clt_soc.send(
            bytes("Word found in the list.", "utf-8")
        )  # to send back to client in case word is not found
