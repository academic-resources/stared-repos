...
class WOFComputerPlayer(WOFPlayer):
    ...
    def smartCoinFlip(self):
        self.flip = random.randint(1, 10)
        if self.flip > self.difficulty:
            return True
        elif self.flip <= self.difficulty:
            return False
    ...
    def getMove(self, category, obscuredPhrase, guessed):
        ...
        self.obscuredPhrase = obscuredPhrase
        self.guessed = guessed
        gl = self.getPossibleLetters(self.guessed)
        if gl == [] or gl == "pass":
            return "pass"
...
# Screenshots are of code executed in Jupyter Notebook
