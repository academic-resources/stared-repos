...
font = pygame.font.Font(
    os.path.join(os.getcwd(), "fontdesign.ttf"), 42
)  # font of game display
...
def draw_lives(display, x, y, lives, image):
    for i in range(lives):
        ...
        img_rect = (
            img.get_rect()
        )  # gets the (x,y) coordinates of the cross icons (lives on the the top rightmost side)
        img_rect.x = int(
            x + 35 * i
        )  # position of lives icon (sets the next cross icon 35pixels awt from the previous one)
        img_rect.y = (
            y
        )  # takes care of how many pixels the cross icon should be positioned from top of the screen
...
while game_running:
    ...
    for key, value in data.items():
        if value["throw"]:
            ...
            if value["y"] <= 800:
                gameDisplay.blit(
                    value["img"], (value["x"], value["y"])
                )  # dynamic display of fruit inside display screen
            else:
                generate_random_fruits(key)

            current_position = (
                pygame.mouse.get_pos()
            )  # gets the current coordinate(x,y) in pixels of the mouse

            if (
                not value["hit"]
                and current_position[0] > value["x"]
                and current_position[0] < value["x"] + 60
                and current_position[1] > value["y"]
                and current_position[1] < value["y"] + 60
            ):
                if key == "bomb":
                    player_lives -= 1
                    if player_lives == 0:
