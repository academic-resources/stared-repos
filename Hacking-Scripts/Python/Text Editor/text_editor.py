...
def on_find(event=None):
    ...
    c = IntVar()
    Checkbutton(t2, text="Ignore Case", variable=c).grid(
    ...
    Button(
        ...
        text="Find All",
        underline=0,
        command=lambda: search_for(v.get(), c.get(), textPad, t2, e),
    ).grid(row=0, column=2, sticky="e" + "w", padx=2, pady=4)
...
def open_file(event=None):
    ...
    if filename == "":  # If no file chosen.
        ...
        root.title(
            os.path.basename(filename) + " - Tkeditor"
...
editmenu.add_command(
    label="Undo", compound=LEFT, image=undoicon, accelerator="Ctrl+Z", command=undo
)
editmenu.add_command(
    label="Redo", compound=LEFT, image=redoicon, accelerator="Ctrl+Y", command=redo
)
editmenu.add_separator()
editmenu.add_command(
    label="Cut", compound=LEFT, image=cuticon, accelerator="Ctrl+X", command=cut
)
editmenu.add_command(
    label="Copy", compound=LEFT, image=copyicon, accelerator="Ctrl+C", command=copy
)
editmenu.add_command(
    labe="Paste", compound=LEFT, image=pasteicon, accelerator="Ctrl+V", command=paste
)
editmenu.add_separator()
editmenu.add_command(label="Find", underline=0, accelerator="Ctrl+F", command=on_find)
editmenu.add_separator()
editmenu.add_command(
    label="Select All", accelerator="Ctrl+A", underline=7, command=select_all
)
