...
def AddWebsite():
    ...
    if "www" not in site or "." not in site:
        messagebox.showinfo(
            "Invalid Website",
            "Make sure the site is of this format 'www.google.com' OR 'google.com'",
        )
    else:
...
if sys.argv[-1] != ASADMIN:
    ...
    try:
        ...
        messagebox.showinfo(
            "Welcome",
            "Welcome to Website Blocker v1.0.Add Websites to block,Once you are done click on Block-Sites to get started!",
        )
        ...
        MyBtn = Button(root, text="Add Website", command=AddWebsite).grid(
            row=1, column=2
        )
        MyBtn2 = Button(root, text="Block-Sites", command=BlockSites).grid(
            row=3, column=2
        )
        root.mainloop()
    except (Exception):
