...
class Create:
    ...
    def __init__(self, root, row, col, txt, bg, fg, num, parent):
        self.root, self.row, self.col, self.txt, self.bg, self.fg, self.num, self.parent = (
            root,
            row,
            col,
            txt,
    ...
    def hint():
        ...
        if Intro.Gme_mode:
            if Main.flag == True:
                ...
                messagebox.showinfo(
                    "Compass", f"One of Enemy Ship is at box location { hin }"
                )
            else:
                messagebox.showinfo("Message", "Kindly place yours ships first !!")
            ...
            if Main.flag:
                if turn % 2 == 0:
                    ...
                    hin = (random.choice(hin) - 64) + random.randint(0, 3)
                    messagebox.showinfo(
                        "Compass", f"One of Player-2 Ship is at box location { hin }"
                    )
                elif turn % 2 == 1:
                    hin = [i.give_id for i in Main.lis_player if i.get_bg == "Red"]
                    hin = (random.choice(hin) - 64) + random.randint(0, 3)
                    messagebox.showinfo(
                        "Compass", f"One of Player-1 Ship is at box location { hin }"
                    )
        ...
        if Main.flag == True:
            ...
            if self.fg == "Red" and self.num > 64:
                ...
                if Create.computer_health == 0:
                    m = messagebox.askquestion(
                        "HURRAH ! ",
                        "YOU SANK Player-2's ARMADA ! "
                        + "\n"
                        + "Do you want to play again ?",
                    )
                    if m == "yes":
                        Main.destroy_comp(self.parent, flag=False)
                    if m == "no":
                        exit()
                    return False
            ...
            if not Intro.Gme_mode:
                if turn % 2 == 1:
                    if self.bg == "Red" and self.num < 65:
                        ...
                        Main.player_health[
                            "text"
                        ] = f"Your-Health : {Create.player_health}"
                        Main.computer_health[
                            "text"
                        ] = f"Comp-Health : {Create.computer_health}"

                        if Create.player_health == 0:
                            m = messagebox.askquestion(
                                "HURRAH ! ",
                                "YOU SANK Player-1's ARMADA ! "
                                + "\n"
                                + "Do you want to play again ?",
                            )
                            if m == "yes":
                    ...
                    if k.give_id == a:
                        if k.bg == "Red":
                            ...
                            if Create.player_health == 0:
                                m = messagebox.askquestion(
                                    "OPSS",
                                    "...You Lost... Enemy Sunk your fleet"
                                    + "\n"
                                    + "Do You want to play again ?",
                                )
                                if m == "yes":
...
class Main:
    ...
    def __init__(self):
        self.app = Tk()
        self.app.geometry("900x500" + "+170+100"), self.app.resizable(
            0, 0
        ), self.app.title("Battle Ship Game")
        ...
        strt_bttle = Button(
            ...
            width=15,
            fg="black",
            font=("veredana italic", 13, "bold"),
            command=start_place,
        )
        strt_bttle.place(x=371, y=140)
        Main.player_health = Label(
            self.app,
            text=f"Your-Health : {Create.player_health}",
            relief="groove",
            bg="white",
            fg="purple",
            font=("veredana italic", 14, "bold"),
        )
        Main.computer_health = Label(
            self.app,
            text=f"Comp-Health : {Create.computer_health}",
            relief="groove",
            bg="white",
            fg="purple",
            font=("veredana italic", 14, "bold"),
        ...
        new_gme = Button(
            ...
            bg="seagreen3",
            fg="black",
            font=("veredana", 12, "bold"),
            command=lambda: Main.destroy_comp(self),
        )
        compas = Button(
            self.app,
            text="Compas",
        ...
        new_gme.place(x=340, y=400), compas.place(x=340, y=450), back_gme.place(
            x=10, y=450
        )
        ...
        compas.bind(
            "<Leave>",
            lambda event, bt_this=compas, bg_color="yellow": on_leave(
                event, bt_this, bg_color
            ),
        )
        back_gme.bind(
            "<Enter>", lambda event, bt_this=back_gme: on_enter(event, bt_this)
        )
        back_gme.bind(
            "<Leave>",
            lambda event, bt_this=back_gme, bg_color="yellow": on_leave(
                event, bt_this, bg_color
            ),
        )
        strt_bttle.bind(
            "<Enter>", lambda event, bt_this=strt_bttle: on_enter(event, bt_this)
        )
        strt_bttle.bind(
            "<Leave>",
            lambda event, bt_this=strt_bttle, bg_clr="gold": on_leave(
                event, bt_this, bg_clr
            ),
        )
    ...
    def destroy_comp(self, flag=True):
        ...
        Create.player_health, Create.computer_health, Main.lis_comp, Main.lis_player = (
            0,
            0,
            [],
            [],
...
class Intro(Main):
    ...
    def __init__(self):
        self.root = Tk()
        self.root.geometry("500x540" + "+300+100"), self.root.resizable(
            0, 0
        ), self.root.title("Welcome Battle-ship")
        ...
        self.diff_lev = Button(
            ...
            fg="black",
            font=("veredana", 15, "bold"),
            command=self.level_difficulty,
        )
        self.diff_lev.place(x=140, y=350)
        self.gme_mode = Button(
            self.root,
            text="Game-Mode",
            relief=SUNKEN,
            width=20,
            ...
            fg="black",
            font=("veredana", 15, "bold"),
            command=self.mode_sel,
        )
        self.gme_mode.place(x=140, y=450)

        self.new_srt.bind(
            "<Enter>", lambda event, bt_this=self.new_srt: on_enter(event, bt_this)
        )
        self.new_srt.bind(
            "<Leave>", lambda event, bt_this=self.new_srt: on_leave(event, bt_this)
        )
        self.instr.bind(
            "<Enter>", lambda event, bt_this=self.instr: on_enter(event, bt_this)
        )
        self.instr.bind(
            "<Leave>", lambda event, bt_this=self.instr: on_leave(event, bt_this)
        )
        self.diff_lev.bind(
            "<Enter>", lambda event, bt_this=self.diff_lev: on_enter(event, bt_this)
        )
        self.diff_lev.bind(
            "<Leave>", lambda event, bt_this=self.diff_lev: on_leave(event, bt_this)
        )

        self.gme_mode.bind(
            "<Enter>", lambda event, bt_this=self.gme_mode: on_enter(event, bt_this)
        )
        self.gme_mode.bind(
            "<Leave>", lambda event, bt_this=self.gme_mode: on_leave(event, bt_this)
        )
    ...
    def instructions(self):
        ...
        self.new_root.geometry("600x380"), self.new_root.title(
            "Instructions"
        ...
        Label(
            ...
            bg="Black",
            fg="Gold",
            font=("helvitca", 12, "bold"),
        ).pack(), Label(self.new_root).pack()
    ...
    def level_difficulty(self):
        ...
        self.diff_wind.geometry("480x250"), self.diff_wind.resizable(
            0, 0
        ), self.diff_wind.title("Difficulty Level")
        ...
        insane = Button(
            ...
            width=10,
            font=("veredana italic", 12, "bold"),
            command=lambda: self.set_level("Insane"),
        )
        easy.pack(pady=5, ipadx=50, ipady=10), hard.pack(
            pady=5, ipadx=50, ipady=10
        ), insane.pack(pady=5, ipadx=50, ipady=10)
    ...
    def mode_sel(self):
        ...
        self.mode_wind.geometry("480x250"), self.mode_wind.resizable(
            0, 0
        ), self.mode_wind.title("Difficulty Level")
        ...
        fps = Button(
            ...
            width=10,
            font=("veredana italic", 12, "bold"),
            command=lambda: self.set_mode("Single-player"),
        )
        mps = Button(
            self.mode_wind,
            text="Multi-player",
            ...
            width=10,
            font=("veredana italic", 12, "bold"),
            command=lambda: self.set_mode("Multi-player"),
        )
    ...
    def set_mode(self, txt):
        if txt == "Single-player":
            Intro.Gme_mode = True
            messagebox.showinfo(
                "Set", "SINGLE PLAYER mode selected", parent=self.mode_wind
            ), self.mode_wind.destroy()
        elif txt == "Multi-player":
            Intro.Gme_mode = False
            messagebox.showinfo(
                "Set", "MULTI PLAYER mode selected", parent=self.mode_wind
            ), self.mode_wind.destroy()
    ...
    def set_level(self, txt):
        if txt == "Easy":
            ...
            messagebox.showinfo(
                "Set", "Difficulty set to Easy", parent=self.diff_wind
            ), self.diff_wind.destroy()
        elif txt == "Hard":
            Intro.diff_level = Intro.inverted_dic["--Hard--"]
            messagebox.showinfo(
                "Set", "Difficulty set to Hard", parent=self.diff_wind
            ), self.diff_wind.destroy()
        elif txt == "Insane":
            Intro.diff_level = Intro.inverted_dic["--Insane--"]
            messagebox.showinfo(
                "Set", "Difficulty set to Insane", parent=self.diff_wind
            ), self.diff_wind.destroy()
...
def show_grid(root, parent, specify=False):
    ...
    for i in range(8):
        for j in range(8):
            if specify == False:
                Main.lis_player.append(
                    Create(root, i, j, "", "White", "Yellow", counter, parent)
                )
            if specify == True:
                Main.lis_comp.append(
                    Create(root, i, j, "", "White", "Yellow", (counter + 65), parent)
                )
            counter += 1
        ...
        if Intro.Gme_mode:
            ...
            if not Main.flag:
                for each in Main.lis_comp:
                    if each.get_fg == "Yellow":
                        check_2 += 1
                    if each.get_fg == "Red":
                        count_2 += 1
                if check_2 == 64:
                    messagebox.showinfo(
                        "Alert", " Player-2 kindly place your ships !  !  "
                    )
                    return True
        if count_2 == 0:
    ...
    if "Sounds" in dirs:
        ...
        if exp_sound in files and shot_sound in files:
            ...
            messagebox.showerror(
                "Error",
                f'The files  "{exp_sound}"  and  "{shot_sound}"  doesnt exist in Sounds directory',
            )
    else:
        root_msg = Tk()
        root_msg.withdraw()
