...
frameCnt = 4
frames = [
    PhotoImage(file="Images/university.gif", format="gif -index %i" % (i))
    ...
    for i in range(0, len(name)):
        if name[i] == selected_name:
            mbox.showinfo(
                ...
                + "\n2.)  Institute ID  :  "
                + str(institute_id[i])
                + "\n3.)  Teaching, Learning and Resources (TLR)  :  "
                + str(tlr[i])
                + "\n4.)  Research and Professional Practice (RPC)  :  "
                + str(rpc[i])
                + "\n5.)  Graduation Outcomes (GO)  :  "
                + str(go[i])
                + "\n6.)  Outreach and Inclusivity (OI)  :  "
                + str(oi[i])
                + "\n7.)  Peer Perception  :  "
                + str(perception[i])
                + "\n8.)  City  :  "
...
# top label
start1 = tk.Label(
    text="INDIAN UNIVERSITY NIRF RANKING", font=("Arial", 40), fg="magenta", underline=0
)  # same way bg
start1.place(x=50, y=10)

# label for universitry name ---------------------------------------------------------------------------------
sel_label = tk.Label(
    text="University Name : ", font=("Arial", 30), fg="brown"
)  # same way bg
...
)  # size 1 is selected as by default, and we can

nirfb = Button(
    window,
    text="ABOUT",
    command=nirf_details,
    font=("Arial", 20),
    bg="light green",
    fg="blue",
    borderwidth=3,
