...
def insertLetter(letter, pos):
    if isBoardFull(
        board
    ):  # if the board is full, them we return the message that, "None of you won!"
...
def isBoardFull(board):
    if (
        board.count(" ") > 1
    ):  # for checking we just count the empty string , then there is some space present
        return False
    else:
        return True
...
def playerMove():
    ...
    while run:
        ...
        )  # taken input from player
        try:
            move = int(move)  # type casting the imput into integer
            if move > 0 and move < 10:
                if spaceIsFree(
                    move
                ):  # if input is valid, then add input at that positoon
                    run = False
                    insertLetter("X", move)
                else:
            ...
            )  # when try code is not run, then this message will be displayed
...
def computerMove():
    ...
    for let in [
        "O",
        "X",
    ]:  # we just loop through all possible values and checks for the correct/optimal position for computer
    ...
    for i in possibleMoves:
        if i in [
            ...
            7,
            9,
        ]:  # 4 corners are at position 1,3,7,9, sowe check for all position
            cornersOpen.append(i)
    if (
        len(cornersOpen) > 0
    ):  # if conditon satisfied, we take any random position from corner position we got
    ...
    if (
        len(edgesOpen) > 0
    ):  # if conditon satisfied, we take any random position from edge position we got
...
def main():
    ...
    if isBoardFull(
        board
    ):  # is board is full, so we tie the game, as no one won and board become full
        print("The game tied.")
