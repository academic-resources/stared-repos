...
def go_extract():
    ...
    # top label
    start1 = tk.Label(
        window_extract, text="EXTRACT IP ADDRESS", font=("Arial", 50), fg="magenta"
    )  # same way bg
    ...
    enter_label = Label(
        window_extract,
        text="Enter Your text and Extract IP Address...",
        font=("Arial", 30),
        fg="brown",
    )
...
def go_validate():
    ...
    def check(ip):
        if "." in ip:
            ...
            for n in ip:
                for c in n:
                    if c not in map(str, range(10)) and c not in map(
                        lambda x: chr(x), range(ord("a"), ord("f") + 1)
                    ):
                        return False
            return 6
        else:
    ...
    # top label
    start1 = tk.Label(
        window_validate, text="Validate IP Address", font=("Arial", 50), fg="magenta"
    )  # same way bg
    ...
    enter_label = Label(
        window_validate,
        text="Enter Your IP Address and see validity...",
        font=("Arial", 30),
        fg="brown",
    )
    ...
    ip_lbl = tk.Label(
        window_validate, text="IP Address :  ", font=("Arial", 30), fg="brown"
    )  # same way bg
...
def start_fun():
    ...
    extract_lbl = Label(
        f1, text="Want to Extract Add. ...", font=("Arial", 40), fg="brown", bg="white"
    )
    ...
    validate_lbl = Label(
        f1, text="Want to Check Validity...", font=("Arial", 40), fg="brown", bg="white"
    )
...
# top label
start1 = tk.Label(
    text="IP Address Extractor-Validator", font=("Arial", 50), fg="magenta"
)  # same way bg
