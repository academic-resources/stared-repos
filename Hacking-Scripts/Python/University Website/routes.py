...
def route(app, db):
    ...
    def register_page():
        ...
        if form.validate_on_submit():
            student_to_create = application.Student(
                id=form.roll_number.data,
                name=form.name.data,
            ...
            flash(
                f"Account created successfully! You are now logged in as {student_to_create.username}",
                category="success",
            )
    ...
    def login_page():
        form = LoginForm()
        if form.validate_on_submit():
            attempted_user = application.Student.query.filter_by(
                username=form.username.data
            ).first()
            if (
                attempted_user
                ...
                flash(
                    "Username, password  or roll number does not match! Please try again",
                    category="danger",
                )
