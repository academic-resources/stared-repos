...
class RegisterForm(FlaskForm):
    def validate_username(self, student_to_check):
        ...
        student = application.Student.query.filter_by(
            username=student_to_check.data
        ).first()
        if student:
    ...
    def validate_email_address(self, email_address_to_check):
        email_address = application.Student.query.filter_by(
    ...
    name = StringField(
        label="Name:", validators=[Length(min=2, max=50), DataRequired()]
    )
    stream = StringField(
        label="Stream:", validators=[Length(min=1, max=50), DataRequired()]
    )
    d_o_b = StringField(
    ...
    password1 = PasswordField(
        label="Password:", validators=[Length(min=2), DataRequired()]
    )
