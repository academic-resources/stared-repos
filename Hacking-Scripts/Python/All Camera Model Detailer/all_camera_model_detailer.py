...
frameCnt = 4
frames = [
    PhotoImage(file="Images/cameras.gif", format="gif -index %i" % (i))
    ...
    for i in range(0, len(Model)):
        if Model[i] == model:
            mbox.showinfo(
                ...
                + str(Release_date[i])
                + "\n\n2.)  Max resolution  :  "
                + str(Max_resolution[i])
                + "\n\n3.)  Low resolution  :  "
                + str(Low_resolution[i])
                + "\n\n4.)  Effective pixels  :  "
                + str(Effective_pixels[i])
                + "\n\n5.)  Zoom wide (W)  :  "
                + str(Zoom_wide[i])
                + "\n\n6.)  Zoom tele (T)  :  "
                + str(Zoom_tele[i])
                + "\n\n7.)  Normal Focus Range  :  "
                + str(Normal_focus_range[i])
                + "\n\n8.)  Macro Focus Range  :  "
                + str(Macro_focus_range[i])
                + "\n\n9.)  Storage Included  :  "
                + str(Storage_included[i])
...
# top label
start1 = tk.Label(
    text="CAMERA MODEL DETAILER", font=("Arial", 50), fg="magenta", underline=0
)  # same way bg
start1.place(x=50, y=10)

# label for camera model ---------------------------------------------------------------------------------
modellbl = tk.Label(
    text="CAMERA MODEL  : ", font=("Arial", 30), fg="brown"
)  # same way bg
