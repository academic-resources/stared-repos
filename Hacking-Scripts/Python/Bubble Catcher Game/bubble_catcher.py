...
def create_bubbles():
    x = randrange(
        10, 740
    )  # randrange function is used from module random, to generate randomness
    y = 40
    new_bubble = c.create_oval(
        x, y, x + bubble_width, y + bubble_height, fill=next(color_cycle), width=0
    )
...
def catch_check():
    ...
    for bubble in bubbles:
        ...
        if (
            catcher_x < bubble_x
            and bubble_x2 < catcher_x2
            and catcher_y2 - bubble_y2 < 40
        ):
            bubbles.remove(bubble)
            c.delete(bubble)
