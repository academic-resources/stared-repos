...
frameCnt = 3
frames = [
    PhotoImage(file="Images/spacex.gif", format="gif -index %i" % (i))
    ...
    for i in range(0, len(year_list)):
        ...
        if year_list[i] == selected_year:
            ...
            s = (
                ...
                + str(customer_type[i])
                + "\n      Customer Country  :  "
                + str(customer_country[i])
                + "\n      Mission Outcome  :  "
                + str(mission_outcome[i])
                + "\n      Failure Reasons  :  "
                + str(failure_reason[i])
                + "\n      Landing Type  :  "
                + str(landing_type[i])
                + "\n      Landing Outcome  :  "
                + str(landing_outcome[i])
            )
...
# top label
start1 = tk.Label(
    text="SPACEX MISSION DETAILER", font=("Arial", 50), fg="magenta", underline=0
)  # same way bg
start1.place(x=50, y=10)

# label for country code ---------------------------------------------------------------------------------
sel_label = tk.Label(
    text="Select  Year : ", font=("Arial", 30), fg="brown"
)  # same way bg
