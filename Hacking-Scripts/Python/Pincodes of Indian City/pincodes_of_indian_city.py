...
frameCnt = 4
frames = [
    PhotoImage(file="Images/pincodes.gif", format="gif -index %i" % (i))
...
def pincode():
    ...
    mbox.showinfo(
        selected_name + " Pin Code",
        "1.)  District Name  :  "
        + str(selected_name)
        + "\n\n2.)  Number of Cities in district "
        + selected_name
        + "  :  "
        + str(y - x)
        + "\n\n3.)  Pin Code Range is  :  ( "
...
# top label
start1 = tk.Label(
    text="PINCODES OF INDIAN CITY", font=("Arial", 50), fg="magenta", underline=0
)  # same way bg
start1.place(x=50, y=10)

# label for universitry name ---------------------------------------------------------------------------------
sel_label = tk.Label(
    text="SELECT CITY NAME : ", font=("Arial", 30), fg="brown"
)  # same way bg
