...
def def_start():
    ...
    def def_cal():
        ...
        r = int(e5.get())
        n = int(e6.get())
        cal_emi = p * (r / 1200) * ((1 + r / 1200) ** n)
        cal_emi = cal_emi / (((1 + r / 1200) ** n) - 1)
    ...
    # for top label
    start1 = tk.Label(
        f1, text="EMI  CALCULATOR", font=("Arial", 50), fg="magenta"
    )  # same way bg
...
def def_emi():
    mbox.showinfo(
        "EMI INFO",
        "EMI, which stands for Equated Monthly Installment, is the monthly amount payments we make towards a loan we opted for.\n\nEMI payments include contributions towards both principal and interest on the loan amount.\n\nThe mathematical formula to calculate EMI is: EMI = P × r × (1 + r)n/((1 + r)n - 1) where P= Loan amount, r= interest rate, n=tenure in number of months.",
    )


# top label
start1 = tk.Label(
    text="EMI  CALCULATOR", font=("Arial", 50), fg="magenta"
)  # same way bg
start1.place(x=180, y=10)
