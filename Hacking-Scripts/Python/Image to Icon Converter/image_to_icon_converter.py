...
def img_choose():
    from PIL import (
        Image,
    )  # we only import Image not ImageTk, beacause we are nit displaying in file, just choosing
    ...
    frame.img_dir = filedialog.askopenfilename(
        initialdir="C:",
...
def preview():
    ...
    lbl = Label(
        prev, text="Check your icon on the title bar!", font=("Arial", 15), fg="brown"
    )
    lbl.pack()
...
# starting label
start1 = Label(
    frame, text="Image to Icon Converter", font=("Arial", 35), fg="magenta", underline=0
)
