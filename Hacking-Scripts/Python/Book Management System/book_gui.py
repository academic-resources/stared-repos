...
def add():
    def add_cmd():
        mydata.insert_in_database(
            title_val.get(), author_val.get(), year_val.get(), isbn_val.get()
        )
        view_table.delete(0, END)
        view_table.insert(
            END, (title_val.get(), author_val.get(), year_val.get(), isbn_val.get())
        )
...
def update():
    ...
    def update_cmd():
        mydata.update_record(
            select[0], title_val.get(), author_val.get(), year_val.get(), isbn_val.get()
        )
...
title = Label(
    ...
    justify="center",
    font="helvetica 21 bold underline",
    pady=20,
    width=30,
)
view_button = Button(
