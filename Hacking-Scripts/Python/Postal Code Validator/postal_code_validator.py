...
def validate_fun():
    ...
    valid = len(re.findall(r"(?=(\d)\d\1)", entered_code)) < 2 and bool(
        re.match(r"^[1-9][0-9]{5}$", entered_code)
    )
    if valid:
        mbox.showinfo(
            "Validity Status", "The Postal Code " + entered_code + " is VALID."
        )
    else:
        mbox.showerror(
            "Validity Error", "THe Postal Code " + entered_code + " is INVALID."
        )
...
# top label
start1 = tk.Label(
    text="POSTAL CODE VALIDATOR", font=("Arial", 40), fg="magenta"
)  # same way bg
...
sel_label = tk.Label(
    text="ENTER POSTAL CODE : ", font=("Arial", 35), fg="brown"
)  # same way bg
sel_label.place(x=50, y=515)

# Created Entry Box
code_entry = Entry(
    window, font=("Arial", 30), fg="orange", bg="light yellow", borderwidth=3, width=14
)
