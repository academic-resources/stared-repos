...
def main_data_window(url_link, tab):
    ...
    # A scrollbar
    my_scrollbar = tk.Scrollbar(tab, orient=tk.VERTICAL, command=my_canvas.yview)
    ...
    date_time_label = tk.Label(frame0, text=f"{date}", font="mssans 21 bold", pady=4)
    date_time_label.pack(side=tk.LEFT, anchor=tk.NE, padx=(19, 0), pady=(9, 9))

    # Heading / Name of the app
    heading_lable = tk.Label(
        frame0, text="The Daily News", font="fixdays 36 bold italic", fg="red"
    )
    ...
    for i in range(len(title_lst)):
        ...
        description_lbl = tk.Label(
            containter,
            text=lineBreaker(description_lst[i]),
            justify=tk.LEFT,
            font="mssans 15",
            fg="#457b9d",
        )
        ...
        # bind urls
        url_lbl.bind("<Button-1>", lambda e, url=url_lst[i]: webbrowser.open_new(url))

        # seperator line
        seperator_lbl = tk.Label(containter, text=f"{'-'*180}", justify=tk.LEFT)
...
# Heading
hdn_lbl = tk.Label(
    frame1, text="Catgories", font="fixdays 31 bold", bg="#d62828", fg="black"
)
...
frame4.pack()
main_data_window(
    f"http://newsapi.org/v2/top-headlines?country=in&category=entertainment&apiKey={API_KEY}",
    frame4,
)
# frame5 or tab5
...
frame5.pack()
main_data_window(
    f"http://newsapi.org/v2/top-headlines?country=in&category=health&apiKey={API_KEY}",
    frame5,
)
# frame6 or tab6
...
frame6.pack()
main_data_window(
    f"http://newsapi.org/v2/top-headlines?country=in&category=science&apiKey={API_KEY}",
    frame6,
)
# frame7 or tab7
...
frame7.pack()
main_data_window(
    f"http://newsapi.org/v2/top-headlines?country=in&category=sports&apiKey={API_KEY}",
    frame7,
)
# frame8 or tab8
...
frame8.pack()
main_data_window(
    f"http://newsapi.org/v2/top-headlines?country=in&category=technology&apiKey={API_KEY}",
    frame8,
)
