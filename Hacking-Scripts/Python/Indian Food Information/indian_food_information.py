...
frameCnt = 4
frames = [
    PhotoImage(file="Images/food.gif", format="gif -index %i" % (i))
...
def get_status():
    mbox.showinfo(
        "INDIA",
        "Indian food is fourth most popular in the world, according to a study of cuisine trade.\n\nThe 5 MOST POPULAR CUISINES IN THE WORLD :\n1.)  Italian\n2.)  Japanese\n3.)  Chinese\n4.)  Indian\n5.)  American",
    )
    ...
    for i in range(0, len(food_name)):
        ...
        if food_name[i] == selected_food:
            mbox.showinfo(
                ...
                + food_diet[i]
                + "\n\nPreparation Time  :  "
                + str(food_prep_time[i])
                + " mins"
                + "\n\nCooking Time  :  "
                + str(food_cook_time[i])
                + " mins"
                + "\n\nFlavour  :  "
                + food_flavor_profile[i]
                + "\n\nCourse  :  "
                + food_course[i]
                + "\n\nState  :  "
                + food_state[i]
...
# top label
start1 = tk.Label(
    text="INDIAN FOOD INFO", font=("Arial", 50), fg="magenta", underline=0
)  # same way bg
start1.place(x=200, y=10)

# label for country code ---------------------------------------------------------------------------------
sel_label = tk.Label(
    text="Select Food : ", font=("Arial", 30), fg="brown"
)  # same way bg
