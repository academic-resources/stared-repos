...
frameCnt = 3
frames = [
    PhotoImage(file="Images/flight.gif", format="gif -index %i" % (i))
    ...
    for i1 in i_list:
        for j1 in j_list:
            if i1 == j1:
                mbox.showinfo(
                    "Find Success",
                    "Yes, There is a flight route between "
                    + src
                    + " and  "
                    + dest
                    ...
                    + src
                    + "  :  "
                    + source_airport_id_list[i1]
                    + "\n"
                    + dest
                    + "  :  "
                    + destination_airport_id_list[j1],
                )
                return

    mbox.showinfo(
        "Find Failure", "There is no flight route between " + src + " and " + dest
    )


# top label
start1 = tk.Label(
    text="DIRECT FLIGHT ROUTE FINDER", font=("Arial", 45), fg="magenta", underline=0
)  # same way bg
start1.place(x=50, y=10)
#
# label for entering time ---------------------------------------------------------------------------------
time1 = tk.Label(
    text="Select src. airport : ", font=("Arial", 30), fg="brown"
)  # same way bg
...
# label for :
colon1 = tk.Label(
    text="Select dest. airport", font=("Arial", 30), fg="brown"
)  # same way bg
