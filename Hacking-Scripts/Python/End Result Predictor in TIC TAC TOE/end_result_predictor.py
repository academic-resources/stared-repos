...
def guess_result():
    ...
    if cntx < cnto:
        mbox.showinfo(
            "Prediction",
            "This Tic Tac Toe board is not reachable.\n\nReason : The no. of X is less than no. of O.",
        )
        return
    if cntx > cnto + 1:
        mbox.showinfo(
    ...
    if cnt1 >= 1 or cnt2 >= 1:
        if cnt1 >= 1:
            mbox.showinfo(
                "Prediction",
                "This Tic Tac Toe board is reachable, and the player with X won.\n\nReason : Beacuse there is atleast one complete sequence of 3 X's present in the board.",
            )
            return
        else:
            mbox.showinfo(
                "Prediction",
                "This Tic Tac Toe board is reachable, and the player with O won.\n\nReason : Beacuse there is atleast one complete sequence of 3 O's present in the board.",
            )
            return
    if cnt_ == 0:
        mbox.showinfo(
            "Prediction",
            "This Tic Tac Toe board is reachable, and the game is drawn.\n\nReason : Since there is no empty cell and also no one either player with X or player with O has won.",
        )
        return
    else:
        mbox.showinfo(
            "Prediction",
            "This Tic Tac Toe board is reachable, and the game will continue for at least one more move.",
        )
        return
...
start1 = tk.Label(
    text="TIC-TAC-TOE PREDICTOR", font=("Arial", 50), fg="blue", underline=0
)  # same way bg
...
# x start label
startlabel = tk.Label(
    text="The game starts with X.", font=("Arial", 30), fg="brown"
)  # same way bg
