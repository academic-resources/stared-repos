...
frameCnt = 4
frames = [
    PhotoImage(file="Images/latlong.gif", format="gif -index %i" % (i))
...
def distance_feature():
    if valid:
        ...
        a = (
            sin(delta_lat / 2) ** 2
            + cos(ours_latitude) * cos(co_latitude) * sin(delta_long / 2) ** 2
        )
        c = 2 * asin(sqrt(a))
        r = 6371
        mbox.showinfo(
            "Distance Info",
            "Distance between Member Location and Co-ordinate Location:\n\n"
            + str(c * r)
            + "  Kms",
        )
    else:
...
def locate_entry():
    if (
        lat1_country == ""
        or lat1_country_code == ""
        or long1_country == ""
        or long1_country_code == ""
    ):
        mbox.showerror(
            "Locate Error", "The latitude and Longitude pair is not verified."
        )
    else:
        mbox.showinfo(
            "Locate Status",
            "The Country around this latitude  :  "
            + lat1_country
            + "\nThe Country Code is  :  "
            + lat1_country_code
            + "\n\nThe Country around this longitude  :  "
            + long1_country
            + "\nThe Country Code is  :  "
            + long1_country_code,
        )
...
# top label
start1 = tk.Label(
    text="Latitude-Longitude Pair Verifier", font=("Arial", 50), fg="magenta"
)  # same way bg
...
# Entry Box
lat_entry = Entry(
    window, font=("Arial", 25), fg="brown", bg="light yellow", borderwidth=3, width=12
)
