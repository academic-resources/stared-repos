...
def search_word():
    word = (
        inputentry.get()
    )  # first we get the word from the inputtxt and store it in word variable
    # print(word)
    ...
    if word == "":
        ...
        print(
            "You Entered Nothing! Please Enter Some Text.", file=buffer
    ...
    # CASE 2 : if word is present in data
        ...
        for i in data[
            word
        ]:  # we get output in list form , so we convert it into different line of string
            cnt = cnt + 1
            str_cnt = f"{cnt}"
            str += str_cnt + ".) "
            str += i
...
dic = tk.Label(
    text="DICTIONARY", font=("Arial", 50), fg="magenta", underline=0
)  # same way bg
dic.pack()

start1 = tk.Label(
    text="Enter the text you want to search...",
    font=("Arial", 30),
    fg="green",
    underline=0,
)  # same way bg
