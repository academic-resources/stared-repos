...
frameCnt = 4
frames = [
    PhotoImage(file="Images/periodic.gif", format="gif -index %i" % (i))
    ...
    for i in range(0, len(name)):
        if name[i] == selected_name:
            mbox.showinfo(
                ...
                + str(period[i])
                + "\n6.)  Element Category  :  "
                + str(element_category[i])
                + "\n7.)  Atomic Weight  :  "
                + str(atomic_weight[i])
                + "\n8.)  Electronic Configuration  :  "
                + str(electron_configuration[i])
                + "\n9.)  Phase  :  "
                + str(phase[i])
                + "\n10.)  Melting Point  :  "
                + str(melting_point[i])
                ...
                + str(heat_of_vaporization[i])
                + "\n13.)  Heat of Fusion  :  "
                + str(heat_of_fusion[i])
                + "\n14.)  Molar Heat Capacity  :  "
                + str(molar_heat_capacity[i])
                + "\n15.)  Oxidation States  :  "
                + str(oxidation_states[i])
                + "\n16.)  Electronegativity  :  "
                + str(electronegativity[i])
                + "\n17.)  Ionization Energies  :  "
                + str(ionization_energies[i])
                + "\n18.)  Covalent Radius  :  "
                + str(covalent_radius[i])
                + "\n19.)  Van Der Waal's Radius  :  "
                + str(van_der_waals_radius[i])
                + "\n20.)  Crystal Structure  :  "
                + str(crystal_structure[i])
                + "\n21.)  Thermal Conductivity  :  "
                + str(thermal_conductivity[i])
                + "\n22.)  Magnetic Ordering  :  "
                + str(magnetic_ordering[i]),
            )


# top label
start1 = tk.Label(
    text="PERIODIC TABLE ELEMENT INFO", font=("Arial", 40), fg="magenta", underline=0
)  # same way bg
start1.place(x=70, y=10)
#
# label for country code ---------------------------------------------------------------------------------
sel_label = tk.Label(
    text="Select  Element Name : ", font=("Arial", 30), fg="brown"
)  # same way bg
