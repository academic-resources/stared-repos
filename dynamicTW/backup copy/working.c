...
void signalMatch(t_dynamicTW *x){
    if(x->compareValue <= x->lcpValue){
        ...
        post("Incoming Signal Matches Stored Signal. comparValue is %f and lcpValue is %f", x->compareValue, x->lcpValue);
    }
    else{
        x->match = 0;
        post("Signal NO Match. compareVal is %f while lcpVal is %f", x->compareValue, x->lcpValue);
    }
}
...
void dtw_onSet_A(t_dynamicTW *x, t_floatarg f){  /*function that gets called when an input is received */
    ...
    if (arr_position <= SIZE_ARRAY){ //checks if array is filled. If not then store incoming value to next index
