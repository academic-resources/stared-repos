$(document).ready(function () {
	...
	function onSearch(e) {
		...
		$icons
			.hide()
			.filter(function () {
				return nameRegex.test(this.getAttribute("data-name"));
			})
			.filter(function () {
				return nameRegex.test(this.getAttribute("data-name"));
			})
	...
	function initTabs(el) {
		...
		$tabs.on("click", function (e) {
			if (e.target.parentElement !== el || $tabs.hasClass("m-inactive"))
				return;
			$tab = $(e.target);
	...
	function onSectionTabClick(e, mode) {
		...
		$togglePreviewChars
			.parent()
			.toggleClass("m-inactive", mode == "cheatsheet");
	}
	...
	function onIconClick(e) {
		...
		if ("grid" == gridMode) openPopup($icon[0].outerHTML);
	}
	...
	function onUsageExampleClick(e) {
		...
		openPopup(
			$('.js-usage-example[data-example="' + example + '"]').html()
		);
	}
