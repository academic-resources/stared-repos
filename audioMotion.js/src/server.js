...
function getDir(directoryPath, showHidden = false) {
	...
	try {
		entries = fs.readdirSync(path.normalize(directoryPath), {
			withFileTypes: true,
		});
	} catch (e) {
		return false;
...
process.argv.forEach((arg, index) => {
	...
	else if (arg == "-s") launchClient = false;
	else if (arg == "-e") host = "";
});
...
if (!musicPath) {
	...
	musicPath = readlineSync.questionPath(
		`\n\tPlease enter full path to music folder (e.g. ${
			process.platform == "win32"
				? "c:\\users\\john\\music"
				: "/home/john/music"
...
server.use("/music", express.static(musicPath), (req, res) => {
	...
	if (files === false) res.status(404).send("Not found!");
	else {
		files.files = files.files.filter((file) => {
			if (file.match(/\.(jpg|jpeg|png|gif|bmp)$/i)) imgs.push(file);
			return (
				file.match(/\.(mp3|flac|m4a|aac|ogg|wav|m3u|m3u8)$/i) !== null
			);
		});
		files.cover =
			findImg(imgs, "cover") ||
...
server.listen(port, host, () => {
	console.log(
		`\n\n\tListening on port ${port} ${
			host
				? "for localhost connections only"
...
server.get("/getCover/:path", (req, res) => {
	const path =
			musicPath +
			decodeURI(req.params.path)
				.replace(/^\/music/, "")
				.replace(/%23/g, "#"),
		entries = getDir(path);

	if (entries === false) res.status(404).send("Not found!");
	else {
		const imgs = entries.files.filter(
			(file) => file.match(/\.(jpg|jpeg|png|gif|bmp)$/i) !== null
		);
		res.send(
			findImg(imgs, "cover") ||
				findImg(imgs, "folder") ||
