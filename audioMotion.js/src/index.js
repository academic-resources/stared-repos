...
const gradients = {
	...
	cool: {
		name: "Cool",
		bgColor: "#0b202b",
		colorStops: ["hsl( 208, 0%, 100% )", "hsl( 208, 100%, 35% )"],
		disabled: false,
	},
	dusk: {
		name: "Dusk",
...
function setProperty(elems, save) {
	...
	for (const el of elems) {
		switch (el) {
			...
			case elBarSpace:
				audioMotion.barSpace = audioMotion.isLumiBars
					? 1.5
					: elBarSpace.value;
				break;
			...
			case elFillAlpha:
				audioMotion.fillAlpha =
					elMode.value == 10 ? 1 : elFillAlpha.value;
				break;
			...
			case elLineWidth:
				audioMotion.lineWidth =
					elMode.value == 10 ? 0 : elLineWidth.value;
				break;
			...
			case elMode:
				...
				if (mode < 10) audioMotion.mode = mode;
				else {
					audioMotion.mode = 10;
					...
					if (mode == 10) {
						...
						// "Line graph" mode with custom line width and fill opacity
			...
			case elRandomMode:
				...
				if (option > 1)
					randomModeTimer = window.setInterval(
						selectRandomMode,
						2500 * option
					);
...
function setCurrentCover(url) {
	if (url) coverImage[currAudio].src = url;
	...
	} else audioMotion.canvas.style.backgroundImage = "";
...
function clearAudioElement(n = currAudio) {
	...
	if (n == currAudio) setCurrentCover(); // clear current background image
}
...
function loadSavedPlaylists(keyName) {
	...
	if (playlists) {
		...
		Object.keys(playlists).forEach((key) => {
			...
			if (key == keyName) item.selected = true;
			elPlaylists.options[elPlaylists.options.length] = item;
		});
	}
			...
			if (content !== false) {
				...
				content.split(/[\r\n]+/).forEach((line) => {
					if (line.charAt(0) != "#" && line.trim() != "") {
						...
						let info = line.split(/\|/);
						if (info.length == 2) {
							elPlaylists.options[elPlaylists.options.length] =
								new Option(info[0].trim(), info[1].trim());
							n++;
						}
					}
				});
				if (n) consoleLog(`${n} playlists loaded from playlists.cfg`);
				else consoleLog("No playlists found in playlists.cfg", true);
			}
		})
		.catch((e) => {});
}
...
function addBatchToPlayQueue(files, autoplay = false) {
	...
	Promise.all(promises).then((added) => {
		...
		notie.alert({
			text: `${total} song${total > 1 ? "s" : ""} added to the queue`,
			time: 5,
		});
	});
}
...
function addToPlayQueue(file, autoplay = false) {
	...
	if (["m3u", "m3u8"].includes(ext)) ret = loadPlaylist(file);
	else
...
function addMetadata(metadata, target) {
	...
	if (sourceData) {
		// just copy metadata from element dataset (playlist item)
...
function addSongToPlayQueue(uri, content = {}) {
	...
	if (len == 1 && !isPlaying()) loadSong(0);
	if (playlistPos > len - 3) loadNextSong();
	...
	fetch(uri)
		...
		.then((stream) => {
			mm.parseReadableStream(stream, "", { skipCovers: true })
				.then((metadata) => {
					if (metadata) {
...
function loadPlaylist(path) {
	...
	return new Promise((resolve) => {
		if (!path) {
			...
			fetch(path)
				.then((response) => {
					if (response.status == 200) return response.text();
						...
						consoleLog(
							`Fetch returned error code ${response.status} for URI ${path}`,
							true
						);
				})
				.then((content) => {
					...
					content.split(/[\r\n]+/).forEach((line) => {
						if (line.charAt(0) != "#" && line.trim() != "") {
							...
							n++;
							if (!songInfo) {
								// if no previous #EXTINF tag, extract info from the filename
								songInfo = line.slice(
									line.lastIndexOf("/") + 1
								);
								songInfo = songInfo
									.slice(0, songInfo.lastIndexOf("."))
									.replace(/_/g, " ");
							}
							if (
								line.slice(0, 4) != "http" &&
							...
							if (sep == -1)
								...
								addSongToPlayQueue(line, {
									artist: songInfo.slice(0, sep),
									title: songInfo.slice(sep + 3),
								});
							songInfo = "";
			...
			// try to load playlist from localStorage
			...
			} else consoleLog(`Unrecognized playlist file: ${path}`, true);
...
function savePlaylist(index) {
	if (elPlaylists[index].value == "") storePlaylist();
		...
		notie.confirm({
			text: `Overwrite "${elPlaylists[index].innerText}" with the current play queue?`,
...
function storePlaylist(name, update = true) {
	...
	if (!name) {
		notie.input({
			...
			submitCallback: (value) => {
				if (value) storePlaylist(value, false);
			},
			cancelCallback: () => {
				notie.alert({ text: "Canceled" });
			},
	...
	if (name) {
		...
		if (!update) {
			...
			while (playlists.hasOwnProperty(safename)) safename += "_1";
		...
		mm.fetchFromUrl(uri)
			.then((metadata) => {
				if (metadata.common.picture && metadata.common.picture.length) {
					...
					// in file mode there's nothing else we can do
					resolve("");
				} else {
					...
					if (folderImages[path] !== undefined)
						...
						fetch(urlToFetch)
							...
							.then((content) => {
								let imageUrl = "";
								if (content) {
									if (serverMode == 1) imageUrl = content;
									else {
										const dirContents =
											fileExplorer.parseWebDirectory(
												content
											);
...
function loadNextSong() {
	const next =
			playlistPos < playlist.children.length - 1 ? playlistPos + 1 : 0,
	...
	if (song) {
		...
		loadCover(song.dataset.file).then(
			(cover) => (coverImage[nextAudio].src = cover)
		);
	}
...
function playPause(play) {
	...
	if (isPlaying() && !play) audioElement[currAudio].pause();
	else
...
function skipTrack(back = false) {
	...
	if (!status) setCanvasMsg(`Already at ${back ? "first" : "last"} track`);
}
...
function playPreviousSong() {
	if (isPlaying()) {
		...
		else if (playlistPos > 0) playSong(playlistPos - 1);
		else return false;
	} else return loadSong(playlistPos - 1);
}
...
function playNextSong(play) {
	...
	if (playlistPos < playlist.children.length - 1) playlistPos++;
	else if (isSwitchOn(elRepeat)) playlistPos = 0;
	else {
		skipping = false;
		return false;
	}
...
function fastSearch(dir = 1) {
	...
	if (audioEl.duration > 0 && audioEl.duration < Infinity) {
		...
		else if (newPos > audioEl.duration - 1) newPos = audioEl.duration - 1;
	...
	if (isFastSearch == "m") scheduleFastSearch("m", dir);
}
...
function isPlaying() {
	...
	return (
		audioEl && audioEl.currentTime > 0 && !audioEl.paused && !audioEl.ended
	);
...
function toggleInfo() {
	if (canvasMsg.info == 2)
		// if already showing all info, turn it off
		setCanvasMsg();
	else {
...
function outlineText(text, x, y, maxWidth) {
	...
	if (isSwitchOn(elNoShadow)) {
		...
		canvasCtx.shadowOffsetX = canvasCtx.shadowOffsetY =
			3 * audioMotion.pixelRatio;
		canvasCtx.fillText(text, x, y, maxWidth);
...
function formatHHMMSS(time) {
	...
	str +=
		(lead + Math.floor(time / 60)).slice(-2) +
		":" +
...
function displayCanvasMsg() {
	...
	if (remaining < 0.1) playNextSong(true);

	// set song info display at the end of the song
	if (
		endTimeout > 0 &&
		remaining <= endTimeout &&
		isSwitchOn(elShowSong) &&
		!canvasMsg.info &&
		isPlaying()
	)
	...
	if ((canvasMsg.timer || canvasMsg.msgTimer) < 1) return;
	...
	const canvas = audioMotion.canvas,
		...
		normalFont = `bold ${fontSize * 0.7}px sans-serif`,
	...
	if (canvasMsg.msgTimer > 0 && canvasMsg.info != 2) {
		canvasCtx.globalAlpha =
			canvasMsg.msgTimer < 60 ? canvasMsg.msgTimer / 60 : 1;
	...
	if (canvasMsg.timer > 0) {
		...
		if (canvasMsg.fade < 0) {
			...
			canvasCtx.globalAlpha =
				canvasMsg.timer < canvasMsg.fade
					? canvasMsg.timer / canvasMsg.fade
					: 1;
		...
		if (canvasMsg.info == 2) {
			...
			outlineText(
				"Gradient: " + gradients[elGradient.value].name,
				centerPos,
				topLine,
				maxWidthTop
			);
			outlineText(
				`Auto gradient is ${onOff(elCycleGrad)}`,
				centerPos,
				secondLine
			);
			...
			outlineText(
				`Random mode: ${getText(elRandomMode)}`,
				leftPos,
				secondLine,
				maxWidthTop
			);
			...
			outlineText(
				getText(elSensitivity).toUpperCase() + " sensitivity",
				rightPos,
				topLine,
				maxWidthTop
			);
			outlineText(
				`Repeat is ${onOff(elRepeat)}`,
				rightPos,
				secondLine,
				maxWidthTop
			);
		...
		if (isMicSource) {
			...
			canvasCtx.font = largeFont;
			outlineText(
				audioEl.src ? trackData.title : "No song loaded",
				leftPos,
				bottomLine2,
				maxWidth
			);
				...
				if (!trackData.duration) {
					trackData.duration =
						audioEl.duration === Infinity
							? "LIVE"
							: formatHHMMSS(audioEl.duration);

					if (playlist.children[playlistPos])
						playlist.children[playlistPos].dataset.duration =
							trackData.duration;
				}
				canvasCtx.textAlign = "right";

				outlineText(
					formatHHMMSS(audioEl.currentTime) +
						" / " +
						trackData.duration,
					rightPos,
					bottomLine3
			...
			if (coverImage[currAudio].width && elShowCover.checked) {
				...
				canvasCtx.drawImage(
					coverImage[currAudio],
					leftPos,
					bottomLine1 - coverSize * 1.3,
					coverSize,
					coverSize
				);
			}
		...
		if (--canvasMsg.timer < 1) canvasMsg.info = canvasMsg.fade = 0;
	}
}
...
function setCanvasMsg(msg, timer = 2, dir = 1) {
	if (!msg) canvasMsg = { timer: 0, msgTimer: 0 };
	// clear all canvas messages
	else {
		if (typeof msg == "number") {
			canvasMsg.info = msg; // set info level 1 or 2
			canvasMsg.timer = Math.round(
				Math.max(timer * 60, canvasMsg.timer || 0)
			); // note: Infinity | 0 == 0
			canvasMsg.fade = Math.round(canvasMsg.timer / 3) * dir;
		} else {
			canvasMsg.msg = msg; // set custom message
			if (canvasMsg.info == 2) canvasMsg.info = 1;
...
function showCanvasInfo(reason, instance) {
	...
	switch (reason) {
		case "create":
			consoleLog(
				`Display resolution: ${instance.fsWidth} x ${instance.fsHeight} pixels`
			);
			consoleLog(`Display pixel ratio: ${window.devicePixelRatio}`);
			msg = "Canvas created";
			break;
		case "lores":
			msg = `Lo-res mode ${instance.loRes ? "ON" : "OFF"} (pixelRatio = ${
				instance.pixelRatio
			})`;
			break;
		case "fschange":
	...
	consoleLog(
		`${msg || reason}. Canvas size is ${instance.canvas.width} x ${
			instance.canvas.height
		} px ${instance.isFullscreen ? "(fullscreen)" : ""}`
	);
}
...
function consoleLog(msg, error, clear) {
	...
	if (error) $("#toggle_console").classList.add("warning");
		...
		if (!micStream) {
			...
			if (navigator.mediaDevices) {
				navigator.mediaDevices
					...
					.catch((err) => {
						consoleLog(
							`Could not change audio source - ${err}`,
							true
						);
			...
			if (isPlaying()) audioElement[currAudio].pause();
...
function changeVolume(incr) {
	let newVal = Number(elVolume.dataset.value || 0) + incr * 0.05;
...
function setVolume(value) {
	...
	elVolume.querySelector(".marker").style.transform = `rotate( ${
		125 + 290 * value
	}deg )`;
}
...
function loadPreset(name, alert, init) {
	...
	if (thisPreset.hasOwnProperty("randomMode"))
		// convert legacy boolean value to integer (version =< 19.12)
		thisPreset.randomMode |= 0;

	if (thisPreset.hasOwnProperty("blackBg"))
		// convert legacy blackBg property (version =< 20.4)
		thisPreset.background = thisPreset.blackBg | 0;

	if (thisPreset.hasOwnProperty("showScale")) {
		// convert legacy showScale property (version =< 20.9)
	...
	if (thisPreset.hasOwnProperty("reflex") && isNaN(thisPreset.reflex))
		// convert legacy string value to integer (version =< 20.6)
		thisPreset.reflex = ["off", "on", "mirror"].indexOf(thisPreset.reflex);

	if (thisPreset.hasOwnProperty("bgImageFit") && isNaN(thisPreset.bgImageFit))
		// convert legacy string value to integer (version =< 20.6)
		thisPreset.bgImageFit = [
			"adjust",
			"center",
			"repeat",
			"pulse",
	...
	if (thisPreset.hasOwnProperty("highSens"))
		// legacy option (version =< 19.5)
		elSensitivity.value = thisPreset.highSens ? 2 : 1;

	if (
		thisPreset.hasOwnProperty("minDb") &&
		thisPreset.hasOwnProperty("maxDb")
	) {
		// legacy options (version =< 19.12)
	...
	if (name == "demo") selectRandomMode(true);

	if (alert) notie.alert({ text: "Settings loaded!" });
}
...
function keyboardControls(event) {
	if (event.target.tagName != "BODY" || event.altKey || event.ctrlKey) return;
	...
	if (event.type == "keydown") {
		...
		switch (event.code) {
			...
			case "Backspace": // for Mac
				...
				if (current !== undefined) playlistPos = current;
				// update playlistPos if current song hasn't been deleted
				else if (playlistPos > queueLength - 1)
					playlistPos = queueLength - 1;
				else playlistPos--;
				if (queueLength) loadNextSong();
				else {
					clearAudioElement(nextAudio);
					if (!isPlaying()) clearAudioElement();
				}
				break;
			...
			case "KeyA": // cycle thru auto gradient / random mode options
				if (isSwitchOn(elCycleGrad) || isShiftKey) {
					if (
						(elRandomMode.selectedIndex ==
							elRandomMode.options.length - 1 &&
							!isShiftKey) ||
						(elRandomMode.selectedIndex == 0 &&
							isSwitchOn(elCycleGrad) &&
							isShiftKey)
					) {
			...
			case "KeyB": // background or image fit (shift)
				...
				setCanvasMsg(
					"Background: " +
						getText(elBackground) +
						(elBackground.value > 1
							? ` (${getText(elBgImageFit)})`
							: "")
				);
				break;
			case "KeyC": // radial
			...
			case "KeyO": // toggle resolution
				elLoRes.click();
				setCanvasMsg(
					(isSwitchOn(elLoRes) ? "LOW" : "HIGH") + " Resolution"
				);
				break;
			...
			case "KeyS": // toggle X and Y axis scales
				setCanvasMsg(
					"Scale: " +
						["None", "Frequency (Hz)", "Level (dB)", "Both"][
							cycleScale(isShiftKey)
						]
				);
				break;
			case "KeyT": // toggle text shadow
				elNoShadow.click();
				setCanvasMsg(
					(isSwitchOn(elNoShadow) ? "Flat" : "Shadowed") +
						" text mode"
				);
				break;
...
function audioOnPlay() {
	...
	if (audioElement[currAudio].currentTime < 0.1) {
		if (elRandomMode.value == "1") selectRandomMode(true);
...
function updateRangeValue(el) {
	...
	if (elVal && elVal.className == "value") elVal.innerText = el.value;
}
...
function selectRandomMode(force = isMicSource) {
	...
	const isEnabled = (prop) =>
		!randomProperties.find((item) => item.value == prop).disabled;
	...
	if (isEnabled("lumi")) {
		...
		elLumiBars.dataset.active =
			elBackground.value > 1 && isSwitchOn(elLedDisplay)
				? 0
				: randomInt();
...
function cycleElement(el, prev) {
	...
	if (idx < 0) el.selectedIndex = el.options.length - 1;
	else if (idx >= el.options.length) el.selectedIndex = 0;
...
function cycleScale(prev) {
	let scale =
		(elScaleX.dataset.active | 0) +
		(elScaleY.dataset.active << 1) +
		(prev ? -1 : 1);
...
function populateVisualizationModes() {
	...
	while (elMode.firstChild) elMode.removeChild(elMode.firstChild);
...
function populateGradients() {
	...
	while (elGradient.firstChild) elGradient.removeChild(elGradient.firstChild);
	...
	for (const key of Object.keys(gradients)) {
		if (!gradients[key].disabled)
			elGradient.options[elGradient.options.length] = new Option(
				gradients[key].name,
				key
			);
	}
...
function setUIEventListeners() {
	...
	$("#console-close").addEventListener("click", () =>
		elToggleConsole.click()
	);
	$("#console-clear").addEventListener("click", () =>
		consoleLog("Console cleared.", false, true)
	);

	// Add event listeners to the custom checkboxes
	$$(".switch").forEach((el) => {
		el.addEventListener("click", () => {
			el.dataset.active = !(el.dataset.active | 0) | 0;
		});
	});
	...
	[
		...
		el.addEventListener("click", () => setProperty(el, true))
	);

	[elCycleGrad, elRepeat, elShowSong, elNoShadow].forEach((el) =>
		el.addEventListener("click", updateLastConfig)
	);
	...
	[
		...
		el.addEventListener("change", () => setProperty(el, true))
	);

	// update range elements' value
	$$('input[type="range"]').forEach((el) =>
		el.addEventListener("change", () => updateRangeValue(el))
	);
	...
	$("#btn_prev").addEventListener("mousedown", () =>
		scheduleFastSearch("m", -1)
	);
	$("#btn_prev").addEventListener("click", (e) => {
		if (!finishFastSearch()) skipTrack(true);
	});

	$("#btn_next").addEventListener("mousedown", () => scheduleFastSearch("m"));
	$("#btn_next").addEventListener("click", () => {
		if (!finishFastSearch()) skipTrack();
	});
		...
		loadPlaylist(elPlaylists.value).then((n) => {
			const text =
				n == -1
					...
					: `${n} song${n > 1 ? "s" : ""} added to the queue`;
	...
	$("#btn_load_url").addEventListener("click", () => {
		notie.input({
			...
			submitText: "Load",
			submitCallback: (url) => {
				if (url.trim()) addToPlayQueue(url, true);
			},
		});
	});
}
...
function doConfigPanel() {
	...
	modeOptions.forEach((mode) => {
		elEnabledModes.innerHTML += `<label><input type="checkbox" class="enabledMode" data-mode="${
			mode.value
		}" ${mode.disabled ? "" : "checked"}> ${mode.text}</label>`;
	});

	$$(".enabledMode").forEach((el) => {
		el.addEventListener("click", (event) => {
			if (!el.checked) {
				const count = modeOptions.filter(
					(item) => !item.disabled
				).length;
				if (count < 2) {
			...
			modeOptions.find((item) => item.value == el.dataset.mode).disabled =
				!el.checked;
	...
	Object.keys(gradients).forEach((key) => {
		elEnabledGradients.innerHTML += `<label><input type="checkbox" class="enabledGradient" data-grad="${key}" ${
			gradients[key].disabled ? "" : "checked"
		}> ${gradients[key].name}</label>`;
	});

	$$(".enabledGradient").forEach((el) => {
		el.addEventListener("click", (event) => {
			if (!el.checked) {
				const count = Object.keys(gradients).reduce(
					(acc, val) => acc + !gradients[val].disabled,
					0
				);
				if (count < 2) {
					notie.alert({
	...
	randomProperties.forEach((prop) => {
		elProperties.innerHTML += `<label><input type="checkbox" class="randomProperty" value="${
			prop.value
		}" ${prop.disabled ? "" : "checked"}> ${prop.text}</label>`;
	});

	$$(".randomProperty").forEach((el) => {
		el.addEventListener("click", (event) => {
			randomProperties.find((item) => item.value == el.value).disabled =
				!el.checked;
	...
	$$("[data-preset]").forEach((el) => {
		if (el.className == "reset-sens") {
			el.addEventListener("click", () => {
				$(`.min-db[data-preset="${el.dataset.preset}"]`).value =
					sensitivityDefaults[el.dataset.preset].min;
				$(`.max-db[data-preset="${el.dataset.preset}"]`).value =
					sensitivityDefaults[el.dataset.preset].max;
				if (el.dataset.preset == elSensitivity.value)
					// current preset has been changed
...
function loadPreferences() {
	...
	if (isLastSession) presets["last"] = JSON.parse(lastConfig);
	// if no data found from last session, use the defaults
	else presets["last"] = JSON.parse(JSON.stringify(presets["default"]));

	// Load custom preset
	const customPreset = localStorage.getItem("custom-preset");
	if (customPreset !== null) presets["custom"] = JSON.parse(customPreset);
	else presets["custom"] = JSON.parse(JSON.stringify(presets["last"]));
	...
	setInfoOptions(
		Object.assign(
			displayOptions,
			JSON.parse(localStorage.getItem("display-options")) || {}
		)
	);
...
function savePreferences(pref) {
	if (!pref || pref == "mode") {
		const disabledModes = modeOptions
	...
	if (!pref || pref == "grad") {
		const disabledGradients = Object.keys(gradients).filter(
			(key) => gradients[key].disabled
		);
		localStorage.setItem(
			"disabled-gradients",
			JSON.stringify(disabledGradients)
		);
	}

	if (!pref || pref == "prop") {
		const disabledProperties = randomProperties
			.filter((item) => item.disabled)
			.map((item) => item.value);
		localStorage.setItem(
			"disabled-properties",
			JSON.stringify(disabledProperties)
		);
	}
	...
	if (!pref || pref == "sens") {
		...
		localStorage.setItem(
			"sensitivity-presets",
			JSON.stringify(sensitivityPresets)
		);
	}
...
(function () {
	...
	window.addEventListener("error", (event) =>
		consoleLog(`Unexpected ${event.error}`, true)
	);
	...
	Sortable.create(playlist, {
		...
		onEnd: (evt) => {
			...
			if (evt.newIndex == 0 && !isPlaying()) loadSong(0);
			loadNextSong();
		},
	});
	...
	consoleLog(
		`Instantiating audioMotion-analyzer v${AudioMotionAnalyzer.version}`
	);
	...
	consoleLog(
		`AudioContext sample rate is ${audioMotion.audioCtx.sampleRate}Hz`
	);
	...
	for (const i of [20, 30, 40, 50, 60, 100, 250, 500, 1000, 2000])
		elRangeMin[elRangeMin.options.length] = new Option(
			(i >= 1000 ? i / 1000 + "k" : i) + "Hz",
			i
		);
	...
	Object.keys(gradients).forEach((key) => {
		if (gradients[key].bgColor && gradients[key].colorStops)
			audioMotion.registerGradient(key, {
				bgColor: gradients[key].bgColor,
				colorStops: gradients[key].colorStops,
			});
	});
	populateGradients();
	...
	// Load last used settings
	consoleLog(
		`Loading ${isLastSession ? "last session" : "default"} settings`
	);
	...
	fileExplorer
		...
		.then(([status, filelist, serversignature]) => {
			...
			if (status == -1) {
				...
				Sortable.create(filelist, {
					...
					onEnd: (evt) => {
						if (evt.to.id == "playlist") {
							let items = evt.items.length
								? evt.items
								: [evt.item];
							items.forEach((item) => {
								addToPlayQueue(
									fileExplorer.makePath(item.dataset.path)
								);
								item.remove();
							});
						}
				...
				btnAddSelected.addEventListener("mousedown", () =>
					addBatchToPlayQueue(
						fileExplorer.getFolderContents(".selected")
					)
				);
				btnAddFolder.addEventListener("click", () =>
					addBatchToPlayQueue(fileExplorer.getFolderContents())
				);
			}
		});
