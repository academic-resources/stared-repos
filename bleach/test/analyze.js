  ...
  .addBatch({
    "bleach.analyze(html)": {
      ...
      "extracts attributes": function (analyze) {
        assert.equal(
          analyze('<input type="text"></input>')[0].attr[0],
          { name: "type", value: '"text"' }.toString()
        );
      },
    },
    ...
    "bleach.sanitize(html, options)": {
      ...
      "whitelist is respected": function (sanitize) {
        var whitelist = ["br"],
          input =
            '<html><body><p>hello<br />world!</p></body></html><script type="text/javascript" />',
