...
class FirefoxAllPage(BasePage):
    ...
    _android_nightly_download_button_locator = (
        By.ID,
    ...
    def _options(self):
        return [
            ProductOptions(self, root=el)
            for el in self.find_elements(*self._product_options_locator)
        ]
    ...
    def is_desktop_download_link_valid(self):
        return "https://download.mozilla.org" in self.find_element(
            *self._desktop_download_button_locator
        ).get_attribute("href")
    ...
    def select_product(self, value):
        ...
        option = [
            o for o in self._options if selected == o.root.get_attribute("data-product")
        ]
        assert len(option) == 1
