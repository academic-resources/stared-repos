...
def session_capabilities(pytestconfig, session_capabilities):
    ...
    if pytestconfig.getoption("driver") == "BrowserStack":
        ...
        if (
            session_capabilities.get("browser").lower() == "internet explorer"
            and session_capabilities.get("browser_version") == "11.0"
        ):
...
def filter_capabilities(request):
    marker = None
    if request.node.get_closest_marker("skip_if_firefox") and request.getfixturevalue(
        "firefox"
    ):
        marker = request.node.get_closest_marker("skip_if_firefox")
    if request.node.get_closest_marker(
        "skip_if_not_firefox"
    ) and not request.getfixturevalue("firefox"):
        marker = request.node.get_closest_marker("skip_if_not_firefox")
    if request.node.get_closest_marker(
        "skip_if_internet_explorer"
    ) and request.getfixturevalue("internet_explorer"):
...
def pytest_generate_tests(metafunc):
    ...
    if markexpr == "download":
        ...
        if "download_path" in metafunc.fixturenames:
            ...
            urls = [
                a["href"] for a in soup.find("ul", class_="download-list").find_all("a")
            ]
            ...
            urls = [
                a["href"] for a in lists.find_all(attrs={"data-link-type": "download"})
            ]
            assert urls
