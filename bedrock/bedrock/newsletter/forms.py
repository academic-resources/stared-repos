...
def get_lang_choices(newsletters=None):
    ...
    for lang in languages:
        if lang in product_details.languages:
            ...
            try:
                locale = [
                    loc for loc in product_details.languages if loc.startswith(lang)
                ][0]
            except IndexError:
                continue
...
class BooleanTabularRadioSelect(widgets.RadioSelect):
    ...
    def format_value(self, value):
        try:
            return {True: "true", False: "false", "true": "true", "false": "false"}[
                value
            ]
        except KeyError:
            return "unknown"
...
class ManageSubscriptionsForm(forms.Form):
    ...
    country = forms.ChoiceField(
        choices=[], required=False  # will set choices based on locale
    )
    lang = forms.ChoiceField(
        choices=[], required=False  # will set choices based on newsletter languages
    )
    ...
    def clean(self):
        ...
        for newsletter in self.newsletters:
            if newsletter not in valid_newsletters:
                msg = ftl(
                    "newsletters-is-not-a-valid-newsletter",
...
class NewsletterFooterForm(forms.Form):
    ...
    newsletters = forms.CharField(
        widget=forms.HiddenInput, required=True, max_length=100
    )
    ...
    def __init__(self, newsletters, locale, data=None, *args, **kwargs):
        ...
        required_args = {"required": "required", "aria-required": "true"}
        country_widget = widgets.Select(attrs=required_args)
        self.fields["country"] = forms.ChoiceField(
            widget=country_widget, choices=regions, initial=country, required=False
        )
        lang_widget = widgets.Select(attrs=required_args)
        self.fields["lang"] = forms.TypedChoiceField(
            widget=lang_widget, choices=lang_choices, initial=lang, required=False
        )
