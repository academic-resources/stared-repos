...
def test_limited_request(req_mock):
    ...
    req_mock.get.assert_called_once_with(
        "some_url", params={"per_page": 10, "page": 1}, timeout=5
    )
...
def test_api_url():
    assert (
        api._api_url("https://moz.blog/", "posts", 4)
        == "https://moz.blog/wp-json/wp/v2/posts/4"
    )
    assert (
        api._api_url("https://moz.blog/", "tags", None)
        == "https://moz.blog/wp-json/wp/v2/tags/"
    )
    assert (
        api._api_url("https://moz.blog", "media", 55)
