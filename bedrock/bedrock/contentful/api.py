...
LOCALE_MAP = {"de": "de-DE"}
ASPECT_RATIOS = {"1:1": "1-1", "3:2": "3-2", "16:9": "16-9"}
...
def _get_card_image_url(image, width, aspect):
    return "https:" + image.url(
        w=width, h=_get_height(width, aspect), fit="fill", f="faces"
    )
...
def _make_logo(entry):
    ...
    data = {
        ...
        "icon_size": WIDTHS.get(fields.get("icon_size"), "")
        if fields.get("icon_size")
        else "md",
    }

    return render_to_string(
        "includes/contentful/logo.html", data, get_current_request()
    )
...
def _make_wordmark(entry):
    ...
    return render_to_string(
        "includes/contentful/wordmark.html", data, get_current_request()
    )
...
class PRenderer(BaseBlockRenderer):
    def render(self, node):
        ...
        # contains only an empty text node
            ...
            len(node["content"]) == 1
            and node["content"][0]["nodeType"] == "text"
            and node["content"][0]["value"] == ""
        ):
...
class ContentfulPage:
    ...
    def get_hero_data(self, entry_obj):
        ...
        product_class = (
            ...
            if fields.get("product_icon") and fields.get("product_icon") != "None"
            else ""
        )
        data = {
            "component": "hero",
    ...
    def get_section_data(self, entry_obj):
        ...
        data = {"component": "sectionHeading", "heading": fields.get("heading")}
    ...
    def get_split_data(self, entry_obj):
        ...
        def get_mobile_class():
            ...
            mobile_classes = [
                ...
                if "Center content" in mobile_display
                else "",
                "mzp-l-split-hide-media-on-sm-md"
                if "Hide image" in mobile_display
                else "",
            ]
    ...
    def get_card_data(self, entry_obj, aspect_ratio):
        ...
        fields = entry_obj.fields()
        card_body = (
            self.render_rich_text(fields.get("body")) if fields.get("body") else ""
        )
    ...
    def get_picto_data(self, picto_obj, image_width):
        ...
        body = (
            self.render_rich_text(fields.get("body")) if fields.get("body") else False
        )
        ...
        if "icon" in fields:
            ...
            image_url = (
                ""
            )  # TODO: this should cause an error, the macro requires an image
    ...
    def get_picto_layout_data(self, entry):
        ...
        def get_layout_class():
            ...
            layout_classes = [
                ...
                "mzp-t-picto-center"
                if fields.get("block_alignment") == "Center"
                else "",
        ...
        image_width = (
            PICTO_ICON_SIZE.get(fields.get("icon_size"))
            if fields.get("icon_size")
            else PICTO_ICON_SIZE.get("Large")
        )

        data = {
            "component": "pictoLayout",
            "layout_class": get_layout_class(),
            "heading_level": fields.get("heading_level")[1:]
            if fields.get("heading_level")
            else 3,
