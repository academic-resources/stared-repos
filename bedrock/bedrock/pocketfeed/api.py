...
def complete_articles_data(articles):
    for _, article in articles:
        ...
        article["time_shared"] = make_aware(
            datetime.datetime.fromtimestamp(int(article["time_shared"])), utc
        )
...
def check_article_image(article):
    ...
    if article["image_src"] and re.match(
        r"^https://", article["image_src"], flags=re.I
    ):
        try:
