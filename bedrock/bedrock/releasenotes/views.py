...
def release_notes_template(channel, product, version=None):
    ...
    return "{dir}/releases/{channel}-notes.html".format(
        dir=dir, channel=channel.lower()
    )


def equivalent_release_url(release):
    equivalent_release = (
        release.equivalent_android_release() or release.equivalent_desktop_release()
    )
...
def system_requirements(request, version, product="Firefox"):
    ...
    return l10n_utils.render(
        request,
        "{dir}/releases/system_requirements.html".format(dir=dir),
        ...
        releases[major_version] = {
            ...
            "minor": sorted(
                [
                    ...
                    if re.findall(major_pattern, x)
                    if x not in major_releases
                ],
...
def nightly_feed(request):
    ...
    for release in releases:
        try:
            link = reverse(
                "firefox.desktop.releasenotes", args=(release.version, "release")
            )
    ...
    return l10n_utils.render(
        request,
        "firefox/releases/nightly-feed.xml",
        {"notes": notes},
