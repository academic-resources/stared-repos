...
class TestReleaseViews(TestCase):
    ...
    def test_get_release_or_404(self, get_release):
        assert (
            views.get_release_or_404("version", "product") == get_release.return_value
        )
    ...
    def test_release_notes(
        self, mock_equiv_rel_url, get_release_or_404, mock_release_notes_template
    ):
        """
        ...
        assert (
            self.mock_render.call_args[0][1] == mock_release_notes_template.return_value
        )
    ...
    def test_non_public_release(self):
        ...
        assert not views.get_release_or_404(
            "58.0a1", "Firefox for Android", True
        ).is_public
...
class TestNotesRedirects(TestCase):
    ...
    @patch(
        ...
        Mock(
            return_value=ProductRelease(
                product="Firefox for Android", version="23.0beta", channel="Beta"
            )
        ),
    )
    def test_android_beta_version(self):
        self._test(
            "/firefox/android/beta/notes/", "/firefox/android/23.0beta/releasenotes/"
        )
    ...
    @patch(
        ...
        Mock(
            return_value=ProductRelease(
                product="Firefox for Android", version="24.0a2", channel="Aurora"
            )
        ),
    )
    def test_android_aurora_version(self):
        self._test(
            "/firefox/android/aurora/notes/", "/firefox/android/24.0a2/auroranotes/"
        )
...
class TestSysreqRedirect(TestCase):
    ...
    def test_desktop_release_version(self):
        self._test(
            "/firefox/system-requirements/", "/firefox/22.0/system-requirements/"
        )
