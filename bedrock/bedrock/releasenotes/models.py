...
ALLOWED_ATTRS = ["alt", "class", "href", "id", "title"]


def process_markdown(value):
    return markdowner.reset().convert(
        bleach.clean(value, tags=ALLOWED_TAGS, attributes=ALLOWED_ATTRS)
    )
...
class ProductReleaseManager(models.Manager):
    ...
    def product(
        self, product_name, channel_name=None, version=None, include_drafts=False
    ):
        return self.get_queryset(include_drafts).product(
            product_name, channel_name, version
        )
...
class ProductRelease(models.Model):
    ...
    def equivalent_release_for_product(self, product):
        ...
        """
        releases = ProductRelease.objects.product(product, self.channel).filter(
            version__startswith="%s." % self.major_version
        )
        if releases:
...
def get_release(product, version, channel=None, include_drafts=False):
    ...
    for channel in channels:
        try:
            return ProductRelease.objects.product(
                product, channel, version, include_drafts
            ).get()
