...
class TestHeaderRedirector(TestCase):
    ...
    def test_header_redirects_case_sensitive(self):
        callback = header_redirector(
            "user-agent", "dude", "/abide/", "/flout/", case_sensitive=True
        )
        url = callback(
            self.rf.get("/take/comfort/", HTTP_USER_AGENT="The Dude Browses")
        )
...
class TestIsFirefoxRedirector(TestCase):
    ...
    def test_firefox_redirects(self):
        ...
        url = callback(
            self.rf.get("/take/comfort/", HTTP_USER_AGENT="Mozilla Firefox/42.0")
        )
        self.assertEqual(url, "/abide/")
...
class TestRedirectUrlPattern(TestCase):
    ...
    def test_merge_query(self):
        ...
        """
        pattern = redirect(
            r"^the/dude$", "abides", query={"aggression": "not_stand"}, merge_query=True
        )
        ...
        self.assertEqual(
            query_dict, {"aggression": ["not_stand"], "hates": ["the-eagles"]}
        )
    ...
    def test_to_view(self, mock_reverse):
        ...
        mock_reverse.assert_called_with(
            "yeah.well.you.know.thats", args=None, kwargs=None
        )
        assert response.status_code == 301
        assert response["Location"] == "/just/your/opinion/man"
    ...
    def test_to_view_args_kwargs(self, mock_reverse):
        ...
        pattern = redirect(
            r"^the/dude$",
            "yeah.well.you.know.thats",
            to_args=["dude"],
            to_kwargs={"tapes": "credence"},
        )
        request = self.rf.get("the/dude")
        response = pattern.callback(request)
        mock_reverse.assert_called_with(
            "yeah.well.you.know.thats", args=["dude"], kwargs={"tapes": "credence"}
        )
        assert response.status_code == 301
        assert response["Location"] == "/just/your/opinion/man"
    ...
    def test_value_capture_and_substitution(self):
        ...
        """
        resolver = get_resolver(
            [redirect(r"^iam/the/(?P<name>.+)/$", "/donnie/the/{name}/")]
        )
        middleware = RedirectsMiddleware(resolver=resolver)
        resp = middleware.process_request(self.rf.get("/iam/the/walrus/"))
        assert resp.status_code == 301
    ...
    def test_no_locale_prefix(self):
        ...
        """
        resolver = get_resolver(
            [redirect(r"^iam/the/(.+)/$", "/donnie/the/{}/", locale_prefix=False)]
        )
        middleware = RedirectsMiddleware(resolver=resolver)
        resp = middleware.process_request(self.rf.get("/iam/the/walrus/"))
        assert resp.status_code == 301
