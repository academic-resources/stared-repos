...
HEALTH_FILES = (("download_database", 600), ("update_locales", 600))
DB_INFO_FILE = getenv(
    "AWS_DB_JSON_DATA_FILE", f"{settings.DATA_PATH}/bedrock_db_info.json"
)
...
def get_extra_server_info():
    ...
    server_info = {"name": server_name, "git_sha": GIT_SHA}
    try:
...
def cron_health_check(request):
    ...
    git_repos = git.GitRepoState.objects.exclude(repo_name="").order_by(
        "repo_name", "-latest_ref_timestamp"
    )
    unique_repos = {}
