...
def csrf_failure(request, reason=""):
    ...
    return l10n_utils.render(
        request, "mozorg/csrf-failure.html", template_vars, status=403
    )
    ...
    if locale.startswith("en-"):
        if switch("contentful-homepage-en"):
            try:
                ...
                ctx.update(
                    ContentfulEntry.objects.get_page_by_id("58YIvwDmzSDjtvpSqstDcL")
                )
            except Exception:
        ...
        if switch("contentful-homepage-de"):
            try:
                template_name = "mozorg/contentful-homepage.html"
                ctx.update(
                    ContentfulEntry.objects.get_page_by_id("4k3CxqZGjxXOjR1I0dhyto")
                )
            except Exception:
                # if anything goes wrong, use the old page
