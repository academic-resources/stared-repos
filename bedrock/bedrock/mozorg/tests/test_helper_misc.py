...
def test_convert_to_high_res():
    assert (
        misc.convert_to_high_res("/media/img/the.dude.png")
...
class TestImgL10n(TestCase):
    ...
    def test_works_for_default_lang(self, media_exists_mock):
        ...
        assert self._render("en-US", "dino/head.png") == static(
            "img/l10n/en-US/dino/head.png"
        )
        assert self._render("en-US", "img/dino/head.png") == static(
            "img/l10n/en-US/dino/head.png"
        )

        assert self._render("en-US", "dino/does-not-exist.png") == static(
            "img/l10n/en-US/dino/does-not-exist.png"
        )

    def test_works_for_other_lang(self, media_exists_mock):
        """Should use the request lang if file exists."""
        media_exists_mock.return_value = True
        assert self._render("de", "dino/head.png") == static(
            "img/l10n/de/dino/head.png"
        )
        assert self._render("de", "img/dino/head.png") == static(
            "img/l10n/de/dino/head.png"
        )
    ...
    def test_defaults_when_lang_file_missing(self, media_exists_mock):
        ...
        assert self._render("is", "dino/head.png") == static(
            "img/l10n/en-US/dino/head.png"
        )
    ...
    def test_latam_spanishes_fallback_to_european_spanish(self, media_exists_mock):
        ...
        assert self._render("es-AR", "dino/head.png") == static(
            "img/l10n/es-ES/dino/head.png"
        )

        media_exists_mock.reset_mock()
        media_exists_mock.side_effect = [False, True]
        assert self._render("es-CL", "dino/head.png") == static(
            "img/l10n/es-ES/dino/head.png"
        )

        media_exists_mock.reset_mock()
        media_exists_mock.side_effect = [False, True]
        assert self._render("es-MX", "dino/head.png") == static(
            "img/l10n/es-ES/dino/head.png"
        )

        media_exists_mock.reset_mock()
        media_exists_mock.side_effect = [False, True]
        assert self._render("es", "dino/head.png") == static(
            "img/l10n/es-ES/dino/head.png"
        )
    ...
    def test_file_not_checked_for_default_lang(self, media_exists_mock):
        ...
        media_exists_mock.assert_called_once_with(
            "img", "is", "dino/does-not-exist.png"
        )
...
class TestL10nCSS(TestCase):
    ...
    @patch(
        "django.contrib.staticfiles.storage.staticfiles_storage.base_url",
        static_url_dev,
    )
...
class TestVideoTag(TestCase):
    ...
    def _render(self, template):
        ...
        req.locale = "en-US"
        return render(
            template, {"request": req, "fluent_l10n": self.get_l10n(req.locale)}
        )
    ...
    def test_video(self):
        ...
        extensions = [
            os.path.splitext(el.attrib["src"])[1] for el in doc("video source")
        ]
...
class TestPlatformImg(TestCase):
    ...
    def _render(self, url, optional_attributes=None):
        ...
        req.locale = "en-US"
        return render(
            "{{{{ platform_img('{0}', {1}) }}}}".format(url, optional_attributes),
            {"request": req},
        )
    ...
    def test_platform_img_with_high_res(self, find_static):
        ...
        self.assertIn(
            u'data-src-windows-high-res="/media/test-windows-high-res.png"', markup
        )
        ...
        self.assertIn(
            u'data-src-windows-high-res="/media/img/test-windows-high-res.png"', markup
        )
        self.assertIn(
            u'data-src-mac-high-res="/media/img/test-mac-high-res.png"', markup
        )
        self.assertIn(u'data-high-res="true"', markup)
...
@override_settings(DONATE_LINK=TEST_DONATE_LINK, DONATE_PARAMS=TEST_DONATE_PARAMS)
...
class TestHighResImg(TestCase):
    ...
    def _render(self, url, optional_attributes=None):
        ...
        req.locale = "en-US"
        return render(
            "{{{{ high_res_img('{0}', {1}) }}}}".format(url, optional_attributes),
            {"request": req},
        )
    ...
    def test_high_res_img_with_optional_attributes(self):
        """Should return expected markup with optional attributes"""
        markup = self._render(
            "img/test.png", {"data-test-attr": "test", "class": "logo"}
        )
        expected = (
...
class TestLazyImg(TestCase):
    ...
    def _render(
        ...
        req.locale = "en-US"
        return render(
            "{{{{ lazy_img('{0}', '{1}', {2}, {3}) }}}}".format(
                image_url, placeholder_url, include_highres_image, optional_attributes
            ),
            {"request": req},
        )
    ...
    def test_lazy_img_no_highres_image(self):
        ...
        markup = self._render(
            image_url="img/test.png", placeholder_url="img/placeholder.png"
        )
    ...
    def test_lazy_img_external(self):
        ...
        markup = self._render(
            image_url="https://www.test.com/test.png",
            placeholder_url="img/placeholder.png",
            include_highres_image=True,
        )
        self.assertIn(u'src="/media/img/placeholder.png"', markup)
...
class TestAbsoluteURLFilter(TestCase):
    ...
    block_template = (
        "{% filter absolute_url %}{% block page_image %}"
        + "{{ static('%s') }}" % image_path
        + "{% endblock %}{% endfilter %}"
    )
    ...
    def test_urls(self):
        ...
        assert (
            misc.absolute_url("https://www.mozilla.org/en-US/firefox/new/") == expected
        )
...
class TestAppStoreURL(TestCase):
    ...
    def test_app_store_url_localized(self):
        """should return localized URL"""
        assert (
            self._render("en-US") == "https://itunes.apple.com/us/app/id1314000270?mt=8"
        )
        assert (
            self._render("es-ES") == "https://itunes.apple.com/es/app/id1314000270?mt=8"
        )
...
class TestPlayStoreURL(TestCase):
    ...
    def test_play_store_url_localized(self):
        ...
        assert (
            self._render("en-US")
            == "https://play.google.com/store/apps/details?id=mozilla.lockbox&amp;hl=en"
        )
        assert (
            self._render("es-ES")
            == "https://play.google.com/store/apps/details?id=mozilla.lockbox&amp;hl=es"
        )
        assert (
            self._render("de")
            == "https://play.google.com/store/apps/details?id=mozilla.lockbox&amp;hl=de"
        )
...
class TestStructuredDataID(TestCase):
    ...
    def _render(self, locale, domain=None):
        ...
        if domain:
            return render(
                "{{{{ structured_data_id('{0}', '{1}') }}}}".format(sd_id, domain),
                {"request": req},
            )
    ...
    def test_structured_data_custom_domain_id(self):
        ...
        assert (
            self._render("en-US", domain)
            == "https://foundation.mozilla.org/#firefoxbrowser"
        )
        assert (
            self._render("es-ES", domain)
            == "https://foundation.mozilla.org/#firefoxbrowser-es-es"
        )
        assert (
            self._render("de", domain)
            == "https://foundation.mozilla.org/#firefoxbrowser-de"
        )
...
class TestFirefoxAdjustUrl(TestCase):
    ...
    def _render(self, locale, redirect, adgroup, creative=None):
        ...
        if creative:
            return render(
                "{{{{ firefox_adjust_url('{0}', '{1}', '{2}') }}}}".format(
                    redirect, adgroup, creative
                ),
                {"request": req},
            )

        return render(
            "{{{{ firefox_adjust_url('{0}', '{1}') }}}}".format(redirect, adgroup),
            {"request": req},
        )
    ...
    def test_firefox_ios_adjust_url(self):
        ...
        assert (
            self._render("en-US", "ios", "test-page")
            == "https://app.adjust.com/2uo1qc?redirect="
    ...
    def test_firefox_ios_adjust_url_creative(self):
        ...
        assert (
            self._render("de", "ios", "test-page", "experiment-name")
            == "https://app.adjust.com/2uo1qc?redirect="
    ...
    def test_firefox_android_adjust_url(self):
        ...
        assert (
            self._render("en-US", "android", "test-page")
            == "https://app.adjust.com/2uo1qc?redirect="
...
class TestFocusAdjustUrl(TestCase):
    ...
    def _render(self, locale, redirect, adgroup, creative=None):
        ...
        return render(
            "{{{{ focus_adjust_url('{0}', '{1}') }}}}".format(redirect, adgroup),
            {"request": req},
        )
    ...
    def test_focus_ios_adjust_url(self):
        ...
        assert (
            self._render("en-US", "ios", "test-page")
            == "https://app.adjust.com/b8s7qo?redirect="
    ...
    def test_focus_ios_adjust_url_creative(self):
        ...
        assert (
            self._render("fr", "ios", "test-page", "experiment-name")
    ...
    def test_klar_ios_adjust_url(self):
        ...
        assert (
            self._render("de", "ios", "test-page")
            == "https://app.adjust.com/jfcx5x?redirect="
    ...
    def test_klar_android_adjust_url(self):
        ...
        assert (
            self._render("de", "android", "test-page")
            == "https://app.adjust.com/jfcx5x?redirect="
...
class TestLockwiseAdjustUrl(TestCase):
    ...
    def _render(self, locale, redirect, adgroup, creative=None):
        ...
        if creative:
            return render(
                "{{{{ lockwise_adjust_url('{0}', '{1}', '{2}') }}}}".format(
                    redirect, adgroup, creative
                ),
                {"request": req},
            )
        ...
        return render(
            "{{{{ lockwise_adjust_url('{0}', '{1}') }}}}".format(redirect, adgroup),
            {"request": req},
        )
...
class TestPocketAdjustUrl(TestCase):
    ...
    def _render(self, locale, redirect, adgroup, creative=None):
        ...
        if creative:
            return render(
                "{{{{ pocket_adjust_url('{0}', '{1}', '{2}') }}}}".format(
                    redirect, adgroup, creative
                ),
                {"request": req},
            )
        ...
        return render(
            "{{{{ pocket_adjust_url('{0}', '{1}') }}}}".format(redirect, adgroup),
            {"request": req},
        )
    ...
    def test_pocket_ios_adjust_url(self):
        ...
        assert (
            self._render("en-US", "ios", "test-page")
            == "https://app.adjust.com/m54twk?redirect="
...
class TestMonitorFxAButton(TestCase):
    ...
    def test_monitor_fxa_button(self):
        ...
        markup = self._render(
            ...
            optional_attributes={
                "data-cta-text": "Sign In to Monitor",
...
class TestFxALinkFragment(TestCase):
    ...
    def _render(self, entrypoint, action="signup", optional_parameters=None):
        ...
        req.locale = "en-US"
        return render(
            "{{{{ fxa_link_fragment('{0}', '{1}', {2}) }}}}".format(
                entrypoint, action, optional_parameters
            ),
            {"request": req},
        )
