...
@override_settings(
    PROD_DETAILS_STORAGE="PDDatabaseStorage",
...
class TestUpdateProductDetailsFiles(TestCase):
    ...
    def test_handle_diff_loads_all(self):
        with patch.multiple(
            self.command,
            update_file_data=DEFAULT,
            validate_data=DEFAULT,
            file_storage=DEFAULT,
            load_changes=DEFAULT,
            repo=DEFAULT,
        ):
            ...
            self.command.load_changes.assert_called_with(
                options, self.command.file_storage.all_json_files()
            )
