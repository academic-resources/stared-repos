...
def test_git(co_mock, os_mock):
    ...
    os_mock.chdir.assert_has_calls([call(g.path_str), call("olddir")])
    assert output == "dude"
...
def test_git_db_latest():
    ...
    assert (
        g.db_latest_key
        == "33ff0192f06306345030004c92533017b466e16489d4c762eab69ad8142ddae4"
    )
...
def test_git_db_latest_auto_name():
    ...
    g = git.GitRepo(
        "hollywood-star-lanes/the-dude", "https://example.com/repo/", "master"
    )
    g.set_db_latest("deadbeef")
    assert g.get_db_latest() == "deadbeef"
    gobj = git.GitRepoState.objects.get(repo_id=g.db_latest_key)
...
def test_git_clone():
    ...
    git_mock["git"].assert_called_with(
        "clone", "--depth", "1", "--branch", "master", "https://example.com", "."
    )
...
def test_git_update(rmtree_mock):
    g = git.GitRepo(".", "https://example.com")
    with patch.multiple(
        g, clone=DEFAULT, path=DEFAULT, diff=DEFAULT, pull=DEFAULT
    ) as git_mock:
