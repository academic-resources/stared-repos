...
def _vpn_product_link(
    ...
    if optional_parameters:
        params = "&".join(
            "%s=%s" % (param, val) for param, val in optional_parameters.items()
        )
        href += f"&{params}"
    ...
    if optional_attributes:
        attrs += " ".join(
            '%s="%s"' % (attr, val) for attr, val in optional_attributes.items()
        )
    ...
    markup = (
        f'<a href="{href}" data-action="{settings.FXA_ENDPOINT}" class="{css_class}" {attrs}>'
        f"{link_text}"
        f"</a>"
    )
...
def vpn_monthly_price(ctx, plan="monthly", lang=None):
    ...
    default_text = ftl(
        "vpn-shared-pricing-monthly", amount=default_amount, ftl_files=FTL_FILES
    )

    # HTML data-attributes are used by client side JS to set the correct display price based on geo.
    attributes = {
        "data-price-usd": ftl(
            "vpn-shared-pricing-monthly", amount=usd_amount, ftl_files=FTL_FILES
        ),
        "data-price-euro": ftl(
            "vpn-shared-pricing-monthly", amount=euro_amount, ftl_files=FTL_FILES
        ),
        "data-price-chf": ftl(
            "vpn-shared-pricing-monthly", amount=chf_amount, ftl_files=FTL_FILES
        ),
    }
    ...
    markup = (
        f'<span class="js-vpn-monthly-price-display" {attrs}>'
        f"{default_text}"
        f"</span>"
    )
...
def vpn_total_price(ctx, plan="12-month", lang=None):
    ...
    default_text = ftl(
        "vpn-shared-pricing-total", amount=default_amount, ftl_files=FTL_FILES
    )

    # HTML data-attributes are used by client side JS to set the correct display price based on geo.
    attributes = {
        "data-price-usd": ftl(
            "vpn-shared-pricing-total", amount=usd_amount, ftl_files=FTL_FILES
        ),
        "data-price-euro": ftl(
            "vpn-shared-pricing-total", amount=euro_amount, ftl_files=FTL_FILES
        ),
        "data-price-chf": ftl(
            "vpn-shared-pricing-total", amount=chf_amount, ftl_files=FTL_FILES
        ),
    }
    ...
    markup = (
        f'<span class="js-vpn-total-price-display" {attrs}>'
        f"{default_text}"
        f"</span>"
    )
...
def vpn_saving(ctx, plan="12-month", lang=None):
    ...
    default_text = ftl(
        "vpn-shared-pricing-save-percent", percent=default_amount, ftl_files=FTL_FILES
    )

    # HTML data-attributes are used by client side JS to set the correct saving based on geo.
    attributes = {
        "data-price-usd": ftl(
            "vpn-shared-pricing-save-percent", percent=usd_amount, ftl_files=FTL_FILES
        ),
        "data-price-euro": ftl(
            "vpn-shared-pricing-save-percent", percent=euro_amount, ftl_files=FTL_FILES
        ),
        "data-price-chf": ftl(
            "vpn-shared-pricing-save-percent", percent=chf_amount, ftl_files=FTL_FILES
        ),
    }
    ...
    markup = (
        f'<span class="js-vpn-saving-display" {attrs}>' f"{default_text}" f"</span>"
    )
