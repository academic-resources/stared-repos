...
class TestVPNInviteWaitlist(TestCase):
    ...
    def test_invalid_country(self):
        ...
        assert not resp_data["success"]
        assert (
            "Select a valid choice. zzzz is not one of the available choices."
            in resp_data["errors"]
        )
        assert not self.mock_subscribe.called
...
class TestVPNLandingPage(TestCase):
    ...
    def test_vpn_landing_page_variant_a_template(self, render_mock):
        req = RequestFactory().get(
            "/products/vpn/?entrypoint_experiment=vpn-landing-page-cta-change&entrypoint_variation=a"
        )
        req.locale = "en-US"
        view = views.vpn_landing_page
        view(req)
    ...
    def test_vpn_landing_page_variant_b_template(self, render_mock):
        req = RequestFactory().get(
            "/products/vpn/?entrypoint_experiment=vpn-landing-page-cta-change&entrypoint_variation=b"
        )
        req.locale = "en-US"
        view = views.vpn_landing_page
        view(req)
