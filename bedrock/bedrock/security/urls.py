...
urlpatterns = (
    ...
    page(
        "bug-bounty/web-eligible-sites", "security/bug-bounty/web-eligible-sites.html"
    ),
    url(
        r"^bug-bounty/hall-of-fame/$",
        HallOfFameView.as_view(program="client"),
        name="security.bug-bounty.hall-of-fame",
    ),
    url(
        r"^bug-bounty/web-hall-of-fame/$",
        HallOfFameView.as_view(program="web"),
        name="security.bug-bounty.web-hall-of-fame",
    ),
    url(r"^advisories/$", AdvisoriesView.as_view(), name="security.advisories"),
    url(
        r"^advisories/mfsa(?P<pk>\d{4}-\d{2,3})/$",
        AdvisoryView.as_view(),
        name="security.advisory",
    ),
    url(
        r"^advisories/cve-feed\.json$",
        mitre_cve_feed,
        name="security.advisories.cve_feed",
    ),
    page("known-vulnerabilities", "security/known-vulnerabilities.html"),
    page(
        "known-vulnerabilities/older-vulnerabilities",
        "security/older-vulnerabilities.html",
    ),
    url(
        r"^known-vulnerabilities/(?P<slug>[a-z-]+)/$",
        ProductView.as_view(),
        name="security.product-advisories",
    ),
    url(
    ...
    url(
        r"^(?:announce|advisories)(?:/.*)?/mfsa(?P<pk>\d{4}-\d{2,3})\.html$",
