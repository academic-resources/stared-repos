...
class ProductVersionView(LangFilesMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        cxt["product_name"] = "{0} {1}".format(
            cxt["product_versions"][0].product, version
        )
...
class KVRedirectsView(CachedRedirectView):
    ...
    def get_redirect_args(self, url_component):
        if url_component == "suite17":
            return dict(
                viewname="security.product-advisories", kwargs={"slug": "mozilla-suite"}
            )
        ...
        if match:
            ...
            return dict(
                viewname="security.product-version-advisories",
                kwargs={"product": product, "version": version},
            )

        if url_component.endswith("ESR"):
            return dict(
                viewname="security.product-advisories",
                kwargs={"slug": url_component[:-3] + "-esr"},
            )

        return dict(
            viewname="security.product-advisories", kwargs={"slug": url_component}
        )
