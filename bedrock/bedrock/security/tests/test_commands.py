...
def test_fix_product_name():
    ...
    assert (
        update_security_advisories.fix_product_name("Seamonkey 2.2") == "SeaMonkey 2.2"
    )
...
def test_filter_advisory_names():
    ...
    assert (
        update_security_advisories.filter_advisory_filenames(filenames)
        == good_filenames
    )
...
class TestDBActions(TestCase):
    ...
    def test_file_name_extension_change(self, git_mock, udbff_mock, df_mock, gafn_mock):
        ...
        make_mfsa("2016-43")
        all_files = [
            os.path.join(update_security_advisories.ADVISORIES_PATH, "mfsa2016-42.yml")
        ]
        ...
        udbff_mock.assert_called_with(
            update_security_advisories.filter_advisory_filenames(all_files)[0]
        )
