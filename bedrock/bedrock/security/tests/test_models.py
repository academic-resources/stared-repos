...
class TestHallOfFamer(TestCase):
    def test_year_quarter(self):
        hf1 = HallOfFamer.objects.create(
            name="The Dude", program="web", date=date(2018, 3, 2)
        )
        hf2 = HallOfFamer.objects.create(
            name="The Dude", program="web", date=date(2017, 4, 2)
        )
        hf3 = HallOfFamer.objects.create(
            name="The Dude", program="web", date=date(2016, 7, 12)
        )
        hf4 = HallOfFamer.objects.create(
            name="The Dude", program="web", date=date(2015, 11, 7)
        )
...
class TestMitreCVE(TestCase):
    ...
    def test_product_versions(self):
        ...
        cve = self._create_cve(
            ["Firefox ESR 60.0.1", "Firefox ESR 52.8.0", "Firefox 60.0.1"]
        )
        assert cve.product_versions() == {
            "Firefox": ["60.0.1"],
            "Firefox ESR": ["60.0.1", "52.8.0"],
        }
    ...
    def test_get_description(self):
        ...
        assert (
            cve.get_description()
            == "There was a problem. This vulnerability affects Firefox < 60 and Firefox < 60.0.1."
        )
        cve = self._create_cve(
            ["Firefox ESR 60.0.1", "Firefox ESR 52.8.0", "Firefox 60.0.1"]
        )
        assert cve.get_description() == (
            "There was a problem. This vulnerability affects Firefox ESR < 60.0.1, "
        ...
        assert (
            cve.get_description()
            == "No punctuation. This vulnerability affects Firefox ESR < 60.0.1."
        )
        cve.description = "Punctuation but extra space. \n"
        assert (
            cve.get_description()
            == "Punctuation but extra space. This vulnerability affects Firefox ESR < 60.0.1."
        )
