...
class Product(models.Model):
    ...
    def get_absolute_url(self):
        product, vers = self.name_and_version
        return reverse(
            "security.product-version-advisories",
...
class HallOfFamer(models.Model):
    ...
    PROGRAM_CHOICES = (("web", "Web"), ("client", "Client"))
...
class MitreCVE(models.Model):
    ...
    def get_reference_data(self):
        reference_data = [
            {
                "url": "https://www.mozilla.org/security/advisories/mfsa{}/".format(
                    mfsa_id
                )
            }
    ...
    def feed_entry(self):
        ...
        return {
            ...
            "affects": {
                "vendor": {
                    "vendor_data": [
                        {
                            "vendor_name": "Mozilla",
                            "product": {"product_data": self.get_product_data()},
                        }
                    ]
                }
            },
            "problemtype": {
                "problemtype_data": [
                    {"description": [{"lang": "eng", "value": self.title}]}
                ]
            },
            "references": {"reference_data": self.get_reference_data()},
            "description": {
                "description_data": [{"lang": "eng", "value": self.get_description()}]
            },
        }
