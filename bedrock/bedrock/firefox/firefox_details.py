...
class _ProductDetails(ProductDetails):
    ...
    def _matches_query(self, info, query):
        ...
        return all(
            (word in info["name_en"].lower() or word in info["name_native"].lower())
            for word in words
        )
...
class FirefoxDesktop(_ProductDetails):
            ...
            for platform, label in self.platform_labels.items():
                build_info["platforms"][platform] = {
                    "download_url": self.get_download_url(
                        channel, version, platform, locale, True, True
                    )
                }
    ...
    def get_filtered_full_builds(self, channel, version=None, query=None):
        ...
        :return: list
        """
        return self._get_filtered_builds(
            self.firefox_primary_builds, channel, version, query
        )
    ...
    def get_filtered_test_builds(self, channel, version=None, query=None):
        ...
        """
        return self._get_filtered_builds(
            self.firefox_beta_builds, channel, version, query
        )
    ...
    def get_download_url(
        ...
        if funnelcake_id:
            fc_platforms = config(
                "FUNNELCAKE_%s_PLATFORMS" % funnelcake_id,
                default="",
                parser=ListOf(str),
            )
            fc_locales = config(
                "FUNNELCAKE_%s_LOCALES" % funnelcake_id, default="", parser=ListOf(str)
            )
            include_funnelcake_param = (
                platform in fc_platforms and _locale in fc_locales
            )
...
class FirefoxAndroid(_ProductDetails):
    ...
    build_map = {"beta": "beta_builds", "release": "builds"}
    ...
    archive_url_base = (
        "https://archive.mozilla.org/pub/mobile/nightly/" "latest-mozilla-%s-android"
    )
    archive_repo = {"nightly": "central"}
    archive_urls = {
    ...
    def get_download_url(
        self, channel="release", arch="arm", locale="multi", force_direct=False
    ):
        """
        Get direct download url for the product.
...
class FirefoxIOS(_ProductDetails):
    # Version property names in product-details
    version_map = {"beta": "ios_beta_version", "release": "ios_version"}
