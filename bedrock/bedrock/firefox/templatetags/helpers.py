...
def download_firefox(
    ...
    dom_id = dom_id or "download-button-%s-%s" % (
        "desktop" if platform == "all" else platform,
        channel,
    )
    ...
    html = render_to_string(
        "firefox/includes/download-button.html", data, request=ctx["request"]
    )
...
def download_firefox_thanks(
    ...
    html = render_to_string(
        "firefox/includes/download-button-thanks.html", data, request=ctx["request"]
    )
    return jinja2.Markup(html)
...
def download_firefox_desktop_list(
    ctx, channel="release", dom_id=None, locale=None, force_full_installer=False
):
    """
    ...
    builds = desktop_builds(
        channel, None, locale, True, force_full_installer, False, False, False, True
    )
    ...
    for plat in builds:
        ...
        if (
            plat["os"] in firefox_desktop.platform_classification["recommended"]
            or channel == "nightly"
            and plat["os"] == "win"
        ):
    ...
    html = render_to_string(
        "firefox/includes/download-list.html", data, request=ctx["request"]
    )
    return jinja2.Markup(html)
