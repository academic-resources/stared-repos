...
class TestDownloadButtons(TestCase):
    ...
    def test_button_locale_in_transition(self):
        ...
        doc = pq(
            render(
                "{{ download_firefox(locale_in_transition=true) }}",
                {
                    "request": get_request,
                    "fluent_l10n": self.get_l10n(get_request.locale),
                },
        ...
        doc = pq(
            render(
                "{{ download_firefox(locale_in_transition=false) }}",
                {
                    "request": get_request,
                    "fluent_l10n": self.get_l10n(get_request.locale),
                },
    ...
    def test_download_nosnippet_attribute(self):
        ...
        assert (
            ...
            == '<div class="unrecognized-download" data-nosnippet="true"></div>'
        )
    ...
    def test_nightly_desktop(self):
        ...
        doc = pq(
            render(
                "{{ download_firefox('nightly', platform='desktop') }}",
                {
                    "request": get_request,
                    "fluent_l10n": self.get_l10n(get_request.locale),
                },
    ...
    def test_beta_desktop(self):
        ...
        doc = pq(
            render(
                "{{ download_firefox('beta', platform='desktop') }}",
                {
                    "request": get_request,
                    "fluent_l10n": self.get_l10n(get_request.locale),
                },
    ...
    def test_firefox_desktop(self):
        ...
        doc = pq(
            render(
                "{{ download_firefox(platform='desktop') }}",
                {
                    "request": get_request,
                    "fluent_l10n": self.get_l10n(get_request.locale),
                },
    ...
    def test_latest_nightly_android(self):
        ...
        doc = pq(
            render(
                "{{ download_firefox('nightly', platform='android') }}",
    ...
    def test_beta_mobile(self):
        ...
        doc = pq(
            render(
                "{{ download_firefox('beta', platform='android') }}",
                {"request": get_request, "fluent_l10n": self.get_l10n("fr")},
            )
        )
...
class TestDownloadList(TestCase):
    ...
    def test_firefox_desktop_list_release(self):
        ...
        get_request.locale = "en-US"
        doc = pq(
            render("{{ download_firefox_desktop_list() }}", {"request": get_request})
        )
    ...
    def test_firefox_desktop_list_aurora(self):
        ...
        doc = pq(
            render(
                "{{ download_firefox_desktop_list(channel='alpha') }}",
                {"request": get_request},
            )
        )
    ...
    def test_firefox_desktop_list_nightly(self):
        ...
        doc = pq(
            render(
                "{{ download_firefox_desktop_list(channel='nightly') }}",
                {"request": get_request},
            )
        )
...
class TestFirefoxURL(TestCase):
    ...
    def test_firefox_all(self):
        ...
        assert self._render("desktop", "all").endswith(
            "/firefox/all/#product-desktop-release"
        )
        assert self._render("desktop", "all", "release").endswith(
            "/firefox/all/#product-desktop-release"
        )
        assert self._render("desktop", "all", "beta").endswith(
            "/firefox/all/#product-desktop-beta"
        )
        assert self._render("desktop", "all", "alpha").endswith(
            "/firefox/all/#product-desktop-developer"
        )
        assert self._render("desktop", "all", "developer").endswith(
            "/firefox/all/#product-desktop-developer"
        )
        assert self._render("desktop", "all", "nightly").endswith(
            "/firefox/all/#product-desktop-nightly"
        )
        assert self._render("desktop", "all", "esr").endswith(
            "/firefox/all/#product-desktop-esr"
        )
        assert self._render("desktop", "all", "organizations").endswith(
            "/firefox/all/#product-desktop-esr"
        )
        assert self._render("android", "all").endswith(
            "/firefox/all/#product-android-release"
        )
        assert self._render("android", "all", "release").endswith(
            "/firefox/all/#product-android-release"
        )
        assert self._render("android", "all", "beta").endswith(
            "/firefox/all/#product-android-beta"
        )
        assert self._render("android", "all", "nightly").endswith(
            "/firefox/all/#product-android-nightly"
        )

    def test_firefox_sysreq(self):
        """Should return a reversed path for the Firefox sysreq page"""
        assert self._render("desktop", "sysreq").endswith(
            "/firefox/system-requirements/"
        )
        assert self._render("desktop", "sysreq", "release").endswith(
            "/firefox/system-requirements/"
        )
        assert self._render("desktop", "sysreq", "beta").endswith(
            "/firefox/beta/system-requirements/"
        )
        assert self._render("desktop", "sysreq", "alpha").endswith(
            "/firefox/developer/system-requirements/"
        )
        assert self._render("desktop", "sysreq", "esr").endswith(
            "/firefox/organizations/system-requirements/"
        )
        assert self._render("desktop", "sysreq", "organizations").endswith(
            "/firefox/organizations/system-requirements/"
        )
    ...
    def test_desktop_notes(self):
        ...
        assert self._render("desktop", "notes", "alpha").endswith(
            "/firefox/developer/notes/"
        )
        assert self._render("desktop", "notes", "esr").endswith(
            "/firefox/organizations/notes/"
        )
        assert self._render("desktop", "notes", "organizations").endswith(
            "/firefox/organizations/notes/"
        )
    ...
    def test_android_notes(self):
        ...
        assert self._render("android", "notes", "release").endswith(
            "/firefox/android/notes/"
        )
        assert self._render("android", "notes", "beta").endswith(
            "/firefox/android/beta/notes/"
        )
        assert self._render("android", "notes", "alpha").endswith(
