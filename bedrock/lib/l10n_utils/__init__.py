...
def render_to_string(
    template_name, context=None, request=None, using=None, ftl_files=None
):
    if request:
        context = context or {}
        ...
        if ftl_files:
            ...
            context["fluent_l10n"] = fluent_l10n(
                [locale, "en"], settings.FLUENT_DEFAULT_FILES
            )
    return loader.render_to_string(template_name, context, request, using)


def render(
    request, template, context=None, ftl_files=None, activation_files=None, **kwargs
):
    """
