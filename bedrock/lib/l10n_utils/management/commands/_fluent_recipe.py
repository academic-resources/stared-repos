...
def get_brand_terms():
    ...
    if BRAND_TERMS is None:
        ...
        tmp_dict = {
            k: bundle._lookup(k, term=True).value.value
            for k, v in bundle._terms.items()
        }
        BRAND_TERMS = {
            k: tmp_dict[k]
            for k in sorted(tmp_dict, key=lambda k: len(tmp_dict[k]), reverse=True)
        }
...
class Recipe:
    ...
    def handle(self, template):
        ...
        for from_path, ids in ids_for_file.items():
            ...
            recipe += ADD_TRANSFORMS.format(
                ftl_file=from_path.with_suffix(".ftl"), transforms=transforms
            )
    ...
    def get_transforms(self, id_stem, lang_ids, from_path):
        ...
        for lang_string in lang_ids:
            ...
            if STR_VARIABLE_RE.search(lang_string) or brands_in_string:
                ...
                replaces.append(
                    self.create_replace(
                        from_path, fluent_id, lang_string, brands_in_string
                    )
                )
            else:
                if replaces:
                ...
                simple_transforms.append(
                    f'{fluent_id} = {"{"}COPY({from_path}, "{escaped_lang_string}",){"}"}'
                )
        if simple_transforms:
            if transforms:
                transforms += " + "
    ...
    def create_replace(self, from_path, fluent_id, lang_string, brands_in_string):
        ...
        if replacers:
            replacements += '                        "%%": "%",\n'
            replacements += "\n".join(
                VAR_REPLACE.format(lit=lit, var=var) for var, lit in replacers.items()
            )
