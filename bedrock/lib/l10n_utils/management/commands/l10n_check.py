...
def list_templates():
    ...
    for app in settings.INSTALLED_APPS:
        if app.startswith(settings.PROJECT_MODULE):
            ...
            tmpl_dir = path.join(
                settings.ROOT, settings.PROJECT_MODULE, app, "templates"
            )
...
class L10nTemplate:
    ...
    def update(self, lang):
        ...
        with closing(StringIO()) as buffer:
            for token in parser.parse_template(
                dest_tmpl, strict=False, halt_on_content=True
            ):
                if not token:
                    ...
                    if not file_version:
                        raise Exception(
                            "l10n file version tag does not exist "
                    ...
                    self._transfer_content(
                        l10n_block, self._get_ref_block(name, blocks)
                    )
...
class L10nParser:
    ...
    def parse_template(
        self, tmpl, strict=True, halt_on_content=False, only_blocks=False
    ):
        """Read a template and parse the l10n blocks"""

        self.tmpl = tmpl
        for token in self.parse(
            codecs.open(tmpl, encoding="utf-8").read(),
            strict,
            halt_on_content,
            only_blocks,
        ):
        ...
        for token in self.tokens:
            ...
            if name == "comment_begin":
                ...
                if matches:
                    ...
                    if not version:
                        raise Exception(
                            "Invalid version metadata in " "template: %s" % self.tmpl
                        )
    ...
    def get_block_version(self, version_str):
        ...
        if not block_version:
            raise Exception(
                "Invalid l10n block declaration: "
                "bad version '%s' in %s" % (block_version, self.tmpl)
            )
    ...
    def parse_block(self, strict=True):
        ...
        if token_type == "integer":
            ...
            raise Exception(
                "Invalid l10n block declaration: "
                "missing date for block '%s' in %s" % (block_name, self.tmpl)
            )
