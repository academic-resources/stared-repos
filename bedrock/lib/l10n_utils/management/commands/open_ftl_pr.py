...
class Command(FTLRepoCommand):
    ...
    def commit_message(self):
        ...
        if GIT_HASH:
            ...
            message += (
                "From file changes in https://github.com/mozilla/bedrock/commits/master"
            )
    ...
    def push_changes(self):
        try:
            ...
            raise CommandError(
                f"There was a problem pushing to {self.l10n_repo.remote_url}"
            )

        commit = self.l10n_repo.git("rev-parse", "--short", "HEAD")
        self.stdout.write(
            f"Pushed {commit} to {self.l10n_repo.remote_url} as {self.branch_name}"
        )
    ...
    def open_pr(self):
        ...
        pr = self.github.create_pull(
            title=title, body=body, base="master", head=self.branch_name
        )
