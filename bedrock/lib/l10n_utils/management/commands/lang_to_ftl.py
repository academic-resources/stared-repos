...
class Command(BaseCommand):
    ...
    def ftl_file_path(self):
        return settings.FLUENT_LOCAL_PATH.joinpath("en", self.filename).with_suffix(
            ".ftl"
        )
    ...
    def get_ftl_strings(self):
        ...
        for str_id, string in translations.items():
            ...
            if "%s" in string:
                self.stderr.write(
                    "WARNING: Place-holder with no variable name found in string. "
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            f"Inspect the file before converting translations: {self.ftl_file_path}"
        )
