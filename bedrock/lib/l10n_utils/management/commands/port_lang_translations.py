...
class Command(BaseCommand):
    ...
    def write_ftl_translations(self):
        ...
        for path in self.translated_filepaths():
            ...
            for str_id, string in translation.items():
                ...
                if ftl_id:
                    ...
                    self.stderr.write(
                        f'WARNING: Could not find a match for "{str_id}"\n'
                        f"in {relative_path}"
                    )
        ...
        if not wrote_all:
            self.stdout.write(
                "Did not modify existing files. Use --force to overwrite."
            )

    @override_settings(DEV=False)
    def record_active_translations(self):
        translations = get_translations_for_langfile(
            self.lang_file_path.with_suffix("")
        )
        data = self.metadata
