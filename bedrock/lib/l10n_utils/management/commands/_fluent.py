...
def template_name(recipe):
    ...
    name = (
        recipe.resolve()
        .relative_to(settings.FLUENT_MIGRATIONS_PATH)
        .with_suffix(".html")
    )
...
def get_migration_context(recipe_or_template, locale="en"):
    ...
    context = MigrationContext(
        locale, ref_dir, settings.LOCALES_PATH / locale, enforce_translated=no_reference
    )
...
GETTEXT_RE = re.compile(
    r'\b_\([\'"](?P<string>.+?)[\'"]\)' r"(\s*\|\s*format\((?P<args>\w.+?)\))?", re.S
)
TRANS_BLOCK_RE = re.compile(
    r"{%-?\s+trans\s+((?P<args>\w.+?)\s+)?-?%\}\s*"
    r"(?P<string>.+?)"
    r"\s*{%-?\s+endtrans\s+-?%\}",
    re.S,
)
