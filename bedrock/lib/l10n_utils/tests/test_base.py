...
class TestRender(TestCase):
    ...
    def test_firefox(self):
        ...
        self._test(
            path, template, "en-US", "en-us,en;q=0.5", 200, active_locales=locales
        )
        # en-GB is activated on /firefox/new/
        self._test(
            path, template, "en-GB", "en-gb,en;q=0.5", 200, active_locales=locales
        )
    ...
    def test_add_active_locales(self, tft_mock):
        ...
        # Nothing to do with a valid locale
        self._test(
            path, template, "en-US", "en-us,en;q=0.5", 200, add_active_locales=locales
        )
        # en-GB is activated on /firefox/new/
        self._test(
            path, template, "en-GB", "en-gb,en;q=0.5", 200, add_active_locales=locales
        )
    ...
    def test_activation_files(self, fal_mock, tft_mock, dr_mock):
        ...
        fal_mock.assert_has_calls([call("dude"), call("walter")], any_order=True)
...
class TestL10nTemplateView(TestCase):
    ...
    def test_ftl_files(self, render_mock):
        view = l10n_utils.L10nTemplateView.as_view(
            template_name="dude.html", ftl_files="dude"
        )
        view(self.req)
        render_mock.assert_called_with(
            self.req, ["dude.html"], ANY, ftl_files="dude", activation_files=None
        )

    def test_ftl_files_map(self, render_mock):
        view = l10n_utils.L10nTemplateView.as_view(
            template_name="dude.html", ftl_files_map={"dude.html": "dude"}
        )
        view(self.req)
        render_mock.assert_called_with(
            self.req, ["dude.html"], ANY, ftl_files="dude", activation_files=None
        ...
        view = l10n_utils.L10nTemplateView.as_view(
            template_name="dude.html", ftl_files_map={"donny.html": "donny"}
        )
        view(self.req)
        render_mock.assert_called_with(
            self.req, ["dude.html"], ANY, ftl_files=None, activation_files=None
        )

    def test_ftl_activations(self, render_mock):
        view = l10n_utils.L10nTemplateView.as_view(
            template_name="dude.html",
            ftl_files="dude",
