...
class TestTemplateTagFuncs(TestCase):
    ...
    def test_get_template_tag_set(
        self, lang_file_tag_set, parse_template_mock, get_template, get_lang_path
    ):
        """Should return a unique set of tags from all lang files."""
        parse_template_mock.return_value = ["dude", "walter"]
        lang_file_tag_set.side_effect = [
            set(["dude", "donny"]),
            set(["dude", "uli", "bunny"]),
            set(["walter", "brandt"]),
        ]
        self.assertSetEqual(
            _get_template_tag_set("stuff", "es"),
...
class TestPOFiles(TestCase):
    good_messages = [
        [u"Said angrily, loudly, and repeatedly.", u"Is this your homework Larry?"],
    ...
    def test_parse_po(self):
        ...
        expected = {
            ...
            u"templates/firefox/fx.html": [
                [None, u"Find out if your device " u"is supported &nbsp;»"]
            ],
    ...
    def test_append_to_lang_file_dir_creation(self, codecs_mock, md_mock):
        ...
        path_dir_exists = os.path.join(
            ROOT, "locale", "templates", "firefox", "new.lang"
        )
    ...
    @override_settings(
        ROOT=ROOT, LOCALES_PATH=LOCALES_PATH, DOTLANG_FILES=DOTLANG_FILES
    )
    ...
    def test_uses_default_lang_files(self, pl_mock):
        ...
        calls = [
            (
                ("{0}/locale/templates/{1}.lang".format(ROOT, lf),),
...
class TestLangfilesForPath(TestCase):
    def test_tmpl_no_lang_files_defined(self):
        ...
        lang_files = langfiles_for_path(
            "lib/l10n_utils/tests/test_files/" "templates/no_lang_files.html"
        )
    ...
    def test_templ_lang_files_defined(self):
        ...
        lang_files = langfiles_for_path(
            "lib/l10n_utils/tests/test_files/" "templates/some_lang_files.html"
        )
    ...
    def test_py_no_lang_files_defined(self):
        ...
        """
        lang_files = langfiles_for_path(
            "lib/l10n_utils/tests/test_files/" "extract_me.py"
        )
    ...
    def test_py_lang_files_defined(self):
        ...
        """
        lang_files = langfiles_for_path(
            "lib/l10n_utils/tests/test_files/" "extract_me_with_langfiles.py"
        )
        assert lang_files == ["lebowski", "dude"]
