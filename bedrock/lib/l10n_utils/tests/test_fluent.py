...
class TestFluentTranslationUtils(TestCase):
    ...
    def test_translate(self):
        ...
        assert (
            fluent.translate(l10n, "fluent-page-desc", lang="Dudeish")
            == "Description in Dudeish"
        )
        # fall back to 'en' string
        assert (
            fluent.translate(l10n, "brand-new-string")
            == "New string not yet available in all languages"
        )
        # will use fallback string
        assert (
            fluent.translate(l10n, "brand-new-string", fallback="fluent-title")
            == "Title in German"
        )
    ...
    def test_has_any_messages(self):
        l10n = get_l10n()
        assert fluent.ftl_has_messages(
            l10n, "fluent-title", "brand-new-string", require_all=False
        )
    ...
    def test_get_active_locales(self, meta_mock):
        ...
        assert fluent.get_active_locales("the/dude", force=True) == [
            "de",
            "en-US",
            "fr",
        ]
...
class TestFluentViewTranslationUtils(TestCase):
    ...
    def test_ftl_view_util(self):
        assert (
            fluent.ftl("fluent-title", locale="de", ftl_files="mozorg/fluent")
            == "Title in German"
        )
    ...
    def test_ftl_view_util_no_mutate_list(self):
        ...
        assert (
            fluent.ftl("fluent-title", locale="de", ftl_files=ftl_files)
            == "Title in German"
        )
