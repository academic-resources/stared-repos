...
class UrlsParser:
    ...
    def parse(config_start_urls):
        ...
        for start_url in config_start_urls:
            ...
            if len(matches) > 0:
                ...
                start_urls = UrlsParser.geturls(
                    start_url, matches[0], matches[1:], values, start_urls
                )
    ...
    def geturls(start_url, current_match, matches, values, start_urls):
        for value in values[current_match]:
            ...
            if len(matches) == 0:
                ...
                start_urls = UrlsParser.geturls(
                    copy_start_url, matches[0], matches[1:], values, start_urls
                )
    ...
    def build_allowed_domains(start_urls, stop_urls):
        ...
        all_urls = [
            _["url"] if not isinstance(_, str) else _ for _ in start_urls
        ] + stop_urls
