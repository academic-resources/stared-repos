...
class SelectorsParser:
        ...
        for key in config_selectors:
            ...
            if "type" in selectors_set[key]:
                if selectors_set[key]["type"] not in ["xpath", "css"]:
                    raise Exception(
                        selectors_set[key]["type"]
                        + "is not a good selector type, it should be `xpath` or `css`"
                    )
            else:
            ...
            # Default value
            selectors_set[key]["default_value"] = (
                selectors_set[key]["default_value"]
                if "default_value" in selectors_set[key]
                else None
            )

            # Strip chars
            selectors_set[key]["strip_chars"] = (
                selectors_set[key]["strip_chars"]
                if "strip_chars" in selectors_set[key]
                else None
            )

            if "attributes" in selectors_set[key]:
                selectors_set[key]["attributes"] = SelectorsParser._parse_selectors_set(
    ...
    def parse_min_indexed_level(min_indexed_level):
        ...
        if isinstance(min_indexed_level, int):
            min_indexed_level_object = {"default": min_indexed_level_object}
