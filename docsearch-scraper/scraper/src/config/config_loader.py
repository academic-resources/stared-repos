...
class ConfigLoader:
    ...
    def _parse(self):
        ...
        if self.index_name_tmp is None:
            self.index_name_tmp = os.environ.get(
                "INDEX_NAME_TMP", self.index_name + "_tmp"
            )
    ...
    def update_nb_hits_value(self, nb_hits):
        if self.config_file is not None:
            ...
            previous_nb_hits = (
                None
                if "nb_hits" not in self.config_content
                else self.config_content["nb_hits"]
            )
            nb_hit_updater = NbHitsUpdater(
                self.config_file, self.config_content, previous_nb_hits, nb_hits
            )
