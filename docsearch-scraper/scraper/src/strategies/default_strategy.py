...
class DefaultStrategy(AbstractStrategy):
    ...
    def get_records_from_dom(self, current_page_url=None):
        ...
        for position, node in enumerate(nodes):
            ...
            hierarchy = self._update_hierarchy_with_global_content(
                hierarchy, current_level_int
            )
            ...
            if "lvl" in current_level and (
                hierarchy[current_level] == "" or hierarchy[current_level] is None
            ):
                continue
            ...
            record = {
                ...
                "hierarchy_radio": Hierarchy.get_hierarchy_radio(
                    hierarchy, current_level, levels
                ),
                "type": current_level,
                "tags": UrlsParser.get_tags(current_page_url, self.config.start_urls),
                "weight": {
                    "page_rank": UrlsParser.get_page_rank(
            ...
            if current_page_url is not None:
                ...
                record["url"] = self._get_url_with_anchor(
                    record["url"], record["anchor"]
                )
                record["url_without_variables"] = self._get_url_with_anchor(
                    record["url_without_variables"], record["anchor"]
                )
                record["no_variables"] = (
                    record["url"] == record["url_without_variables"]
                )

            # Define our own ObjectID to enable proper analytics
            hierarchy_to_hash = {
                lvl: x for lvl, x in hierarchy.items() if x is not None
            }
        ...
        if "attributes" in selectors[current_level]:
            ...
            for attribute_name in list(selectors[current_level]["attributes"].keys()):
                ...
                attributes[attribute_name] = self.get_text_from_nodes(
                    matching_nodes,
                    self.get_strip_chars(
                        attribute_name, selectors[current_level]["attributes"]
                    ),
                )
            return attributes
    ...
    def _handle_default_values(hierarchy, content, selectors, levels):
        ...
        for level in selectors:
            if level in levels:
                if (
                    hierarchy[level] is None
                    and selectors[level]["default_value"] is not None
                ):
    ...
    def _get_url_with_anchor(self, current_page_url, anchor):
        if (
            not self.config.js_render or not self.config.use_anchors
