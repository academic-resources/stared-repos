    ...
    def get(config, levels):
        ...
        for level in levels:
            for selectors_key in config.selectors:
                ...
                if (
                    level in config.selectors[selectors_key]
                    and attr_to_index not in searchable_attributes
                ):
        ...
        for selectors_key in config.selectors:
            if (
                "content" in config.selectors[selectors_key]
                and "content" not in searchable_attributes
            ):
        ...
        settings = {
            ...
            "userData": {"lastCrawl": str(datetime.datetime.now().isoformat())},
        }
