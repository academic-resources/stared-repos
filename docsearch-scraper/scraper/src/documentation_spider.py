...
class DocumentationSpider(CrawlSpider, SitemapSpider):
    ...
    def to_any_scheme(url):
        ...
        return (
            url
            if not re.match(DocumentationSpider.match_capture_any_scheme, url)
            else re.sub(
                DocumentationSpider.match_capture_any_scheme,
                DocumentationSpider.backreference_any_scheme,
                url,
            )
        )
    ...
    def to_other_scheme(url):
        ...
        if not (match and match.group(1) and match.group(2)):
            raise ValueError(
                "Must have a match and split the url into the scheme and the rest. url: "
                + url
            )
    ...
    def __init__(self, config, algolia_helper, strategy, *args, **kwargs):
        ...
        self.stop_urls = [
            DocumentationSpider.to_any_scheme(stop_url) for stop_url in config.stop_urls
        ]
        ...
        start_urls_any_scheme = [
            DocumentationSpider.to_any_scheme(start_url)
        ...
        if config.sitemap_urls:
            ...
            self.sitemap_urls_regexs = (
                config.sitemap_urls_regexs
                if config.sitemap_urls_regexs
                else start_urls_any_scheme
            )
            ...
            self.__init_sitemap_(
                config.sitemap_urls, sitemap_rules, config.sitemap_alternate_links
            )
        ...
        for rule in self._rules:
            if not self.strict_redirect:
                ...
                if rule.link_extractor._link_allowed(
                    response
                ) and rule.link_extractor._link_allowed(response.request):
                    continue
            return False
    ...
    def __init_sitemap_(
        self, sitemap_urls, custom_sitemap_rules, sitemap_alternate_links
    ):
