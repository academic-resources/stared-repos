...
class AbstractBuildDocker(AbstractCommand):
    @staticmethod
    def build_docker_file(file, image="algolia/docsearch-scraper-dev", local_tag=False):
        tags = [image]
        ...
        cmd = (
            ["docker", "build"]
            + [param for tag in tags for param in ["-t", tag]]
            + ["-f", file, "."]
        )
    ...
    def get_local_tag():
        ...
        return check_output(["git", "describe", "--abbrev=0", "--tags"]).strip()
