...
class BuildDockerScraper(AbstractBuildDocker):
    ...
    def get_options(self):
        ...
        options.append(
            {"name": "test", "description": "build the test image", "optional": False}
        )
        return options
    ...
    def run(self, args):
        ...
        if test:
            return self.build_docker_file(
                "scraper/dev/docker/Dockerfile.test",
                "algolia/docsearch-scraper-test",
                local_tag=local_tag,
            )

        code = self.build_docker_file(
        ...
        if code != 0:
            return code
        code = self.build_docker_file(
            "scraper/dev/docker/Dockerfile.dev", local_tag=local_tag
        )
        if code != 0:
            return code
        return self.build_docker_file(
