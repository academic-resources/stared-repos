...
def get_helpscout_app_secret():
    ...
    if not hs_api_secret:
        raise ValueError("None HELPSCOUT_APP_SECRET from environment variables")
...
def get_conversation_ID_from_url(hs_url):
    ...
    if not RepresentsInt(cuid):
        raise ValueError("Conversation ID : {} must be an integer".format(cuid))
...
def get_emails_from_conversation(conversation_with_threads):
    ...
    if len(emails) > 1:
        print(
            "Conversation sent by \033[1;33m"
            + customers_mail
            + "\033[0m"
            + (" with " + " ".join(emails[1:]))
        )
...
def add_draft(cuid, body):
    ...
    data = {"text": body, "draft": True, "customer": {"id": customer_id}}
...
def is_docusaurus_conversation(conversation):
    return check_if_has_tag(
        conversation, ["docusaurus", "ds_docusaurus", "gen-docusaurus"]
    )


def is_docusaurus_v2_conversation(conversation):
    return check_if_has_tag(
        conversation, ["docusaurus_v2", "ds_docusaurus_v2", "gen-docusaurus_v2"]
    )
...
def is_pkgdown_conversation(conversation):
    return check_if_has_tag(conversation, ["pkgdown", "ds_pkgdown", "gen-pkgdown"])


def is_vuepress_conversation(conversation):
    return check_if_has_tag(conversation, ["vuepress", "ds_vuepress", "gen-vuepress"])
...
def is_publii_conversation(conversation):
    return check_if_has_tag(conversation, ["publii", "ds_publii", "gen-publii"])


def is_jsdoc_conversation(conversation):
    return check_if_has_tag(conversation, ["jsdoc", "ds_jsdoc", "gen-jsdoc"])
