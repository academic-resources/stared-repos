...
def get_sitemap_if_available(url):
    if not url.endswith("sitemap.xml"):
        ...
        url = (
            url + "/sitemap.xml" if reMatch is None else reMatch.group() + "sitemap.xml"
        )
...
def to_fixme_config(config, urls):
    ...
    if not mainSelector == "FIXME":
        config["selectors"]["lvl0"] = OrderedDict(
            (("selector", ""), ("global", True), ("default_value", "Documentation"))
        )
        ...
        config["selectors"]["text"] = mainSelector + " p, " + mainSelector + " li"
...
def to_docusaurus_config(config, urls):
    ...
    config["custom_settings"] = {"attributesForFaceting": ["language", "version"]}
    start_url = urls[0]
...
def to_pkgdown_config(config, urls=None):
    if urls:
        ...
        config["sitemap_urls"] = get_sitemap_if_available(root + "sitemap.xml")
...
def to_jsdoc_config(config):
    config["stop_urls"] = ["\\.js\\.html", "/index\\.html$"]
    ...
    config["selectors_exclude"] = [".signature", ".type-signature", ".details"]
...
def create_config(u=None):
    ...
    config["index_name"] = (
        tldextract.extract(u).subdomain
        if tldextract.extract(u).domain == "github"
        else tldextract.extract(u).domain
    )
    ...
    user_index_name = helpers.get_user_value(
        "index_name is \033[1;33m{}\033[0m [enter to confirm]: ".format(
            config["index_name"]
        )
    )
    ...
    if user_index_name != "":
        ...
        print("index_name is now \033[1;33m{}\033[0m".format(config["index_name"]))
