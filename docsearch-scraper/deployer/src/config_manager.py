...
from .helpdesk_helper import (
    add_draft,
    get_conversation_with_threads,
    get_emails_from_conversation,
    get_conversation_url_from_cuid,
)
...
class ConfigManager:
    ...
    class __ConfigManager:
        ...
        def init(self):
            output = helpers.check_output_decoded(
                ["git", "stash", "list"], cwd=self.public_dir
            )
            self.initial_public_nb_stash = len(output.split("\n"))
            helpers.check_output_decoded(
                ["git", "stash", "--include-untracked"], cwd=self.public_dir
            )
            output2 = helpers.check_output_decoded(
                ["git", "stash", "list"], cwd=self.public_dir
            )
            self.final_nb_public_stash = len(output2.split("\n"))
            helpers.check_output_decoded(
                ["git", "pull", "-r", "origin", "master"], cwd=self.public_dir
            )

            output = helpers.check_output_decoded(
                ["git", "stash", "list"], cwd=self.private_dir
            )
            self.initial_private_nb_stash = len(output.split("\n"))
            helpers.check_output_decoded(
                ["git", "stash", "--include-untracked"], cwd=self.private_dir
            )
            output2 = helpers.check_output_decoded(
                ["git", "stash", "list"], cwd=self.private_dir
            )
        ...
        def add_config(self, config_name):
            ...
            if "conversation_id" in config:
                ...
                analytics_statuses = emails.add(
                    config_name, self.private_dir, emails_to_add=emails_from_conv
                )

                note_content = snippeter.get_email_for_config(
                    config_name, analytics_statuses
                )
                ...
                if helpers.confirm(
                    ...
                    print(
                        snippeter.get_email_for_config(config_name, analytics_statuses)
                    )
                else:
