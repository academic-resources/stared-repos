...
for x in range(0, len(word_array)):
    if (
        word_array[x].startswith('"')
        and word_array[x][1].isupper()
        or word_array[x].istitle()
    ):
        start_words.append(word_array[x])
    elif (
        word_array[x].endswith(".")
        or word_array[x].endswith("?")
        or word_array[x].endswith("!")
        or word_array[x].endswith('"')
        and word_array[x][len(word_array[x]) - 2] == "."
        or word_array[x].endswith('"')
        and word_array[x][len(word_array[x]) - 2] == "!"
        or word_array[x].endswith('"')
        and word_array[x][len(word_array[x]) - 2] == "?"
