...
for (x = 0; x < graduates.length; x++) {
  universities.push(`${graduates[x].university}`);
}
...
for (x = 0; x < graduates.length; x++) {
  if (graduates[x].university.includes("Uni")) {
    unisWithUni.push(graduates[x].university)
  }
}
...
*/
const displayNames = [];
zooAnimals.forEach(items => displayNames.push(`Name: ${items.animal_name}, Scientific: ${items.scientific_name}`));
