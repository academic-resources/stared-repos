...
for (let x = 0; x < inventory.length; x++) {
  carModels[x] = inventory[x].car_model;
}
...
let carYears = [];
for (let x = 0; x < inventory.length; x++) {
  carYears[x] = inventory[x].car_year;
}
...
for (let x = 0; x < carYears.length; x++) {
  if (carYears[x] <= 1999) {
    oldCars.push(carYears[x]);
  }
}
...
let BMWAndAudi = [];
console.log(fBMWAudi(inventory));
function fBMWAudi(inventory) {
for (let x = 0; x < inventory.length; x++) {
