...
def toyless_blue_cats
  ...
  execute(<<-SQL)
    ...
    WHERE cats.color = 'Navy Blue' AND cattoys.toy_id IS NULL;
  SQL
end
...
def cats_like_johnson
  ...
  execute(<<-SQL)
   ...
   --WHERE cats_1.name = 'Johnson' AND cats_1.color = 'Lavender'
   --ORDER BY cats_2.name;
   SELECT cats.name
   FROM cats
   WHERE cats.breed = (
     SELECT cats.breed
...
def cheap_toys_and_their_cats
  ...
  # Get your overall cost lower than: 
  execute(<<-SQL)
    SELECT cats.name
    FROM cats
    JOIN cattoys ON cats.id = cattoys.cat_id
