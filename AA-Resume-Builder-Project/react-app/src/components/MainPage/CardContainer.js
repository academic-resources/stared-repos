...
const CardContainer = ({ templates, resumes, loaded }) => {
  ...
  if (resumes) {
    visibleResumes = Object.values(resumes).slice(
      Object.values(resumes).length - 4
    );
  }
          ...
          <div className="flex flex-nowrap lg:ml-40 md:ml-20 ml-10 ">
            {visibleTemplates.map((template_name) => (
              ...
              >
                <Card
                  title={template_name}
                  tags={templates[template_name]["default_tags"]}
                />
              </a>
            ))}
            <a href="/templates">
          ...
          <div className="flex flex-nowrap lg:ml-40 md:ml-20 ml-10">
            {visibleResumes.map((resume) => (
              <a href={`resumes/${resume.id}`} key={resume.id}>
                <Card
                  title={resume["user_tags"][0]}
                  tags={resume["user_tags"]}
                />
              </a>
            ))}
            <a href="/resumes">
