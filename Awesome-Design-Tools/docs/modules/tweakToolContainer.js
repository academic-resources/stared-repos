...
const makeTextLogo = (unsplittedTitle) => {
  let titleObject = removeEmojis(unsplittedTitle)
    .split('">')[1]
    .split("</a")[0]
    .split(" ");
  let result = "";
...
const createTag = (title, className) => `
	<div
		...
		title="this tool is ${
      title !== "Mac only" ? title.toLowerCase() : `for Mac users only`
    }"
	>
		${title}
	</div>
...
const tweakToolContainer = ({ document }) => {
  ...
  toolsContainer.forEach((tool) => {
    ...
    const targetBlankTitle = `${title.split('">')[0]}" target="_blank">${
      title.split('">')[1]
    }`;
    const descriptionFromMarkdown = tool.innerHTML.split(" — ")[1];
    const description = `<p>${capitalizeFirstLetter(
      descriptionFromMarkdown
    )}</p>`;
    let toolLink = title.split('href="')[1].split('"')[0];
    // const emailRegex = /^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[A-Z0-9.-]+\.[A-Z]{2,}$/;
    tool.innerHTML = `
			<a href="${toolLink}" class="tool__asset" target="_blank">${makeTextLogo(
      title
    )}</a>
