...
const parseTweaks = (html, config) => {
  ...
  const { title, head, bodyColorScheme, logoClassName, nav, welcomeArticle } =
    config;

  const isTool =
    document.querySelector("h1").textContent.split(" ")[2] === "Tools"
      ? true
      : false;
  const isPlugin = document.querySelector("h1").textContent.includes("Plugin")
    ? true
    : false;
  const isKit = document.querySelector("h1").textContent.includes("Kits")
    ? true
    : false;
