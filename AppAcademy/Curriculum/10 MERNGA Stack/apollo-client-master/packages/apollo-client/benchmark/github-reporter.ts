        ...
        if (element != "baseline") {
          if (!thresholds[element]) {
            ...
            if (normalizedMean > thresholds[element]) {
              const perfDropMessage = `Performance drop detected for benchmark: "${element}", ${res[element].mean} / ${res["baseline"].mean} = ${normalizedMean} > ${thresholds[element]}`;
              ...
              console.log(
                `No performance drop detected for benchmark: "${element}", ${res[element].mean} / ${res["baseline"].mean} = ${normalizedMean} <= ${thresholds[element]}`
              );
            }
          }
        }
