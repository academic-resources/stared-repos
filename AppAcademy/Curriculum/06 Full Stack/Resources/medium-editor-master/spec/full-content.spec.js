    ...
    describe("Selection", function () {
        it("should preserve selection after multiple full-content commands", function () {
            ...
            expect(this.el.innerHTML).toBe(
                "<p><u>lorem <strike>ipsum</strike> dolor</u></p>"
            );
        });

        it("should justify all contents including multiple block elements", function () {
            this.el.innerHTML =
                '<p align="center">lorem ipsum dolor</p><p align="left">lorem ipsum dolor</p>';
            var editor = this.newMediumEditor(".editor");
            selectElementContentsAndFire(editor.elements[0].firstChild);
            expect(
                window
                    .getComputedStyle(editor.elements[0].childNodes[0])
                    .getPropertyValue("text-align")
                    .indexOf("center")
            ).not.toBe(-1);
            expect(
                window
                    .getComputedStyle(editor.elements[0].childNodes[1])
                    .getPropertyValue("text-align")
                    .indexOf("left")
            ).not.toBe(-1);
