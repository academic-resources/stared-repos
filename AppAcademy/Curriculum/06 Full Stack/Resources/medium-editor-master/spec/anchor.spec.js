...
describe("Anchor Button TestCase", function () {
    ...
    describe("Anchor Form", function () {
        it("should add class for visible state and remove it for invisivble", function () {
            ...
            expect(
                anchorExtension.getForm().classList.contains(activeClass)
            ).toBe(true);

            fireEvent(
                anchorExtension
                    .getForm()
                    .querySelector("a.medium-editor-toolbar-save"),
                "click"
            );
            expect(
                anchorExtension.getForm().classList.contains(activeClass)
    ...
    describe("Link Creation", function () {
        it("should create a link when user presses enter", function () {
            ...
            expect(
                this.el.innerHTML.indexOf(
                    '<a href="http://test.com">lorem ipsum</a>'
                )
            ).toBe(0);
        });
        ...
        it("should remove the extra white spaces in the link when user presses enter", function () {
            ...
            // A trailing <br> may be added when insertHTML is used to add the link internally.
            expect(
                this.el.innerHTML.indexOf('<a href="test">lorem ipsum</a>')
            ).toBe(0);
        });
        ...
        it("should not set any href if all user passes is spaces in the link when user presses enter", function () {
            ...
            expect(
                this.el.innerHTML.indexOf('<a href="">lorem ipsum</a>')
            ).toBe(-1);
        });

        it("should create only one anchor tag when the user selects across a boundary", function () {
            this.el.innerHTML =
                "Hello world, this <strong>will become a link, but this part won't.</strong>";
            ...
            MediumEditor.selection.select(
                ...
                "Hello world, ".length,
                this.el.childNodes[1].childNodes[0],
                "will become a link".length
            );
            button = toolbar
                .getToolbarElement()
                .querySelector('[data-action="createLink"]');
        ...
        it("should change spaces to %20 for a valid url if linkValidation option is set to true", function () {
            ...
            expect(editor.execAction).toHaveBeenCalledWith(
                "createLink",
                expectedOpts
            );
        ...
        it("should not encode an encoded query param if linkValidation option is set to true", function () {
            ...
            selectElementContentsAndFire(editor.elements[0]);
            anchorExtension.showForm(
                "a.com/?q=http%3A%2F%2Fb.com&q2=http://c.com"
            );
            fireEvent(
                anchorExtension
                    .getForm()
        ...
        it('should add target="_blank" when "open in a new window" checkbox is checked', function () {
            ...
            expect(anchorExtension.isDisplayed()).toBe(true);
            targetCheckbox = anchorExtension
                .getForm()
                .querySelector("input.medium-editor-toolbar-anchor-target");
        ...
        it("should create a button when user selects this option and presses enter", function () {
            ...
            button = anchorExtension
                .getForm()
                .querySelector("input.medium-editor-toolbar-anchor-button");
        ...
        it(
            "should create a button when user selects this option, even if selected text carries tags " +
                "and the selection doesn't exactly match those tags",
            function () {
                spyOn(MediumEditor.prototype, "createLink").and.callThrough();
                ...
                expect(link.classList.contains("btn")).toBe(
                    true,
                    "Link does not contain class btn"
                );
                expect(link.classList.contains("btn-default")).toBe(
                    true,
                    "Link does not contain class btn-default"
                );
            }
        );

        it(
            "should remove the target _blank from the anchor tag when the open in a new window checkbox," +
                " is unchecked and the form is saved",
            function () {
                var editor = this.newMediumEditor(".editor", {
                        anchor: {
                            targetCheckbox: true,
        ...
        it(
            "should fire editableInput only once when the user creates a link open to a new window," +
                " and it should fire at the end of the DOM and selection modifications",
            function () {
                spyOn(MediumEditor.prototype, "createLink").and.callThrough();
                ...
                expect(window.getSelection().toString()).toBe(
                    "ipsum",
                    "selected text should remain selected"
                );
                expect(selectionWhenEventsFired.length).toBe(
                    1,
                    "only one editableInput event should have been registered"
                );
                expect(selectionWhenEventsFired[0]).toBe(
                    "ipsum",
                    "selected text should have been the same when event fired"
                );
            }
        );
        ...
        it("should not select empty paragraphs when link is created at beginning of paragraph after empty paragraphs", function () {
            spyOn(MediumEditor.prototype, "createLink").and.callThrough();
            this.el.innerHTML =
                "<p>Some text</p><p><br/></p><p><br/></p><p>link text more text</p>";
            var editor = this.newMediumEditor(".editor"),
            ...
            ).toBe(true, "The start of the selection is incorrect");
            ...
            expect(
                ...
                true,
                "The end of the selection is not contained within the link"
            );
        });
        ...
        it("should not remove the <p> container when adding a link inside a top-level <p> with a single text-node child", function () {
            spyOn(MediumEditor.prototype, "createLink").and.callThrough();
            this.el.innerHTML =
                "<p>Some text</p><p><br/></p><p><br/></p><p>some text link text</p>";
            var editor = this.newMediumEditor(".editor"),
                lastP = this.el.lastChild,
                anchorExtension = editor.getExtensionByName("anchor"),
                toolbar = editor.getExtensionByName("toolbar");
            ...
            if (range.startContainer === lastP.lastChild.firstChild) {
                expect(range.startOffset).toBe(
                    0,
                    "The start of the selection is not at the front of the link"
                );
            } else {
        ...
        it("should update the href of a link containing only an image", function () {
            ...
            if (!edgeVersion || edgeVersion >= 13) {
                expect(this.el.innerHTML).toContain(
                    '<a href="http://www.google.com"><img src="../demo/img/medium-editor.jpg"></a>'
                );
            }
        });
    });
    ...
    describe("Cancel", function () {
        it("should close the link form when user clicks on cancel", function () {
            ...
            button = toolbar
                .getToolbarElement()
                .querySelector('[data-action="createLink"]');
            cancel = anchorExtension
                .getForm()
                .querySelector("a.medium-editor-toolbar-close");
            fireEvent(button, "click");
            expect(anchorExtension.isDisplayed()).toBe(true);
    ...
    describe("Click", function () {
        it("should display the anchor form when toolbar is visible", function () {
            ...
            button = toolbar
                ...
                .querySelector('[data-action="createLink"]');
            fireEvent(button, "click");
            expect(toolbar.getToolbarActionsElement().style.display).toBe(
                "none"
            );
            expect(anchorExtension.isDisplayed()).toBe(true);
        ...
        it("should allow more than one link creation within a paragraph", function () {
            ...
            this.el.innerHTML =
                '<p><a href="#">beginning</a> some text middle some text end</p>';
            var editor = this.newMediumEditor(".editor"),
                anchorExtension = editor.getExtensionByName("anchor"),
                toolbar = editor.getExtensionByName("toolbar"),
