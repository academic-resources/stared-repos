...
describe("Pasting content", function () {
    ...
    describe("using cleanPastedHTML option", function () {
        ...
        it("should add paragraphs to pasted plain-text", function () {
            ...
            pasteHandler.handlePaste(evt, editorEl);
            jasmine.clock().tick(100);
            expect(editorEl.innerHTML).toEqual(
        ...
        it('should filter multi-line rich-text pastes when "insertHTML" command is not supported', function () {
            ...
            multiLineTests.forEach(
                function (test) {
                    this.el.innerHTML =
                        '<span id="editor-inner">lorem ipsum</span>';
    ...
    describe("using keyboard", function () {
        it("should insert a custom paste-bin on keydown of CTRL + V", function () {
            ...
            var contentEditables = document.body.querySelectorAll(
            ...
            contentEditables = document.body.querySelectorAll(
                "[contentEditable=true]"
            );
            ...
            ).toBe(true, "Select is not within the paste bin");
    ...
    describe("using cleanPaste", function () {
        it("should filter inline rich-text", function () {
            ...
            for (i = 0; i < inlineTests.length; i += 1) {
                // move caret to editor
                editorEl.innerHTML =
                    'Before&nbsp;<span id="editor-inner">&nbsp;</span>&nbsp;after.';
                ...
                expect(editorEl.innerHTML).toMatch(
                    new RegExp(
                        '^Before(&nbsp;|\\s)(<span id="editor-inner">)?' +
                            inlineTests[i].output +
                            "(</span>)?(&nbsp;|\\s)after\\.$"
                    )
                );
            }
            ...
            inlineTests.forEach(
                function (test) {
                    ...
                    selectElementContents(
                        document.getElementById("editor-inner")
                    );
        ...
        it("should cleanup only pasted element on multi-line when nothing is selected", function () {
            ...
            this.el.innerHTML =
                '<div><img src="http://0.0.0.0/ohyeah.png" /></div>';

            selectElementContents(this.el.firstChild, { collapse: true });

            editor.cleanPaste(
                "<table><tr><td>test</td><td><br/></td></tr></table>"
            );

            expect(this.el.innerHTML).toContain(
                '<img src="http://0.0.0.0/ohyeah.png"></div>'
            );
        });
    });
    ...
    describe("using pasteHTML", function () {
        it("should remove certain attributes and tags by default", function () {
            ...
            editor.pasteHTML(
                '<p class="some-class" style="font-weight: bold" dir="ltr"><meta name="description" content="test" />test</p>'
            );
        ...
        it('should not remove node with "empty" content', function () {
            ...
            this.el.innerHTML =
                '<div>this is a div</div><figure id="editor-inner">and this is a figure</figure>.';
            ...
            expect(this.el.innerHTML).toContain(
                "<table><tbody><tr><td>test</td><td><br></td></tr></tbody></table>"
            );
        });
        ...
        it("should accept a list of tags to unwrap", function () {
            ...
            expect(editor.elements[0].innerHTML).toBe(
                "<div><i>test</i><b>test</b>test</div>"
            );
        });
