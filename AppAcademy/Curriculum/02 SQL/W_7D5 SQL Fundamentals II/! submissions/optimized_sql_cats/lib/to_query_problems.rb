...
def price_like_shiny_mouse
  ...
  execute(<<-SQL) 
    ...
    JOIN
      toys 
      ON
        shiny.price = toys.price  
    WHERE
      shiny.name = 'Shiny Mouse' AND toys.name != 'Shiny Mouse'
    ORDER BY
      toys.name
...
def just_like_orange
  ...
  execute(<<-SQL)
    SELECT
      cat_breed.name, cat_breed.breed
    FROM
      cats AS orange
    JOIN
      cats AS cat_breed
        ON orange.id = cat_breed.id
    WHERE
      orange.breed = 'Brazilian Shorthair'
    AND 
      cat_breed.name != 'Orange'
    ORDER BY
      cat_breed.name
    ;
  SQL
...
def toys_that_jet_owns
  ...
  execute(<<-SQL)
    ...
    JOIN
      cats ON cats.id = cattoys.cat_id
    JOIN
      cattoys as non_jet ON non_jet.toy_id = toys.id
    JOIN
      cats as non_jet_cats ON non_jet_cats.id = non_jet.cat_id
    WHERE
      cats.name = 'Jet'
      AND
      non_jet_cats.name != 'Jet'
    ORDER BY
      non_jet_cats.name
    ;
  SQL
