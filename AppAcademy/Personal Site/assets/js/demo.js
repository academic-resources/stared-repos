  ...
  $.fn.demo_controls = function (styles, userOptions) {
    ...
    if (styles) {
      ...
      for (i in styles) {
        ...
        $x = $('<span data-class="' + i + '">, ' + i + "</span>").appendTo(
          $stylePropertyClasses
        );
          ...
          for (k in styles[i][j]) {
            $z = $(
              '<span data-class="' +
                k +
                '">, ' +
                styles[i][j][k].replace("*", "") +
                "</span>"
            ).appendTo($y);

            if (styles[i][j][k].substr(-1, 1) == "*") $z.addClass("default");

            if (current && $this.hasClass(k)) $z.addClass("active");
          }
    ...
    $controls.on("click", ".property.active", function (event) {
      ...
      // Determine next.
      if ($current.length == 0 || $current.index() == $classes.length - 1)
        $next = $classes.first();
    ...
    $controls.on("click", '.property[data-name="style"]', function (event) {
      ...
      $controls
        .find('.property[data-requires="' + $current.data("class") + '"]')
        .removeClass("active");

      $controls
        .find(
          '.property[data-requires="' +
            $current.data("class") +
            '"] > .classes > .active'
        )
        .each(function () {
      ...
      $controls
        .find('.property[data-requires="' + $next.data("class") + '"]')
        .addClass("active");
